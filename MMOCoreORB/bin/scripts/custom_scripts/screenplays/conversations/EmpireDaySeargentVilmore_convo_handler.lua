...
function empiredaySeargentVilmore_convo_handler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "Response_01" or "Response_02")then
		if pGhost ~= nil then
			local empiredayRebWpID_2 = PlayerObject(pGhost):addWaypoint("tatooine", "Speak to and assist Captain Edwards", "Speak to and assist Captain Edwards", -6634, 5522, WAYPOINT_COLOR_PURPLE, true, true, 0)
...
function empiredaySeargentVilmore_convo_handler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	elseif (empiredayRebStatus == "3" or empiredayRebStatus == "4" or empiredayRebStatus == "5" or empiredayRebStatus == "6") then
