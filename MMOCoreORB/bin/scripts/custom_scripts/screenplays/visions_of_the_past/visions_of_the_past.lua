	...
	for i = 1, #scnOTable, 1 do
		...
		if isZoneEnabled(sceneObject[1]) then
			...
			if (pSceneObject ~= nil) then
				...
				writeData(spawnedSceneObject:getObjectID() .. ":visionsWreckage", i)
...
function VisionsOfThePast:itemLooted(pSceneObject, radialSelected)
	local pLootContainer = pSceneObject
	
	if radialSelected ~= 16 then
		return
	end
	
	if pLootContainer ~= nil then
		--createEvent(self.lootRespawnTime, "VisionsOfThePast", "spawnCollectionItems", pSceneObject, "")
		createEvent(20, "VisionsOfThePast", "spawnCollectionItems", pLootContainer, "")
	end
end
