...
VisionsDianeBrack_convo_handler = conv_handler:new {}

function VisionsDianeBrack_convo_handler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "Turnin_Response_01" or screenID == "Turnin_Response_02") then
		if VisionsOfThePast:hasCollectionItem(pPlayer) then
			...
			--deleteData(playerID .. ":visionsState", 2)
			writeData(playerID .. ":visionsState", 3)
			ThemeParkLogic:giveFaction(pPlayer, "imperial", 2000)
...
function VisionsDianeBrack_convo_handler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	local visionsRebelStatus = readData(SceneObject(pPlayer):getObjectID() .. ":visionsRebelHelper")

	if (playerFaction == FACTIONREBEL or visionsRebelStatus == 1) then
