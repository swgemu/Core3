...
function VisionsRichKoster_convo_handler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "Response_01" or screenID == "Response_02") then
		if pGhost ~= nil then
			local visionsWpID_1 = PlayerObject(pGhost):addWaypoint("yavin4", "Imperial Security Bureau Officer", "Imperial Security Bureau Officer", -6988, -5748, WAYPOINT_COLOR_PURPLE, true, true, 0)

			VisionsOfThePast:setQuestStatus(pPlayer, ":visionsStatus", 1)
			VisionsOfThePast:setQuestStatus(pPlayer, ":visionsHelper", 2)
