...
describe("Holocron Jedi Manager", function()
	it("Shall define the number of professions to master to 6", function()
		assert.same(EXPECTEDNUMBEROFPROFESSIONSTOGRIND, NUMBEROFPROFESSIONSTOMASTER)
	end)

	it("Shall on call to get grindable profession list return a list of at least the number of professions that has to be mastered", function()
		assert.is_true(EXPECTEDNUMBEROFPROFESSIONSTOGRIND <= table.getn(HolocronJediManager:getGrindableProfessionList()))
	end)

	it("Shall always return the same number of grindable professions in a list that is not affected when items are removed from it locally", function()
