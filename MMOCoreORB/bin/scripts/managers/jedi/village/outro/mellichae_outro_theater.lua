...
function MellichaeOutroTheater:onLoot(pLootedCreature, pLooter, nothing)
	...
	if (SceneObject(pLooter):getObjectID() == ownerID) and (SpawnMobiles.isFromSpawn(pLooter, self.taskName, pLootedCreature)) then
...
function MellichaeOutroTheater:onEnteredActiveArea(pCreatureObject, spawnedSithShadowsList)
	...
	foreach(spawnedSithShadowsList, function(pMobile)
		if (pMobile ~= nil) and (CreatureObject(pMobile):getFirstName() ~= "Mellichae" or CreatureObject(pMobile):getFirstName() ~= "Daktar") then
