...
function VillageJediManagerTownship:spawnMobiles(currentPhase, spawnStaticMobs)
	if (spawnStaticMobs == true) then
		...
		for i = 1, table.getn(mobileTable), 1 do
			...
			local pMobile = spawnMobile("dathomir", mobile[1], 0, mobile[2], mobile[3], mobile[4], mobile[5], mobile[6])
			if (pMobile ~= nil) then
				CreatureObject(pMobile):setPvpStatusBitmask(0)
				if (mobile[7] ~= "") then
					self[mobile[7]](pMobile)
				end
				if (mobile[8] ~= "") then
					CreatureObject(pMobile):setOptionsBitmask(136)
					AiAgent(pMobile):setConvoTemplate(mobile[8])
				end
			end
			Logger:log("Spawning a Village Static NPC at " .. mobile[2] .. " - " .. mobile[4], LT_INFO)
		end
	end
	...
	for i = 1, table.getn(mobileTable), 1 do
		local mobile = mobileTable[i]
		local pMobile = spawnMobile("dathomir", mobile[1], 0, mobile[2], mobile[3], mobile[4], mobile[5], mobile[6])
		Logger:log("Spawning a Village Phase " .. currentPhase .. " NPC at " .. mobile[2] .. " - " .. mobile[4], LT_INFO)
		if (pMobile ~= nil) then
			CreatureObject(pMobile):setPvpStatusBitmask(0)
			if (mobile[7] ~= "") then
				self[mobile[7]](pMobile)
...
function VillageJediManagerTownship:despawnMobiles(currentPhase)
	...
	for i = 1, table.getn(mobileTable), 1 do
		...
		if (pMobile ~= nil) then
			...
			Logger:log("Despawning " .. mobile[1], LT_INFO)
		end
	end
end
...
function VillageJediManagerTownship:spawnSceneObjects(currentPhase, spawnStaticObjects)
	if (spawnStaticObjects == true) then
		...
		foreach(objectTable, function(sceneObject)
			local pMobile = spawnSceneObject("dathomir", sceneObject[1], sceneObject[2], sceneObject[3], sceneObject[4], 0, sceneObject[5])
			Logger:log("Spawning a Village Static Scene Object at " .. sceneObject[2] .. " - " .. sceneObject[4], LT_INFO)
		end)
	end
	...
	for i = 1, table.getn(objectTable), 1 do
		...
		local pMobile = spawnSceneObject("dathomir", sceneObject[1], sceneObject[2], sceneObject[3], sceneObject[4], 0, sceneObject[5])
		Logger:log("Spawning a Village Phase " .. currentPhase .. " Scene Object at " .. sceneObject[2] .. " - " .. sceneObject[4], LT_INFO)
		if (pMobile ~= nil) then
			local mobileID = SceneObject(pMobile):getObjectID()
			writeData("village:scene:object:" .. i, mobileID)
		end
	end
end
...
function VillageJediManagerTownship:despawnSceneObjects(currentPhase)
	...
	for i = 1, table.getn(objectTable), 1 do
		...
		if (pObject ~= nil) then
			...
			deleteData("village:npc:object:" .. i)
			Logger:log("Despawning " .. object[1], LT_INFO)
		end
	end
end
