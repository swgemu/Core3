...
--local SithOutroTheater = require("managers.jedi.village.sith_shadow_outro_theater")

local OLD_MAN_GREETING_STRING = "@quest/force_sensitive/exit:oldman_greeting"
local OLD_MAN_DESPAWN_TIME = 10 * 1000

OldManEncounterOutro = Encounter:new {
	-- Task properties
	taskName = "OldManEncounterOutro",
...
function OldManEncounterOutro:getPlayerFirstName(pCreatureObject)
...
-- @param pCreatureObject pointer to the creature object of the player.
function OldManEncounterOutro:sendGreetingString(pOldMan, pCreatureObject)
...
function OldManEncounterOutro:onEncounterClosingIn(pCreatureObject, oldManPointerList)
...
function OldManEncounterOutro:handleScheduledDespawn(pCreatureObject)
...
-- @param pCreatureObject pointer to the creature object of the player.
function OldManEncounterOutro:scheduleDespawnOfOldMan(pCreatureObject)
	if (pCreatureObject == nil) then
		return
	end
...
function OldManEncounterOutro:doesOldManBelongToThePlayer(pConversingPlayer, pConversingOldMan)
...
function OldManEncounter:taskFinish(pCreatureObject)
	...
	Logger:log("Finishing " .. self.taskName .. " and starting SithOutroTheater.", LT_INFO)
