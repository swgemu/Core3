...
function FsMedicPuzzle:giveSymptoms(pPlayer, pNpc)
	...
	for i = 1, maxSymptoms, 1 do
		...
		while self:hasSpecificSymptom(pPlayer, pNpc, symptomNum) do
...
function FsMedicPuzzle:hasSpecificSymptom(pPlayer, pNpc, symptom)
	return readData(SceneObject(pPlayer):getObjectID() .. ":fs_medic_puzzle_npc:" .. SceneObject(pNpc):getObjectID() .. ":symptom" .. symptom) == 1
end

function FsMedicPuzzle:hasAnySymptoms(pPlayer, pNpc)
	for i = 0, self.totalSymptoms - 1, 1 do
		if (readData(SceneObject(pPlayer):getObjectID() .. ":fs_medic_puzzle_npc:" .. SceneObject(pNpc):getObjectID() .. ":symptom" .. i) == 1) then
			return true
		end
	end
...
function FsMedicPuzzle:getSymptoms(pPlayer, pNpc)
	...
	for i = 0, self.totalSymptoms - 1, 1 do
		if self:hasSpecificSymptom(pPlayer, pNpc, i) then
