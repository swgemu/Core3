...
function FsSurvey:surveyEventHandler(pPlayer, pResourceSpawn, density)
	...
	local questName = self:hasSurveyorQuest(pPlayer, 2)

	if (questName == "") then
		questName = self:hasSurveyorQuest(pPlayer, 3)
	end
...
function FsSurvey:getResourceValue(average)
	...
	if (rand > math.min(average + 150, 1000)) then
		rand = math.min(average + 150, 1000)
	elseif (rand < math.max(average - 150, 150)) then
		rand = math.max(average - 150, 150)
	end
