...
FsSad1Theater7 = GoToTheater:new {
	...
	mobileList = {
		...
		{ template = "sith_shadow_taskmaster", minimumDistance = 4, maximumDistance = 12, referencePoint = 0 },
	},
...
function FsSad1Theater7:onSuccessfulSpawn(pCreatureObject, mobileList)
	if (pCreatureObject == nil) then
		return
	end

	local playerID = SceneObject(pCreatureObject):getObjectID()
	writeData(playerID .. self.taskName .. ":killableCount", #mobileList)

	for i = 1, #mobileList, 1 do
		writeData(SceneObject(mobileList[i]):getObjectID() .. self.taskName .. "ownerID", playerID)
		createObserver(OBJECTDESTRUCTION, self.taskName, "notifyKilledMobile", mobileList[i])
	end

	local theaterId = readData(playerID .. self.taskName .. "theaterId")
	local pTheater = getSceneObject(theaterId)
	printf("location spawned at " .. SceneObject(pTheater):getWorldPositionX() .. " " .. SceneObject(pTheater):getWorldPositionY() .. "\n")
