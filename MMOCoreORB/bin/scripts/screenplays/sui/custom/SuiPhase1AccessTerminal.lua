...
function SuiPhase1AccessTerminal:retrieveComponent(pPlayer, suiPageData, rowIndex)
	...
	if (rowStatus == 1) then
		TangibleObject(pItem):setLuaStringData("componentStatus", "1")
	else
		TangibleObject(pItem):setLuaStringData("componentStatus", "-1")
	end
...
function SuiPhase1AccessTerminal:replaceComponent(pPlayer, suiPageData, rowIndex)
	...
	local componentStatus = TangibleObject(pComponent):getLuaStringData("componentStatus")

	writeScreenPlayData(pPlayer, "FsCrafting1", "accessTerminalStatus" .. rowIndex, componentStatus)
