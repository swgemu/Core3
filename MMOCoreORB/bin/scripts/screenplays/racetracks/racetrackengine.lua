...
function RaceTrackManager:createRaceTrack(trackConfig)
	...
	RaceTrackManager:spawnActiveArea(trackConfig.planetName,trackConfig.waypoints[table.getn(trackConfig.waypoints)],trackConfig.waypointRadius,table.getn(trackConfig.waypoints),true)
end
...
function RaceTrackManager:loadVariablesFromScreenPlay(pObject)
	...
	trackConfig.numOfWPs =  readScreenPlayData(pObject, trackConfig.trackName, "numberofwaypoints")
	if trackConfig.numOfWPs ~= "" then
		for lc = 1, tonumber(trackConfig.numOfWPs), 1 do
...
function  RaceTrackManager:finalWaypoint(pActiveArea, pObject)
	ObjectManager.withCreatureObject(pObject, function(creatureObject)
		...
		if lastIndex ~= "" then
			if tonumber(lastIndex)==tonumber(trackConfig.numOfWPs) then
				local startTime = readScreenPlayData(pObject, trackConfig.trackName, "starttime")
				local seconds = os.difftime(os.time(), startTime)
				local lastWaypointID =  readScreenPlayData(pObject, trackConfig.trackName, "waypointID")
				if lastWaypointID~="" then
				...
				if recordTime > seconds then
					...
					if playerObject:hasBadge(tonnumber(trackConfig.badge)) == false then
						playerObject:awardBadge(tonumber(trackConfig.badge))
					end
				end
				clearScreenPlayData(pObject,trackConfig.trackName )
...
function RaceTrackManager:displayTrackBestTime(pObject,trackConfig)
	ObjectManager.withCreatureObject(pObject, function(creatureObject)
		...
		if recordTime == 0 then
			...
			creatureObject:sendSystemMessage(readStringSharedMemory(trackConfig.trackName ..".recordholder") .. " with a time of " .. recordTime .. "s")
		end
	end)
end
