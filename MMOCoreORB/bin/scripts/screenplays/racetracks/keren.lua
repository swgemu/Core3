...
keren_racetrack_screenplay = ScreenPlay:new {
	numberOfActs = 1,
	states = {},
	trackConfig={
...
function keren_racetrack_screenplay:spawnObservers()
	RaceTrackManager:createRaceTrack(self.trackConfig)
end
function keren_racetrack_screenplay:startRace(pObject)
	RaceTrackManager:saveVariablesToScreenPlay(pObject,self.trackConfig)
	RaceTrackManager:spawnFirstWaypoint(pObject)
end
function keren_racetrack_screenplay:displayPersonalBestTime(pObject)
	RaceTrackManager:displayPersonalBestTime(pObject,self.trackConfig)
end

function keren_racetrack_screenplay:displayTrackBestTime(pObject)
...
function keren_racetrack_convo_handler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	if ( screenID == "cs_jsPlumb_1_116" ) then
		keren_racetrack_screenplay:startRace(conversingPlayer)
