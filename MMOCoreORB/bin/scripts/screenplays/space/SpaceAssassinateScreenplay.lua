...
function SpaceAssassinateScreenplay:completeQuest(pPlayer, notifyClient)
	...
	if (self.sideQuest and (self.sideQuestSplitType == self.SIDE_QUEST_SPLIT_TYPES.COMPLETION or self.SIDE_QUEST_SPLIT_TYPES.BIDIRECTIONAL)) then
...
function SpaceAssassinateScreenplay:resetQuest(pPlayer, notifyClient)
	if (pPlayer == nil) then
