...
--[[
		...
		- After all rounds are completed the level increments or boss is spawned. Defined as: totalLevels
		- Each round awards credits.
...
function SpaceDutyDestroyScreenplay:spawnAttackWave(pPlayer)
	...
	-- Spawn regular attack wave
		...
		local targetsMsg = LuaStringIdChatParameter("@spacequest/destroy_duty/" .. self.questName .. ":duty_update")
		targetsMsg:setTO("@spacequest/destroy_duty/" .. self.questName .. ":targets_detected")
		-- "targets_behind", "\\#pcontrast3 Tactical Computer: < \\#pcontrast1 Be aware, vessels trying to target this ship with their weapon systems. Attack originating direction 180 degrees.\\#pcontrast3  >");
		-- "targets_detected", "\\#pcontrast3 Tactical Computer: < \\#pcontrast1 Vessels fitting hunted ship profile on attack vector, weapon systems armed. \\#pcontrast3 >");
...
function SpaceDutyDestroyScreenplay:notifyAttackShipDestroyed(pShipAgent, pKillerShip)
	...
	if (#newIDs > 0) then
		...
		-- All waves have been destroyed
		else
			local currentRound = readData(playerID .. ":" .. self.className .. ":currentRound:") + 1
			deleteData(playerID .. ":" .. self.className .. ":currentRound:")

			-- Give Reward
			local rewardCredits = 300 -- Amount calc?

			local targetsMsg = LuaStringIdChatParameter("@space/quest:destroy_duty_round_reward") -- "destroy_duty_round_reward", "\\#pcontrast3 > \\#pcontrast1 All waves eliminated. Reward: \\#pcontrast2 %DI \\#pcontrast1 credits. \\#pcontrast3 <"
			targetsMsg:setDI(rewardCredits)

			CreatureObject(pPlayer):sendSystemMessage(targetsMsg:_getObject())

			ThemeParkLogic:giveCredits(pPlayer, rewardCredits)

			if (currenRound >= self.totalRounds) then


			end

			-- Give new waypoint nearby
