...
function SpaceEscortScreenplay:spawnAttackWave(pEscortAgent)
	...
	if (self.DEBUG_SPACE_ESCORT) then
		print(self.className .. ":spawnAttackWave -- Spawn Table Size: " .. #spawnTable .. " Spawn Zone: " .. spawnZone)
	end
...
function SpaceEscortScreenplay:notifyEnteredQuestArea(pActiveArea, pShip)
	...
	elseif (SceneObject(pShip):isShipAiAgent()) then
		...
		if (shipProgress <= 0) then
			...
			createEvent(200, self.className, "removeAttackShips", pShip, "")

			-- hyperspace out escort ship with thanks message
			createEvent(200, self.className, "removeEscortShip", pShip, "")
...
function SpaceEscortScreenplay:notifyEscortShipDestroyed(pShipAgent, pKillerShip)
	...
	-- Hyperspace out any remaining attack waves
	createEvent(1000, self.className, "removeAttackShips", pShipAgent, "true")
...
function SpaceEscortScreenplay:notifyAttackShipDestroyed(pShipAgent, pKillerShip)
	...
	for i = 1, #shipIDs, 1 do
		if (agentID == shipIDs[i]) then
			index = i
		end
	end

	if (index > 0) then
		-- Remove agentID from the stored table
		table.remove(shipIDs, index)
	end
	...
	if (#shipIDs > 0) then
		...
		local messageString = LuaStringIdChatParameter("@space/quest:enemies_remain")
		messageString:setDI(#shipIDs)
