...
function SpaceEscortScreenplay:spawnEscortShip(pPlayer)
	...
	-- TODO REMOVE
	CreatureObject(pPlayer):sendSystemMessage("This is where the escort begins")
...
function SpaceEscortScreenplay:checkEscort(pShipAgent)
	...
	if (not SpaceHelpers:isSpaceQuestTaskActive(pPilot, self.questType, self.questName, 3)) then
		return 0
	end
...
function SpaceEscortScreenplay:notifyEnteredEscortArea(pActiveArea, pShip)
	if ((pActiveArea == nil) or (pShip == nil) or (not SceneObject(pShip):isPlayerShip())) then
		return 0
	end

	if (self.DEBUG_SPACE_ESCORT) then
		print(self.className .. ":notifyEnteredEscortArea - Ship: " .. SceneObject(pShip):getDisplayedName() .. " entered SpaceActiveArea - X: " .. SceneObject(pActiveArea):getPositionX() .. " Z: " .. SceneObject(pActiveArea):getPositionZ() .. " Y: " .. SceneObject(pActiveArea):getPositionY() .. " Object Position - X: " .. SceneObject(pShip):getPositionX() .. " Z: " .. SceneObject(pShip):getPositionZ() .. " Y: " .. SceneObject(pShip):getPositionY())
	end
