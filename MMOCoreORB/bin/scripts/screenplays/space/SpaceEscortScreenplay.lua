...
function SpaceEscortScreenplay:setupEscort(pPlayer)
	...
	SpaceHelpers:sendQuestUpdate(pPilot, "@spacequest/escort/" .. self.questName .. ":found_loc") -- "Meet the convoy at the following location."
end
...
function SpaceEscortScreenplay:notifyEnteredQuestArea(pActiveArea, pShip)
	...
	if (SceneObject(pShip):isPlayerShip()) then
		...
		SpaceHelpers:sendQuestUpdate(pPilot, "@spacequest/escort/" .. self.questName .. ":arrived_at_loc")

		-- Arrived at escort rendevous, send them the initial progress message
