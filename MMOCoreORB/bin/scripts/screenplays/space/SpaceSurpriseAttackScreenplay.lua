...
function SpaceSurpriseAttackScreenplay:spawnSurpriseAttack(pPilot)
	...
	for i = 1, #shipsTable, 1 do
		...
		for j = 1, count, 1 do
			...
			if (pShipAgent ~= nil) then
				...
				CreatureObject(pPilot):addMissionCriticalObject(agentID, true)
	...
	for i = 1, #shipIDs, 1 do
		...
		--CreatureObject(pPilot):addMissionCriticalObject(shipIDs[i], (i == #shipIDs))
	end
...
function SpaceSurpriseAttackScreenplay:notifyShipDestroyed(pShipAgent, pKillerShip)
	...
	CreatureObject(pPlayer):removeMissionCriticalObject(agentID, 0)
