...
function SpaceRecoveryScreenplay:spawnRecoveryShip(pPlayer)
	...
	-- Ship Observers
	createObserver(DESTROYEDSHIP, self.className, "notifyRecoverShipDestroyed", pShipAgent)
...
function SpaceRecoveryScreenplay:assignEscortPoints(pShipAgent, flightPath)
	...
	for i = 1, #flightPath, 1 do
		ShipAiAgent(pShipAgent):addFixedPatrolPoint(flightPath[i], true)
	end
end
...
function SpaceRecoveryScreenplay:startRecovery(pRecoveryShip)
	...
	if (self.DEBUG_SPACE_RECOVERY) then
		print(self.className .. ":notifyRecoveryShipDisabled -- Called for Destructed ShipAgent Object: " .. ShipObject(pRecoveryShip):getShipName() .. " Destructor: " .. SceneObject(pPlayer):getDisplayedName())
	end
...
function SpaceRecoveryScreenplay:startRecovery(pRecoveryShip)
	...
	createEvent(10 * 1000, self.className, "assignRecoverPoints", pRecoveryShip, "")
end

function SpaceRecoveryScreenplay:assignRecoverPoints(pRecoveryShip)
	if (pRecoveryShip == nil) then
		Logger:log(self.className .. ":assignRecoverPoints -- pRecoveryShip is nil.", LT_ERROR)
		return
	end

	-- Need to clear the agents points

	local flightPath = self.recoveryPoints

	if (self.DEBUG_SPACE_RECOVERY) then
		print(self.className .. ":assignRecoverPoints to -- Ship: " .. ShipObject(pRecoveryShip):getShipName() .. " Total Points to assign: " .. #flightPath)
	end

	for i = 1, #flightPath, 1 do
		ShipAiAgent(pRecoveryShip):addFixedPatrolPoint(flightPath[i], true)
	end
end
...
function SpaceRecoveryScreenplay:notifyRecoverShipDestroyed(pShipAgent, pPlayer)
	...
	if (self.DEBUG_SPACE_RECOVERY) then
		print(self.className .. ":notifyRecoverShipDestroyed -- Called for Destructed ShipAgent Object: " .. ShipObject(pShipAgent):getShipName() .. " Destructor: " .. SceneObject(pPlayer):getDisplayedName())
	end
...
function SpaceRecoveryScreenplay:notifyRecoveryShipDisabled(pPlayer, pShipAgent)
	if (pShipAgent == nil or not SceneObject(pShipAgent):isShipAiAgent()) then
		return 1
	end
	...
	if (self.DEBUG_SPACE_RECOVERY) then
		print(self.className .. ":notifyRecoveryShipDisabled -- Called for Destructed ShipAgent Object: " .. ShipObject(pShipAgent):getShipName() .. " Destructor: " .. SceneObject(pPlayer):getDisplayedName())
	end
