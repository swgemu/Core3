...
evinConvoHandler = conv_handler:new {}

function evinConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	local questOneComplete = SpaceHelpers:isSpaceQuestComplete(pPlayer, VortexSquadronScreenplay.QUEST_STRING_1.type, VortexSquadronScreenplay.QUEST_STRING_1.name)
	...
	local questThreeComplete = SpaceHelpers:isSpaceQuestComplete(pPlayer, VortexSquadronScreenplay.QUEST_STRING_3.type, VortexSquadronScreenplay.QUEST_STRING_3.name)
	...
	-- Player has an active story quest or duty mission from Evin
	...
	elseif (SpaceHelpers:hasCompletedPilotTier(pPlayer, "rebel_navy", 1)) then
		return convoTemplate:getScreen("completed_evin")
	...
	elseif (questFourComplete and getQuestStatus(playerID .. VortexSquadronScreenplay.QUEST_STRING_4.name .. ":reward") == "1") then
...
function evinConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "more_training") then
		...
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_starships_01") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_starships_01")) then
			clonedConversation:addOption("@conversation/naboo_rebel_trainer_1:s_c06c7aa9", "train_player_fighters") -- Starship Piloting
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_weapons_01") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_weapons_01")) then
			clonedConversation:addOption("@conversation/naboo_rebel_trainer_1:s_d1431f95", "train_player_component") -- Weapons Systems
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_procedures_01") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_procedures_01")) then
			clonedConversation:addOption("@conversation/naboo_rebel_trainer_1:s_8523e1fc", "train_player_basics") -- Flight Procedures
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_droid_01") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_droid_01")) then
			clonedConversation:addOption("@conversation/naboo_rebel_trainer_1:s_b7fc5e5d", "train_player_droid") -- Droid Interface
		end
	...
	elseif (screenID == "start_mission3") then
		escort_naboo_rebel_3:startQuest(pPlayer, pNpc)
