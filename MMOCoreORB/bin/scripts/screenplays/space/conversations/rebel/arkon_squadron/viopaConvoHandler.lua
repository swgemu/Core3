...
function viopaConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	elseif (questThreeComplete and not questFourStarted and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_3.name .. ":reward") == "1") then
	...
	elseif (questTwoComplete and not questThreeStarted and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_2.name .. ":reward") == "1") then
	...
	elseif (questOneComplete and not questTwoStarted and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_1.name .. ":reward") == "1") then
	...
	elseif (questOneComplete and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_1.name .. ":reward") ~= "1") then
		...
		return convoTemplate:getScreen("excellent_work")
	-- New pilot to Viopa
	elseif (getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_1.name .. ":attempted") ~= "1") then
...
function viopaConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "more_training") then
		...
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_starships_02") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_starships_02")) then
			clonedConversation:addOption("@conversation/lok_rebel_trainer_2:s_c06c7aa9", "train_player_fighters") -- Advanced fighters.
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_weapons_02") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_weapons_02")) then
			clonedConversation:addOption("@conversation/lok_rebel_trainer_2:s_d1431f95", "train_player_component") -- Advanced starship components.
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_procedures_02") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_procedures_02")) then
			clonedConversation:addOption("@conversation/lok_rebel_trainer_2:s_8523e1fc", "train_player_basics") -- Advanced training
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_droid_02") and skillManager:fulfillsSkillPrerequisitesAndXp(pPlayer, "pilot_rebel_navy_droid_02")) then
			clonedConversation:addOption("@conversation/lok_rebel_trainer_2:s_b7fc5e5d", "train_player_droid") -- Advanced droid interface.
		end
