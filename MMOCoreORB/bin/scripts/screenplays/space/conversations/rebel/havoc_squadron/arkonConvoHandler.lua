...
function arkonConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	if (questFourComplete and SpaceHelpers:hasCompletedPilotTier(pPlayer, "rebel_navy", 4)) then
	...
	if (not SpaceHelpers:hasCompletedPilotTier(pPlayer, "rebel_navy", 4) and SpaceHelpers:hasPilotTierSkill(pPlayer, "rebel_navy", 4)) then
...
function arkonConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	clonedConversation:setDialogTextNU(CreatureObject(pPlayer):getFirstName())
