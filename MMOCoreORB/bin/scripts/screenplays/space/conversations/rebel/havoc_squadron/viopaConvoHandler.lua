...
function viopaConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	elseif (SpaceHelpers:hasPilotTierSkill(pPlayer, "rebel_navy", 2)) then
		...
		if (getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_1.name .. ":introduced") == "1"
			and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER2_QUEST_STRING_1.name .. ":attempted") ~= "1"
			and not questOneComplete) then
