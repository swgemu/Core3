...
function aqzowConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	--[[
		...
		reward is skipped when all tier 3 skills are already trained.
	]]

	-- Player has completed quest 4 and needs reward
	if (questFourComplete and getQuestStatus(playerID .. HavocSquadronScreenplay.TIER3_QUEST_STRING_4.name .. ":reward") ~= "1") then
	...
	-- Check if players have all the tier 3 skill boxes, send them to Warvog Arkon for Tier 4
	if (SpaceHelpers:hasCompletedPilotTier(pPlayer, "rebel_navy", 3)) then
	...
	-- Player has a Tier 3 skill box but not all 4 (needs training - safety net for interrupted conversations)
	if (not SpaceHelpers:hasCompletedPilotTier(pPlayer, "rebel_navy", 3) and SpaceHelpers:hasPilotTierSkill(pPlayer, "rebel_navy", 3)) then
	...
	if (not questFourComplete) then
		...
		-- Player is ready for first mission (has a tier 3 skill from training)
		elseif (not questOneComplete and SpaceHelpers:hasPilotTierSkill(pPlayer, "rebel_navy", 3)) then
	...
	-- New pilot to Aqzow (first meeting)
...
function aqzowConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	--[[
		Skill Training Menu - Dynamically add skill options based on missing Tier 3 skills
	]]
	if (screenID == "ready_train_pilot") then
		local responseString = "initial_"

		if (SpaceHelpers:isSpaceQuestComplete(pPlayer, HavocSquadronScreenplay.TIER3_QUEST_STRING_4.type, HavocSquadronScreenplay.TIER3_QUEST_STRING_4.name)) then
			responseString = "final_"
		elseif (SpaceHelpers:isSpaceQuestComplete(pPlayer, HavocSquadronScreenplay.TIER3_QUEST_STRING_3.type, HavocSquadronScreenplay.TIER3_QUEST_STRING_3.name)) then
			responseString = "mission4_"
		elseif (SpaceHelpers:isSpaceQuestComplete(pPlayer, HavocSquadronScreenplay.TIER3_QUEST_STRING_2.type, HavocSquadronScreenplay.TIER3_QUEST_STRING_2.name)) then
			responseString = "mission3_"
		elseif (SpaceHelpers:isSpaceQuestComplete(pPlayer, HavocSquadronScreenplay.TIER3_QUEST_STRING_1.type, HavocSquadronScreenplay.TIER3_QUEST_STRING_1.name)) then
			responseString = "mission2_"
		end

		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_starships_03")) then
			clonedConversation:addOption("@conversation/corellia_rebel_tier3:s_89f4d28f", responseString .. "train_fighters") -- I want to know about space superiority fighters.
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_weapons_03")) then
			clonedConversation:addOption("@conversation/corellia_rebel_tier3:s_a70b470e", responseString .. "train_components") -- I want to know about advanced starship components.
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_procedures_03")) then
			clonedConversation:addOption("@conversation/corellia_rebel_tier3:s_2d6808ba", responseString .. "train_procedures") -- I want to know about advanced space combat.
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_rebel_navy_droid_03")) then
			clonedConversation:addOption("@conversation/corellia_rebel_tier3:s_c28a300f", responseString .. "train_droid") -- I want to know about system balance programming.
		end
