...
function ramnaConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (string.find(screenID, "complete_mission")) then
		if (not CreatureObject(pPlayer):hasSkill("pilot_neutral_starships_03")) then
			clonedConversation:addOption("@conversation/corellia_privateer_tier3:s_3b154409", "train_warships") -- How about special warships?
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_neutral_weapons_03")) then
			clonedConversation:addOption("@conversation/corellia_privateer_tier3:s_44a12a06", "train_components") -- How about advanced starship component use?
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_neutral_procedures_03")) then
			clonedConversation:addOption("@conversation/corellia_privateer_tier3:s_25c1d490", "train_techniques") -- How about advanced techniques?
		end
		if (not CreatureObject(pPlayer):hasSkill("pilot_neutral_droid_03")) then
			clonedConversation:addOption("@conversation/corellia_privateer_tier3:s_88451b48", "train_programming") -- How about system balance programming?
		end

	-- Handle Skill box granting
	elseif (string.find(screenID, "train_")) then
		if (screenID == "train_warships") then
	...
	elseif (screenID == "got_it_100") then
		...
		-- REMOVE AFTER DELIVERY IMPLEMENTATION
		...
		--createEvent(2000, "delivery_corellia_privateer_tier3_3", "failQuest", pPlayer, "true")
	...
	elseif (screenID == "dont_care" or screenID == "keep_going" or screenID == "exactly1") then
		...
		-- REMOVE AFTER RECOVERY IMPLEMENTATION
