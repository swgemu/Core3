...
function rheaConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	local isNeutral = SpaceHelpers:isNeutralPilot(pPlayer)

	if (isNeutral and not SpaceHelpers:isCorsecSquadron(pPlayer)) then
	...
	-- Quests
	if (ghost:isJournalQuestActive(QuestManager.questCRC.QUEST_CORELLIA_PRIV_PATROL_1) or ghost:isJournalQuestActive(QuestManager.questCRC.QUEST_CORELLIA_PRIV_DESTROY_1)) then
...
function rheaConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "yes_im_ready") then
		print("activating quest for player: " .. SceneObject(pPlayer):getDisplayedName())

		-- Active Mission 1 Patrol
		ghost:activateJournalQuest(QuestManager.questCRC.QUEST_CORELLIA_PRIV_PATROL_1, true)
		ghost:activateJournalQuestTask(QuestManager.questCRC.QUEST_CORELLIA_PRIV_PATROL_1, 0, true)
		ghost:activateJournalQuestTask(QuestManager.questCRC.QUEST_CORELLIA_PRIV_PATROL_1, 1, true)

		ghost:activateJournalQuest(QuestManager.questCRC.QUEST_CORELLIA_PRIV_DESTROY_1, true)
		ghost:activateJournalQuestTask(QuestManager.questCRC.QUEST_CORELLIA_PRIV_DESTROY_1, 0, true)
		ghost:activateJournalQuestTask(QuestManager.questCRC.QUEST_CORELLIA_PRIV_DESTROY_1, 1, true)
