...
function rheaConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	local activeFirstQuest = (SpaceHelpers:isSpaceQuestActive(pPlayer, "patrol", "corellia_privateer_1") or SpaceHelpers:isSpaceQuestActive(pPlayer, "patrol", "corellia_destroy_1"))
	...
	elseif (not hasShip and not activeFirstQuest) then
	...
	elseif (not ghost:isJournalQuestComplete(QuestManager.questCRC.QUEST_CORELLIA_PRIV_PATROL_1)) then
...
function rheaConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "yes_im_ready") then
		...
		SpaceHelpers:activateSpaceQuest(pPlayer, pNpc, "patrol", "corellia_privateer_1")
