...
function GoToTheater:taskStart(pPlayer)
	...
	writeData(SceneObject(pPlayer):getObjectID() .. self.taskName .. "spawnEnterAreaId", SceneObject(pActiveArea):getObjectID())
	createObserver(ENTEREDAREA, self.taskName, "enteredTheaterSpawnArea", pActiveArea)

	pActiveArea = spawnActiveArea(CreatureObject(pPlayer):getZoneName(), "object/active_area.iff", spawnPoint[1], 0, spawnPoint[3], 300, 0)

	if (pActiveArea == nil) then
		return false
	end

	writeData(SceneObject(pPlayer):getObjectID() .. self.taskName .. "spawnExitAreaId", SceneObject(pActiveArea):getObjectID())
...
function GoToTheater:exitedTheaterSpawnArea(pActiveArea, pPlayer)
	...
	if (storedActiveAreaId == SceneObject(pActiveArea):getObjectID()) then
		createEvent(10000, self.taskName, "despawnTheaterObjects", pPlayer, "")
	end
...
function GoToTheater:taskFinish(pPlayer)
	...
	deleteData(SceneObject(pPlayer):getObjectID() .. self.taskName .. "spawnEnterAreaId")

	local activeAreaId = readData(playerID .. self.taskName .. "spawnExitAreaId")
	local pArea = getSceneObject(activeAreaId)
	...
	deleteData(SceneObject(pPlayer):getObjectID() .. self.taskName .. "spawnExitAreaId")
