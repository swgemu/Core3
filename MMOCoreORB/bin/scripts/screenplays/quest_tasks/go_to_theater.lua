...
function GoToTheater:taskStart(pPlayer)
	...
	while (spawnPoint == nil and attempts < 20) do
		...
		attempts = attempts + 1
		spawnPoint = getSpawnArea(zoneName, SceneObject(pPlayer):getWorldPositionX(), SceneObject(pPlayer):getWorldPositionY(), self.minimumDistance + distDiff, self.maximumDistance + distDiff, 20, 10, true)
	end
