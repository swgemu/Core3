...
function ThemeParkLogic:spawnDestroyBuilding(mission, pConversingPlayer)
	...
	if buildingData == nil then
		...
		return false
	elseif buildingData.type ~= "destructible" or buildingData.building == nil or buildingData.terminal == nil then
		printf("invalid destructible building data in ThemeParkLogic:spawnDestroyBuilding for %s", self.className);
		return false
	end
	...
	return ObjectManager.withCreatureObject(pConversingPlayer, function(creature)
		...
		local buildingObject = LuaBuildingObject(pBuilding)
...
function ThemeParkLogic:spawnMissionNpcs(mission, pConversingPlayer)
	...
	if (currentMissionType == "destroy") then
		...
		numberOfChildNpcs = table.getn(childNpcs)
