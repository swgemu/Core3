...
function ThemeParkLogic:followPlayer(pConversingNpc, pConversingPlayer)
	...
	if (playerFaction == FACTIONREBEL or playerFaction == FACTIONIMPERIAL) and not CreatureObject(pConversingNpc):isOnLeave() then
		CreatureObject(pConversingNpc):setFaction(playerFaction)

		if CreatureObject(pConversingNpc):isOvert() then
