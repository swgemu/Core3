...
function ThemeParkLogic:handleMissionAccept(npcNumber, missionNumber, pConversingPlayer)
	...
	if (mission.staticLoc ~= nil and #mission.staticLoc > 0) then
		...
		if (areaSpawnPoint == nil) then
			areaSpawnPoint = getSpawnPoint(zoneName, SceneObject(pConversingPlayer):getWorldPositionX(), SceneObject(pConversingPlayer):getWorldPositionY(), spawnDistance * 0.75, ((spawnDistance / 2) * 3) * 1.25, true)
		end
	end
