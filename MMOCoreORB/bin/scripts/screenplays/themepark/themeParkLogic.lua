...
function ThemeParkLogic:spawnDestroyMissionNpcs(mission, pConversingPlayer)
	...
	for i = 1, numberOfChildNpcs, 1 do
		...
		local pNpc = spawnMobile(buildingData.building.planet, childNpcs[i].npcTemplate, 0, childNpcs[i].x, childNpcs[i].z, childNpcs[i].y, getRandomNumber(-180, 180), targetCellObject:getObjectID())
...
function ThemeParkLogic:spawnNpc(npcTemplate, position, pConversingPlayer, spawnNumber)
	...
	local pNpc = spawnMobile(npcTemplate.planetName, npcTemplate.npcTemplate, 0, position[1], position[2], position[3], getRandomNumber(-180, 180), position[4])
