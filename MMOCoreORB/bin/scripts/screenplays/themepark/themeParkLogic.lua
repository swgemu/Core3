...
function ThemeParkLogic:spawnNpcs()
	...
	for i = 1, # self.npcMap do
		...
		if not isZoneEnabled(planetName) then
			printf("ERROR: Unable to spawn quest NPC " .. npcSpawnData.npcTemplate .. " for screenplay " .. self.className .. ", zone " .. planetName .. " is not enabled.\n")
			return false
		end
...
function ThemeParkLogic:spawnSceneObjects()
	...
	for i = 1, # self.sceneObjectMap do
		...
		if not isZoneEnabled(planetName) then
			printf("ERROR: Unable to spawn scene object " .. objectSpawnData.objectTemplate .. " for screenplay " .. self.className .. ", zone " .. planetName .. " is not enabled.\n")
			return false
		end
...
function ThemeParkLogic:permissionObservers()
	...
	for i = 1, # self.permissionMap, 1 do
		...
		if not isZoneEnabled(permission.planetName) then
			printf("ERROR: Unable to spawn permission observer for screenplay " .. self.className .. ", zone " .. permission.planetName .. " is not enabled.\n")
			return
		end
...
function ThemeParkLogic:spawnDestroyBuilding(mission, pConversingPlayer, pActiveArea)
	...
	local pBuilding = spawnBuilding(pConversingPlayer, buildingData.building.template, SceneObject(pActiveArea):getWorldPositionX(), SceneObject(pActiveArea):getWorldPositionY(), 0)
...
function ThemeParkLogic:spawnMissionNpcs(mission, pConversingPlayer, pActiveArea)
	...
	if (currentMissionType == "destroy") then
		...
		spawnPoints = self:getSpawnPoints(numberOfSpawns, SceneObject(pActiveArea):getWorldPositionX(), SceneObject(pActiveArea):getWorldPositionY(), pConversingPlayer)
	end
	...
	for i = 1, #mission.primarySpawns, 1 do
		local pNpc = self:spawnNpc(mainNpcs[i], spawnPoints[i], pConversingPlayer, i, mission)
		...
		if i == 1 then
			if (self:isValidConvoString(stfFile, ":npc_breech_" .. missionNumber)) then
				local planetName
				if (mainNpcs[i].genericQuest ~= nil and mainNpcs[i].genericQuest == true) then
				...
				if planetName == nil or planetName == "" then
					printf("ERROR: Unable to spawn breech active area for mission number " .. missionNumber .. " for screenplay " .. self.className .. ", planetName is invalid.\n")
					return false
				end
	...
	for i = 1 + #mission.primarySpawns, numberOfSpawns, 1 do
		...
		local pNpc = self:spawnNpc(secondaryNpc, spawnPoints[i], pConversingPlayer, i, mission)
...
function ThemeParkLogic:spawnDestroyMissionNpcs(mission, pConversingPlayer)
	...
	if (planetName == nil or planetName == "") then
		printf("ERROR: Unable to spawn destroy mission npcs in screenplay " .. self.className .. ", planetName invalid.\n")
		return false
	end
...
function ThemeParkLogic:spawnNpc(npcTemplate, position, pConversingPlayer, spawnNumber, mission)
	if pConversingPlayer == nil then
		return nil
	end

	local planetName
	if (npcTemplate.genericQuest ~= nil and npcTemplate.genericQuest == true) then
		planetName = CreatureObject(pConversingPlayer):getZoneName()
	else
		local planetName = self.planetName
	...
	if (planetName == nil or planetName == "") then
		printf("ERROR: Unable to spawn quest target " .. npcTarget .. " in screenplay " .. self.className .. ", planetName invalid.\n")
		return nil
	end
...
function ThemeParkLogic:createEscortReturnArea(pNpc, pPlayer)
	...
	if (self:isValidConvoString(stfFile, ":npc_dropoff_" .. missionNumber) and (escortAreaID == nil or escortAreaID == 0)) then
		...
		if (planetName == nil or planetName == "") then
			printf("ERROR: Unable to spawn escort return active area for mission number " .. missionNumber .. " in screenplay " .. self.className .. ", planetName invalid.\n")
			return
		end
