...
function ThemeParkNym:spawnLootObjects()
	...
	if (droidPointer == nil) then
		return 0
	end
	local droidContainer = LuaSceneObject(droidPointer)
	...
	if (afuPointer == nil) then
		return 0
	end
	local afuContainer = LuaSceneObject(afuPointer)
	...
	if (csuPointer == nil) then
		return 0
	end
	local csuContainer = LuaSceneObject(csuPointer)
	...
	if (cratePointer == nil) then
		return 0
	end
	local crateContainer = LuaSceneObject(cratePointer)
...
function ThemeParkNym:notifyNymContainerUsed(pDroid, pPlayer, radialSelected)
	...
	local container = LuaSceneObject(pDroid)
	local questItem, questMsg

	local cPlayer = LuaCreatureObject(pPlayer)
...
function ThemeParkNym:completePark(pPlayer)
	...
	local player = LuaSceneObject(pPlayer)
	local playerCreo = LuaCreatureObject(pPlayer)
	local pInventory = player:getSlottedObject("inventory")
...
function NymContainerComponent:transferObject(pContainer, pObj, slot)
	local object = LuaSceneObject(pObj)
	...
	local nymNpc = LuaSceneObject(pContainer)
...
function ThemeParkNym:spawnNpcs()
	for i = 1, # self.npcMap do
		...
		if isZoneEnabled(npcSpawnData.planetName) then
			...
			if (pNpc ~= nil) then
				local nymNpc = LuaSceneObject(pNpc)
