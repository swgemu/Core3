...
function VillageRaids:doEnemySpawnPulse()
	...
	for i = 1, #spawnWaveData, 1 do
		...
		local loc = getSpawnPoint("dathomir", self.enemySpawnLocs[i][1], self.enemySpawnLocs[i][2], self.enemyData.minDistance, self.enemyData.maxDistance, true)
...
function VillageRaids:getPlayersInVillage(pMaster)
	...
	local nearbyObjects = SceneObject(pMaster):getPlayersInRange(192)
	local nearbyPlayers = 0

	for i = 1, #nearbyObjects, 1 do
		if (nearbyObjects[i] ~= nil) then
...
function VillageRaids:spawnVictims()
	local pMaster = VillageJediManagerTownship:getMasterObject()

	if (pMaster == nil) then
		printf("Error in VillageRaids:spawnVictims(), unable to get master village object.\n")
		return
	end
