...
-- The valid questStatus val's for branch unlock are as follows:
-- (none): Locked branch.
-- unlocked: player has unlocked the branch, but has not finished mastering it yet.
-- trained: player has trained the top box in the branch.
...
function ExperienceConverter:getNextUnlockableBranches(pCreatureObject)
	...
	foreach(unlockableFSBranches, function(theTable)
		local checkTrees = getQuestStatus(CreatureObject(pCreatureObject):getObjectID() .. ":" .. theTable.topBox)
		if (checkTrees ~= nil and checkTrees ~= "unlocked") then
...
function ExperienceConverter:setBranchUnlocked(oid, branchBox)
	foreach(unlockableFSBranches, function(theTable)
		if (theTable.topBox == branchBox) then
			setQuestStatus(oid .. ":" .. theTable.topBox, "unlocked")
		end
	end)
end
