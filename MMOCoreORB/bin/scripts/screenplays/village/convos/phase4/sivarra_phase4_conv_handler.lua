...
function villageSivarraPhase4ConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	elseif (QuestManager.hasCompletedQuest(pPlayer, QuestManager.quests.FS_COMBAT_HEALING_1) and healCount >= 100) then
		return convoTemplate:getScreen("intro_give_second_set_reward")
	elseif (not QuestManager.hasCompletedQuest(pPlayer, QuestManager.quests.FS_COMBAT_HEALING_1) and healCount >= 50) then
