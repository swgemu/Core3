...
function villageChiefEngineerPhase4ConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "waypoint_keren") then
		PlayerObject(pGhost):addWaypoint("naboo", "Keren - Gadget Specialist", "", 1215, 0, 2740, WAYPOINT_YELLOW, true, true, WAYPOINTQUESTTASK)
		FsCrafting4:activateQuest(pPlayer)
	elseif (screenID == "waypoint_coronet") then
		PlayerObject(pGhost):addWaypoint("corellia", "Coronet - Gadget Specialist", "", 19, 0, -4775, WAYPOINT_YELLOW, true, true, WAYPOINTQUESTTASK)
		FsCrafting4:activateQuest(pPlayer)
	elseif (screenID == "waypoint_mos_entha") then
		PlayerObject(pGhost):addWaypoint("tatooine", "Mos Entha - Gadget Specialist", "", 1209, 0, 2923, WAYPOINT_YELLOW, true, true, WAYPOINTQUESTTASK)
		FsCrafting4:activateQuest(pPlayer)
