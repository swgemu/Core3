...
function fs_experience_converter_conv_handler:notifyBranchUnlocked(pCreature, pSui, cancelPressed, arg0)
	...
	local oid = tostring(CreatureObject(pCreature):getObjectID())
	
	local string = oid .. ":" .. branchTier4
	
	CreatureObject(pCreature):sendSystemMessage(string)
