...
function fs_experience_converter_conv_handler:transferExperiencePoints(pCreature, pSui, cancelPressed, arg0, arg1)
	...
	if (experienceTypeFS ~= nil) then
		ObjectManager.withCreatureAndPlayerObject(pCreature, function(creatureObject, playerObject)
			...
			-- If they are capped (or will be), don't let them transfer any more xp.
			if ((playerObject:getExperience(expFsUnformatted)) >= 510000 or (playerObject:getExperience(expFsUnformatted) + tonumber(arg1) >= 510000)) then
			...
			messageString:setDI(playerObject:getExperience(optionsNameFromUnformatted) - tonumber(arg0))
			creatureObject:awardExperience(optionsNameFromUnformatted, tonumber(arg0) - playerObject:getExperience(optionsNameFromUnformatted))
