...
function fs_experience_converter_conv_handler:transferExperiencePoints(pCreature, pSui, cancelPressed, arg0, arg1)
	...
	if (experienceTypeFS ~= nil) then
		ObjectManager.withCreatureAndPlayerObject(pCreature, function(creatureObject, playerObject)
			...
			-- If they are capped (or will be), don't let them transfer any more xp. The cap for any FS exp type seems to be 510000.
			if (expFsCurrent >= 510000 or (expFsCurrent + tonumber(arg1) >= 510000)) then
				creatureObject:sendSystemMessage("@quest/force_sensitive/utils:convert_at_fs_skill_cap")
				return
			elseif ((tonumber(arg0) - expNormal) < expNormal) then -- Don't go negative xp.
			...
			messageString:setDI(expNormal - tonumber(arg0))
			creatureObject:awardExperience(optionsNameFromUnformatted, tonumber(arg0) - expNormal)
