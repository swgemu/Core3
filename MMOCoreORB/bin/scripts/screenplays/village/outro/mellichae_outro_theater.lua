...
function MellichaeOutroTheater:onMellichaeKilled(pMellichae, pKiller)
	...
	if (CreatureObject(pOwner):isDead() or CreatureObject(pOwner):isIncapacitated()) then
		return 1
	end
...
function MellichaeOutroTheater:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	createEvent(5 * 1000, "MellichaeOutroTheater", "finish", pPlayer, "")
...
function MellichaeOutroTheater:doHealingPulse(pPlayer)
	...
	if (pDaktar ~= nil and numRed > 0) then
	...
	if (pMellichae ~= nil and numRed > 0) then
...
function MellichaeOutroTheater:onTheaterDespawn(pPlayer)
	...
	for i = 1, 2, 1 do
		...
		local shrineID = readData(playerID .. ":powershrines:green:" .. i)
