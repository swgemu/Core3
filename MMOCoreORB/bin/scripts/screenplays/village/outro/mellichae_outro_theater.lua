...
function MellichaeOutroTheater:onLoot(pLootedCreature, pLooter, nothing)
	...
	local oidOfMellichae = SceneObject(pLooter):getObjectID() .. ":onMellichaeMission:oid"
	
	if (oidOfMellchae == nil) or (oidOfMellichae ~= SceneObject(pLootedCreature):getObjectID()) then
		return 1
	end
...
function MellichaeOutroTheater:onEnteredActiveArea(pCreatureObject, spawnedSithShadowsList)
	...
	local oidOfMellichae = SceneObject(pCreatureObject):getObjectID() .. ":onMellichaeMission:oid"
