...
function FsCounterStrike:giveCampWaypoint(pPlayer)
	...
	ObjectManager.withCreaturePlayerObject(pPlayer, function(playerObject)
		playerObject:addWaypoint("dathomir", "Aurilian Enemy", "", randCamp[2], randCamp[4], WAYPOINTYELLOW, true, true, 0)
	end)
end
...
function FsCounterStrike:resetCamp(pTheater, attackerID)
	...
	for i = 1, #self.campLayout, 1 do
		...
		if (objectData[1] == "object/installation/battlefield/destructible/bfield_base_gate_impl.iff" or objectTemplate == "object/installation/battlefield/destructible/antenna_tatt_style_1.iff") then
			local pObject = spawnSceneObject("dathomir", objectData[1], campLoc[2] + objectData[2], campLoc[3]  + objectData[3], campLoc[4] + objectData[4], 0, math.rad(objectData[5]))
