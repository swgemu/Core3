...
function FsCounterStrike:spawnLootMobGroup(pTheater)
	...
	local pSpawner = QuestSpawner:createQuestSpawner("FsCounterStrike", "lootMobDataTable", "lootMobSpawnList", spawnPoint[1], spawnPoint[2], spawnPoint[3], 0, "dathomir", pTheater)

	if (pSpawner ~= nil) then
		writeData(SceneObject(pSpawner):getObjectID() .. ":theaterID", theaterID)
	end
end
...
function FsCounterStrike:spawnCamps()
	...
	for i = 1, #campTable, 1 do
		...
		printf("Spawning camp " .. campName .. " at " .. campLoc[2] .. " " .. campLoc[4] .. "\n")
		...
		for i = 1, #self.campLayout, 1 do
			...
			if (pObject ~= nil) then
				...
				elseif (objectTemplate == "object/installation/battlefield/destructible/antenna_tatt_style_1.iff") then
					...
					createObserver(OBJECTDESTRUCTION, "FsCounterStrike", "notifyDestructibleDisabled", pObject)
					TangibleObject(pObject):setOptionBit(INVULNERABLE)
...
function FsCounterStrike:spawnDefenseWaves(pTheater)
	printf("FsCounterStrike:spawnDefenseWaves(pTheater)\n")
	if (pTheater == nil) then
		return
	end
	...
	for i = 1, #waveData, 1 do
		...
		printf("Created spawner at " .. spawnPoint[1] .. " " .. spawnPoint[3] .. "\n")
