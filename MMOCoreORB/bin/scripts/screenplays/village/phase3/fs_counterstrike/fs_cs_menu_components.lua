...
function FsCampCommanderMenuComponent:handleObjectMenuSelect(pCommander, pPlayer, selectedID)
	...
	if (selectedID == 20) then
		...
		if (shieldKillerID ~= playerID or not CreatureObject(pShieldKiller):isGroupedWith(pPlayer) or not QuestManager.hasActiveQuest(pPlayer, QuestManager.quests.FS_CS_INTRO)) then
			CreatureObject(pPlayer):sendSystemMessage("@fs_quest_village:camp_commander_no_auth")
			return 0
		end
