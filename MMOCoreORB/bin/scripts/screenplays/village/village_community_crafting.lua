...
function VillageCommunityCrafting:getCurrentPhaseSchematic()
	local currentPhase = VillageJediManagerTownship.getCurrentPhase()

	if (currentPhase ~= 2 and currentPhased ~= 3) then
		printf("Error in VillageCommunityCrafting:getCurrentPhaseSchematic(), trying to get schematic during non community crafting phase.\n")
		return ""
	end

	return self.phaseInfo[currentPhase].schematic
end
...
function VillageCommunityCrafting:activateCraftingSession(masterSchematic)
...
function VillageCommunityCrafting:createProjectStatsTables()
	...
	local pMap = createQuestVectorMap("VillageCCStatsTable:" .. phaseID .. ":quality")
...
function VillageCommunityCrafting:sendPlayerProjectSlotAttributes(pPlayer, pNpc, type, slot)
	...
	if (type == "quality") then
		...
		table.sort(statTable, self.quantitySory)
	end
...
function VillageCommunityCrafting.quantitySory(a,b)
	return a[3] < b[3]
end
...
function VillageCommunityCrafting:calculateIngredientQuality(pObj, pPlayer)
	...
	for i = 1, #ingredientData.attributes, 1 do
		...
		printf("attrib " .. attribName .. " value " .. attribValue .. "\n")
	...
	if (finalQuality > 0) then
		...
		printf("Final quality: " .. finalQuality .. "\n")
