...
function fs_experience_converter_conv_handler:runScreenHandlers(pConversationTemplate, pConversingPlayer, pConversingNpc, selectedOption, pConversationScreen)
	...
	elseif screenID == "cs_jsPlumb_1_11" then -- Transferring Experience.
		fs_experience_converter_conv_handler:chooseExperienceTypeDialog(pConversingPlayer)
	end

	return pConversationScreen
end

function fs_experience_converter_conv_handler:chooseExperienceTypeDialog(pCreatureObject)
		local suiManager = LuaSuiManager()
		local options = {"Combat Prowess", "Heightened Senses", "Enhanced Reflexes", "Crafting Mastery"}
		suiManager:sendListBox(pCreatureObject, pCreatureObject, "@sui:swg", "@ui:experience_name", 2, "@ok", "", "@cancel", "fs_experience_converter_conv_handler", "chooseExperienceTypeForRatio", options)
end

function fs_experience_converter_conv_handler:chooseExperienceTypeForRatio(pCreature, pSui, cancelPressed, arg0)
		local suiManager = LuaSuiManager()
		local options = ExperienceConverter:getExperienceForConversion(pCreature, tonumber(arg0))
