...
function fs_experience_converter_conv_handler:notifyTransfer(pCreature, pSui, cancelPressed, arg0)
  if (not cancelPressed) then
    ...
    local optionsTo = {optionsNameFrom, amount, ratio}
    local optionsFrom = {"@exp_n:" .. optionsNameTo, 0, 1}
    local suiManager = LuaSuiManager()
    suiManager:sendTransferBox(pCreature, pCreature, "@sui:swg", "Transfer Experience", "@ok", "fs_experience_converter_conv_handler", "transferExperiencePoints", optionsFrom, optionsTo)
  end
end

function fs_experience_converter_conv_handler:transferExperiencePoints(pCreature, pSui, cancelPressed, arg0, arg1, arg2)
  if (not cancelPressed) then
