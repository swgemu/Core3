...
function trainerConvHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "msg2_1") then
		...
		if (#skillList == 0) then
			...
			pConvScreen = self:addSkillResponses(pConvScreen, skillList, stringTable, "learn_")
		end
	elseif (screenID == "msg2_2") then
		skillList = SkillTrainer:getTeachableSkills(pPlayer, trainerType, false)
		pConvScreen = self:addSkillResponses(pConvScreen, skillList, stringTable, "info_")
	...
	elseif (string.find(screenID, "nsf_skill_points_")) then
		skillList = SkillTrainer:getTeachableSkills(pPlayer, trainerType, true)
		local skillNum = tonumber(string.sub(screenID, 18))
	...
	elseif (string.find(screenID, "confirm_learn_")) then
		skillList = SkillTrainer:getTeachableSkills(pPlayer, trainerType, true)
		local skillNum = tonumber(string.sub(screenID, 15))
	...
	elseif (string.find(screenID, "learn_")) then
		skillList = SkillTrainer:getTeachableSkills(pPlayer, trainerType, true)
		local skillNum = tonumber(string.sub(screenID, 7))
	...
	elseif (string.find(screenID, "info_")) then
		skillList = SkillTrainer:getTeachableSkills(pPlayer, trainerType, false)
		local skillNum = tonumber(string.sub(screenID, 6))
...
function trainerConvHandler:addSkillResponses(pConvScreen, skillList, stringTable, optionTag)
	...
	for i = 1, #skillList, 1 do
		clonedConversation:addOption("@skl_n:" .. skillList[i], optionTag .. curResponse)
		curResponse = curResponse + 1
	end
...
function trainerConvHandler:handleLearnScreen(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen, trainerType, stringTable, skillList, skillNum)
	...
	clonedConversation:addOption("@skill_teacher:yes", "confirm_learn_" .. skillNum)
