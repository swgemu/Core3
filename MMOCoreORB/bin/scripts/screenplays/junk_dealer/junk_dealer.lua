...
function JunkDealer:getEligibleJunk(pInventory, dealerNum)
	...
	for i = 0, SceneObject(pInventory):getContainerObjectsSize() - 1, 1 do
		...
		if pItem ~= nil then
			...
			if tano:getJunkDealerNeeded() == dealerNum and tano:getCraftersName() == "" and not tano:isBroken() and not tano:isSliced() and not tano:isNoTrade() and sceno:getContainerObjectsSize() == 0 then
