...
function GeonosianLabScreenPlay:spawnSceneObjects()
	...
	ObjectManager.withSceneObject(pGasLeak, function(gasLeak)
		writeData("geonosian_lab:gasLeakID", gasLeak:getObjectID()) --set the gas leak as started
	end)
	...
	for i = 1, 8, 1 do
		...
		ObjectManager.withSceneObject(pSceneObject, function(sceneObject)
			writeData(sceneObject:getObjectID() .. ":geonosian_lab:keypad_index", i)
	...
	for i = 1, 15, 1 do
		...
		ObjectManager.withSceneObject(pDebris, function(debris)
			writeData(debris:getObjectID() .. ":geonosian_lab:debris_index", i)
...
function GeonosianLabScreenPlay:spawnMobiles()
	...
	spawnMobile("yavin4", "acklay",7200,101.1,-34.3,-321.6,-136,1627823)
	spawnMobile("yavin4", "enhanced_gaping_spider",3600,-130,-22.1,-85,-180,1627794)
	...
	if (radialSelected == 20) then
		ObjectManager.withCreatureObject(pPlayer, function(player)
			...
			if (isGasLeaking == 1) then
				...
				ObjectManager.withSceneObject(pGasLeak, function(gasLeak)
					gasLeak:destroyObjectFromWorld()
...
function GeonosianLabScreenPlay:restartGasLeak()
	local pGasLeak = spawnSceneObject("yavin4", "object/tangible/dungeon/poison_gas_cloud.iff", 21.7, 3.3, -29.1, 1627783, 0, 0, 0, 0)
	ObjectManager.withSceneObject(pGasLeak, function(gasLeak)
		writeData("geonosian_lab:gasLeakID", gasLeak:getObjectID())
	end)
...
function GeonosianLabScreenPlay:keypadSuiCallback(pCreature, pSui, cancelPressed, enteredCode)
	...
	ObjectManager.withSceneObject(pUsingObject, function(usingObject)
		...
		printf("Code entered: " .. enteredCode .. ", correct code: " .. keypadCode .. "\n")
...
function GeonosianLabScreenPlay:notifyEnteredPoisonGas(pActiveArea, pMovingObject)
	return ObjectManager.withSceneObject(pMovingObject, function(movingObject)
...
function GeonosianLabScreenPlay:hasRebreather(scno)
	...
	return ObjectManager.withSceneObject(pRebreather, function(rebreather)
...
function GeonosianLabScreenPlay:respawnDebris(pDebris)
	ObjectManager.withTangibleObject(pDebris, function(debris)
...
function GeonosianLabScreenPlay:hasGeoItem(pPlayer, itemTemplate)
	return ObjectManager.withSceneObject(pPlayer, function(player)
		...
		local pDatapadPack
		local pDatapadMain = getContainerObjectByTemplate(pInventory, itemTemplate, true)

		if (pBackpack ~= nil) then
			pDatapadPack = getContainerObjectByTemplate(pBackpack, itemTemplate, true)
		end
...
function GeonosianLabScreenPlay:removeGeoItem(pPlayer, itemTemplate)
	return ObjectManager.withSceneObject(pPlayer, function(player)
		...
		if (pDatapadMain ~= nil) then
			ObjectManager.withSceneObject(pDatapadMain, function(item)
				item:destroyObjectFromWorld()
				item:destroyObjectFromDatabase()
			end)
		end
		if (pDatapadPack ~= nil) then
			ObjectManager.withSceneObject(pDatapadPack, function(item)
				item:destroyObjectFromWorld()
				item:destroyObjectFromDatabase()
			end)
		end
...
function GeonosianLabScreenPlay:giveGeoItem(pPlayer, itemTemplate)
	ObjectManager.withSceneObject(pPlayer, function(player)
		...
		ObjectManager.withPlayerObject(pPlayer, function(playerObject)
			local pInventory = player:getSlottedObject("inventory")
