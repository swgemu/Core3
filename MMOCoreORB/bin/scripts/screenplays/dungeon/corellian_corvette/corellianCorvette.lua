...
CorellianCorvetteScreenPlay = ScreenPlay:new {
		...
		patrol1 = {
			{0,0,30,"hall3"}, {0, 0, 10,"hall3"}, {0, 0, 10.3, "hall3"}, {-0.2, 0.0, 2.5, "hall2"}, {-2.7, 0, 0.2,"hall2"}, {-25.25,0, 0, "hall2"}
		},
		...
		escapePoints = {
			{ faction = "neutral", planet = "tatooine", x = -6150, y = -6350 },
			{ faction = "imperial", planet = "naboo", x = 2430, y = -3415 },
		...
		vipComputerTerminals = {
			{ x = 4.03, z = 12.75, y = -1.45, cell =  "vipquarters21", dw = 0, dx = 0, dy = 1, dz = 0 } -- vip computer (imp or reb only)
		},
		...
		bridgeControlTerminal = { --object/tangible/dungeon/wall_terminal_s4.iff
			{ x = -7.5, z = 0, y = 152, cell = "bridge66", dw =0.707107, dx = 0, dy = 0.707107, dz = 0, name = "Bridge Controls" , quest="engine"}
		},

		elevatorControlTerminal = {--  object/tangible/dungeon/terminal_free_s1.iff
...
function CorellianCorvetteScreenPlay:initialize()
	...
	for i = 1, #self.buildings do
		...
		for j = 1, #building.buildingIds do
...
function CorellianCorvetteScreenPlay:lockRooms(pCorvette)
	...
	for i = 1 ,#self.lockedRooms, 1 do
		...
		if (pCell ~= nil) then
			...
			SceneObject(pCell):setContainerAllowPermission(corvetteID..self.lockedRooms[i], WALKIN)
			SceneObject(pCell):setContainerDenyPermission(corvetteID..self.lockedRooms[i], MOVEIN)
		end
	end
end
...
function CorellianCorvetteScreenPlay:givePermission(pPlayer, permissionGroup)
	...
	if (pPlayerObj ~= nil) then
		PlayerObject(pPlayerObj):addPermissionGroup(permissionGroup, true)
	end
end
...
function CorellianCorvetteScreenPlay:removePermission(pPlayer, permissionGroup)
	...
	if (pPlayerObj ~= nil) then
		if (PlayerObject(pPlayerObj):hasPermissionGroup(permissionGroup)) then
			PlayerObject(pPlayerObj):removePermissionGroup(permissionGroup, true)
		end
	end
end
...
function CorellianCorvetteScreenPlay:spawnSceneObjects(pCorvette)
	...
	for i = 1, #self.escapeTerminals, 1 do
		...
		local pCell = BuildingObject(pCorvette):getNamedCell(escTerm.cell)
		if (pCell ~= nil) then
			local cellID = SceneObject(pCell):getObjectID()
			local pObject = spawnSceneObject("dungeon1", "object/tangible/terminal/terminal_geo_bunker.iff", escTerm.x, escTerm.z, escTerm.y, cellId, escTerm.dw, escTerm.dx, escTerm.dy, escTerm.dz)
			if (pObject ~= nil) then
				SceneObject(pObject):setCustomObjectName("Escape Pod Controls")
				createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyEscapePod", pObject)
				local pActiveArea = spawnActiveArea("dungeon1", "object/active_area.iff", SceneObject(pObject):getWorldPositionX(), SceneObject(pObject):getWorldPositionZ(), SceneObject(pObject):getWorldPositionY(), 3, 0)
	...
	for i = 1, #self.keypadTerminals do
		local kp = self.keypadTerminals[i]
		local pCell = BuildingObject(pCorvette):getNamedCell(kp.cell)
		if (pCell ~= nil) then
			local cellId = SceneObject(pCell):getObjectID()
			local pObject = spawnSceneObject("dungeon1", "object/tangible/dungeon/keypad_terminal.iff", kp.x, kp.z, kp.y, cellId, kp.dw, kp.dx, kp.dy, kp.dz)
			if (pObject ~= nil) then
				writeData(corvetteId .. kp.room,SceneObject(pObject):getObjectID())
				writeStringData(SceneObject(pObject):getObjectID()..":room",kp.room)
				writeData(SceneObject(pObject):getObjectID()..":locked",1)
				createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyKeypadUsed", pObject)
			end
		end
	end
	...
	for i = 1,#self.computerTerminals,1 do
		...
		local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(ct.cell)):getObjectID()
		...
		if (spawnedPointer ~= nil) then
			writeData(BuildingObject(pCorvette):getObjectID()..":"..ct.name,SceneObject(spawnedPointer):getObjectID())
			writeStringData(SceneObject(spawnedPointer):getObjectID() .. ":computerName",ct.name)
			writeData(BuildingObject(pCorvette):getObjectID()..":"..ct.name..":used",0)
	...
	for i = 1,4,1 do
		...
		local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(et.cell)):getObjectID()
	...
	-- spawn electric trap
	local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell("thrustersubroom28")):getObjectID()
	for i = 1,#self.electric_trap,1 do
		...
		if pShock ~= nil then
			writeData(BuildingObject(pCorvette):getObjectID()..":shock"..i, SceneObject(pShock):getObjectID())
	...
	writeData(BuildingObject(pCorvette):getObjectID()..":electricity_on",1)
	...
	writeData(BuildingObject(pCorvette):getObjectID()..":pDroid1", SceneObject(pDroid1):getObjectID())
	writeData(BuildingObject(pCorvette):getObjectID()..":pDroid2", SceneObject(pDroid2):getObjectID())
	writeData(BuildingObject(pCorvette):getObjectID()..":DroidFixed", 0)

	for i = 1,#self.computerMainframeOne,1 do
		local cm = self.computerMainframeOne[i]
		local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(cm.cell)):getObjectID()
		...
		writeData(BuildingObject(pCorvette):getObjectID()..":"..cm.name..":used",0)
		createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyComputerUsed", spawnedPointer)
	end
	...
	for i = 1,#self.computerMainframeTwo,1 do
		...
		writeData(BuildingObject(pCorvette):getObjectID()..":"..cm.quest,0)
		SceneObject(spawnedPointer):setObjectMenuComponent("CorvetteTerminalMenuComponent")
	end

	local eCT = self.elevatorControlTerminal[1]
	local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(eCT.cell)):getObjectID()
	local pObject = spawnSceneObject("dungeon1", "object/tangible/dungeon/terminal_free_s1.iff", eCT.x, eCT.z, eCT.y, cellId, eCT.dw, eCT.dx, eCT.dy, eCT.dz)
	SceneObject(pObject):setCustomObjectName(eCT.name)
	writeData(BuildingObject(pCorvette):getObjectID()..":"..eCT.quest,SceneObject(pObject):getObjectID())
	writeStringData(SceneObject(pObject):getObjectID() .. ":computerName",eCT.quest)
	writeData(BuildingObject(pCorvette):getObjectID()..":"..eCT.quest,0)
	createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyComputerUsed", pObject)

	local bCT = self.bridgeControlTerminal[1]
	local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(bCT.cell)):getObjectID()
	local pObject = spawnSceneObject("dungeon1", "object/tangible/dungeon/wall_terminal_s4.iff", bCT.x, bCT.z, bCT.y, cellId, bCT.dw, bCT.dx, bCT.dy, bCT.dz)
	SceneObject(pObject):setCustomObjectName(bCT.name)
	writeStringData(SceneObject(pObject):getObjectID() .. ":quest",bCT.quest)
	writeData(corvetteId..":"..bCT.quest,0)
	SceneObject(pObject):setObjectMenuComponent("CorvetteTerminalMenuComponent")

	local sr = self.security38[1]
	local cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(sr.cell)):getObjectID()
	local pObject = spawnSceneObject("dungeon1", "object/tangible/dungeon/computer_mainframe.iff", sr.x, sr.z, sr.y, cellId, sr.dw, sr.dx, sr.dy, sr.dz)
	SceneObject(pObject):setCustomObjectName(sr.name)
	writeStringData(SceneObject(pObject):getObjectID() .. ":computerName",sr.quest)
	writeData(BuildingObject(pCorvette):getObjectID()..":"..sr.quest,0)
	createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyComputerUsed", pObject)

	if (self:getBuildingFaction(pCorvette) == "imperial") or (self:getBuildingFaction(pCorvette) == "rebel") then
		local it = self.vipComputerTerminals[1]
		local pCell = BuildingObject(pCorvette):getNamedCell(it.cell)
		if (pCell ~= nil) then
			local cellId = SceneObject(pCell):getObjectID()
			local pObject = spawnSceneObject("dungeon1", "object/tangible/dungeon/computer_desktop.iff", it.x, it.z, it.y, cellId, it.dw, it.dx, it.dy, it.dz)
			writeData(corvetteId..":"..SceneObject(pObject):getObjectID())
			if self:getBuildingFaction(pCorvette) == "rebel" then
				writeStringData(SceneObject(pObject):getObjectID() .. ":quest","Nine")
			else
				writeStringData(SceneObject(pObject):getObjectID() .. ":quest","Ten")
			end
			createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "notifyComputerUsed", pObject)
		end
	end
...
function CorellianCorvetteScreenPlay:spawnMobs(pCorvette, faction)
	if (pCorvette == nil) or (faction == "") then
		return
	end
	local cellId = 0
	local corvetteId = SceneObject(pCorvette):getObjectID()
	...
	if faction == "rebel" then
		for i,v in ipairs(rebelStaticSpawns) do
			print("spawning rebelStaticSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
			--AiAgent(pNpc):setFollowState(4)
		end

		for i,v in ipairs(rebelPatrolSpawns) do
			print("spawning rebelPatrolSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			pNpc = spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
		end
	elseif faction == "imperial" then
		for i,v in ipairs(imperialStaticSpawns) do
			print("spawning imperialStaticSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			local pNpc = spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
		end

		for i,v in ipairs(imperialPatrolSpawns) do
			print("spawning imperialPatrolSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			local pNpc = spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
			--AiAgent(pNpc):generatePatrol(1, 30)
			--AiAgent(pNpc):setFollowState(4)
		end
	elseif faction == "neutral" then
		for i,v in ipairs(neutralStaticSpawns) do
			print("spawning neutralStaticSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
			--AiAgent(pNpc):setFollowState(4)
		end
		for i,v in ipairs(neutralPatrolSpawns) do
			print("spawning neutralPatrolSpawns ",v[1])
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			local pNpc = spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
		end
		for i,v in ipairs(neutralDroidStaticSpawns) do
			cellId = SceneObject(BuildingObject(pCorvette):getNamedCell(v[7])):getObjectID()
			--pNpc =
			local pNpc=spawnMobile("dungeon1", v[1], v[2], v[3], v[4], v[5], v[6],cellId)
			--AiAgent(pNpc):setAiTemplate("idlewander")
			--local droid = TangibleObject(pNpc)
			--droid:setPvpStatusBitmask(0)
...
function CorellianCorvetteScreenPlay:spawnLootContainers(pCorvette)
	...
	if (pCell ~= nil) then
		...
		writeData(SceneObject(spawnedPointer):getObjectID() .. "lootbox", 1)
	...
	if (pCell ~= nil) then
		local cellID = SceneObject(pCell):getObjectID()
		local spawnedPointer = spawnSceneObject("dungeon1", "object/tangible/container/general/tech_armoire.iff",22.7,-12,52.08,cellID,0.707107,0,-0.707107,0)
		self:setLootBoxPermissions(spawnedPointer)
		--SceneObject(spawnedPointer):setCustomObjectName("")
		writeData(SceneObject(spawnedPointer):getObjectID() .. "lootbox", 2)
		createLoot(spawnedPointer, corvFaction.."_corvette_loot", 1, false)
		--createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "spawnSecurity", spawnedPointer)
	end
	...
	if (pCell ~= nil) then
		local cellID = SceneObject(pCell):getObjectID()
		local spawnedPointer = spawnSceneObject("dungeon1", "object/tangible/container/general/tech_armoire.iff",6.45,12,13,cellID,0.707107,0,-0.707107,0)
		self:setLootBoxPermissions(spawnedPointer)
		--SceneObject(spawnedPointer):setCustomObjectName("")
		writeData(SceneObject(spawnedPointer):getObjectID() .. "lootbox", 3)
		createLoot(spawnedPointer, corvFaction.."_corvette_loot", 1, false)
		--createObserver(OBJECTRADIALUSED, "CorellianCorvetteScreenPlay", "spawnSecurity", spawnedPointer)
	end
	...
	if (pCell ~= nil) then
		local cellID = SceneObject(pCell):getObjectID()
		local spawnedPointer = spawnSceneObject("dungeon1", "object/tangible/container/general/tech_chest.iff",0.5,12,-8.91,cellID,1,0,0,0)
		self:setLootBoxPermissions(spawnedPointer)
		--SceneObject(spawnedPointer):setCustomObjectName("")
		writeData(SceneObject(spawnedPointer):getObjectID() .. "lootbox", 4)
	...
	if (pCell ~= nil) then
		local cellID = SceneObject(pCell):getObjectID()
		local spawnedPointer = spawnSceneObject("dungeon1", "object/tangible/container/loot/astromech_container.iff",-13.9,0,51.44,cellID,0,0,1,0)
		SceneObject(spawnedPointer):setCustomObjectName("spare droid (for parts)")
		self:setLootBoxPermissions(spawnedPointer)
		writeData(SceneObject(spawnedPointer):getObjectID() .. "lootbox", 5)
...
function CorellianCorvetteScreenPlay:transportPlayer(pPlayer)
	...
	--self:startQuest(pCorvette, questType)
	--local pCell = BuildingObject(pCorvette):getNamedCell("airlock1")
	--local cellId = SceneObject(pCell):getObjectID()
...
function CorellianCorvetteScreenPlay:broadcastMessage(pCorvette, message)
	local corvette = BuildingObject(pCorvette)
	local playersToSend = {}
	for i = 1, 66, 1 do
...
function CorellianCorvetteScreenPlay:cleanOutDungeon(pCorvette)
	...
	for i = 1, #self.keypadTerminals, 1 do
		local kp = self.keypadTerminals[i]
		local keypadId = readData(BuildingObject(pCorvette):getObjectID()..":"..kp.room..":keypad")
		deleteData(keypadId..":locked")
		deleteStringData(keypadId..":room")
		deleteData(corvetteId..":"..kp.room..":keypad")
	end

	for i = 1,#self.computerTerminals,1 do
		local ct = self.computerTerminals[i]
		local compId = readData(BuildingObject(pCorvette):getObjectID()..":"..ct.name)
		deleteStringData(compId .. ":computerName")
		deleteData(corvetteId..":"..ct.name..":used")
		deleteData(corvetteId..":"..ct.name)
	end

	for i = 1,#self.computerMainframeOne do
		local cm = self.computerMainframeOne[i]
		local compId = readData(BuildingObject(pCorvette):getObjectID()..":"..cm.name)
		deleteStringData(SceneObject(compId):getObjectID() .. ":computerName")
		deleteData(BuildingObject(pCorvette):getObjectID()..":"..cm.name..":used")
		deleteData(BuildingObject(pCorvette):getObjectID()..":"..cm.name)
	end

	for i = 1,#self.computerMainframeTwo do
		local cm = self.computerMainframeTwo[i]
		local compId = readData(BuildingObject(pCorvette):getObjectID()..":"..cm.name)
		deleteStringData(SceneObject(compId):getObjectID() .. ":quest")
		deleteData(BuildingObject(pCorvette):getObjectID()..":"..cm.quest)
	end

	local tf = self.terminalFree[1]
	local terminalId = readData(BuildingObject(pCorvette):getObjectID()..":"..tf)
	deleteStringData(SceneObject(terminalId):getObjectID() .. ":computerName")
	deleteData(BuildingObject(pCorvette):getObjectID()..":"..tf.quest)
	deleteData(BuildingObject(pCorvette):getObjectID()..":"..tf)

	local tf = self.terminalS4[1]
	local terminalId = readData(BuildingObject(pCorvette):getObjectID()..":"..tf)
	deleteStringData(SceneObject(terminalId):getObjectID() .. ":quest")
	deleteData(BuildingObject(pCorvette):getObjectID()..":"..tf.quest)

	local sr = self.security38[1]
	local secTermId = readData(BuildingObject(pCorvette):getObjectID()..":"..sr)
	deleteStringData(SceneObject(secTermId):getObjectID() .. ":computerName")
	deleteData(BuildingObject(pCorvette):getObjectID()..":"..sr.quest)

	deleteData(BuildingObject(pCorvette):getObjectID()..":shock1")
	deleteData(BuildingObject(pCorvette):getObjectID()..":shock2")
	deleteData(BuildingObject(pCorvette):getObjectID()..":shock3")
	deleteData(BuildingObject(pCorvette):getObjectID()..":shock4")
	deleteData(BuildingObject(pCorvette):getObjectID()..":pDroid1")
	deleteData(BuildingObject(pCorvette):getObjectID()..":pDroid2")
	deleteData(BuildingObject(pCorvette):getObjectID()..":DroidFixed")
end
...
function CorellianCorvetteScreenPlay:ejectPlayer(pPlayer)
	...
	local faction = self:getBuildingFaction(pCorvette)
	print("escape points faction: "..faction)
	...
	if (isZoneEnabled(point.planet)) then
		...
		SceneObject(pPlayer):switchZone("tatooine", 0, 0, 0, 0)
	end
end
