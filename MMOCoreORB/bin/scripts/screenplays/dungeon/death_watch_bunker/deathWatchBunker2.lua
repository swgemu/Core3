...
function DeathWatchBunker:start()
	if (isZoneEnabled("endor")) then
		...
		createEvent(1000 * 45, "DeathWatchBunker", "poisonEvent", pBunker, "")
		createObserver(ENTEREDBUILDING, "DeathWatchBunker", "onEnterDWB", pBunker)
		createObserver(EXITEDBUILDING, "DeathWatchBunker", "onExitDWB", pBunker)
	end
end
...
function DeathWatchBunker:spawnObjects()
	...
	if pActiveArea ~= nil then
		createObserver(ENTEREDAREA, "DeathWatchBunker", "notifyEnteredOutsideLockedDoorArea", pActiveArea)
	end
	...
	if pActiveArea ~= nil then
		createObserver(ENTEREDAREA, "DeathWatchBunker", "notifyEnteredVoiceTerminalArea", pActiveArea)
	end
	...
	if (spawnedPointer ~= nil) then
		...
		createEvent(100, "DeathWatchBunker", "setBombDroidTemplate", spawnedPointer, "")
	end
	...
	if (spawnedPointer ~= nil) then
		spawnedSceneObject:_setObject(spawnedPointer)
		createObserver(DAMAGERECEIVED, "DeathWatchBunker", "haldoDamage", spawnedPointer)
	end
	...
	if (spawnedPointer ~= nil) then
		...
		createEvent(1000, "DeathWatchBunker", "refillContainer", spawnedPointer, "")
		createObserver(OBJECTRADIALUSED, "DeathWatchBunker", "boxLooted", spawnedPointer)
	end
...
function DeathWatchBunker:setBombDroidTemplate(pDroid)
...
function DeathWatchBunker:onEnterDWB(sceneObject, pCreature)
	if (not SceneObject(pCreature):isPlayerCreature()) then
		return 0
	end

	createObserver(SPATIALCHATSENT, "DeathWatchBunker", "notifyTerminalChatSent", pCreature)
...
function DeathWatchBunker:onExitDWB(sceneObject, pCreature, long)
	...
	dropObserver(SPATIALCHATSENT, "DeathWatchBunker", "notifyTerminalChatSent", pCreature)

	return 0
end

function DeathWatchBunker:diedWhileCrafting(pCreature, pAttacker, long)
...
function DeathWatchBunker:enableAccess(pSceneObject)
...
function DeathWatchBunker:respawnHaldo(creatureObject)
	local halNum = getRandomNumber(1,3)
	...
	if (pMobile ~= nil) then
		createObserver(DAMAGERECEIVED, "DeathWatchBunker", "haldoDamage", pMobile)
	end
end

function DeathWatchBunker:boxLooted(pSceneObject, pCreature, radialID)
	if (radialID ~= 16) then
		return 0
	end
	...
	if readData(objectID .. ":dwb:spawned") ~= 1 then
		...
		if boxId == 1 then
			...
			if (pMobile ~= nil) then
				createEvent(self.containerRespawnTime, "DeathWatchBunker", "despawnMobile", pMobile, "")
			end
		elseif boxId == 2 then
		...
		createEvent(self.containerRespawnTime, "DeathWatchBunker", "refillContainer", pSceneObject, "")
	end

	return 0
end

function DeathWatchBunker:despawnMobile(pMobile)
	if (pMobile == nil) then
		return
	end

	if (CreatureObject(pMobile):isInCombat()) then
		createEvent(15 * 1000, "DeathWatchBunker", "despawnMobile", pMobile, "")
		return
	end
...
function DeathWatchBunker:poisonEvent(pSceneObject)
				...
				for j = 0, cellSize - 1, 1 do
					...
					if pObject ~= nil then
						if (SceneObject(pObject):isPlayerCreature() and not self:hasRebreather(pObject)
							...
							createEvent(500, "DeathWatchBunker", "doPoison", pObject, "")
						end
					end
				end
			end
	...
	createEvent(1000 * 45, "DeathWatchBunker", "poisonEvent", pSceneObject, "")
end

function DeathWatchBunker:timeWarning(pCreature)
	if pCreature == nil then
		return
	end
...
function DeathWatchBunker:removeFromBunker(pCreature)
	if (pCreature == nil) then
		return 0
	end
	...
	if (CreatureObject(pCreature):isGrouped()) then
		...
		for i = 0, groupSize - 1, 1 do
			...
			if pMember ~= nil then
				if CreatureObject(pMember):getParentID() > 5996313 and CreatureObject(pMember):getParentID() < 5996380 then
					createEvent(500, "DeathWatchBunker", "teleportPlayer", pMember, "")
				end
			end
		end
	else
		createEvent(500, "DeathWatchBunker", "teleportPlayer", pCreature, "")
	end
end

function DeathWatchBunker:teleportPlayer(pCreature)
	if (pCreature == nil) then
		return 0
	end
...
function DeathWatchBunker:doBombDroidAction(pBombDroid)
...
function DeathWatchBunker:notifyEnteredVoiceTerminalArea(pArea, pPlayer)
...
function DeathWatchBunker:notifyTerminalChatSent(pPlayer, pChatMessage)
	...
	createEvent(500, "DeathWatchBunker", "doBombDroidAction", pBombDroid, "")

	return 0
end


function DeathWatchBunker:notifyEnteredOutsideLockedDoorArea(pArea, pPlayer)
	if (not SceneObject(pPlayer):isPlayerCreature()) then
		return 0
	end
...
function DeathWatchBunker:bombDroidDetonated(pBombDroid)
	...
	if (pDebris ~= nil and SceneObject(pBombDroid):isInRangeWithObject(pDebris, 5)) then
		...
		createEvent(500, "DeathWatchBunker", "destroyDebris", pDebris, "")
	elseif (pDebris2 ~= nil and SceneObject(pBombDroid):isInRangeWithObject(pDebris2, 5)) then
		...
		createEvent(500, "DeathWatchBunker", "destroyDebris", pDebris2, "")
	end

	createEvent(2000, "DeathWatchBunker", "despawnCreature", pBombDroid, "")
	deleteData("dwb:bombDroid")

	return 1
end

function DeathWatchBunker:respawnDebris(pOldDebris)
...
function DeathWatchBunker:destroyDebris(pDebris)
	if (pDebris ~= nil) then
		createEvent(self.debrisRespawnTime, "DeathWatchBunker", "respawnDebris", pDebris, "")
		SceneObject(pDebris):destroyObjectFromWorld()
	end
end

function DeathWatchBunker:respawnBombDroid(pDroid)
	local spawn = deathWatchSpecialSpawns["bombdroid"]
	...
	createEvent(100, "DeathWatchBunker", "setBombDroidTemplate", pBombDroid, "")
end

function DeathWatchBunker:haldoTimer(pCreature)
...
function DeathWatchBunker:haldoKilled(pHaldo, pPlayer)
	createEvent(1000 * 240, "DeathWatchBunker", "respawnHaldo", pPlayer, "")
...
function DeathWatchBunker:haldoDamage(pHaldo, pPlayer, damage)
...
function DeathWatchBunker:spawnAggroHaldo(pOldHaldo, pPlayer)
...
function DeathWatchBunker:destroyIngredient(pIngredient)
...
--   Key Spawn Events                                        -
--------------------------------------------------------------
function DeathWatchBunker:spawnDefender(spawnData, spawnName)
	local pMobile = spawnMobile("endor", spawnData[1], spawnData[2], spawnData[3], spawnData[4], spawnData[5], spawnData[6], spawnData[7])

	if (pMobile ~= nil) then
		createEvent(300 * 1000, "DeathWatchBunker", "despawnMobile", pMobile, "")
		createEvent(10, "DeathWatchBunker", "startDefenderPath", pMobile, spawnName)
	end

	return pMobile
end

function DeathWatchBunker:startDefenderPath(pMobile, spawnName)
	if (pMobile == nil) then
		return
	end
	...
	elseif (string.find(spawnName, "fenri_dalso")) then
		...
		printLuaError("DeathWatchBunker:startDefenderPath, invalid spawnName " .. spawnName)
	end
	...
	if (pCell == nil) then
		printLuaError("Invalid cellid " .. patrolPoint[4] .. " in DeathWatchBunker:startDefenderPath for spawn " .. spawnName)
		return
	end
...
function DeathWatchBunker:spawnNextA(pCreature)
	if (pCreature == nil) then
		return
	end
	...
	elseif nextSpawn == 1 then
		...
		if (pDefender ~= nil) then
			createObserver(DAMAGERECEIVED, "DeathWatchBunker", "spawnADefenderDamageReceived", pDefender)
		end
...
function DeathWatchBunker:spawnADefenderDamageReceived(pDefender, pPlayer, damage)
	if pDefender == nil then
		return 1
	end

	spatialChat(pDefender, "@dungeon/death_watch:call_back_up")
	createEvent(30 * 1000, "DeathWatchBunker", "spawnNextA", pPlayer, "")

	return 1
end

function DeathWatchBunker:spawnNextB(pCreature)
	if (pCreature == nil) then
		return 0
	end
	...
	elseif nextSpawn == 1 then
		...
		if (pDefender ~= nil) then
			createObserver(DAMAGERECEIVED, "DeathWatchBunker", "spawnBDefenderDamageReceived", pDefender)
		end
	elseif nextSpawn == 2 then
...
function DeathWatchBunker:spawnBDefenderDamageReceived(pDefender, pPlayer, damage)
	if pDefender == nil then
		return 1
	end
	...
	spatialChat(pDefender, "@dungeon/death_watch:call_back_up")
	createEvent(30 * 1000, "DeathWatchBunker", "spawnNextB", pPlayer, "")

	return 1
end

function DeathWatchBunker:spawnNextC(creatureObject)
	...
	elseif nextSpawn == 1 then
		...
		if (pDefender ~= nil) then
			createObserver(DAMAGERECEIVED, "DeathWatchBunker", "spawnCDefenderDamageReceived", pDefender)
		end
	elseif nextSpawn == 2 then
...
function DeathWatchBunker:spawnCDefenderDamageReceived(pDefender, pPlayer, damage)
	if pDefender == nil then
		return 1
	end
	...
	spatialChat(pDefender, "@dungeon/death_watch:call_back_up")
	createEvent(30 * 1000, "DeathWatchBunker", "spawnNextC", pPlayer, "")

	return 1
end

--   DeathWatchBunker whether a creature has sufficient skill to access a particular crafting room
function DeathWatchBunker:hasRequiredSkill(room, pCreature)
...
function DeathWatchBunker:hasAlumMineral(pCreature)
	if pCreature == nil then
		return false
	end
...
function DeathWatchBunker:findRequiredItem(room, pCreature)
	if (pCreature == nil) then
		return nil
	end
...
function DeathWatchBunker:hasRebreather(pSceneObject)
...
function DeathWatchBunker:doPoison(pCreature)
	if pCreature == nil then
		return 0
	end
...
function DeathWatchBunker:lockAll(pCreature)
...
function DeathWatchBunker:spawnDefenders(number, pCreature)
	...
	if number == 1 then
		...
		createEvent(2 * 1000, "DeathWatchBunker", self.spawnEvents[number], pCreature, "")
	end
end

function DeathWatchBunker:unlockForGroup(number, pCreature, cells)
...
function DeathWatchBunker:checkDoor(pSceneObject, pCreature)
	...
	elseif doorType == 3 then
		...
		createEvent(1000 * 60 * 5, "DeathWatchBunker", "removeFromBunker", pCreature, "")
		createEvent(1000 * 60 * 4.5, "DeathWatchBunker", "timeWarning", pCreature, "")
		createEvent(1000 * 60 * 5.5, "DeathWatchBunker", "despawnCell", pCell, "")
	end

	deleteData(SceneObject(pSceneObject):getObjectID() .. ":dwb:accessEnabled")
	createEvent(1000 * 60 * self.doorData[doorNumber].lockTime, "DeathWatchBunker", "enableAccess", pSceneObject, "")
end

function DeathWatchBunker:despawnCell(pCell)
	if pCell == nil then
		return
	end
		...
		if pObject ~= nil then
			if SceneObject(pObject):isCreatureObject() then
				...
				if string.find(template, "death_watch") ~= nil or string.find(template, "battle_droid") ~= nil then
					createEvent(1000, "DeathWatchBunker", "despawnCreature", pObject, "")
				end
			end
		end
	end
...
function DeathWatchBunker:despawnCreature(pObject)
	if pObject ~= nil then
		SceneObject(pObject):destroyObjectFromWorld()
	end
end

function DeathWatchBunker:startForemanQuestStage(number, pCreature)
	if pCreature == nil then
		return
	end

	if number == 1 then
		createEvent(1000 * 60 * 60, "DeathWatchBunker", "haldoTimer", pCreature, "")
	elseif number == 2 then
		createEvent(1000 * 60 * 60, "DeathWatchBunker", "pumpTimer", pCreature, "")
	end
end

function DeathWatchBunker:storeTime(pCreature)
	if pCreature ~= nil then
		local time = os.time()
		writeScreenPlayData(pCreature, "DeathWatchBunker", "time", time)
	end
end

function DeathWatchBunker:checkTime(pCreature)
	if pCreature == nil then
		return false
	end

	local currentTime = os.time()
	local receivedTime = readScreenPlayData(pCreature, "DeathWatchBunker", "time")
...
function DeathWatchBunker:cancelCrafting(pTerm)
	if pTerm == nil then
		return
	end
...
function DeathWatchBunker:stopCraftingProcess(pCreature, pTerm, successful, teleport)
	...
	if teleport == true then
		if (number == 4) then
			createEvent(5000, "DeathWatchBunker", "removeFromBunker", pCreature, "")
		else
			createEvent(500, "DeathWatchBunker", "teleportPlayer", pCreature, "")
		end
	end
...
function DeathWatchBunker:sendUseTerminalMessage(pCreature)
	if pCreature ~= nil then
		CreatureObject(pCreature):sendSystemMessage("@dungeon/death_watch:use_terminal")
	end
end

function DeathWatchBunker:swapValveState(pCreature, valveNumber, notifyPlayer)
...
function DeathWatchBunker:spawnValveEffect(valveNumber, valveState)
...
function DeathWatchBunker:doValveSwitch(pCreature, valveNumber)
...
function DeathWatchBunker:startCraftingProcess(pCreature, pTerminal)
	...
	createEvent(1000 * 30, "DeathWatchBunker", "cancelCrafting", pTerminal, "")
	createEvent(1000, "DeathWatchBunker", "sendUseTerminalMessage", pCreature, "")
	...
	createObserver(OBJECTDESTRUCTION, "DeathWatchBunker", "diedWhileCrafting", pCreature)
end

function DeathWatchBunker:nextCraftingStep(pTerm)
	if pTerm == nil then
		return
	end
	...
	if creoId ~= 0 then
		...
		writeData(statusPrefix .. "stepStartTime", os.time())
		createEvent(1000 * 30, "DeathWatchBunker", "cancelCrafting", pTerm, "")
	end
end

function DeathWatchBunker:finishCraftingStep(pTerm)
	if pTerm == nil then
		return
	end
...
function DeathWatchBunker:getObjOwner(pObj)
...
function DeathWatchBunker:craftingConfirmCallback(pCreature, pSui, eventIndex)
	...
	if step == 1 then
		...
		createEvent(10 * 1000, "DeathWatchBunker", "nextCraftingStep", pUsingObject, "")
	elseif step == 3 then
		...
		createEvent(10 * 1000, "DeathWatchBunker", "finishCraftingStep", pUsingObject, "")
	end

	return 0
end

function DeathWatchBunker:doVentDroidStep(pDroid)
	...
	if (curStep == 0) then -- Initial spawn at technician, wait 10 secs
		...
		createObserver(DESTINATIONREACHED, "DeathWatchBunker", "ventDroidDestinationReached", pDroid)
		AiAgent(pDroid):setAiTemplate("manualescortwalk") -- Don't move unless patrol point is added to list, walking speed
		AiAgent(pDroid):setFollowState(4) -- Patrolling
		createEvent(10 * 1000, "DeathWatchBunker", "doVentDroidStep", pDroid, "")
	...
	elseif (curStep == 2) then -- Wait 120 seconds
		...
		createEvent(120 * 1000, "DeathWatchBunker", "doVentDroidStep", pDroid, "")
		writeData("dwb:ventDroidStep", curStep + 1)
	...
	elseif (curStep == 5) then -- End effect, Despawn droid, complete quest
		...
		createEvent(60 * 60 * 1000, "DeathWatchBunker", "disableVents", pStarter, "") -- Disable vents after an hour
	end
end

function DeathWatchBunker:ventDroidDestinationReached(pDroid)
	createEvent(1500, "DeathWatchBunker", "doVentDroidMove", pDroid, "")
	return 0
end

function DeathWatchBunker:doVentDroidMove(pDroid)
	local curStep = readData("dwb:ventDroidStep")
	local patrolPoints
	if (curStep == 1) then
	...
	elseif (curStep == 3) then
		...
		printf("ERROR: DeathWatchBunker:doVentDroidMove called on wrong step.\n")
		return
	end
...
function DeathWatchBunker:disableVents()
	deleteData("dwb:ventsEnabled")
end

-- Not used until droid is made attackable by spawns
function DeathWatchBunker:failVentEscort(pDroid)
	deleteData("dwb:droidEscortStatus")
	deleteData("dwb:droidEscortStarter")
	...
	createEvent(5 * 60 * 1000, "DeathWatchBunker", "createNewVentDroid", pDroid, "") -- New vent droid available 5 mins after failing
end

function DeathWatchBunker:createNewVentDroid()
	writeData("dwb:ventDroidAvailable", 1)
	deleteData("dwb:ventDroidStep")
end

function DeathWatchBunker:sendMessageToCell(cellID, string)
