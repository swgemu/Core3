...
[added] Imperial Destroy Corvette mission files, Lt Velso and

Sabol conversing is disabled until more can be added.
[added] Main corvette screenplay, all spawns disabled but Lt Velso and
Lt Sabol.
[moved] Sabol spawn from Tatooine spawn manager.
[moved] Elevator terminals from Imperial Corvette to screenplay.

Change-Id: I41feb4a97385c28876e14ac4e8604d562f678460
...
function escape_pod_menu_component:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	if (playerCreature:isImperial()) then
		for i,v in ipairs(self.spawnMapImperial) do
			local deletionCheck = readData(i .. ":imperial:vette:ob