...
function CityControlLanding:chanceToSpawn()
	...
	if (chance <= 50) then --if (chance <= 5) then
		createEvent(10 * 60 * 1000, "CityControlLanding", "spawnCityLanding", "", "") --10min after chance roll to run in case server is loading
	else
...
function CityControlLanding:handleShuttlePosture(shuttle)
	...
	elseif (landingType == "REBEL") then
		if (shuttlePosture == "PRONE") then
			...
			createEvent(20000, "CityControlLanding", "spawnRebelLandingParty", pShuttle, "")
			return
		else
			CreatureObject(pShuttle):setPosture(PRONE)
			createEvent(60 * 1000, "CityControlLanding", "cleanUp", pShuttle, "")
