	...
	for i = 2, totalSpawns, 1 do
		...
		if (template == "darth_vader") then
			...
			if (pMobile ~= nil) then
				...
				writeData(i .. ":imperialLandingParty", mobileID)
				createObserver(CREATUREDESPAWNED, "CityControlLanding", "onDespawnImperial", pMobile)
			end
		end
	end
...
function CityControlLanding:onDespawnRebel(mobile)
...
function CityControlLanding:onDespawnImperial(mobile)
	local pMobile = mobile
	...
	if (landingFaction == "REBEL") then
		...
		if (spawnNumber ~= nil) then
			local rebelLanding = self.rebelLanding
			spawn = rebelLanding[spawnNumber]

			local xOffset = xLoc + getRandomNumber(6)
			local yOffset = yLoc - 15

			--template, level, x, z, y, direction, cell, mood},
			local pMobile = spawnMobile(planet, spawn, 0, xLoc, cityCoords[2], yLoc, cityCoords[4], 0, "")
		...
		if (spawnNumber ~= nil) then
			local imperialLanding = self.imperialLanding
			spawn = imperialLanding[spawnNumber]

			local yOffset = (yLoc - 10 - spawnNumber)

			--template, level, x, z, y, direction, cell, mood},
			local pMobile = spawnMobile(planet, spawn, 0, xLoc, cityCoords[2], yOffset, cityCoords[4], 0, "")
