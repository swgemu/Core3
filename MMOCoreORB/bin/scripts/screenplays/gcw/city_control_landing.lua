CityControlLanding = ScreenPlay:new {
	...
	coordinates = {
		...
		{109, 52, -5340, 171}, -- 6 Anchorhead
	},

	rebelLanding = {"object/creature/npc/theme_park/player_shuttle.iff", "lukeLeia", "rebel_commandant", "rebel_commandant", "rebel_commandant", "rebel_commandant", "rebel_commandant", "rebel_commandant", "rebel_commandant"},
...
function CityControlLanding:chanceToSpawn()
	...
	if (chance <= 50) then --if (chance <= 5) then
		createEvent(2 * 60 * 1000, "CityControlLanding", "spawnCityLanding", "", "") --10min after chance roll to run in case server is loading
	else
...
function CityControlLanding:spawnCityLanding()
	...
	if (city ~= "bestine") and (controllingFaction == FACTIONREBEL or city == "anchorhead" or city == "moenia") then
		...
		if (pShuttle ~= nil) then
			...
			local templateRoll = 2 --Disables Luke from spawning
...
function CityControlLanding:broadcastMessage(pShuttle)
	...
	if (landingType == "REBEL") then
		...
		if (celebrity == 1) then
			broadcastTemplate= "<<Incoming Alliance Transmission: Ground control, this is shuttle Narrow Margin. We have the Hero of the Alliance and are coming in for a landing.>>"
		else
...
function CityControlLanding:handleShuttlePosture(shuttle)
	...
	if (landingType == "IMPERIAL") then
		if (shuttlePosture == "UPRIGHT") then
			...
			createEvent(60 * 1000, "CityControlLanding", "cleanUp", pShuttle, "")
			return
		end
...
function CityControlLanding:doSpeech(pMobile)
    ...
    createEvent(15 * 1000, "CityControlLanding", "doSpeech", pMobile, "")
end
...
function CityControlLanding:despawnLanding()
	...
	createEvent(23 * 1000, "CityControlLanding", "handleShuttlePosture", pShuttle, "")
end
	...
	for i = 2, 9, 1 do
		...
		if (pMobile ~= nil) then
			if (CreatureObject(pMobile):isInCombat()) then
				...
				createEvent(14 * 1000, "CityControlLanding", "mobileDespawn", pMobile, "")
			end
		end
	end
end
