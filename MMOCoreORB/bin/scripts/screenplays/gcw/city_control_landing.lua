...
function CityControlLanding:chanceToSpawn()
	...
	if (chance <= 50) then --if (chance <= 5) then
		createEvent(2 * 60 * 1000, "CityControlLanding", "spawnCityLanding", "", "") --15min after chance roll to allow for load time
	else
...
function CityControlLanding:spawnLandingParty()
	...
	for i = 2, totalSpawns, 1 do
		if (template[i] == "leia_organa_city_control" or template[i] == "darth_vader_city_control") then
			...
			if (landingType == "REBEL") then
				...
				yLoc = cityCoords[3] - self.yReb
			else
				yLoc = cityCoords[3] - self.yImp - i

				if (i == 3 or i == 5 or i == 7) then
					xLoc = cityCoords[1] - 1
				elseif (i == 4 or i == 6 or i == 8) then
					xLoc = cityCoords[1] + 1
				else
					xLoc = cityCoords[1]
				end
...
function CityControlLanding:despawnLanding()
	...
	createEvent(2 * 60 * 1000, "CityControlLanding", "cleanUp", "", "")
end
...
function CityControlLanding:onDespawn(mobile)
	...
	createEvent(30 * 1000, "CityControlLanding", "respawnMobile", pMobile, "")

	return 1
end

function CityControlLanding:respawnMobile(pMobile)
	if (pMobile == nil) then
		return
	end
	...
	local template = ""
	local xLoc = spawnCoords[1]
	local yLoc = spawnCoords[3]

	if (landingType == "REBEL") then
		template = self.rebelLanding[spawnNumber]
		xLoc = xLoc + getRandomNumber(-3, 3)
		yLoc = yLoc - self.yReb
	else
		template = self.imperialLanding[spawnNumber]
		yLoc = yLoc - self.yImp - i
	end

	local pMobile = spawnMobile(planet, template, 0, xLoc, spawnCoords[2], yLoc, spawnCoords[4], 0, "")
