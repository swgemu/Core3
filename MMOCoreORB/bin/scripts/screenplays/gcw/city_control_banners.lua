...
function CityControlBanners:spawnGcwControlBanners(zoneName)
	for i = 1, #self.planets do
		...
		if (isZoneEnabled(planetName) and planetName == zoneName) then
			...
			for j = 1, #location do
				self:spawnSceneObjects(location, j, planetName)
			end
		end
	end
end

function CityControlBanners:spawnSceneObjects(location, num, planetName)
	...
	if pSceneObject ~= nil then
		...
		writeData("control:banner:" .. num, pSceneObjectId)
	end
end

function CityControlBanners:despawnGcwControlBanners(zoneName)
	for i = 1, #self.planets do
		if zoneName == self.planets[i] then
			local despawnObjects = self.bannerLocations[zoneName]
	
			for j = 1, #despawnObjects do
				local objectID = readData("control:banner:" .. j)
				local pObject = getSceneObject(objectID)
				
				if (pObject ~= nil) then
					SceneObject(pObject):destroyObjectFromWorld()
					deleteData("control:banner" .. j)
				end
			end
		end
	end
