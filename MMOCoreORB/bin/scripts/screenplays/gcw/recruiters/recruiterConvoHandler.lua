...
RecruiterConvoHandler = conv_handler:new {}
 ...
 function RecruiterConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "confirm_resign") then
		...
		createEvent(timer, "recruiterScreenplay", "handleResign", pPlayer, "")

		return pConvScreen
	elseif (screenID == "dont_resign11111") then
		local convoTemplate = LuaConversationTemplate(pConvTemplate)
		local mainScreen

		if (CreatureObject(pPlayer):isCovert()) then
			mainScreen = convoTemplate:getScreen("greet_member_start_covert2")
		else
			mainScreen = convoTemplate:getScreen("greet_member_start_overt2")
		end

		local screenObject = LuaConversationScreen(mainScreen)

		pConvScreen = screenObject:cloneScreen()

		screenObject = LuaConversationScreen(pConvScreen)
		screenObject:setDialogTextStringId("@faction_recruiter:resign_faction_no")
		screenObject:addOption("@faction_recruiter:option_purchase_items", "faction_purchase")
	elseif (screenID == "greet_member_start_covert2" or screenID == "greet_member_start_overt2" or screenID == "dont_resign") then
		...
		if (not CreatureObject(pPlayer):isChangingFactionStatus()) then
	...
	elseif (screenID == "cancel_resignation") then
		writeData(CreatureObject(pPlayer):getObjectID() .. ":GCWRecruiter:cancelResignation:", 1)
