...
function heraldScreenPlay:createLoc(pPlayer, heraldNum)
	...
	local waypointID = PlayerObject(pGhost):addWaypoint(heraldData.planet, stfFile .. ":waypoint_name_1", stfFile .. ":waypoint_description_1", heraldData.destX, 0, heraldData.destY, WAYPOINT_PURPLE, true, true, 0, 0)
...
function heraldScreenPlay:giveMultiDestWaypoint(pPlayer, heraldNum, locNum)
	...
	if pWaypoint ~= nil then
		...
		PlayerObject(pGhost):addWaypoint(heraldData.planet, wpName, "", x, 0, y, WAYPOINT_BLUE, true, true, 0)
	end
end
