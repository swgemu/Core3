...
function ImperialDefectorConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if pDefectorTasks == nil then
		Logger:log("ERROR: Could not load DefectorQuest Tasks in ImperialDefectorConvoHandler.", LT_ERROR)
		return 0
	end
	...
	elseif (screenID == "response_start") then
		...
		deleteScreenPlayData(pPlayer, "DefectorQuest", "impQuestGiverWaypointID")
		deleteScreenPlayData(pPlayer, "DefectorQuest", "impDefectorWaypointID")
