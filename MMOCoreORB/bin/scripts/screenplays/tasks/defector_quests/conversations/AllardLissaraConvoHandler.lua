...
function AllardLissaraConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "response_correct_1" or screenID == "response_correct_2") then
		ghost:completeJournalQuestTask(questCrc, DefectorQuest.WAIT_FOR_SIGNAL2, true)
		ghost:activateJournalQuestTask(questCrc, DefectorQuest.REWARD1, true)
		ghost:activateJournalQuestTask(questCrc, DefectorQuest.REWARD2, true)
		ghost:activateJournalQuestTask(questCrc, DefectorQuest.COMPLETE_QUEST, true)
