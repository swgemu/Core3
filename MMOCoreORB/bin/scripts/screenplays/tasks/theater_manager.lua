...
function TheaterManagerScreenPlay:startAudition(pPlayer)
	ObjectManager.withCreatureObject(pPlayer, function(player)
		...
		if (inAuditionArea == 0) then
			...
			createEvent(math.random(5,8) * 1000, "TheaterManagerScreenPlay", "runAuditionPhase", pPlayer)
		end
	end)
end
...
function TheaterManagerScreenPlay:isExpectedPerformance(pPlayer, performance, auditionType)
	...
	return ObjectManager.withPlayerObject(pPlayer, function(playerObject)
		if (auditionType == 1 and playerObject:isDancing() == true and playerObject:getPerformanceName() == expectedPerformance) then
			return true
		elseif (auditionType == 2 and playerObject:isPlayingMusic() == true and playerObject:getPerformanceName() == expectedPerformance) then
			return true
		else
			return false
		end
...
function TheaterManagerScreenPlay:getExpectedPerformance(pPlayer, type)
	...
	return ObjectManager.withPlayerObject(pPlayer, function(playerObject)
		...
		if (playerObject:isDancing() == true or playerObject:isPlayingMusic() == true) then
			currentPerformance = playerObject:getPerformanceName()
		end
		if (type == 1) then
