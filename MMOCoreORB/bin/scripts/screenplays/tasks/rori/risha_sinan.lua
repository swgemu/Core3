...
function RishaSinan:notifyLootMite(pLootedCreature, pLooter, nothing)
	...
	if (PlayerObject(pGhost):questIsQuestActive(questHashCode) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 0) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 3) == true) then
		return 0
	end
...
function RishaSinan:notifyLootWorker(pLootedCreature, pLooter, nothing)
	...
	if (PlayerObject(pGhost):questIsQuestActive(questHashCode) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 0) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 4) == true) then
		return 0
	end
...
function RishaSinan:notifyLootHatchling(pLootedCreature, pLooter, nothing)
	...
	if (PlayerObject(pGhost):questIsQuestActive(questHashCode) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 0) == false or
		PlayerObject(pGhost):questIsQuestTaskComplete(questHashCode, 5) == true) then
		return 0
	end
