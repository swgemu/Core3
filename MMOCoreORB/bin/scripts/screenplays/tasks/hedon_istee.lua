...
function HedonIstee:spawnActiveArea()
	local pActiveArea = spawnSceneObject("tatooine", "object/active_area.iff", mapX, mapZ, mapY, 0, 0, 0, 0, 0)
	...
	if (not cancelPressed) then
		if (pCreature ~= nil) then
			...
			if (pPlayerObject ~= nil) then
				...
				local waypointID = playerObject:addWaypoint("tatooine", "Ancient Treasure Location", "", mapX, mapY, WAYPOINTGREEN, true, true)
...
function HedonIstee:spawnTreasure(pTreasureMap, pPlayer)
	...
	if (pPlayerObject ~= nil) then
		...
		local x = mapX - math.random(200)
		local y = mapY - math.random(200)
		...
		if (pChest ~= nil) then
			...
			createEvent(treasure_life, "HedonIstee", "removeTreasureChest", pChest)
		end
