...
function DarnDroid2:notifyEnteredBlackSunArea(pArea, pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_FINDSTAGING) == true) then
		return 0
	end

	local playerID = CreatureObject(pPlayer):getObjectID()

	PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_FINDSTAGING, true);
	PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_KILLMINION, false);
	PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_KILLSMUGGLER, false);
	PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_KILLINITIATE, true);
...
function DarnDroid2:notifyMinionDead(pMinion, pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLMINION) == true) then
		return 0
	end

	PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_KILLMINION, true);
...
function DarnDroid2:notifySmugglerDead(pSmuggler, pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLSMUGGLER) == true) then
		return 0
	end
	...
	if (count >= 5) then
		PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_KILLSMUGGLER, true)
		self:returnToValance(pPlayer)
	end
...
function DarnDroid2:notifyInitiateDead(pSmuggler, pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLINITIATE) == true) then
		return 0
	end
	...
	if (count >= 8) then
		PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_KILLINITIATE, true);
		self:returnToValance(pPlayer)
	end
...
function DarnDroid2:returnToValance(pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLMINION) == true and
		PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLSMUGGLER) == true and
		PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLINITIATE) == true) then

		DarnDroid2:clearValanceWaypoint(pPlayer)
		DarnDroid2:giveWaypoint(pPlayer,  "talus", "Return to Valance Serth", -2389, 2043)
		PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_RETURNTOVALANCE, true);
	end
end
...
function DarnDroid2:giveSpaceMission(pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(quest2Crc, self.TASK_RETURNTOVALANCE) == false and
		PlayerObject(pGhost):isJournalQuestTaskActive(quest2Crc, self.TASK_KILLTRANSPORT) == false) then

		PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_RETURNTOVALANCE, true);
		PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_KILLTRANSPORT, true);
...
function DarnDroid2:giveMemWipeTask(pPlayer)
	...
	PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_KILLTRANSPORT, true);
	PlayerObject(pGhost):activateJournalQuestTask(quest2Crc, self.TASK_WIPEMEM, true);
...
function DarnDroid2:completeQuest(pPlayer)
	...
	if (self:awardQuestItem(pPlayer, "object/tangible/loot/loot_schematic/droid_head_schematic.iff")) then
		...
		PlayerObject(pGhost):completeJournalQuestTask(quest2Crc, self.TASK_WIPEMEM, true);
...
function DarnDroid2:giveQuest(pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestActive(quest2Crc) or	PlayerObject(pGhost):isJournalQuestComplete(quest2Crc)) then
		return
	end
...
function DarnDroid2:isPlayerOnQuest1(pPlayer)
	...
	if (PlayerObject(pPlayer):isJournalQuestTaskActive(quest1Crc, 5) and PlayerObject(pPlayer):isJournalQuestActive(quest1Crc)) then
		return true
	end
...
function DarnDroid2:isPlayerReadyForQuest2(pPlayer)
	...
	if (PlayerObject(pPlayer):isJournalQuestComplete(quest1Crc) and PlayerObject(pPlayer):isJournalQuestActive(quest2Crc)) then
		return true
	end
...
function DarnDroid2:isPlayerDoneQuest2Part2(pPlayer)
	...
	if (PlayerObject(pPlayer):isJournalQuestTaskActive(quest2Crc, self.TASK_RETURNTOVALANCE) == true and
		PlayerObject(pPlayer):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLMINION) == true and
		PlayerObject(pPlayer):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLSMUGGLER) == true and
		PlayerObject(pPlayer):isJournalQuestTaskComplete(quest2Crc, self.TASK_KILLINITIATE) == true) then
		return true
	end
...
function DarnDroid2:isPlayerOnQuest2Part1(pPlayer)
	...
	if (PlayerObject(pPlayer):isJournalQuestTaskActive(quest2Crc, self.TASK_FINDSTAGING) == true or
		PlayerObject(pPlayer):isJournalQuestTaskActive(quest2Crc, self.TASK_KILLMINION) == true or
		PlayerObject(pPlayer):isJournalQuestTaskActive(quest2Crc, self.TASK_KILLSMUGGLER) == true or
		PlayerObject(pPlayer):isJournalQuestTaskActive(quest2Crc, self.TASK_KILLINITIATE) == true) then
		return true
	end
...
function DarnDroid2:isPlayerCompletedAll(pPlayer)
	...
	if (PlayerObject(pPlayer):isJournalQuestComplete(quest1Crc) and	PlayerObject(pPlayer):isJournalQuestComplete(quest2Crc)) then
		return true
	end
