...
function DarnDroid1:notifyEnteredCrashedShipArea(pArea, pPlayer)
	...
	if (PlayerObject(pGhost):isJournalQuestTaskComplete(questCrc, TASK_FINDSHIP) == true) then
		return 0
	end
...
function DarnDroid1:completeQuest(pPlayer)
	...
	self:awardQuestItem(pPlayer, "object/tangible/loot/collectible/collectible_parts/blue_rug_adhesive.iff")
...
function DarnDroid1:giveShipTask(pPlayer, taskType)
	...
	self:giveShipWaypoint(pPlayer)
end

function DarnDroid1:giveShipWaypoint(pPlayer)
