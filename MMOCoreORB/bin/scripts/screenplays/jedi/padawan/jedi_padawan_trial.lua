...
function JediPadawanTrial:jediPadawanTrialsRestartCallback(pPlayer, pSui, eventIndex, args)
	...
	CreatureObject(pPlayer):removeScreenPlayState(self.SCREEN_PLAY_NAME, 8)
...
--Remove All States.
	...
	CreatureObject(pPlayer):removeScreenPlayState("JediPadawanTrialArchitectScreenPlay", state)
	
	state = CreatureObject(pPlayer):getScreenPlayState("JediPadawanTrialArtistScreenPlay")
	CreatureObject(pPlayer):removeScreenPlayState("JediPadawanTrialArtistScreenPlay", state)
	
	-- TODO: Add more trials when implemented.
end
