...
function trialsPadawanArchitect02ConvoHandler:runScreenHandlers(pConversationTemplate, pConversingPlayer, pConversingNpc, selectedOption, pConversationScreen)
	...
	if (screenID == "nice_away") then
		CreatureObject(pConversingPlayer):setScreenPlayState(4, "JediPadawanTrialArchitectScreenPlay")
		createEvent(60 * 1000, "JediPadawanTrialArchitectScreenPlay", "despawnNPC02", pConversingNpc)
	...
	elseif (screenID == "initiate_attack_fail") then
		...
		createEvent(10 * 1000, "JediPadawanTrialArchitectScreenPlay", "despawnNPC02", pConversingNpc)
	end
