...
function PadawanTrials:resetAllPadawanTrials(pPlayer)
	...
	JediTrials:resetTrialData(pPlayer)
...
function PadawanTrials:createFirstLocation(pPlayer)
	...
	writeData(playerID .. ":trialActiveArea", areaID)
	writeData(areaID .. ":ownerID", playerID)
	createObserver(ENTEREDAREA, "PadawanTrials", "notifyEnteredTrialNpcSpawnArea", pActiveArea)
...
function PadawanTrials:notifyEnteredTrialNpcSpawnArea(pArea, pPlayer)
	...
	if pActiveArea == nil then
		printLuaError("PadawanTrials:notifyEnteredTrialTargetArea, failed to create npc destroy active area.")
		return 1
	end

	createObserver(EXITEDAREA, "PadawanTrials", "notifyExitedTrialNpcDestroyArea", pActiveArea)
	local areaID = SceneObject(pActiveArea):getObjectID()
	writeData(areaID .. ":ownerID", playerID)
...
function PadawanTrials:notifyExitedTrialNpcDestroyArea(pArea, pPlayer)
	if (pArea == nil or pPlayer == nil or not SceneObject(pPlayer):isPlayerCreature()) then
		return 0
	end
...
function PadawanTrials:createSecondLocation(pPlayer)
	...
	writeData(SceneObject(pActiveArea):getObjectID() .. ":ownerID", playerID)
	createObserver(ENTEREDAREA, "PadawanTrials", "notifyEnteredSecondaryLocationArea", pActiveArea)
...
function PadawanTrials:notifyEnteredSecondaryLocationArea(pArea, pPlayer)
	if (pArea == nil or pPlayer == nil or not SceneObject(pPlayer):isPlayerCreature()) then
		return 0
	end
	...
	local pNpc = spawnMobile(SceneObject(pPlayer):getZoneName(), trialData.targetNpc, 0, locX, locZ, locY, getRandomNumber(180) - 180, 0)

	if (pNpc == nil) then
		printLuaError("PadawanTrials:notifyEnteredSecondaryLocationArea, failed to spawn quest mobile.")
		return 1
	end

	writeData(SceneObject(pNpc):getObjectID() .. ":ownerID", playerID)
	...
	if pActiveArea == nil then
		printLuaError("PadawanTrials:notifyEnteredSecondaryLocationArea, failed to create npc destroy active area.")
		return 1
	end
