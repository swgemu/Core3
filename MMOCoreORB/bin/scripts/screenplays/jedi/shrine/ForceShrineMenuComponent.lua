...
-- Gets the closest Force Shrine to the player. Should return an objectID.
function ForceShrineMenuComponent:findNearestForceShrine(pPlayer)
	...
	local objectID = 0 -- Returning.
	local lastDist = 128000 -- Initialize as anything higher than a SWG zone.

	for i=1, #self.ForceShrineOids.planet, 1 do
    		local thisObjectID = self.ForceShrineOids.planet[i]
		local pSceneObject = getSceneObject(thisObjectID)
