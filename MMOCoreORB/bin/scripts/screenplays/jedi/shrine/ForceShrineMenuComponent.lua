...
function ForceShrineMenuComponent:doMeditate(pObject, pPlayer)
	...
	if (not isJediPadawan) then
		-- Unlock Padawan
		if (CreatureObject(pPlayer):hasScreenPlayState(64, "VillageJediProgression") == 1) then
			ForceShrineMenuComponent:unlockJediPadawan(pPlayer)
		elseif (CreatureObject(pPlayer):hasScreenPlayState(32, "VillageJediProgression") == 1) then
