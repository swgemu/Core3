...
function ForceShrineMenuComponent:startNextJediPadawanTrial(pObject, pPlayer)
	...
	-- JediPadawanTrialCraftLightsaberScreenPlay:start(pPlayer)
		...
		if (uncompletedTrials == nil or #uncompletedTrials <= 0) then -- Finished all trials.
			CreatureObject(pPlayer):setScreenPlayState(64, "VillageJediProgression") -- Unlock for padawan.
		elseif (uncompletedTrials ~= nil or #uncompletedTrials > 0) then -- Still more.
			local rand = getRandomNumber(1, #uncompletedTrials)
			local screenPlay = ForceShrineMenuComponent:getScreenplayFromGlobalTable(self.PadawanTrialQuests, uncompletedTrials[rand])
			screenPlay:start(pPlayer)
			writeScreenPlayData(pPlayer, "JediPadawanTrial", "CurrentTrial", rand)
		end
	end
