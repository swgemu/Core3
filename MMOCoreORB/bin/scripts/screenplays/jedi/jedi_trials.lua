...
function JediTrials:isOnPadawanTrials(pPlayer)
	...
	return tonumber(readScreenPlayData(pPlayer, "JediPadawanTrial", "startedTrials")) == 1
end
...
function JediTrials:setTrialLocation(pPlayer, x, z, y, planet)
	local playerID = SceneObject(pPlayer):getObjectID()
...
function JediTrials:resetTrialData(pPlayer)
	if (self:isEligibleForPadawanTrials(pPlayer)) then
