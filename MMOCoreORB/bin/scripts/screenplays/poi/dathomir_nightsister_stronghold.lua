...
function NightSisterStrongholdScreenPlay:getEvent(fourProtectAxkva)
	if (pAxkvaMin ~= nil) then
		createObserver(STARTCOMBAT, "NightSisterStrongholdScreenPlay", "@dungeon/nightsister_rancor_cave:protect", pAxkvaMin)
		createEvent("NightSisterStrongholdScreenPlay", "fourProtectAxkva", pAxkvaMin)
	elseif (pAxkvaMin == nil or CreatureObject(pAxkvaMin):isDead()) then
		return 0
	end

function NightSisterStrongholdScreenPlay:fourProtectAxkva(pFourMobiles)
	local pFourMobiles = spawnMobile("dathomir", "nightsister_protector",0,-80.5,-99.7,-101.6,-171,4115629)
	pFourMobiles = spawnMobile("dathomir", "nightsister_protector",0,-94.5,-100.8,-106.8,166,4115629)
	pFourMobiles = spawnMobile("dathomir", "nightsister_protector",0,-85.9,-100.8,-105.4,-118,4115629)
	pFourMobiles = spawnMobile("dathomir", "nightsister_protector",0,-99.1,-99.8,-105.1,168,4115629)
	createObserver(EXITEDAREA, "NightSisterStrongholdScreenPlay", "despawnProtectors", pFourMobiles)
	end
end

function NightSisterStrongholdScreenPlay:despawnProtectors(PlayerCreature)
	if PlayerCreature == nil then
	CreatureObject(pFourMobiles):destroyObjectFromWorld(getObjectID)
	return 0
	end
end
