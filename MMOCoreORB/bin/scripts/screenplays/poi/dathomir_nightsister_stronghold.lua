...
function NightSisterStrongholdScreenPlay:spawnAxkva()
  ...
  if (pAxkvaMin ~= nil) then
    ...
    createObserver(CALLFORHELP, "NightSisterStrongholdScreenPlay", "startCombatLogic", pAxkvaMin)
  end
...
function NightSisterStrongholdScreenPlay:spawnCombatArea()
  ...
  if (pActiveArea ~= nil) then
    ...
    createObserver(DEFENDERDROPPED, "NightSisterStrongholdScreenPlay", "clearProtectors", pActiveArea)
  end
end
...
function NightSisterStrongholdScreenPlay:saveID(pCreatureObject, id)
  CreatureObject(pCreatureObject):getObjectID()
...
function NightSisterStrongholdScreenPlay:despawnProtectors(pActiveArea)
  for i = 1 , 4 , 1 do
    ...
    self:destroy(deleteData("NightSisterStrongholdScreenPlay:CreatureID_" .. i))
  end
  return 0
end

function NightSisterStrongholdScreenPlay:forcePeace(pActiveArea)
  self:forcePeace(getCreatureObject(readData("NightSisterStrongholdScreenPlay:CreatureID_0")))
  return 0
end

function NightSisterStrongholdScreenPlay:clearProtectors(pActiveArea)
  local sObj = getSceneObject(objectID)
  if (sObj ~= nil) then
    if (pAxkvaMin ~= nil and (AiAgent(pAxkvaMin):isPeaced())) then
      sObj:destroyObjectFromWorld(getCreatureObject(deleteData("NightSisterStrongholdScreenPlay:CreatureID_0" .. i)))
    end
   end
  return 0
end
...
function NightSisterStrongholdScreenPlay:destroy(objectID)
  ...
  if (sObj ~= nil) then
    for i = 1 , 4 , 1 do
      sObj:destroyObjectFromWorld(getCreatureObject(readData("NightSisterStrongholdScreenPlay:CreatureID_0")))
      sObj:destroyObjectFromWorld(getCreatureObject(deleteData("NightSisterStrongholdScreenPlay:CreatureID_0")))
      sObj:destroyObjectFromWorld(getCreatureObject(readData("NightSisterStrongholdScreenPlay:CreatureID_0" .. i)))
      sObj:destroyObjectFromWorld(getCreatureObject(deleteData("NightSisterStrongholdScreenPlay:CreatureID_0" .. i)))
    end
  end
  return 0
