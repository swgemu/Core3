...
function NightSisterStrongholdScreenPlay:spawnMobiles()
  ...
  --in the cave, make difficulty scale as player progresses into the cave
  ...
  local pAxkvaMin = spawnMobile("dathomir", "axkva_min",6400,-90.5,-101,-102.2,172,4115629)
    if (pAxkvaMin ~= nil) then
      createObserver(DAMAGERECEIVED, "NightSisterStrongholdScreenPlay", "startCombatLogic", pAxkvaMin)
    end

end

function NightSisterStrongholdScreenPlay:startCombatLogic(pAxkvaMin, pAttacker, damage)
    if pAxkvaMin == nil or pAttacker == nil then
      return 1
    end

    return ObjectManager.withCreatureObject(pAxkvaMin, function(axkvamin)
     if (pAxkvaMin ~= nil and ((axkvamin:getHAM(0) <= (axkvamin:getMaxHAM(0) * 0.9)) or (axkvamin:getHAM(3) <= (axkvamin:getMaxHAM(3) * 0.9)) or (axkvamin:getHAM(6) <= (axkvamin:getMaxHAM(6) * 0.9)))) then
          local pDefender = spawnMobile("dathomir", "nightsister_protector",0,-80.5,-99.7,-101.6,-171,4115629)
          self:saveID(pDefender,1)
          pDefender = spawnMobile("dathomir", "nightsister_protector",0,-94.5,-100.8,-106.8,166,4115629)
          self:saveID(pDefender,2)
          pDefender = spawnMobile("dathomir", "nightsister_protector",0,-85.9,-100.8,-105.4,-118,4115629)
          self:saveID(pDefender,3)
          pDefender = spawnMobile("dathomir", "nightsister_protector",0,-99.1,-99.8,-105.1,168,4115629)
          self:saveID(pDefender,4)
          spatialChat(pAxkvaMin, "@dungeon/nightsister_rancor_cave:protect")
          createObserver(PEACE, "NightSisterStrongholdScreenPlay", "forceDespawnProtectors", pAxkvaMin)
          writeData("nightsisterstronghold:protectors",1)
      return 1
     else
      return 0
     end
...
function NightSisterStrongholdScreenPlay:forceDespawnProtectors(pAxkvaMin, pDefender)
    if (readData("nightsisterstronghold:protectors") == 1) and not (AiAgent(pAxkvaMin):isInCombat()) then
      for i = 1 , 4 , 1 do
        pDefender:destroyObjectFromWorld(getCreatureObject(readData("NightSisterStrongholdScreenPlay:CreatureID_0")))
        pDefender:destroyObjectFromWorld(getCreatureObject(deleteData("NightSisterStrongholdScreenPlay:CreatureID_0")))
        pDefender:destroyObjectFromWorld(getCreatureObject(readData("NightSisterStrongholdScreenPlay:CreatureID_0" .. i)))
        pDefender:destroyObjectFromWorld(getCreatureObject(deleteData("NightSisterStrongholdScreenPlay:CreatureID_0" .. i)))
      end
      writeData("nightsisterstronghold:protectors", 0)
    end

    return 1
end

function NightSisterStrongholdScreenPlay:saveID(pDefender, id)
  CreatureObject(pDefender):getObjectID()
    writeData("NightSisterStrongholdScreenPlay:CreatureID_" .. id)

end

--TO DO: what if players try to DRAG Axkva OUT of the combatarea out of aggro range of the Protector spawns?
--TO DO: what happens if/when Axkva DIES but Protector(s) are still alive?
