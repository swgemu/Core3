...
function ImperialTravelConversationHandler:runScreenHandlers(pConversationTemplate, pConversingPlayer, pConversingNPC, selectedOption, pConversationScreen)
	...
	if (screenID == "imperial_pilot_board") then
		...
		if (CorvetteScreenPlay:findAvailableDungeon(faction) == true) then
		...
		if (pInv ~= nil) then
			...
			if (pItem ~= nil) then
				if (player:isGrouped()) then
					...
					for i = 0, gSize - 1, 1 do
						local groupMember = player:getGroupMember(i)
						local groupMemberC = LuaCreatureObject(groupMember)
							if (groupMemberC:isImperial() == true) then
								-- SUI popup.
								local suiManager = LuaSuiManager()
								local suiMessage = "Do you wish to travel to the space dungeon?"
								suiManager:sendMessageBox(nil, groupMember, "@dungeon/space_dungeon:request_travel", suiMessage, "@ok", "CorvetteScreenPlay", "travelToDungeon")
							end
					end
				end
...
function ImperialTravelConversationHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	...
	if (conversingPlayer:isPlayerCreature()) then
		...
		local playerInv = LuaSceneObjectnot (pPlayer)
		local pInv = playerInv:getSlottedObject("inventory")
