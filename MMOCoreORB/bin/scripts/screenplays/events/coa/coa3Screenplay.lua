...
function Coa3Screenplay:setupCommanderMission(pPlayer, faction)
	...
	print("setupCommanderMission called -- faction = " .. faction .. " with commanderID: " .. commanderID)
...
function Coa3Screenplay:spawnCommanderObjective(pPlayer, pCommander, faction)
	...
	if (faction == "imperial") then
		...
		local pTerminal = spawnSceneObject(planet, "object/tangible/furniture/imperial/data_terminal_s1.iff", terX, z, termY, 0, 0)

		if (pTerminal ~= nil) then
			SceneObject(pTerminal):setObjectMenuComponent("DestructibleBuildingMenuComponent")
			spawnedObjects[#spawnedObjects + 1] = tostring(SceneObject(pTerminal):getObjectID())
		end
	end
...
function Coa3Screenplay:spawnFinalMobiles(pPlayer, pArea)
	...
	print(" spawnFinalMobiles called -- faction = " .. faction)
	...
	if (faction == "rebel") then
		mobiles = self.finalRebelMobiles
	else
		mobiles = self.finalImperialMobiles
	end
...
function Coa3Screenplay:spawnAssaultWave(pArea)
	...
	for i = 1, waveNum + 2, 1 do
		...
		if (pMobile1 ~= nil) then
			...
			AiAgent(pMobile1):setNextPosition(x + getRandomNumber(2, 14), getWorldFloor(x, y, planet), y + 35, 0)
	...
	for j = 1, waveNum, 1 do
		...
		if (pMobile2 ~= nil) then
			...
			AiAgent(pMobile2):setNextPosition(x + getRandomNumber(2, 14), getWorldFloor(x, y, planet), y + 35, 0)
	...
	print(" spawning assualt wave #" .. waveNum .. " totalAttackers = " .. totalAttackers)
...
function Coa3Screenplay:onLootCaravanLeader(pLootedCreature, pLooter, nothing)
	...
	if (state == self.M2_FIND_CARAVAN_LEADER) then
		...
		Coa3Screenplay:abortMission(pLooter, 1, 1)
		return 1
	end
...
function Coa3Screenplay:rebelDefendMobileKilled(pMobile, pKiller)
	...
	if (mobFaction == FACTIONREBEL) then
		...
		print("rebelDefendMobileKilled -- totalGuards = " .. totalGuards .. " owner ID =" .. ownerID)
		...
		if (totalGuards <= 0) then
			...
			self:abortMission(pOwner, Coa3Screenplay.COMMANDER_MISSION, 1)

			local stateKey = self:getStateKey(mobFaction)
	...
	elseif (mobFaction == FACTIONIMPERIAL) then
		...
		if (totalAttackers <= 0 and waveNum >= 3) then
			...
			if (pOwner ~= nil and SceneObject(pOwner):isPlayerCreature()) then
				CreatureObject(pOwner):sendSystemMessage("@theme_park/alderaan/act3/shared_rebel_missions:mission_complete")
