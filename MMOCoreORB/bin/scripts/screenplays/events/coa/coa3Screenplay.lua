...
function Coa3Screenplay:setupCommanderMission(pPlayer, faction)
	...
	writeVector3Data(playerID .. ":CoA3:commanderLocation:", x, 0, y)
...
function Coa3Screenplay:spawnCommanderObjective(pPlayer, pCommander, faction)
	...
	local planet = SceneObject(pCommander):getZoneName()
	local commanderLoc = readVector3Data(playerID .. ":CoA3:commanderLocation:")

	local missionLoc = getSpawnArea(planet, commanderLoc[1], commandLoc[3], 1500, 2500, 20, 10, true)

	local spawnedObjects = {}
	local spawnName = ""

	if (faction == "rebel") then
		spawnName = "object/building/poi/coa3_rebel_defend.iff"
	else

	end

	-- Spawn Mission Building
	local pBuilding = spawnBuilding(pPlayer, spawnName, x, y, 0)
	...
	if (faction == "rebel") then
		createEvent(2000, "Coa3Screenplay", "spawnFinalMobiles", pBuilding, faction)
	end


	--TODO: Rebel mission will spawn 3 attack waves. If all the guards are killed the player fails. Waves are every 60 to 90s

end

function Coa3Screenplay:spawnFinalMobiles(pBuilding)
	if (pBuilding == nil) then
		return
	end
	...
	if (pPlayer == nil) then
		self:abortMission(pPlayer, Coa3Screenplay.COMMANDER_MISSION, true)
		return
	end
...
function Coa3Screenplay:getRandomCommander(faction)
	...
	for i = 1, #coa3commanders, 1 do
		...
		if string.find(commander[1], faction) ~= nil and isZoneEnabled(commander[2]) then
