...
function Coa3Screenplay:createMissionWaypoint(pPlayer, x, y, planet, missionNum, faction)
	...
	local wayID = PlayerObject(pGhost):addWaypoint(planet, wayName, wayDesc, location[1], location[3], WAYPOINTYELLOW, true, true, WAYPOINTQUESTTASK)
...
function Coa3Screenplay:updateMissionLocation(pPlayer, nill, zoneNameHash)
	...
	local location = readVector3Data(playerID .. ":CoA3:missionLoc:")
	...
	deleteVector3Data(playerID .. ":CoA3:missionLoc:")
...
function Coa3Screenplay:getFactionString(pPlayer)
	if (pPlayer == nil) then
		return "neutral"
	end
