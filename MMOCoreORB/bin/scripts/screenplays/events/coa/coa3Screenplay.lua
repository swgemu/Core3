...
function Coa3Screenplay:enteredMissionArea(pArea, pPlayer)
	...
	--print("eneteredMissionArea -- areaRadius = " .. areaRadius .. " zoneName = ".. zoneName ..  " missionNum = " .. missionNum .. " spawnName = " .. spawnName)
...
function Coa3Screenplay:spawnCommanderObjective(pPlayer, pCommander, faction)
	...
	if (faction == "imperial") then
		...
		local pTerminal = spawnSceneObject(planet, "object/tangible/furniture/imperial/data_terminal_s1.iff", termX, termZ, termY, 0, 0)
...
function Coa3Screenplay:notifyDestroyedWarehouse(pBuilding, pBuilding2)
	...
	for i = 1, 5, 1 do
		...
		playClientEffectLoc(pPlayer, "clienteffect/dth_watch_water_pressure.cef", planet, x, z, y, 0)
	end
	...
	if (state >= self.M5_GO_TO_FACILITY) then
