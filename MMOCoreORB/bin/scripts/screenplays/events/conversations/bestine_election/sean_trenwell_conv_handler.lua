...
function seanTrenwellConvoHandler:hasNegativeEvidence(pPlayer)
	...
	if (pInventory ~= nil) then
		return (getContainerObjectByTemplate(pInventory, "object/tangible/loot/quest/victor_questn_hlist.iff" , true) 
...
function seanTrenwellConvoHandler:removeEvidence(pPlayer)
	...
	if (pInventory ~= nil) then
		...
		local evidenceItem = getContainerObjectByTemplate(pInventory,"object/tangible/loot/quest/victor_questn_dseal.iff", true)
		if (evidenceItem ~= nil) then
			SceneObject(evidenceItem):destroyObjectFromWorld()
			SceneObject(evidenceItem):destroyObjectFromDatabase()
		end
		local evidenceItem = getContainerObjectByTemplate(pInventory, "object/tangible/loot/quest/victor_questn_journal.iff", true)
		if (evidenceItem ~= nil) then
			SceneObject(evidenceItem):destroyObjectFromWorld()
			SceneObject(evidenceItem):destroyObjectFromDatabase()
		end
...
function seanTrenwellConvoHandler:GiveHistoryReward(pPlayer)
	...
	writeScreenPlayData(pPlayer, "BestineElection", completedSeanHistoryQuest, electionNum)
	deleteScreenPlayData(pPlayer, "BestineElection", ":bestine_election:disk_destroyed")
