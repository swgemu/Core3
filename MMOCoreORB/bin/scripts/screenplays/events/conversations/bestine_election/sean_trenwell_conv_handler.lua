...
function seanTrenwellConvoHandler:returned_noroom(pPlayer)
	if (pPlayer == nil) then
		return false
	end
	...
	local playerCampaign = tonumber(readData(playerID..":bestine_election:sean_noroom"))
	return (playerCampaign ~= nil) and (playerCampaign == electionNum)
end

function seanTrenwellConvoHandler:hasSeanNegQuest(pPlayer)
	if (pPlayer == nil) then
		return false
	end
	...
	local playerID = CreatureObject(pPlayer):getObjectID()
	local electionNum = BestineElectionScreenPlay:getElectionNumber()
	local playerCampaign = tonumber(readScreenPlayData(pPlayer, "BestineElection", "seanNegQuest"))
	return (playerCampaign ~= nil) and (playerCampaign == electionNum)
end
...
function seanTrenwellConvoHandler:giveDiskandJoinCampaign(pPlayer)
	...
	if (pInventory ~= nil) then
		...
		deleteScreenPlayData(pPlayer, "BestineElection", "historyq_searched")
	end
end
...
function seanTrenwellConvoHandler:GiveHistoryReward(pPlayer)
	...
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	if (pInventory ~= nil) then
		local reward = giveItem(pInventory, historyReward, -1)
		if (reward == nil) then
	...
	deleteScreenPlayData(pPlayer, "BestineElection", "disk_destroyed")
...
function seanTrenwellConvoHandler:giveHistoryQuest(pPlayer)
	...
	deleteScreenPlayData(pPlayer, "BestineElection", "completedSeanHistoryQuest")
	writeScreenPlayData(pPlayer, "BestineElection", "SeanHistoryQuest", electionNum)
	local historyWaypointID = PlayerObject(pObj):addWaypoint("tatooine", "Historical Site", "", -758, -3907, WAYPOINT_COLOR_BLUE, true, true, 0,0)
...
function seanTrenwellConvoHandler:removeHistoryQuest(pPlayer)
	...
	PlayerObject(playerID):removeWaypoint(wayID, true)
	deleteScreenPlayData(pPlayer, "BestineElection", "SeanHistoryQuest")
...
function seanTrenwellConvoHandler:giveContactQuest(pPlayer)
	...
	local electionNum = BestineElectionScreenPlay:getElectionNumber()
	writeScreenPlayData(pPlayer, "BestineElection", "seanContact",electionNum)
	--writeData(playerID .. ":bestine_election:seanContact",1)
	local pObj = CreatureObject(pPlayer):getPlayerObject()
	local contactWaypointID = PlayerObject(pObj):addWaypoint("tatooine", "Sean's Contact", "", -1448, -3765, WAYPOINT_COLOR_BLUE, true, true, 0,0)
	local playerID = CreatureObject(pPlayer):getObjectID()
	writeData(playerID .. ":bestine_election:contactWaypointID", contactWaypointID)
end

function seanTrenwellConvoHandler:tellAboutHistoryQuest(pPlayer)
	if (pPlayer == nil) then
		return false
	end
	...
	if readScreenPlayData(pPlayer, "BestineElection", "SeanHistoryQuest") == nil then
		if readScreenPlayData(pPlayer, "BestineElection", "hquest_find") == nil then
			if readScreenPlayData(pPlayer, "BestineElection", "hquest_itemFound") == nil then
				if readScreenPlayData(pPlayer, "BestineElection", "seanContact") == nil then
					if readScreenPlayData(pPlayer, "BestineElection", "DestroyedDisk") == nil then
						if readScreenPlayData(pPlayer, "BestineElection", "completedSeanHistoryQuest") ==nil then
							return true
						else
							local playerCampaign = tonumber(readScreenPlayData(pPlayer, "BestineElection", "completedSeanHistoryQuest"))
...
function seanTrenwellConvoHandler:historyItemfound(pPlayer)
	...
	if tonumber(readScreenPlayData(pPlayer, "BestineElection", "historyq_searched")) == 1 then
		if getQuestStatus("bestine_election:electionWinner") == "Sean" then
			local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
			if pInventory ~= nil then
				return (getContainerObjectByTemplate(pInventory, "object/tangible/loot/quest/sean_history_disk.iff", true) ~= nil)
			end
		end
	end
	return false;
...
function seanTrenwellConvoHandler:inOffice_newElection(pPlayer)
	...
	if (BestineElectionScreenPlay:getCurrentPhase() == 1) then
		if (not BestineElectionScreenPlay:votedSean(pPlayer)) and (not BestineElectionScreenPlay:votedVictor(pPlayer)) then
			return true
		end
	end
	return false
...
function seanTrenwellConvoHandler:getInitialScreen(pPlayer, npc, pConversationTemplate)
	...
	if (BestineElectionScreenPlay:getCurrentPhase() == 1) then
		if self:returned_noroom(pPlayer) then
...
function seanTrenwellConvoHandler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	if (screenID == "historyQuest_DestroyedDisk_noroom") then
		writeScreenPlayData(conversingPlayer, "BestineElection", "noRoomForHistoryReward",1)
