...
function seanTrenwellConvoHandler:inOffice_newElection(pPlayer)
	...
	if (BestineElectionScreenPlay:getCurrentPhase() == 1) then
		if (not BestineElectionScreenPlay:votedFor(pPlayer,"Sean")) or (not BestineElectionScreenPlay:votedFor(pPlayer,"Victor")) then
			return true
		end
		...
		if (playerCampaign == nil) then
			...
			if (playerCampaign == nil) then
				...
				if (playerCampaign ~= currentElection) then
					return true
				end
			end
		end
...
function seanTrenwellConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "sean_inOffice") then
		...
		elseif CreatureObject(pPlayer):hasScreenPlayState(2, "bestine_history_quest") then
		...
		elseif CreatureObject(pPlayer):hasScreenPlayState(4, "bestine_history_quest") then
...
function seanTrenwellConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	if self:inCampaign(pPlayer) then
