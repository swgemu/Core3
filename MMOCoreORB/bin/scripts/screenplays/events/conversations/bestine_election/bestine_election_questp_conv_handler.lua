...
function BestineElectionQuestpConvoHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	...
	print("current phase"..BestineElectionScreenPlay:getCurrentPhase())
	local blah = BestineElectionScreenPlay:isOnNegativeQuest(pPlayer, "Sean")
	if blah then s = "true" else s = "false" end
	print("isonneg quest sean:"..s)
	blah = BestineElectionScreenPlay:isOnNegativeQuest(pPlayer, "Victor")
	if blah then s = "true" else s = "false" end
	print("isonneg quest victor:"..s)
	blah = BestineElectionScreenPlay:joinedCampaign(pPlayer, "Sean")
	if blah then s = "true" else s = "false" end
	print("joined campaign sean:"..s)
	blah = BestineElectionScreenPlay:joinedCampaign(pPlayer, "Victor")
	if blah then s = "true" else s = "false" end
	print("joined campaign victor:"..s)
	local campaign = tonumber(readScreenPlayData(pPlayer, "BestineElection", "SeanCampaign"))
	if campaign == nil then campaign = "No" end
	print("joined campiagn sean?: "..campaign)
	campaign = tonumber(readScreenPlayData(pPlayer, "BestineElection", "VictorCampaign"))
	if campaign == nil then campaign = "No" end
	print("joined campaign victor?: "..campaign)
