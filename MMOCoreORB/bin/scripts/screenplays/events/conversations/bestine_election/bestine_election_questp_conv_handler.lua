...
function BestineElectionQuestpConvoHandler:giveQuestpItem(pPlayer,quest)
	...
	if pInventory ~= nil then
		if quest == "slums" then
			local pItem = giveItem(pInventory, SLUMS, -1)
			if (pItem == nil) then
				CreatureObject(pPlayer):sendSystemMessage("Error: Unable to generate item.")
				return
			end
		elseif quest == "hospital" then
			local pItem = giveItem(pInventory, HOSPITAL, -1)
			if (pItem == nil) then
				CreatureObject(pPlayer):sendSystemMessage("Error: Unable to generate item.")
				return
			end
		elseif quest == "market" then
			local pItem = giveItem(pInventory, MARKET, -1)
			if (pItem == nil) then
				CreatureObject(pPlayer):sendSystemMessage("Error: Unable to generate item.")
				return
			end
		elseif quest == "house" then
			local pItem = giveItem(pInventory, HOUSE, -1)
			if (pItem == nil) then
				CreatureObject(pPlayer):sendSystemMessage("Error: Unable to generate item.")
				return
			end
...
function BestineElectionQuestpConvoHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	...
	elseif (string.find(objectName,"sean") and not seanTrenwellConvoHandler:seanNegQuest(pPlayer)) or (string.find(objectName,"victor") and not victorVisalisConvoHandler:victorNegQuest(pPlayer)) then
	...
	elseif self:hasNoRoom(pPlayer,questID) then
		return convoTemplate:getScreen("noroom_returned")
	elseif BestineElectionScreenPlay:joinedVictorCampaign(pPlayer) or BestineElectionScreenPlay:joinedSeanCampaign(pPlayer) then
