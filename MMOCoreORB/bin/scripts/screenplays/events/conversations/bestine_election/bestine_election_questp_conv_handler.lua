...
function BestineElectionQuestpConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	elseif ((string.find(npcObject,"victor") ~=nil) and (BestineElectionScreenPlay:joinedCampaign(pPlayer, "victor"))) then
		return convoTemplate:getScreen("convo_start")
	elseif ((string.find(npcObject,"sean") ~= nil) and (BestineElectionScreenPlay:joinedCampaign(pPlayer, "sean"))) then
		return convoTemplate:getScreen("convo_start")
	end
	return convoTemplate:getScreen("no_business")
end
