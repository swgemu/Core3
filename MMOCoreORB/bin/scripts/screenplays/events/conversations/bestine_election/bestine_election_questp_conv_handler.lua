...
function BestineElectionQuestpConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	local npcObject = SceneObject(pNpc):getTemplateObjectPath()
	local questID = self:getQuestID(pNpc)
	...
	if BestineElectionScreenPlay:noroomReturned(pPlayer,questID) then
		return convoTemplate:getScreen("noroom_returned")
	end
	if ((string.find(npcObject,"victor") ~=nil) and (BestineElectionScreenPlay:joinedCampaign(pPlayer, "Victor"))) then
		return convoTemplate:getScreen("convo_start")
	end
	
	if ((string.find(npcObject,"sean") ~= nil) and (BestineElectionScreenPlay:joinedCampaign(pPlayer, "Sean"))) then
		return convoTemplate:getScreen("convo_start")
	end
