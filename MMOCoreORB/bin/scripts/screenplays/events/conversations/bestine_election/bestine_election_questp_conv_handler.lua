...
function BestineElectionQuestpConvoHandler:hasNoRoom(pPlayer,quest)
	if (BestineElectionScreenPlay:getCurrentPhase() == 1) then	
		...
		if BestineElectionScreenPlay:VictorCampaign(pPlayer) or BestineElectionScreenPlay:SeanCampaign(pPlayer) then
...
function BestineElectionQuestpConvoHandler:giveQustpItem(pPlayer,quest)
	local playerID = CreatureObject(pPlayer):getObjectID()
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
...
function BestineElectionQuestpConvoHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	...
	if string.find(SceneObject(pNpc):getTemplateObjectPath(), "sean") ~= nil and (not BestineElectionScreenPlay:SeanCampaign(pPlayer)) then
		return convoTemplate:getScreen("default_reply")
	end
	
	if string.find(SceneObject(pNpc):getTemplateObjectPath(), "victor") ~= nil and (not BestineElectionScreenPlay:VictorCampaign(pPlayer)) then
		return convoTemplate:getScreen("default_reply")
	end
...
function BestineElectionQuestpConvoHandler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	if (screenID == "give_item") then
		self:giveQustpItem(conversingPlayer,questID)
	end
