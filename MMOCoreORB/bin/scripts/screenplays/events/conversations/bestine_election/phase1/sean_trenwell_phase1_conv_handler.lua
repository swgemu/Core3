...
seanTrenwellPhase1ConvoHandler = conv_handler:new {	}
...
function seanTrenwellPhase1ConvoHandler:gaveDiskToHutt(pPlayer)
	if (pPlayer ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:hasNegativeEvidence(pPlayer)
	if (pPlayer ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:canDoHistoryQuest(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function seanTrenwellPhase1ConvoHandler:giveHistoryQuest(pPlayer)
	if (pPlayer ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:giveContactQuest(pPlayer)
	if (pPlayer ~= nil) then
		local playerID = SceneObject(pPlayer):getObjectID()
		local pPlayerObj = CreatureObject(pPlayer):getPlayerObject()
		if (pPlayerObj ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:removeHistoryQuest(pPlayer)
	if (pPlayer ~= nil) then
		local playerID = SceneObject(pPlayer):getObjectID()
		local pPlayerObj = CreatureObject(pPlayer):getPlayerObject()
		if (pPlayerObj ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:checkRewardGiven(pPlayer)
	if (pPlayer ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:giveVoteReward(pPlayer)
	if (pPlayer ~= nil) then
		local electionNum = BestineElectionScreenPlay:getElectionNumber()
		local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
		if (pInventory ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:giveHistoryReward(pPlayer)
	if (pPlayer ~= nil) then
		local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
		if (pInventory ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:giveDiskandJoinCampaign(pPlayer)
	if (pPlayer ~= nil) then
		local electionNum = BestineElectionScreenPlay:getElectionNumber()
		local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
		if (pInventory ~= nil) then
...
function seanTrenwellPhase1ConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "returned_noroom") then
		if BestineElectionScreenPlay:hasFullInventory(pPlayer) then
			...
			clonedConversation:addOption("@conversation/sean_trenwell:s_ff7aeed7","joincampaign_givedisk") -- Yes, that was me. (maderoom)
		end
	...
	elseif (screenID == "nquest_found") then
		if BestineElectionScreenPlay:hasFullInventory(pPlayer) then
			...
			clonedConversation:addOption("@conversation/sean_trenwell:s_641d98d5","joincampaign_givedisk") --Yes, of course!
		end
	...
	elseif (screenID == "here_to_vote_continue") then
		if BestineElectionScreenPlay:hasFullInventory(pPlayer) then
			...
			clonedConversation:addOption("@conversation/sean_trenwell:s_ef067f71","joincampaign_givedisk")--Alright. I want to join your campaign.
		...
		clonedConversation:addOption("@conversation/sean_trenwell:s_dea56128","goodbye_pleasant")--I have to go.
	elseif (screenID == "ask_voted") then
		if BestineElectionScreenPlay:votedCurrentElection(pPlayer, "sean") then
	...
	elseif (screenID == "joincampaign_givedisk") then
...
function seanTrenwellPhase1ConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
