...
function victorVisalisConvoHandler:inOffice_newElection(pPlayer)
	...
	if (BestineElectionScreenPlay:getCurrentPhase() == 1) then
		if (not BestineElectionScreenPlay:votedForWho(pPlayer,"Victor")) or (not BestineElectionScreenPlay:votedForWho(pPlayer,"Sean")) then
			return true
		end
...
function victorVisalisConvoHandler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	elseif (screenID == "victor_inOffice") then
		...
		elseif (not self:hasTuskenHead(conversingPlayer)) and CreatureObject(conversingPlayer):hasScreenPlayState(1, "bestine_tusken_quest") then
		...
		if (self:inOffice_newElection(conversingPlayer)) then
			...
			elseif seanTrenwellConvoHandler:votedSeanCurrentElection(conversingPlayer) then
	...
	elseif (screenID == "tusken_quest_complete") then
		if not self:checkForStones(conversingPlayer) then
