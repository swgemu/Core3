...
function victorVisalisConvoHandler:giveTuskenReward(pPlayer)
	...
	return ObjectManager.withCreatureAndPlayerObject(pPlayer, function(player, playerObject)
		...
		if (pInventory ~= nil) then
			...
			local waypointID = readData(player:getObjectID() .. ":bestine_election:tuskenWaypointID")
			deleteData(player:getObjectID() .. ":bestine_election:tuskenWaypointID")
			playerObject:removeWaypoint(waypointID, true)
			player:removeScreenPlayState(1, "victorTuskenQuest")
		end
	end)
end
...
function victorVisalisConvoHandler:giveTuskenQuest(pPlayer)
	...
	local electionNum = BestineElectionScreenPlay:getElectionNumber()
	return ObjectManager.withCreatureAndPlayerObject(pPlayer, function(player, playerObject)
		local tuskenWaypointID = playerObject:addWaypoint("tatooine", "Fort Tusken", "", -3960, 6233, WAYPOINT_COLOR_PURPLE, true, true, 0,0)
		player:setScreenPlayState(1, "victorTuskenQuest")
		writeData(player:getObjectID() .. ":bestine_election:tuskenWaypointID", tuskenWaypointID)
	end)
end
...
function victorVisalisConvoHandler:removeTuskenQuest(pPlayer)
	...
	return ObjectManager.withCreatureAndPlayerObject(pPlayer, function(player, playerObject)
		local wayID = readData(player:getObjectID() .. ":bestine:tuskenWaypointID")
