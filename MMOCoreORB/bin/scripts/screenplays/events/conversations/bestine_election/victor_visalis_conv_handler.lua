...
function victorVisalisConvoHandler:giveTuskenReward(pPlayer)
	if (pPlayer ~= nil) then
		...
		if (pInventory ~= nil) then
			...
			CreatureObject(pPlayer):setScreenPlayState(2, "bestine_tusken_quest")
		end
	end
end


function victorVisalisConvoHandler:receivedTuskenReward(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function victorVisalisConvoHandler:giveTuskenQuest(pPlayer)
	if (pPlayer ~= nil) then
		deleteScreenPlayData(pPlayer, "BestineElection", "receivedTuskenReward")
		self:tuskenWaypoint(pPlayer)
		CreatureObject(pPlayer):setScreenPlayState(1, "bestine_tusken_quest")
		CreatureObject(pPlayer):removeScreenPlayState(2, "bestine_tusken_quest")
	end
end
...
function victorVisalisConvoHandler:removeTuskenQuest(pPlayer)
	if (pPlayer ~= nil) then
		local playerID = SceneObject(pPlayer):getObjectID()
		local waypointID = readData(playerID.. ":bestine:tuskenWaypointID")
...
function victorVisalisConvoHandler:giveDiskandJoinCampaign(pPlayer)
	if (pPlayer ~= nil) then
		...
		if (pInventory ~= nil) then
			...
			deleteScreenPlayData(pPlayer, "BestineElection", "receivedVictorReward")
...
function victorVisalisConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "nquest_found") then
		...
		clonedConversation:addOption("@conversation/victor_visalis:s_5c46daeb","joincampaign_changedmind")--I've changed my mind.
	...
	elseif (screenID == "victor_inOffice") then
		if (CreatureObject(pPlayer):hasScreenPlayState(1, "bestine_tusken_quest")) then
		...
		elseif (CreatureObject(pPlayer):hasScreenPlayState(2, "bestine_tusken_quest")) then
		...
		if (self:hasTuskenHead(pPlayer) and CreatureObject(pPlayer):hasScreenPlayState(1, "bestine_tusken_quest")) then
