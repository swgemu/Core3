...
function seansHistorianConvoHandler:hasHistoryQuest(pPlayer)
	...
	local playerCampaign = tonumber(readScreenPlayData(pPlayer, "BestineElection", "SeanHistoryQuest" ))
...
function seansHistorianConvoHandler:getInitialScreen(pPlayer, npc, pConversationTemplate)
	...
	if tonumber(readScreenPlayData(conversingPlayer, "BestineElection", "hquest_find")) == 1 then
...
function seansHistorianConvoHandler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	if (screenID == "tellme_history_3") then
		writeScreenPlayData(conversingPlayer, "BestineElection", "hquest_find",1)
		deleteScreenPlayData(conversingPlayer, "BestineElection", "SeanHistoryQuest")
	...
	elseif (screenID == "found_something") then
			writeScreenPlayData(conversingPlayer, "BestineElection", "hquest_itemFound", 1)
			deleteScreenPlayData(conversingPlayer, "BestineElection", "hquest_find")
			deleteScreenPlayData(conversingPlayer, "BestineElection", "SeanHistoryQuest")
			local playerID = CreatureObject(conversingPlayer):getPlayerObject()
