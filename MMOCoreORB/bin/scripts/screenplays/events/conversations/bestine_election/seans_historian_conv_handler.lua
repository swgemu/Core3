...
function seansHistorianConvoHandler:hasHistoryQuest(pPlayer)
	if (pPlayer ~= nil) then
		if (getQuestStatus("bestine_election:electionWinner") == "sean") and (CreatureObject(pPlayer):hasScreenPlayState(1, "bestine_history_quest")) then
			return true
		end
	end
	return false
...
function seansHistorianConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	if (not BestineElectionScreenPlay:isElectionEnabled()) or getQuestStatus("bestine_election:electionWinner") ~= "sean" then
	...
	elseif CreatureObject(pPlayer):hasScreenPlayState(2, "bestine_history_quest") or
		CreatureObject(pPlayer):hasScreenPlayState(4, "bestine_history_quest") then
