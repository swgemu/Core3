...
function BestineElectionQuestnConvoHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	...
	local pNpcName = SceneObject(pNpc):getObjectName()
	...
	elseif self:hasNoRoomVar(pPlayer,questID) then
			return convoTemplate:getScreen("noroom_returned")
	elseif (pNpcName == "Victor Visalis" and seanTrenwellConvoHandler:seanNegQuest(pPlayer)) or (pNpcName == "Sean Trenwell" and victorVisalisConvoHandler:victorNegQuest(pPlayer)) then
