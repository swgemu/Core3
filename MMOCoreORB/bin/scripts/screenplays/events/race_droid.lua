...
function raceDroidEventPerk:suiSetupCallback(pPlayer, pSui, eventIndex, args)
	...
	if (curStep == 0) then
		...
		CreatureObject(pPlayer):sendSystemMessage("Number of waypoints: " .. numWaypoints)
...
function raceDroidEventPerk:doCleanup(pPlayer)
	...
	if (readData(playerID .. ":eventPerkRace:currentlyRacing") == 1) then
		...
		local areaID = readData(playerID .. ":eventPerkRace:nextPointWaypointID")
...
function raceDroidEventPerk:sendStoredData(pNpc, pPlayer)
	...
	CreatureObject(pPlayer):sendSystemMessage("@event_perk:race_show_status0") -- 	Please review your waypoint data. If you need to make changes you can use "Set Race Parameters". If everything is correct choose "Initialize Droid" to begin race operation. NOTE: You may not make changes after the droid is initialized.

	for i = 1, totalWaypoints, 1 do
		local waypoint = self:getRacePoint(pNpc, i)
		CreatureObject(pPlayer):sendSystemMessage(getStringId("@event_perk:race_waypoint") .. " " .. i .. ": " .. waypoint[1] .. " " .. waypoint[2])
	end
end

function raceDroidEventPerk:initializeDroid(pNpc)
...
function RaceDroidPerkMenuComponent:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	elseif (selectedID == 122) then
		raceDroidEventPerk:initializeDroid(pSceneObject)
	end
