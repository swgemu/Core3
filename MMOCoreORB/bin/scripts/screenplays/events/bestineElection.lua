...
function BestineElectionScreenPlay:setupTerminals()
	...
	if (pTerminal ~= nil) then
		SceneObject(pTerminal):setObjectMenuComponent("BestineElectionTerminalMenuComponent")
		print(SceneObject(pTerminal):getObjectID())
...
function BestineElectionScreenPlay:canSearch(pSceneObject, pPlayer)
	...
	if (tblQuest[1] == "victor") then
		if (tblQuest[2] == "questn" and victorVisalisConvoHandler:victorNegQuest(pPlayer)) then
			return true
		elseif (tblQuest[2] == "questp" and self:joinedVictorCampaign(pPlayer)) then
			return true
		end
	elseif (tblQuest[1] == "sean") then
		if (tblQuest[2] == "questn" and seanTrenwellConvoHandler:seanNegQuest(pPlayer)) then
			return true
		elseif (tblQuest[2] == "questp" and self:joinedSeanCampaign(pPlayer)) then
			return true
		end
		return false
	end
