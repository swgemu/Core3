...
BestineElectionScreenPlay = ScreenPlay:new {
		...
		CAMPAIGN_TIME = 60 * 60 * 1000,
		TUNE_TIME = 60 * 60 * 1000,
	...
	for i = 1, #self.electionMobiles do
		...
		if (pNpc ~= nil) then
			if (not BestineElectionScreenPlay:isElectionEnabled()) then
				CreatureObject(pNpc):setOptionsBitmask(128)
				AiAgent(pNpc):setConvoTemplate("")
			else
...
function BestineElectionScreenPlay:triggerHistoryTerminal(pActiveArea, pMovingObject)
	...
	if CreatureObject(pMovingObject):hasScreenPlayState(4, "bestine_history_quest") then
		CreatureObject(pMovingObject):sendSystemMessage("@city/bestine/terminal_items:history_disk_found_already")
		return 0
	end
	...
	if CreatureObject(pMovingObject):hasScreenPlayState(2, "bestine_history_quest") then
		...
		if (pInventory ~= nil) then
			...
			CreatureObject(pMovingObject):sendSystemMessage("@city/bestine/terminal_items:history_disk_found")
...
function TourContainerComponent:transferObject(pContainer, pObj, slot)
	...
	if BestineElectionScreenPlay:tableHas(BestineElectionScreenPlay.seanEvidence, template )
		or BestineElectionScreenPlay:tableHas(BestineElectionScreenPlay.victorEvidence, template)
