...
--local CAMPAIGN_TIME = 3 * 60 * 1000 -- 23 days Phase 1
--local TUNE_TIME = 3 * 60 * 1000 -- 7 days (TUNE = Time Until Next Election) Phase 2
...
function BestineElectionScreenPlay:spawnMerchants()
    for i = 1, #merchMobiles do
      ...
      local pMobile = spawnMobile("tatooine", merchData[1], 0, merchData[2], merchData[3], merchData[4], merchData[5], 0)
...
function BestineElectionScreenPlay:despawnMerchants()
 ...
 local objectID = readData("bestine_election:merchants:marketsound")
 local pMobile = getSceneObject(objectID)
  SceneObject(pMobile):destroyObjectFromWorld()
