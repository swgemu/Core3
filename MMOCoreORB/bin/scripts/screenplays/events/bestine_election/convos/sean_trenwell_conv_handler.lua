...
function seanTrenwellConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif (screenID == "punch_in_location") then
		...
		if (pGhost ~= nil) then
			...
			local historyWaypointID = PlayerObject(pGhost):addWaypoint("tatooine", "Historical Site", "", -758, 0, -3907, WAYPOINT_BLUE, true, true, 0,0)
	...
	elseif (screenID == "seek_out_contact") then
		...
		if (pGhost ~= nil) then
			...
			local contactWaypointID = PlayerObject(pGhost):addWaypoint("tatooine", "Sean's Contact", "", -1448, 0, -3765, WAYPOINT_BLUE, true, true, 0,0)
