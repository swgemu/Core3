...
function TutorialScreenPlay:start(pPlayer)
	...
	-- Lock cell door to cell 9
	updateCellPermission(BuildingObject(pBuilding):getCell(9), 0, pPlayer)
	...
	writeData(playerID .. ":tutorial:message1",0)
	createEvent(4000, "TutorialScreenPlay", "firstMessage", pPlayer)
...
function TutorialScreenPlay:spawnObjects(pPlayer)
	...
	if BuildingObject(pBuilding):getServerObjectCRC() ~= 3369536646 then
		return
	end
	...
	local pCell, cellID, pMobile, pObject, mobileID

	-- ** ROOM 1: Welcome **
	pCell = BuildingObject(pBuilding):getCell(1)
	cellID = SceneObject(pCell):getObjectID()

	spawnMobile("tutorial", "stormtrooper_filler_tutorial", 0, -2.4, 0, -17.4, 0, cellID)
	spawnMobile("tutorial", "stormtrooper_filler_tutorial", 0, 2.5, 0, -17.4, 0, cellID)

	-- ** ROOM 2: Conversations & Items **
	pCell = BuildingObject(pBuilding):getCell(2)
	cellID = SceneObject(pCell):getObjectID()

	--shared_dressed_commoner_fat_zabrak_male_02.iff says refugee1 darn_empire
	pMobile = spawnMobile("tutorial", "tutorial_commoner_talking", 0, 30.2, 0, -43.1, -11.81, cellID)
	createObserver(OBJECTINRANGEMOVED, "TutorialScreenPlay", "refugees", pMobile)

	mobileID = SceneObject(pMobile):getObjectID()
	writeData(mobileID .. ":tutorial:commonerNum", 1)
	writeData(playerID .. ":tutorial:refugee1", mobileID)

	--shared_dressed_commoner_naboo_human_female_04.iff says leave_me_alone
	pMobile = spawnMobile("tutorial", "tutorial_commoner_talking", 0, 28.6, 0, -42.9, -66.58, cellID)
	mobileID = SceneObject(pMobile):getObjectID()

	writeData(mobileID .. ":tutorial:commonerNum", 1)
	writeData(playerID .. ":tutorial:refugee2", mobileID)

	--shared_dressed_commoner_naboo_human_male_01.iff says
	pMobile = spawnMobile("tutorial", "tutorial_commoner_talking", 0, 25, 0, -43.5, -28.87, cellID)
	mobileID = SceneObject(pMobile):getObjectID()

	writeData(mobileID .. ":tutorial:commonerNum", 1)
	writeData(playerID .. ":tutorial:refugee3", mobileID)

	--spawn officer
	pMobile = spawnMobile("tutorial", "imperial_officer_tutorial_1", 0, 26.7, 0, -31.9, -90.47, cellID)
	createObserver(OBJECTINRANGEMOVED, "TutorialScreenPlay", "officer1", pMobile) --move observer

	writeData(playerID.. ":tutorial:officer1", SceneObject(pMobile):getObjectID())

	--drum
	local pDrum = spawnSceneObject("tutorial", "object/tangible/container/drum/tatt_drum_1.iff", 19, 0, -23, cellID, 0.71, 0, -0.71, 0)
	SceneObject(pDrum):setContainerAllowPermission("player", OPEN + MOVEOUT);
	SceneObject(pDrum):setContainerInheritPermissionsFromParent(false)

	writeData(playerID .. ":tutorial:drum", SceneObject(pDrum))

	-- add the starting items (except weapons) to the drum
	addStartingItemsInto(pPlayer, pDrum)

	-- Find maroj melon and register observer on radial
	for i = 0, spawnedSceneObject:getContainerObjectsSize() - 1, 1 do
		local pItem = SceneObject(pDrum):getContainerObject(i)

		if pItem ~= nil and SceneObject(pItem):getGameObjectType() == 8202 then -- Food objectType
			createObserver(OBJECTRADIALUSED, "TutorialScreenPlay", "foodUsed", pItem)
		end
	end

	-- ** ROOM 3: Banks & Bazaars **
	pCell = BuildingObject(pBuilding):getCell(3)
	cellID = SceneObject(pCell):getObjectID()

	--spawn second officer banker
	pMobile = spawnMobile("tutorial", "imperial_officer_tutorial_2", 0, 44, 0, 2.2, 130.53, cellID)
	createObserver(STARTCONVERSATION, "TutorialScreenPlay", "bankerStartConversation", pMobile)

	writeData(playerID .. ":tutorial:officer2", SceneObject(pMobile):getObjectID())

	--spawn commoner shared_dressed_commoner_naboo_bothan_male_02.iff
	pMobile = spawnMobile("tutorial", "tutorial_commoner_talking", 0, 51.1, 0, -16, -88.21, cellID)
	writeData(SceneObject(pMobile):getObjectID() .. ":tutorial:commonerNum", 7)

	--spawn bank
	local pBank = spawnSceneObject("tutorial", "object/tangible/terminal/terminal_bank.iff", 50.8, -0.5, -3.5, cellID, 0.71, 0, -0.71, 0)
	writeData(playerID .. ":tutorial:bankID", SceneObject(pBank):getObjectID())
...
function TutorialScreenPlay:officerTargetEvent(creatureObject, targetCreature)
	...
	local officer1ObjectID = readData(player:getObjectID() .. ":tutorial:officer1")
