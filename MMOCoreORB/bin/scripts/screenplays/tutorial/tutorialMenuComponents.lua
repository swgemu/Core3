...
tutorialWrongBankTerminal = {  }

function tutorialWrongBankTerminal:fillObjectMenuResponse(pSceneObject, pMenuResponse, pPlayer)
end

function tutorialWrongBankTerminal:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	if (selectedID == 20) then
		CreatureObject(pPlayer):sendSystemMessage("@newbie_tutorial/system_messages:wrong_bank")
	end
...
--[[
tutorialTravelTerminal = {  }

function tutorialTravelTerminal:fillObjectMenuResponse(pSceneObject, pMenuResponse, pPlayer)
end

function tutorialTravelTerminal:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	if (pPlayer == nil) then
		return
	end
	
	if (selectedID == 20) then
		if (readData(SceneObject(pPlayer):getObjectID() .. ":tutorial:hasTurnedInDocs") ~= 1) then
			local quartermasterID = readData(SceneObject(pPlayer):getObjectID() .. ":tutorial:roomElevenQuartermaster")
			local pQuartermaster = getSceneObject(quartermasterID)

			if (pQuartermaster ~= nil) then
				spatialChat(pQuartermaster, "@newbie_tutorial/newbie_convo:quarter_wait")
			end
		end
	end
