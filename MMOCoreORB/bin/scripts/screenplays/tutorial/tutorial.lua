...
TODO:
- On logout, player logs in to a different building and screenplay:start doesn't run, resulting in no npcs or objects in the building
	SOLVED by moving spawnObjects out of the conditional block for isRoomComplete(r1)
	Results in a second instance of the screenplay running, creating duplicate objects throughout the building
		SOLVED by removing unnecessary onLoggedIn function
...
- Rewrite function Tutorial:getPlayerProfession(pPlayer) to pull from readStringData(playerID .. ":profession:", playerProfession)
	...
	ERROR - Error running function handleRoomOne scripts/screenplays/tutorial/tutorial.lua:2102: attempt to index a userdata value (local 'pPlayer')
		SOLVED by pulling player first name for printing
	changedRoomEvent for r8 is returning a nil value on line 2099
		SOLVED by commenting line 624 (r8 is completed in pirateKilled)
