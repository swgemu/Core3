...
TODO:
- 1. On logout, player logs in to a different building and screenplay:start doesn't run, resulting in no npcs or objects in the building
	a. This results in a blockage for the player; an admin can use /findo to teleport to the correct building, resulting in a second instance of the screenplay running
...
- 4. After tutorial completion, move any items in player bank to inventory and reset bank location

- 5. Any additional 'flair' stuff that may still be missing
...
function Tutorial:onLoggedIn(pPlayer)
	print("***** TUTORIAL: onLoggedIn for player " .. tostring(pPlayer) .. " *****")

	if (pPlayer == nil) then
		return 0
	end

	createEvent(1000, "Tutorial", "start", pPlayer, "")
end

function Tutorial:onLoggedOut(pPlayer)
	print("***** TUTORIAL: onLoggedOut *****")
	local pBuilding = self:getTutorialBuilding(pPlayer)
	local playerID = CreatureObject(pPlayer):getObjectID()
	...
	createObserver(LOGGEDIN, "Tutorial", "onLoggedIn", pPlayer)
...
function Tutorial:clearPlayers(pBuilding)
	...
	print("TUTORIAL: Clearing Players...")
	local ownerID = readData(SceneObject(pBuilding):getObjectID() .. ":ownerID")
...
function Tutorial:destroyTutorial(pBuilding)
	...
	print("TUTORIAL: Destroying tutorial building " .. tostring(SceneObject(pBuilding):getObjectID()))
