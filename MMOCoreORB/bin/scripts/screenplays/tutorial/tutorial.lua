...
function TutorialScreenPlay:start(pPlayer)
	...
	-- Lock door to room 9
	updateCellPermission(BuildingObject(pBuilding):getNamedCell("r9"), 0, pPlayer)
...
function TutorialScreenPlay:spawnObjects(pPlayer)
	...
	-- Makes sure building is a tutorial building
...
-- Explains item dispenser/bazaar use, reschedules if player is receiving bazaar explanation
...
function TutorialScreenPlay:handleRoomSix(pPlayer)
	...
	if (curStep == 0) then -- Welcome
		...
		createEvent(1000, "TutorialScreenPlay", "doStartPanic", pPlayer)
...
function TutorialScreenPlay:doStartPanic(pPlayer)
	if (pPlayer == nil) then
		return
	end
	...
	local panicID = readData(SceneObject(pPlayer):getObjectID() .. ":tutorial:roomSixPanicCommoner")
	local pPanicNpc = getSceneObject(panicID)
...
function TutorialScreenPlay:doNervousGuySpeak(pGuy)
	...
	if (pPlayer ~= nil and self:isInRoom(pPlayer, "r7") and getRandomNumber(1,20) > 10) then
...
function TutorialScreenPlay:handleRoomEight(pPlayer)
	...
	if (curStep == 0) then -- Explain combat message 1, pirate taunts
...
function TutorialScreenPlay:doCelebSpeak(pCeleb)
	...
	-- Throttle messages so they dont always occur every iteration
	if (pPlayer ~= nil and self:isInRoom(pPlayer, "r9") and getRandomNumber(1,20) > 10) then
...
function TutorialScreenPlay:doCelebAnimation(pCeleb)
	...
	if (pPlayer == nil or not self:isInRoom(pPlayer, "r9")) then
		return
	end
