...
TODO:
- 1. Add functionality to store user room step and recover on logout/login
	a. This seems unnecessary; the player logs into room1 on every login, and the room completion state is stored and saved, so this is likely moot
	b. Line 2111 returning a nil value on changedRoomEvent

- 2. Rewrite function Tutorial:getPlayerProfession(pPlayer) to pull from readStringData(playerID .. ":profession:", playerProfession) 
	a. readStringData for playerProfession is returning nil --> marksman trainer is spawned.
	b. changedRoomEvent for r8 is returning a nil value on line 2133

- 3. Get waypoints working inside terminal (to planets and JTL trainers)

- 4. Figure out greeter1 functionality (see greeter1_bark strings, add to bin/scripts/mobile/conversations/tutorial/room2_greeter_conv.lua)
	a. TODO: research - this might be unnecessary at this point - might only be necessary for the JTL or NGE greetings
	b. TODO: Fix Room 1 greeter using convo template from Room 2 greeter

- 5. After tutorial completion, move any items in player bank to inventory and reset bank location

- 6. Any additional 'flair' stuff that may still be missing
	a. Seems we have everything now

- **** THE NEXT TWO SEEM TO BE CAUSED BY MULTIPLE TUTORIAL START EVENTS
	**** but both seem to have been caused by repeated tutorial starts during the same save cycle - I haven't seen it since documenting this

- 7. Banking room is not telling the player to advance to r4

- 8. Cloning droid active area seems non-functional - no wave or greeting from droid currently
...
roomNumbers = {"1", "2", "3", "4", "5", "6", "7", "14", "8", "9", "10", "11"}
...
function Tutorial:start(pPlayer)
	...
	local roomNames = self.roomNumbers
	for i = 1, #roomNames, 1 do
		local roomNum = "r" .. roomNames[i]
		print("tutorial: room " .. roomNum .. " complete: " .. (tostring(readScreenPlayData(pPlayer, "tutorial", roomNum .. "Complete") == 1)))
	end
	...
	if (self:isInRoom(pPlayer, "r1") and not self:isRoomComplete(pPlayer, "r1")) then
		...
		print("start .. getPlayerProfession == " .. tostring(playerProfession))
		print("start .. readStringData == " .. tostring(readStringData(playerID .. ":profession:")))
...
function Tutorial:onLoggedOut(pPlayer)
	...
	position = readVector3Data(playerID .. ":Tutorial:position:")
	print("playerID savedZoneName: " .. readStringData(playerID .. ":savedZoneName:") .. " savedParent: " .. readData(playerID .. ":savedParentID:") .. " savedPosition: " .. position[1] .. ", " .. position[2] .. ", " .. position[3])
...
function Tutorial:spawnObjects(pPlayer)
	...
	if (pCell ~= nil) then
		...
		-- Greeter
		pMobile = spawnMobile("tutorial", "tutorial_room1_greeter", 0, -9.5, 0, -3.9, 75, cellID)
		...
		spawnMobile("tutorial", "tutorial_stormtrooper_filler", 0, -9.2, 0, 3.6, 120, cellID)
		spawnMobile("tutorial", "tutorial_stormtrooper_filler", 0, -1.9, 0, -16.5, 0, cellID)

		-- Patrolling mouse droid
		spawnMobile("tutorial", "mouse_droid", 0, 0.2, 0, -6.6, 0, cellID)
	...
	--TODO: Add MSE droid patrol dest to 0.2, 0, -6.6 in r1
	...
	if (pCell ~= nil) then
		...
		-- Patrolling mouse droid
		spawnMobile("tutorial", "mouse_droid", 0, 40.0, 0, -13.1, 0, cellID)
	...
	-- TODO: Add MSE Droid patrol dest at 47.1, -7, -51.5 in r4
	...
	if (not self:isRoomComplete(pPlayer, "r14")) then
		...
		if (pCell ~= nil) then
			...
			spawnMobile("tutorial", "mouse_droid", 0, 76.8, -4, -102, 0, cellID)
		end
	end
	...
	if (pCell ~= nil) then
		...
		if (pMobile ~= nil) then
			...
			createEvent(1000, "Tutorial", "setupTrooperPathing", pMobile, "")
		end
	end
end

function Tutorial:setupTrooperPathing(pMobile)
	createEvent(45000, "Tutorial", "doRoomElevenTrooperPathing", pMobile, "")
	createObserver(DESTINATIONREACHED, "Tutorial", "trooperDestReached", pMobile)
...
function Tutorial:handleRoomTwo(pPlayer)
	...
	elseif (curStep == 5) then -- Move to next room
		...
		createEvent(1000, "Tutorial", "doCommerceRoomReminder", pPlayer, "")
	end
...
function Tutorial:doRoomElevenTrooperPathing(pMobile)
	if (pMobile == nil) then
		return
	end
...
function Tutorial:trooperDestReached(pMobile)
	if (pMobile == nil) then
		return 0
	end
...
function Tutorial:markRoomComplete(pPlayer, roomName)
	...
	--print("Tutorial:markRoomComplete(" .. pPlayer .. ", " .. roomName)
