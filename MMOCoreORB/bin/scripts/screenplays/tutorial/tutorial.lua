...
Tutorial = ScreenPlay:new {
	...
	patrolMobiles = {
		--{patrolPoints, template, x, z, y, direction, cell, mood},

		{"mse_1", "mouse_droid", 76.8, -4, -102, 0, "r14", ""},
		{"mse_2", "mouse_droid", 46.6, 0, -31.4, 0, "r3", ""},
		{"panic_guy", "tutorial_commoner_noconverse", 18.3, -4, -76.9, 0, "r6", "panicked"},
	},

	patrolPoints = {
		--{table_name = {{x, z, y, cell, delayAtNextPoint}}

		mse_1 = {{76.8, -4, -102, "r14", false}, {47.1, -7, -51.5, "r4", false}},
		mse_2 = {{46.6, 0, -31.4, "r3", false}, {32.6, 0, -3.4, "r3", false}, {31.1, 0, -3.7, "r2", false}, {0.1, 0, -28.9, "r2", false}, {0.2, 0, -6.6, "r1", false}},
		panic_guy = {{18.3, -4, -76.9, "r6", "false"}, {47.1, -7, -51.5, "r4", true}},
	},
}
...
function Tutorial:spawnPatrol(pPlayer, num)
	...
	local pMobile = spawnMobile("tutorial", template, 0, x, y, z, direction, cellID)

	if (pMobile ~= nil and points ~= nil) then
		if mood ~= "" then
			CreatureObject(pMobile):setMoodString(mood)
		end
		...
		createEvent(1000, "Tutorial", "mobilePatrol", pMobile, "")
...
function Tutorial:mobileDestinationReached(pMobile)
	...
	local currentSet = pointSet[currentLoc]

	local delay = currentSet[5]

	if (currentSet == nil or delay ~= true) then
		createEvent(5000, "Tutorial", "mobilePatrol", pMobile, "")
	else
		local delayTime = getRandomNumber(5, 10)

		createEvent(delayTime * 1000, "Tutorial", "mobilePatrol", pMobile, "")
