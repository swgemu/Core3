...
TEST_F(NameManagerTest, ProfanityTest) {
	...
	if (lua->runFile("scripts/managers/name/test_names.lua")) {
		...
		for (String name : testNames) {
			if (nameManager->checkNamingFilter(name, NameManagerResult::DECLINED_PROFANE) == NameManagerResult::ACCEPTED)
				continue;
...
TEST_F(NameManagerTest, DeveloperTest) {
	...
	if (lua->runFile("scripts/managers/name/test_names.lua")) {
		...
		// Run test on List
		for (String name : testNames) {
			if (nameManager->checkNamingFilter(name, NameManagerResult::DECLINED_DEVELOPER) == NameManagerResult::ACCEPTED)
				continue;
...
TEST_F(NameManagerTest, FictionalTest) {
	...
	if (lua->runFile("scripts/managers/name/test_names.lua")) {
		...
		// Run test on List
		for (String name : testNames) {
			if (nameManager->checkNamingFilter(name, NameManagerResult::DECLINED_FICT_RESERVED) == NameManagerResult::ACCEPTED)
				continue;
