...
void ClientCoreOptions::resolveDependencies() {
	...
	for (int i = 0; i < actions.size(); i++) {
		...
		if (strcmp(name, "selectCharacter") == 0) hasSelectCharacter = true;
		...
		// Auto-insert selectCharacter after connectToZone, before character-needing action
		if (action->needsCharacter() && hasConnectToZone && !hasSelectCharacter) {
			ActionBase* selector = ActionManager::createAction("selectCharacter");
			if (selector != nullptr) {
				final.add(selector);
				hasSelectCharacter = true;
			}
		}
	...
	// Ensure logoutCharacter at end if we selected a character (for cleanup)
	if (hasSelectCharacter && !hasLogoutCharacter) {
