...
void dumpOBJ(String filename, Vector<Reference<MeshData*> > data) {
	...
	for(int x=0; x<data.size(); x++) {
		...
		for (int i=0; i<tris->size(); i++) {
			...
			outputStream.write("f " + String::valueOf(lastIndex+tri.getVerts()[2]) + " " + String::valueOf(lastIndex+tri.getVerts()[1]) + " " + String::valueOf(lastIndex+tri.getVerts()[0])+"\n");
		}
...
void ClientRenderer::loadSnapshotNode(WorldSnapshotNode *node, WorldSnapshotIff *wsiff) {
	...
	if(sceno == NULL) {
		...
		ClientBuilding *building = sceno.castTo<ClientBuilding*>();
		if (false && building != NULL) {
			String fileName = templateName.subString(templateName.lastIndexOf('/')+1);
			//Vector<Reference<MeshData*> > buildingData = sceno->getTransformedMeshData();
			//dumpOBJ(fileName+".obj", buildingData);
			//info("Dumping "+templateName, true);
		}
	...
	for (int i=0; i<regions.size(); i++) {
		...
		if(region->sceneData.size() > 0) {
			for( int s=0; s<region->regionBounds.size(); s++) {
				...
				region->sceneData.add(getTerrainMesh(sphere.getCenter(), sphere.getRadius()*2, 128, 0.5));
			 }
...
osg::ref_ptr<osg::Node> ClientRenderer::createRegions() {
	...
	Reference<ClientRegion*> region = new ClientRegion();
	region->name = "jinta_shit";
	regions.put(region->name, region);
	
	region->regionBounds.add(Sphere(Vector3(0, 0, terrainManager->getHeight(0, 0)), 128));
