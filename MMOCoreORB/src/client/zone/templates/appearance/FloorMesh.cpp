...
osg::ref_ptr<osg::Node> FloorMesh::draw() {
	...
	for (int i = 0; i < getTriangleCount(); ++i) {
		FloorMeshTriangleNode* triangle = getTriangle(i);
	...
	PathGraph* pathGraph = getPathGraph();
...
void FloorMesh::parseBTRE(IffStream* iffStream) {
	...
	for (int i = 0; i < nodeSize; ++i) {
		BoxTreeNode *nod = new BoxTreeNode;
		nod->readObject(iffStream);
