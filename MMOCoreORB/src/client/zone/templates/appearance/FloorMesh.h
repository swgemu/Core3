...
class BoxTreeNode : public Object {
public:
	AABB bbox;
	int index;
	int child1;
	...
	BoxTreeNode() : bbox(Vector3(0, 0, 0), Vector3(0, 0, 0)) {
		index = child1 = child2 = 0;
	}
	...
	void readObject(IffStream* iffStream) {
		...
		index = iffStream->getInt();
		userId = iffStream->getInt();
		child1 = iffStream->getInt();
		child2 = iffStream->getInt();
