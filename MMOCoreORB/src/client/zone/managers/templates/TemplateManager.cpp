...
//	int count = ComponentManager::instance()->size();
	Vector<String>* files = DataArchiveStore::instance()->getTreeArchive()->getFilesAndSubDirectoryFiles("/");
	for(int i=0; i<files->size(); i++) {
		String &file = files->get(i);
		//info("Adding " + file + " with " + String::hexvalueOf((int64)file.hashCode()), true);
		addTemplate(file.hashCode(), file);
	}
//	try {
//		bool val = luaTemplatesInstance->runFile("scripts/object/main.lua");
//		luaTemplatesInstance->runFile("scripts/object/clientmain.lua");
//
//		if (!val)
...
//	info("loaded " + fullName, true);

	//info("Adding " + fullName, true);
	if (templateCRCMap->put(fullName.hashCode(), templateObject) != NULL) {
...
void TemplateManager::registerTemplateObjects() {
	...
	templateFactory.registerObject<SharedDraftSchematicObjectTemplate>(SharedObjectTemplate::DRAFTSCHEMATIC);
...
//	boundingVolumeFactory.registerObject<BoxVolume>('BBOX');
//	boundingVolumeFactory.registerObject<
}
...
String TemplateManager::getAppearanceTemplateString(const AppearanceTemplate* tmpl) {

	
	Locker locker(&appearanceMapLock);
	return appearanceMap->getKeyForValue(tmpl);
}
...
void TemplateManager::addTemplate(uint32 key, const String& fullName) {
	clientTemplateCRCMap->put(key, fullName);
}

PortalLayout* TemplateManager::getPortalLayout(const String& fileName) {
	Locker _locker(&appearanceMapLock);

	Reference<PortalLayout*> portalLayout = portalLayoutMap->get(fileName);
	...
	if (portalLayout == NULL) {
		...
		if (iffStream != NULL) {
			try {
				...
				if(portalLayout->getCRC() != 0) {
					clientTemplateCRCMap->put(portalLayout->getCRC(), fileName);
					info("Adding portal layout " + fileName + " [" + String::valueOf(portalLayout->getCRC()), true);
				}
...
SharedObjectTemplate* TemplateManager::getTemplate(uint32 key) {
	SharedObjectTemplate *shot =  templateCRCMap->get(key);
	if(shot == NULL) {
		String fullName = getTemplateFile(key);
		if(!fullName.isEmpty()) {
			
			IffStream* iffStream = openIffFile(fullName);
			uint32 formType = iffStream->getNextFormType();
			//iffStream->close();
			delete iffStream;
			
			shot = templateFactory.createObject(formType);
			
			if (shot == NULL) {
				error("error creating templateType " + String::hexvalueOf((int)formType));
			...
			shot->setFullTemplateString(fullName);
			shot->setTemplateFileName(fileName);
			
			if (!fileName.isEmpty()) {
				IffStream* iffStream = openIffFile(fullName);
				
				if (iffStream != NULL) {
					shot->readObject(iffStream);
...
//				templateObject->addDerivedFile(clientTemplateFile);
			...
			//info("Adding " + fullName, true);
			if (templateCRCMap->put(fullName.hashCode(), shot) != NULL) {
				//error("duplicate template for " + fullName);
			}
