...
public:
	ClientMeshAppearance(const AppearanceTemplate *tmpl) : ClientAppearance(tmpl) {
		const MeshAppearanceTemplate *mesh = cast<const MeshAppearanceTemplate*>(tmpl);
		assert(mesh != NULL);
		if (tmpl->getFileName().contains("tiki_torch_s01")) {
			geode = new osg::Geode();
		}
		geode = new osg::Geode();
	...
	virtual osg::ref_ptr<osg::Node> draw() {
		...
		//group->addChild(ClientAppearance::draw());
		return group;
	}
	
	
	static osg::ref_ptr<osg::Geometry> createMeshGeometry(const MeshData* meshData) {
		if(meshData == NULL) {
			Logger logger;
			logger.error("NULL MeshData");
			return NULL;
		}
