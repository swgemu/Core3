...
public:
	...
	SamplePolyAreas getType() { return type; }
	...
	float getMinHeight() const { return hmin; }
	float getMaxHeight() const { return hmax; }

	void setMinHeight(float h) { hmin = h; }
	void setMaxHeight(float h) { hmin = h; }


	friend class RecastNavMeshBuilder;
	friend class RecastTileBuilder;
