/*
 * RecastNavMeshBuilder.cpp
 *
 *  Created on: 03/2016
 *      Author: gslomin
 *  Originally adapted from Sample_TileMesh.cpp in the RecastDemo project
 */
...
void RecastNavMeshBuilder::buildAllTiles(AABB area)
{
	...
	float zStart = floor(fabs(bounds.getZMin() - area.getZMin()) / tcs) - 1.0f;
	float xStart = floor(fabs(bounds.getXMin() - area.getXMin()) / tcs) - 1.0f;
	...
	for (int y = zStart; y < th; ++y)
	{
		...
		for (int x = xStart; x < tw; ++x)
		{
			...
			if (data)
			{
				...
				info("No tile data", true);
			}
			
			
		}
		progress += y-zStart;
		info("Generating tiles: " + String::valueOf(progress*100 / ((th-zStart) * (tw-xStart))) +"% complete", true);
