...
bool RecastNavMeshBuilder::rebuildArea(AABB area, RecastNavMesh* existingMesh) {
	...
	area = AABB(Vector3(center.getX() - longest, -100000, -center.getY() - longest),
...
void RecastNavMeshBuilder::buildAllTiles(RecastNavMesh* recastNavMesh, AABB area) {
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void
RecastNavMeshBuilder::initialize(Vector <Reference<MeshData*>> meshData, AABB bounds, float distanceBetweenHeights) {
