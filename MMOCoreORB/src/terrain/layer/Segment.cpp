...
void Segment::createRoadwayHeights() {
	...
	for (int i = 1; i < totalPositions - 1; ++i) {
		...
		/* Disabled for now to figure out flattening of slopes
...
#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "segmentBefore - X: " << segmentBefore->getX() << " Z: " << segmentBefore->getZ() << " Y: " << segmentBefore->getY() << " Distance Sq: " << distSq1;
		Logger::console.info(true) << "ourSegment - X: " << ourSegment->getX() << " Z: " << ourSegment->getZ() << " Y: " << ourSegment->getY();
		Logger::console.info(true) << "segmentAfter - X: " << segmentAfter->getX() << " Z: " << segmentAfter->getZ() << " Y: " << segmentAfter->getY() << " Distance Sq: " << distSq2;

		Logger::console.info(true) << "RotatedX = " << positionX << " RotatedY = " << positionY;
#endif

		// Set the segment line above or below. The given point falls betwee our points line and the next closest one.
		Point3D* closestSegment = (distSq1 < distSq2 ? segmentBefore : segmentAfter);
		// Set distance squared1 to the nearest distance to use in calc for the Z adjustment
		distSq1 = distSq1 < distSq2 ? distSq1 : distSq2;

		// Total distance from world point to the next segment and world point to our segment
		float totalSquared = distSq1 + ourSegment->squaredDistanceTo(closestSegment->getX(), closestSegment->getY());

		if (distSquared > 0.f) {
			float distanceVar = distSquared / totalSquared;

			newHeight = ourSegment->getZ() + (distanceVar * (closestSegment->getZ() - ourSegment->getZ()));

#ifdef DEBUG_AFFECTOR_ROAD
			Logger::console.info(true) << "distSquared = " << distSquared << " totalSquared = " << totalSquared << " distanceVar = " << distanceVar;
#endif
		} else {
			newHeight = ourSegment->getZ();
		}
