...
void Segment::findNearestHeight(float& baseValue, Vector3 worldPosition) {
	...
	if (heightSegment > 0 && heightSegment < (positionsSize - 1)) {
		...
		if (posDistanceSq > 0.f) {
			...
			float segmentDiff = segmentBefore->getZ() - ourSegment->getZ();

			newHeight = segmentBefore->getZ() + (-distanceVar * segmentDiff);
			...
			Logger::console.info(true) << "distSquared = " << distSquared << " distanceVar = " << distanceVar << " Segment Diff: " << segmentDiff;
#endif
		} else {
