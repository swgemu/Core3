...
void Segment::findNearestHeight(float& baseValue, float positionX, float positionY) {
	int positionsSize = positions.size();
	...
	if (heightSegment > 0 && heightSegment < (positionsSize - 1)) {
		...
		// Get the points before and after the closest segment
		Point3D* pointBefore = positions.get(heightSegment - 1);
		Point3D* pointAfter = positions.get(heightSegment + 1);
		Point3D* ourPoint = positions.get(heightSegment);

		float distSq1 = pointBefore->squaredDistanceTo(positionX, positionY);
		float distSq2 = pointAfter->squaredDistanceTo(positionX, positionY);

#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "PointBefore - X: " << pointBefore->getX() << " Y: " << pointBefore->getY() << " Distance Sq: " << distSq1;
		Logger::console.info(true) << "OurPoint - X: " << ourPoint->getX() << " Y: " << ourPoint->getY();
		Logger::console.info(true) << "PointAfter - X: " << pointAfter->getX() << " Y: " << pointAfter->getY() << " Distance Sq: " << distSq2;
#endif

		Point3D* closestPoint = (distSq1 < distSq2 ? pointBefore : pointAfter);
		distSq1 = distSq1 < distSq2 ? distSq1 : distSq2;

		float totalDistance = Math::sqrt(Math::sqr((ourPoint->getX() - closestPoint->getX()) + Math::sqr(ourPoint->getY() - closestPoint->getY())));

#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "Total Distance: " << totalDistance;
#endif

		newHeight += (Math::sqrt(distSq1) /  totalDistance) * (closestPoint->getZ() - newHeight);
	}
