...
void Segment::findNearestHeight(float& baseValue, Vector3 worldPosition) {
	...
	if (heightSegment > 0 && heightSegment < (positionsSize - 1)) {
		...
		if (posDistanceSq > 0.f) {
			float distanceVar = 1.f - (posDistanceSq / sqrDistance);

			newHeight = ourSegment->getZ() + (distanceVar * (segmentBefore->getZ() - ourSegment->getZ()));

#ifdef DEBUG_AFFECTOR_ROAD
			Logger::console.info(true) << "dotProduct: " << dotProduct << " sqrDistance: " << sqrDistance << " posDistanceSq: " << posDistanceSq << " Position: " << position.toString();

			Logger::console.info(true) << "distSquared = " << distSquared << " distanceVar = " << distanceVar;
#endif
		} else {
