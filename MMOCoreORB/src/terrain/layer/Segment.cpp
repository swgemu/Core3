...
void Segment::findNearestHeight(float& baseValue, Vector3 worldPosition) {
	...
	if (heightSegment > 0 && heightSegment < (positionsSize - 1)) {
		...
		Vector3 ourSeg(ourSegment->getX(), ourSegment->getY(), ourSegment->getZ());
		...
		Logger::console.info(true) << "segmentBefore - X: " << beforeSeg.getX() << " Z: " << beforeSeg.getZ() << " Y: " << beforeSeg.getY() << " Distance Sq: " << distSq1;
		Logger::console.info(true) << "ourSegment - X: " << ourSeg.getX() << " Z: " << ourSeg.getZ() << " Y: " << ourSeg.getY();
#endif

		Vector3 lineDirection(beforeSeg.getX() - ourSeg.getX(), beforeSeg.getY() - ourSeg.getY(), 0.f);
		lineDirection.normalize();

		Vector3 midPoint = beforeSeg.midPoint(ourSeg);

		float deltaX = midPoint.getX() - worldPosition.getX();
		float deltaY = midPoint.getY() - worldPosition.getY();
		float directionAngle = Math::PI / 4;

		float rotatedX = deltaX * Math::cos(-directionAngle) - deltaY * Math::sin(-directionAngle);
		float rotatedY = deltaX * Math::sin(-directionAngle) + deltaY * Math::cos(-directionAngle);

		// This is the difference from the midpoint between segments to the player position
		Vector3 rotated(rotatedX, rotatedY, 0.f);
		Vector3 playerPos(midPoint.getX(), midPoint.getX() + rotatedY, 0.f);

  		float yPercent = rotatedY / lineDirection.getY();
		float totalDistance = beforeSeg.distanceTo2d(ourSeg);
		float positionDistance = (totalDistance / 2.0f) + yPercent;

		float distanceVar = Math::clamp(0.f, positionDistance / totalDistance, 1.f);
		float segmentDiff = ourSeg.getZ() - beforeSeg.getZ();

		newHeight = beforeSeg.getZ() + (distanceVar * segmentDiff);

#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "segements - midPoint = " << midPoint.toString() << " world rotated = " << rotated.toString();

		Logger::console.info(true) << " positionDistance = " << positionDistance << " totalDistance = " << totalDistance << " distanceVar: " << distanceVar;
