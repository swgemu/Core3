...
void Segment::findNearestHeight(float& baseValue, Vector3 worldPosition, Vector3 roadCenter, float direction) {
...
#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "World Distance Y Squared: " << rotatedYSq << " Segments half size: " << halfSize;
#endif
	...
	if (heightSegment > 0 && heightSegment <= (positionsSize - 1)) {
		...
		Logger::console.info(true) << "segements - midPoint = " << midPoint.toString();
		Logger::console.info(true) << "===== END SEGMENTS =====";
#endif

		float totalDistance = beforeSeg.distanceTo2d(ourSeg);

		float ourSegDistFromCenter = ourSeg.distanceTo2d(roadCenter);
		float positionDistance = totalDistance - fabs(ourSegDistFromCenter - fabs(rotatedY));
		...
		Logger::console.info(true) << "World Positon - X = " << positionX << " Y = " << positionY << " world rotated Y = " << rotatedY << " Angle in Rad: " << direction;
		Logger::console.info(true) << "segements - midPoint = " << midPoint.toString();

		Logger::console.info(true) << "ourSeg distance from center: " << ourSegDistFromCenter << " positionDistance = " << positionDistance << " totalDistance = " << totalDistance << " distanceVar: " << distanceVar;
