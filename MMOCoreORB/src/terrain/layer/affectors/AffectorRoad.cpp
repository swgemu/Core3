...
void AffectorRoad::process(float x, float y, float transformValue, float& baseValue, TerrainGenerator* terrainGenerator) {
	...
	/*	Process the RoadData
		...
		2. Find nearest segment to pass point from PTA and return Z coordinate
	*/
	...
	for (int i = 0; i < rectangles.size(); ++i) {
		BoundaryRectangle* rect = rectangles.get(i);

		msg << "Checking Rectangle #" << i << " LL X: " << rect->getMinX() << " LL Y: " << rect->getMinY() << endl;
		...
		onRoadway = true;
		roadStart.x = rect->getUpperMiddleX();
		roadStart.y = rect->getUpperMiddleY();
...
#ifdef DEBUG_AFFECTOR_ROAD
	msg << "Point is within bounds of roadway -- Road Start - X: " << roadStart.x << " Y: " << roadStart.y << endl;
#endif

	hdta.findNearestHeight(baseValue, x, y, roadStart.x, roadStart.y);
...
void AffectorRoad::addNewRectangle(float x1, float y1, float x2, float y2) {
	...
	float deltaX = x1 - x2;
	float deltaY = y1 - y2;
	float directionAngle = atan2(deltaX, deltaY);

	directionAngle = Math::PI / 2 - directionAngle;

	if (directionAngle < 0) {
		float a = Math::PI + directionAngle;
		directionAngle = Math::PI + a;
	}

	float xLowerLeft = x2 + (widthHalf * Math::cos(directionAngle) + 0.f * Math::sin(directionAngle));
	float yLowerLeft = y2 + ((-1.f * widthHalf) * Math::sin(directionAngle) + 0.f * Math::cos(directionAngle));

	float xUppperRight = x1 + ((-1.f * widthHalf) * Math::cos(directionAngle) + 0.f * Math::sin(directionAngle));
	float yUppperRight = y1 + (widthHalf * Math::sin(directionAngle) + 0.f * Math::cos(directionAngle));

	BoundaryRectangle* rect = new BoundaryRectangle(xLowerLeft, yLowerLeft, xUppperRight, yUppperRight, x1, y1);
...
#ifdef DEBUG_AFFECTOR_ROAD
	StringBuffer msg;
	msg << "New Rectangle Road Added - LowerLeft X: " << xLowerLeft << " LowerLeft Y: " << yLowerLeft << " UpperRight X: " << xUppperRight << " UpperRightY: " << yUppperRight << endl;
	sendDebugMessage(msg.toString());
#endif
}
