...
public:
	RoadRectangle(float x0, float y0, float x1, float y1, float startX, float startY, float roadWidth) {
		lowerLeftX = x0;
		lowerLeftY = y0;
		upperRightX = x1;
		upperRightY = y1;
	...
	bool containsPoint(float x, float y) const {
...
#ifdef DEBUG_AFFECTOR_ROAD
		Logger::console.info(true) << "Direction Angle: " << directionAngle;
#endif
		...
		return (fabs(rotatedX) <= width / 2.f) && (fabs(rotatedY) <= height / 2.f);
	}
