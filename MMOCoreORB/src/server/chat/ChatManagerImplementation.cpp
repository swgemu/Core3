...
void ChatManagerImplementation::handleSocialInternalMessage(CreatureObject* sender, const UnicodeString& arguments) {
	...
	if (vec != nullptr) {
		...
		zone->getInRangeObjects(worldPosition.getX(), worldPosition.getZ(), worldPosition.getY(), (zone->isSpaceZone() ? ZoneServer::SPACEOBJECTRANGE : ZoneServer::CLOSEOBJECTRANGE), &closeEntryObjects, true);
	}
