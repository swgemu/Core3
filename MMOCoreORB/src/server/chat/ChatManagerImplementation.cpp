...
void ChatManagerImplementation::notifySpatialChatObservers(CreatureObject* source, CreatureObject* target, const UnicodeString& message) {
...
void ChatManagerImplementation::broadcastChatMessage(CreatureObject* sourceCreature, const UnicodeString& message, uint64 chatTargetID, uint32 spatialChatType, uint32 moodType, uint32 chatFlags, int languageID) {
	...
	if (sourceCreature->isPlayerCreature()) {
		...
		if (myGhost != NULL) {
			...
			if (spatialChatTypeSkillNeeded.contains(spatialChatType) && !myGhost->hasAbility(spatialChatTypeSkillNeeded.get(spatialChatType)) && !myGhost->hasGodMode())
				return;
