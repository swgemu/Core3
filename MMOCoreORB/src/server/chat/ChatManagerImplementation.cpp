...
void ChatManagerImplementation::destroyRoom(ChatRoom* room) {
	//Pre: ChatManager locked
	//Post: ChatManager locked
	...
	ChatOnDestroyRoom* msg = new ChatOnDestroyRoom("SWG", server->getGalaxyName(), room->getOwnerName(), room->getRoomID());
	...
	ManagedReference<CreatureObject*> owner = server->getObject(room->getOwnerID()).castTo<CreatureObject*>();
	if (owner != NULL) {
		clocker.release();

		Locker plocker(owner, _this.getReferenceUnsafeStaticCast());
...
void ChatManagerImplementation::disableRoom(ChatRoom* room) {
	//Pre: ChatManager and room locked
	//Post: ChatManager and room locked
