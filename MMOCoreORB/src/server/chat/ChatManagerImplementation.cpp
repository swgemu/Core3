...
int ChatManagerImplementation::sendMail(const String& sendername, const UnicodeString& subject, const StringIdChatParameter& body, const String& recipientName, WaypointObject* waypoint) {
	uint64 receiverObjectID = playerManager->getObjectID(recipientName);
	ManagedReference<SceneObject*> obj = server->getObject(receiverObjectID);
	ManagedReference<CreatureObject*> sender = NULL;
	bool godMode = false;
