...
int APIProxyObjectManager::deleteObject(APIRequest& apiRequest, uint64 oid, String& resultMessage) {
	...
	auto exportFileName = exportJSON(obj.get(), exportMsg.toString());

	StringBuffer result;

	result << "DELETED; Exported to " << exportFileName;
		...
		if (auctionManager != nullptr) {
			...
			if (auctionsMap != nullptr) {
				if (auctionsMap->containsItem(oid)) {
					...
					if (aitem != nullptr) {
						Locker lock(aitem);
						result << "; destroyed AuctionItem with oid: " << aitem->getObjectID();
						aitem->destroyAuctionItemFromDatabase();
					}
				}
			}
		}
