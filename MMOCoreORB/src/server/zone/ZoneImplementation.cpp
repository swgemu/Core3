...
void ZoneImplementation::updateCityRegions() {
	...
	bool anyPerformanceLocations = false;
	...
	for (int i = 0; i < cityRegionUpdateVector.size(); ++i) {
		...
		for(int i = 0; i < city->getStructuresCount(); i++){
			...
			if (!anyPerformanceLocations && structure->getPlanetMapCategory() != NULL) {
	...
	if (anyPerformanceLocations && getPlanetManager() != NULL) {
