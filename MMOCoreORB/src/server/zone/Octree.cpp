...
void Octree::safeInRange(TreeEntry* obj, float range) {
	...
	const auto& position = obj->getWorldPosition();
	...
	for (int i = 0; i < inRangeObjects.size(); ++i) {
		try {
			...
			if (Octree::doLog()) {
				...
				Logger::console.info(true)
				<< "Octree::safeInRange -- Close Object #" << i << " " << nearSceneO->getDisplayedName() << "[" << objectID << "]" << " position: " << x << ", " << y << ", " << z << " range: " << range << " entryOutOfRange: " << entryOutOfRange;
			}
#endif // DEBUG_SAFE_IN_RANGE
