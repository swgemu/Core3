...
class CloneSpawnPoint : public DistributedObject {
	SceneObject* parent;
...
public:
	CloneSpawnPoint(float x, float z, float y) : DistributedObject() {
		positionX = x;
		positionZ = z;
		positionY = y;
	}
	...
	bool spawnAt(CreatureObject* creature) {
		creature->warpTo(getPositionX(), getPositionZ(), getPositionY(), getParent()->getObjectID());
	...
	inline void setParent(SceneObject* parnt) {
		parent = parnt;
	}
