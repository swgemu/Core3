...
String BuildingObjectImplementation::getBuildingType() {
...
void BuildingObjectImplementation::registerProfessional(CreatureObject* player) {
	...
	if(! registeredPlayerIdList.contains(player->getObjectID())) {
		...
		String buildingType = BuildingObjectImplementation::getBuildingType();
		...
		if ( player->isNeutral() && (!this->isNeutral())) {
			// 36646: stringFiles[555].addEntry("no_neutrals", "Neutrals may only register at neutral (non-aligned) locations.");
			player->sendSystemMessage("@faction/faction_hq/faction_hq_response:no_neutrals");
			return;
		}
		// if (player->getPlayerObject()->getFactionStatus() == FactionStatus::OVERT) { }
		if ( (player->isImperial() && this->isRebel()) || (player->isRebel() && this->isImperial())) {
			// 36647: stringFiles[555].addEntry("no_opposition", "You may not register at a location that is factionally opposed.");
		...
		for (int j = 0; j < planetaryLocs.size(); j++) {
			...
			if (this->getObjectID() == obj->getObjectID()) {
				...
				// Notify the player ...
				// 36656: stringFiles[555].addEntry("success", "You successfully register with this location.");
		...
		// 36639: stringFiles[555].addEntry("already_registered", "But you are already registered at this location.");
