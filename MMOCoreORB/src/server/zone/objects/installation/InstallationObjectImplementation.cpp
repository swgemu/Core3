...
void InstallationObjectImplementation::initializeTransientMembers() {
	...
	staticObject = false;
	setLoggingName("InstallationObject");

	// Convert to new installation variables
	if (surplusPower > 0 && power == 0.f) {
		power = surplusPower;
		surplusPower = 0;

		powerRate = getBasePowerRate();
	}
}
...
void InstallationObjectImplementation::doTick() {
	...
	if (surplusMaintenance < 0) {
		info(true) << "doTick() surplusMaint < 0";
...
void InstallationObjectImplementation::activate(CreatureObject* player) {
	...
	info(true) << "activate() curPower " << getPower() << " baseRate " << getBasePowerRate();

	int curPower = getPower();
...
void InstallationObjectImplementation::deactivate() {
	info(true) << "deactivate()";
	if (!activated)
		return;
	info(true) << "deactivate() success";
	activated = false;
...
uint64 InstallationObjectImplementation::getOutOfPowerTime() {
	if (isActive() && powerRate > 0) {
		return lastTickTime.getMiliTime() + (uint64)((power / powerRate) / (3600 * 1000));
	} else {
		return 0;
	}
}
