...
float TurretDataComponent::getArmorResistReduction(float value) {
	...
	// Total damage mitigated by armor resists
	int resistMitigation = turret->getResistMitigation();

	// Percent of damage mitigated compared to maxCondition
	float mitigatedAmount = resistMitigation / (float)maxCondition;
	...
	// Decay resists when mitigated damage is greater than 25% of maxConiditon
	if (mitigatedAmount > 0.25f) {
		// Reduce resists 2% for every 1% of damage mitigated by armor greater than 25% of max condition
...
#ifdef DEBUG_RESIST_DECAY
		turret->info(true) << " max condition = " << maxCondition << " Total Damage Mitigated = " << resistMitigation << " Start Armor value: " << value << " New Armor Value = " << newValue << " Reduction percent = " << reduction;
#endif
	}
