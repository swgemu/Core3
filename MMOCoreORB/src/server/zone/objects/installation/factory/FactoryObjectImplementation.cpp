...
void FactoryObjectImplementation::sendOutputHopper(CreatureObject* player) {
	...
	//outputHopper->sendTo(player, false, false);
	...
	//outputHopper->getContainerObjects(objects);
	...
	for (int j = 0; j < hopperSize; ++j) {
		...
		if (child == nullptr) {
			info(true) << "child object is null";
			continue;
		}

		child->info(true) << "Sending Object To Player: " << player->getDisplayedName() << " Object: " << child->getDisplayedName();

		//outputHopper->broadcastObject(child, false);

		//child->sendTo(player, true, false);
...
void FactoryObjectImplementation::openHopper(Observable* observable, ManagedObject* arg1) {
	...
	//if (sceneObserv->getObjectID() == outputHopper->getObjectID()) {
	//	operatorList.add(player);
	//}
...
void FactoryObjectImplementation::closeHopper(Observable* observable, ManagedObject* arg1) {
	...
	if (player == nullptr || hopper != nullptr || outputHopper == nullptr || !player->isPlayerCreature())
		return;

	//if (observable == outputHopper)
		//operatorList.removeElement(player);
