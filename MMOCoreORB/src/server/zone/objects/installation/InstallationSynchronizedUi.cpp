...
void InstallationSynchronizedUi::getResourceData(Vector<InstallationResourceData>* resourceData) {
	...
	for (int i = 0; i < resourcePools.size(); i++) {
		...
		resourceData->add(makeInstallationResourceData(resourceId, zoneName, posX, posY));
	}
	...
	for (int i = 0; i < hopperContents.size(); i++) {
		...
		uint64 resourceId = entry.getKey();
		resourceData->add(makeInstallationResourceData(resourceId, zoneName, posX, posY));
	}
}

InstallationResourceData InstallationSynchronizedUi::makeInstallationResourceData(uint64 resourceId, float posX, float posY, String zoneName) {
...
void InstallationSynchronizedUi::refresh() {
	...
	VectorMap<uint64, float> newHopperContents;
	...
	for (int i = 0; i < hopperContents.size(); i++) {
		VectorMapEntry<uint64, float> entry = hopperContents->elementAt(i);
		VectorMapEntry<uint64, float> entry2 = newHopperContents->elementAt(i);

		addResource(entry2.getKey());
	...
	if (hopperModified) {
		...
		hopperContents = newHopperContents;
	}
