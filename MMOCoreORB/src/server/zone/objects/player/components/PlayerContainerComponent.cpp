	...
	if (object->isTangibleObject() && containmentType == 4) {
		...
		if (object->isWeaponObject()) {
			...
			if (weapon->isJediWeapon()) {
				...
				if (craftersID == 0 && zoneServer != nullptr) {
					...
					if (chatMan != nullptr) {
						...
						if (playerMap != nullptr) {
							CreatureObject* crafterCreo = playerMap->get(weapon->getCraftersName());

							if (crafterCreo != nullptr) {
								craftersID = crafterCreo->getObjectID();
								weapon->setCraftersID(craftersID);
							}
						}
					}
				}

				if (craftersID != creo->getObjectID() && !ghost->isPrivileged()) {
