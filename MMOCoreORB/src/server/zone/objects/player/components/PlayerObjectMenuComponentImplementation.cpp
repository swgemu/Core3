...
void PlayerObjectMenuComponentImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, PlayerCreature* player) {
	if (!sceneObject->isCreatureObject())
		return;

	CreatureObject* creature = (CreatureObject*) sceneObject.get();
...
int PlayerObjectMenuComponentImplementation::handleObjectMenuSelect(PlayerCreature* player, byte selectedID) {
	PlayerCreature* ownerPlayer = dynamic_cast<PlayerCreature*>(sceneObject.get());
