...
public:
	...
	bool toBinaryStream(ObjectOutputStream* stream) {
		...
		for (int i = 0; i < wideParameters.size(); ++i) {
			printf("Writing to wideparam: %s \n", wideParameters.get(i).toCharArray());
		...
		for (int i = 0; i < narrowParameters.size(); ++i) {
			printf("Writing to narrowparam: %s \n", narrowParameters.get(i).toCharArray());
	...
	bool parseFromBinaryStream(ObjectInputStream* stream) {
		...
		for (int i = 0; i < size; ++i) {
			...
			printf("parsing from wideparam: %s \n", param.toCharArray());
			wideParameters.add(param);
		}
		...
		for (int i = 0; i < size; ++i) {
			String param;
			param.parseFromBinaryStream(stream);
			printf("parsing from narrowparam: %s \n", param.toCharArray());
			narrowParameters.add(param);
		}
