...
public:
	...
	void run(CreatureObject* player, SuiBox* suiBox, bool cancelPressed, Vector<UnicodeString>* args) {
		...
		if (resource == NULL || !resource->isResourceContainer())
			return;
		...
		if (spawn->isType("organic")) {
			...
			printf("nutrient level: %i, attribOne: %i, attribTwo: %i, factor: %i, newQuality: %i \n", nutrientLevel, critAttribOne, critAttribTwo, factor, newQuality);
			...
			player->sendSystemMessage("@plant_grow:apply_nutrients"); // The plant's soil is already filled with nutrients.
		} else {
			int waterLevel = plant->getWaterLevel();

			if (waterLevel > 100) {
				player->sendSystemMessage("@plant_grow:too_much_water"); // The plant's soil is already filled with nutrients.
				return;
			}
			...
			player->sendSystemMessage("@plant_grow:apply_water"); // The plant's soil is already saturated with water.
		}

		if (quantity == 25) {
			resource->destroyObjectFromWorld(true);
			resource->destroyObjectFromDatabase();
		} else {
