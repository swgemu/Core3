...
public:
	...
	void run(CreatureObject* player, SuiBox* suiBox, bool cancelPressed, Vector<UnicodeString>* args) {
		...
		if (curFireworks == 20) {
			player->sendSystemMessage("This firework show is at its limit of 20 fireworks.");
			return;
		}
		...
		if(firework->getUseCount() > 1) {
			...
			fireworkShowData->addFirework(curFireworks, fireworkClone, "10");
		} else {
			fireworkShowData->addFirework(curFireworks, firework, "10");
