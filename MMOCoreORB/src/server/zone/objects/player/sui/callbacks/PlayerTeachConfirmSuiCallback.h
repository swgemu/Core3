...
public:
	PlayerTeachConfirmSuiCallback(ZoneServer* serv, const String& name) :
		SuiCallback(serv) {
		skillname = name;
	}
	...
	void run(CreatureObject* creature, SuiBox* sui, bool cancelPressed,
		...
		if (!cancelPressed) {
			...
			if (SkillManager::instance()->canLearnSkill(skill->getSkillName(),
					creature, false)) {
				canLearnSkill = true;
			} else {
				//Check if the player lacks skill points.
				playerLacksSkillPoints =
						SkillManager::instance()->fullfillsSkillPrerequisitesAndXp(
								skill->getSkillName(), creature);
			}

			if (canLearnSkill) {
				//Award skill.
				SkillManager::instance()->awardSkill(skill->getSkillName(),
						creature, true, false, false);

				//Send message to client
				StringIdChatParameter systemMessageSkillLearned(
						"@skill_teacher:prose_skill_learned");
				systemMessageSkillLearned.setTO(
						"@skl_n:" + skill->getSkillName());
				...
				ManagedReference<CreatureObject*> player =
						cast<CreatureObject*>(sui->getUsingObject());
