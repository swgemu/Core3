...
bool PlayerObjectImplementation::readObjectMember(ObjectInputStream* stream, const String& _name) {
	...
	if (_name == "PlayerObject.bountyLockList") {
		TypeInfo<VectorMap<unsigned long long, BountyHunterTefRemovalTask*> >::parseFromBinaryStream(&bountyLockList, stream);
		return true;
	}
...
int PlayerObjectImplementation::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<VectorMap<unsigned long long, BountyHunterTefRemovalTask*> >::toBinaryStream(&bountyLockList, stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
...
bool PlayerObjectImplementation::isInBountyLockList(unsigned long long playerId) {
	// server/zone/objects/player/PlayerObject.idl():  		return bountyLockList.contains(playerId);
