...
void PlayerObject::setJediState(int state) {
	PlayerObjectImplementation* _implementation = static_cast<PlayerObjectImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_SETJEDISTATE__INT_);
		method.addSignedIntParameter(state);

		method.executeWithVoidReturn();
	} else
		_implementation->setJediState(state);
}
...
void PlayerObjectImplementation::setJediState(int state) {
	// server/zone/objects/player/PlayerObject.idl():  		jediState = state;
	jediState = state;
}
...
void PlayerObjectAdapter::setJediState(int state) {
	(static_cast<PlayerObject*>(stub))->setJediState(state);
}
