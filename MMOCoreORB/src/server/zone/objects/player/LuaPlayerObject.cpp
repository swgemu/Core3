...
int LuaPlayerObject::findJediTrainer(lua_State* L) {
	...
	for (int i=0; i < zServ->getZoneCount(); ++i) {
		...
		for (int j=0; j < trainerTypes.size(); ++j) {
			...
			for (int k=0; k < objectList.size(); ++k) {
				trainers.add(objectList.get(k).get());
			}
		}
	}
	...
	while (!found) {
		...
		CreatureObject* trainerCreo = cast<CreatureObject*>(trainer);
		...
		CityRegion* city = trainerCreo->getCityRegion().get();
