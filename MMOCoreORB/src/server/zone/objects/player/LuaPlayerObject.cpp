...
int LuaPlayerObject::findNearestForceShrine(lua_State* L) {
	...
	zone->getInRangeObjects(realObject->getWorldPositionX(), realObject->getWorldPositionY(), range, &objects, true);
	...
	if (sendWaypoint) {
		...
		waypoint->setCustomObjectName(zoneName + " Force Shrine", false);
