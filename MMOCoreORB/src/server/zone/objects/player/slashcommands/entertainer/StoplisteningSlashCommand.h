...
class StoplisteningSlashCommand : public SlashCommand {
public:

	StoplisteningSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(Player* player, Message* packet) {
		...
		uint64 listenID = packet->parseLong();
