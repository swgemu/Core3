...
public:
	SlashCommand(const String& name, ZoneProcessServerImplementation* serv) {
		server = serv;
	 ...
	 * Override me
	 */
	virtual bool doSlashCommand(Player* player, Message* packet) {
		return false;
	}
	 ...
	 */
	bool checkStateMask(Player* player) {
	 ...
	 */
	bool checkInvalidPostures(Player* player) {
	 ...
	 */
	virtual void onFail(uint32 actioncntr, Player* player) {
	 ...
	 */
	virtual void onComplete(uint32 actioncntr, Player* player) {
