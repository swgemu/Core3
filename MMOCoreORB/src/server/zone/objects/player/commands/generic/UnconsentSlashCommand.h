...
class UnconsentSlashCommand : public QueueCommand {
public:

	UnconsentSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		packet->parseUnicode(unicodeNames);
