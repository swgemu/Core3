...
class SetCurrentSkillTitleSlashCommand : public QueueCommand {
public:

	SetCurrentSkillTitleSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		/*
		packet->shiftOffset(8); //shift past the blank id.
		UnicodeString title;
		packet->parseUnicode(title);
		...
		PlayerCreature* player->getPlayerObject();
