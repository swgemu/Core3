...
class SetMoodInternalSlashCommand : public QueueCommand {
public:

	SetMoodInternalSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		chatManager->handleMood(player, packet);*/
		return true;
	}
