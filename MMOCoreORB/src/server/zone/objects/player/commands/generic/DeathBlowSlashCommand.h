...
class DeathBlowSlashCommand : public QueueCommand {
public:
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		try {
			...
			System::out << "Unreported exception caught in ObjectControllerMessage::handleDeathblow(PlayerCreature* player, Message* packet)\n";
			target->unlock();
			return false;
		}*/
		return true;
