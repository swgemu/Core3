...
public:
	 ...
	 * Override me
	 */
	virtual bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		return false;
	}
	 ...
	 */
	bool checkStateMask(PlayerCreature* player) {
		return (player->getStateBitmask() & stateMask) == 0;
	}

	/*
	 * Checks each invalid posture with the player's current posture
	 */
	bool checkInvalidPostures(PlayerCreature* player) {
		for (int i = 0; i < invalidPostures.size(); ++i) {
			if (invalidPostures.get(i) == player->getPosture())
				return false;
		}
		return true;
	}
	 ...
	 */
	virtual void onFail(uint32 actioncntr, PlayerCreature* player) {
		uint64 states = stateMask & player->getStateBitmask();
	 ...
	 */
	virtual void onComplete(uint32 actioncntr, PlayerCreature* player) {
