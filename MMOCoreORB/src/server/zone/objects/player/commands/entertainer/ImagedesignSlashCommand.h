...
class ImagedesignSlashCommand : public QueueCommand {
public:
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		try {
			...
			if (object->isPlayer()) {
				...
				if (targetPlayerCreature* player->getObjectID()
...
/*
				...
				if (targetPlayerCreature* player->getObjectID()) {
					ImageDesignStartMessage* msgTarget =
							new ImageDesignStartMessage(targetPlayerCreature* player,
									targetPlayer, tent);
			...
			System::out
					<< "Unreported exception in ObjectControllerMessage::parseImageDesign(PlayerCreature* player, Message *pack)";
			if (object != player)
				object->unlock();
			return false;
		}*/
