...
public:
	...
	void run() {
		...
		Locker lockerCt(creatureTarget);
		if(creature != NULL && creatureTarget != NULL) {
			...
			if (counter < amountOfTicks && !creatureTarget->isIncapacitated() && !creatureTarget->isDead() && (creatureTarget->getPvpStatusBitmask() != CreatureFlag::NONE)) {
				float damage = 500;
				SceneObject* psg = creatureTarget->getSlottedObject("utility_belt");
				if (psg != NULL && psg->isPsgArmorObject()) {
						if (cast<ArmorObject*>(psg)->isVulnerable(1)){
						float armorReduction =  cast<ArmorObject*>(psg)->getKinetic();
						if (armorReduction > 0) damage *= (1.f - (armorReduction / 100.f));
						Locker locker(cast<ArmorObject*>(psg));
						cast<ArmorObject*>(psg)->inflictDamage(cast<ArmorObject*>(psg), 0, damage * 0.1, true, true);
						}
					}
