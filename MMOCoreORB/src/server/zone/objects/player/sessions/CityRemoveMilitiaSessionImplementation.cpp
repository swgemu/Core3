...
int CityRemoveMilitiaSessionImplementation::initializeSession() {
	...
	if (!cityRegion->isMayor(creature->getObjectID()))
		return cancelSession();
	...
	StringBuffer text;
	text << "@city/city:remove_militia_prefix " << militiaMember->getObjectName()->getDisplayedName() << " @city/city:remove_militia_suffix";
	...
	sui->setCallback(new CityRemoveMilitiaMemberSuiCallback(creatureObject->getZoneServer(), cityRegion));
