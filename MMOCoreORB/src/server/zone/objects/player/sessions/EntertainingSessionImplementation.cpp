...
void EntertainingSessionImplementation::activateEntertainerBuff(CreatureObject* creature, int performanceType) {
	...
	try {
		...
		if(timeElapsed < 120) {
			creature->sendSystemMessage("You must listen or watch a performer for at least 2 minutes in order to gain the entertainer buffs.");
			return;
		}
...
void EntertainingSessionImplementation::increaseEntertainerBuff(CreatureObject* patron){
	...
	addEntertainerBuffDuration(patron, performance->getType(), 1.0f * buffAcceleration);
