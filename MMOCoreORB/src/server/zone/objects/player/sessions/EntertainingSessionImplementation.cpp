...
void EntertainingSessionImplementation::awardEntertainerExperience() {
	...
	if (player->isPlayerCreature()) {
		...
		if (flourishXp > 0 && (isDancing() || isPlayingMusic())) {
			...
			int audienceSize = MIN(getBandAudienceSize(), 50);
...
SortedVector<ManagedReference<CreatureObject*> > EntertainingSessionImplementation::getAudience() {
	SortedVector<ManagedReference<CreatureObject*> > audienceList;
...
int EntertainingSessionImplementation::getBandAudienceSize() {
	SortedVector<ManagedReference<CreatureObject*> > audienceList = getAudience();
	...
	for (int i = 0; i < group->getGroupSize(); ++i) {
		...
		if (groupMember != NULL && groupMember->isPlayerCreature()) {
			...
			if (groupMember != player && groupMember->isEntertaining() && groupMember->isInRange(player, 40.0f) && groupMember->hasSkill("social_entertainer_novice")) {
				...
				SortedVector<ManagedReference<CreatureObject*> > memberAudienceList = session->getAudience();

				for (int j = 0; j < memberAudienceList.size(); j++) {
					ManagedReference<CreatureObject*> audienceMember = memberAudienceList.get(j);

					if (audienceMember != NULL && !audienceList.contains(audienceMember))
						audienceList.put(audienceMember);
				}
			}
		}
	}
