...
void PlaceStructureSessionImplementation::placeTemporaryNoBuildZone(SharedStructureObjectTemplate* serverTemplate) {
	...
	temporaryNoBuildZone = (zone->getZoneServer()->createObject(String::hashCode("object/active_area.iff"), 0)).castTo<ActiveArea*>();
...
int PlaceStructureSessionImplementation::completeSession() {
	...
	if (ghost != NULL) {
		...
		//Create Waypoint
		ManagedReference<WaypointObject*> waypointObject = ( zone->getZoneServer()->createObject(String::hashCode("object/waypoint/world_waypoint_blue.iff"), 1)).castTo<WaypointObject*>();
