...
void PlayerObjectImplementation::notifySceneReady() {
	...
	for (int i = chatRooms.size() - 1; i >= 0; i--) {
		...
		if (room != NULL) {
			...
			if (roomType == ChatRoom::AUCTION || roomType == ChatRoom::PLANET)
...
bool PlayerObjectImplementation::canCreateChatRoom() {
	Locker locker(_this.getReferenceUnsafeStaticCast());
	...
	for (int i = 0; i < ownedChatRooms.size(); ++i) {
