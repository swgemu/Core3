	...
	if (isOnline()) {
		...
		if (creature->isInCombat() && creature->getTargetID() != 0 && !creature->isPeaced() && 
			...
			if (targetObject != NULL) {
				if ((targetObject->getWorldPosition().distanceTo(creature->getWorldPosition()) - targetObject->getTemplateRadius() - creature->getTemplateRadius()) <= MAX(10, creature->getWeapon()->getMaxRange())) {
