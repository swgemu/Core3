...
void CityRegion::updateNoBuildArea(float x, float y, float radius) {
	CityRegionImplementation* _implementation = static_cast<CityRegionImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_UPDATENOBUILDAREA__FLOAT_FLOAT_FLOAT_);
		method.addFloatParameter(x);
		method.addFloatParameter(y);
		method.addFloatParameter(radius);
		...
		_implementation->updateNoBuildArea(x, y, radius);
}
...
void CityRegionAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_GETBAZAARCOUNT__:
		...
		break;
	case RPC_UPDATENOBUILDAREA__FLOAT_FLOAT_FLOAT_:
		{
			updateNoBuildArea(inv->getFloatParameter(), inv->getFloatParameter(), inv->getFloatParameter());
		}
		break;
...
void CityRegionAdapter::updateNoBuildArea(float x, float y, float radius) {
	(static_cast<CityRegion*>(stub))->updateNoBuildArea(x, y, radius);
}
