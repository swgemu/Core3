...
void CityRegion::getStructureReport(CreatureObject* creature) {
	CityRegionImplementation* _implementation = static_cast<CityRegionImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_GETSTRUCTUREREPORT__CREATUREOBJECT_);
		method.addObjectParameter(creature);

		method.executeWithVoidReturn();
	} else
		_implementation->getStructureReport(creature);
}
...
bool CityRegionImplementation::readObjectMember(ObjectInputStream* stream, const String& _name) {
	...
	if (_name == "CityRegion.structures") {
...
void CityRegionAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETTAX__INT_INT_:
		...
		break;
	case RPC_GETSTRUCTUREREPORT__CREATUREOBJECT_:
		{
			getStructureReport(static_cast<CreatureObject*>(inv->getObjectParameter()));
		}
		break;
	default:
...
void CityRegionAdapter::getStructureReport(CreatureObject* creature) {
	(static_cast<CityRegion*>(stub))->getStructureReport(creature);
}
