...
void CityRegion::cleanupDecorations(int decorationLimit) {
	CityRegionImplementation* _implementation = static_cast<CityRegionImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
		...
		method.addSignedIntParameter(decorationLimit);

		method.executeWithVoidReturn();
	} else
		_implementation->cleanupDecorations(decorationLimit);
}
...
void CityRegionAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_CLEANUPDECORATIONS__INT_:
		{
			cleanupDecorations(inv->getSignedIntParameter());
		}
		break;
	default:
...
void CityRegionAdapter::cleanupDecorations(int decorationLimit) {
	(static_cast<CityRegion*>(stub))->cleanupDecorations(decorationLimit);
}
