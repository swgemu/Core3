...
void SpaceSpawnAreaImplementation::tryToSpawn(ShipObject* playerShip) {
	...
	Vector3 position = getRandomPosition(playerShip);
...
Vector3 SpaceSpawnAreaImplementation::getRandomPosition(SceneObject* playerShip) {
...
#ifdef DEBUG_SPACE_SPAWNING
	info(true) << getAreaName() << " -- getRandomPosition -- for Player " << playerShip->getDisplayedName() << " ID: " << playerShip->getObjectID();
	info(true) << getAreaName() << " Location = " << getPositionX() << " , " << getPositionY();
#endif // DEBUG_SPACE_SPAWNING

	const auto worldPosition = playerShip->getWorldPosition();

	position = areaShape->getRandomPosition(worldPosition, 256.f, ZoneServer::SPACEOBJECTRANGE);
