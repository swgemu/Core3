...
void PerformanceBuffImplementation::activate(bool applyModifiers) {
	...
	if(type == PerformanceBuffType::DANCE_MIND) {
		int mindStrength = round(strength * (float)creature->getBaseHAM(CreatureAttribute::MIND));
		...
		int focusStrength = round(strength * (float)creature->getBaseHAM(CreatureAttribute::FOCUS));
		...
		int willStrength = round(strength * (float)creature->getBaseHAM(CreatureAttribute::WILLPOWER));
