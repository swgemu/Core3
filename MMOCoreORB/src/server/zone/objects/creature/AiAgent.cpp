...
String AiAgent::getPvPFaction() {
	AiAgentImplementation* _implementation = static_cast<AiAgentImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_GETPVPFACTION__);

		String _return_getPvPFaction;
		method.executeWithAsciiReturn(_return_getPvPFaction);
		return _return_getPvPFaction;
	} else
...
void AiAgentAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ISEVENTMOB__:
		...
		break;
	case RPC_GETPVPFACTION__:
		{
			resp->insertAscii(getPvPFaction());
		}
		break;
	default:
...
String AiAgentAdapter::getPvPFaction() {
	return (static_cast<AiAgent*>(stub))->getPvPFaction();
}
