...
#ifdef DEBUG_QUEUE
		creature->info(true) << "CommandQueueTask :run()";
#endif
...
void CommandQueue::initialize(CreatureObject* creature) {
	...
	queueTask = new CommandQueueTask(creature, this);
}
...
void CommandQueue::run() {
	...
	creature->info(true) << "CommandQueue - run() #" << String::valueOf(runNum) << " called with a state of " << String::valueOf(state) << " and queueVector size = " << size;
#endif
...
#ifdef DEBUG_QUEUE
			creature->info(true) << "CommandQueue:run() - State = RUNNING -- Queue Task Scheduled in " << time << " ms - run() #" << String::valueOf(runNum);
#endif
...
#ifdef DEBUG_QUEUE
			creature->info(true) << "CommandQueue:run() -- State = DELAY -  #" << String::valueOf(runNum);
#endif

			if (!creature->hasAttackDelay() && checkForImmediateActions(creature))
				state = RUNNING;

			queueTask->reschedule(DELAYCHECK);
		}
		default:
			break;
	}
...
#ifdef DEBUG_QUEUE
	creature->info(true) << "CommandQueue:run() #" << String::valueOf(runNum) << " finished with a state of " << String::valueOf(state);
#endif
}
