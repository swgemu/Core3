...
void CommandQueue::handleRunningState(CreatureObject* creature, Reference<CommandQueueActionVector*> queue) {
	if (creature == nullptr || queue == nullptr)
		return;

	QueueTask* task = new QueueTask(creature, queue);
	Core::getTaskManager()->executeTask(task);
}

int CommandQueue::run() {
	ManagedReference<CreatureObject*> creature = weakCreature.get();
	Reference<CommandQueueActionVector*> commandQueue = queue.get();
	...
	switch (state) {
		...
		case RUNNING:
			...
			handleRunningState(creature, commandQueue);
			break;
		case POSTUREDELAYED:
...
void CommandQueue::enqueueCommand(unsigned int actionCRC, unsigned int actionCount, uint64 targetID, const UnicodeString& arguments, int priority, int compareCounter) {
	...
	if (commandQueue->size() != 0 || (nextAction != nullptr && !nextAction->isPast())) {
