...
public:
	QueueTask(CreatureObject* creature, CommandQueue* queue) {
		weakCreo = creature;
		commandQueue = queue;
	}
...
int CommandQueue::handleRunningState() {
	...
	if (action != nullptr) {
		...
		if (priority == QueueCommand::NORMAL && nextAction->isFuture()) {
			Time currTime;
		...
		if (queueCommand->addToCombatQueue()) {
			if (creature->hasPostureChangeDelay() && priority != QueueCommand::IMMEDIATE) {
...
void CommandQueue::run() {
	...
	switch (state) {
		case WAITING: {
			...
			QueueTask* queueTask = new QueueTask(creature, this);
