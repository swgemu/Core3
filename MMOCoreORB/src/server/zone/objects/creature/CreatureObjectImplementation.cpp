...
void CreatureObjectImplementation::sendStateCombatSpam(const String& stringName, byte color, int damage, bool broadcast) {
	...
	ManagedReference<CreatureObject*> attacker = _this.get();
	...
	if (broadcast) {
		...
		if (vec != NULL) {
			...
			info("Null closeobjects vector in CreatureObjectImplementation::sendStateCombatSpam", true);
			zone->getInRangeObjects(getWorldPositionX(), getWorldPositionY(), 128, &closeObjects, true);
		}

		for (int i = 0; i < closeObjects.size(); ++i) {
			SceneObject* object = cast<SceneObject*>( closeObjects.get(i).get());

			if (object->isPlayerCreature() && isInRange(object, 70)) {
				CreatureObject* receiver = cast<CreatureObject*>( object);
				CombatSpam* spam = new CombatSpam(attacker, NULL, receiver, NULL, damage, "cbt_spam", stringName, color);
				receiver->sendMessage(spam);
			}
		}

	} else { //Send spam only to originating player.
		CreatureObject* receiver = attacker;
