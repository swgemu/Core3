...
void CreatureObjectImplementation::setIntimidatedState(uint32 mod, uint32 crc, int durationSeconds) {
	...
	if (!hasState(CreatureState::INTIMIDATED)) {
		...
		Reference<StateBuff*> state = cast<StateBuff*>(getBuff(Long::hashCode(CreatureState::INTIMIDATED)));
...
bool CreatureObjectImplementation::removeStateBuff(unsigned long long state) {
	Reference<StateBuff*> buff = cast<StateBuff*>(getBuff(Long::hashCode(state)));
