...
void CreatureObjectImplementation::setDizziedState(int durationSeconds) {
	...
	// If the creature is already dizzied then add to the duration.
	Reference<Buff*> buff = getBuff(buffCRC);
	if (buff != NULL) {
		Locker locker(buff);

		buff->renew(buff->getTimeLeft() + durationSeconds);
		return;
	}
...
void CreatureObjectImplementation::setStunnedState(int durationSeconds) {
	...
	// If the creature is already stunned then add to the duration.
	Reference<Buff*> buff = getBuff(buffCRC);
	if (buff != NULL) {
		Locker locker(buff);
...
void CreatureObjectImplementation::setBlindedState(int durationSeconds) {
	...
	// If the creature is already blinded then add to the duration.
	Reference<Buff*> buff = getBuff(buffCRC);
	if (buff != NULL) {
		Locker locker(buff);
