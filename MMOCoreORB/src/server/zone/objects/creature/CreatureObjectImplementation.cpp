	...
	if (skill->getSkillName().contains("_novice") && isPlayerCreature()) {
		...
		if (ghost != nullptr && ghost->getActivePetsSize() > 0) {
			for (int i = 0; i < ghost->getActivePetsSize(); i++) {
				...
				if (petAgent != nullptr && petAgent->isDroidObject()) {
					Reference<DroidObject*> petDroid = cast<DroidObject*>(petAgent);

					if (petDroid != nullptr && petDroid->isHelperDroid()) {
						Locker clock(petDroid, ghost);
						petDroid->notifyHelperDroidSkillTrained(asCreatureObject(), skill->getSkillName());
					}
				}
			}
		}
