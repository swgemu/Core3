...
bool AiAgentImplementation::runAwarenessLogicCheck(SceneObject* pObject) {
	...
	if (creoObject->isVehicleObject() || creoObject->isMount()) {
		ManagedReference<SceneObject*> rider = creoObject->getSlottedObject("rider");

		if (rider != NULL && rider->isCreatureObject())
			creoObject = rider.castTo<CreatureObject*>();
	}
