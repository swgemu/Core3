...
void AiAgentImplementation::notifyPositionUpdate(QuadTreeEntry* entry) {
	...
	if (target != NULL && (target->isVehicleObject() || target->isMount()) && target->getSlottedObject("rider").castTo<CreatureObject*>() != NULL)
...
bool AiAgentImplementation::isAttackableBy(CreatureObject* object) {
	...
	if (object->isPet() || object->isVehicleObject()) {
		...
		if (!object->isVehicleObject() && pcd != NULL && pcd->getPetType() == PetManager::FACTIONPET && isNeutral()) {
			return false;
		}
