...
void AiAgentImplementation::enqueueAttack() {
	SceneObject* followCopy = getFollowObject();
...
bool AiAgentImplementation::validateStateAttack() {
	...
	return validateStateAttack(cast<CreatureObject*>(followCopy), nextActionArgs);
}
...
bool AiAgentImplementation::validateTarget() {
	...
	if (followCopy->isCreatureObject() && (!cast<CreatureObject*>(followCopy)->isAttackableBy(_this.get()) || cast<CreatureObject*>(followCopy)->isDead() || cast<CreatureObject*>(followCopy)->isIncapacitated()))
		return false;

	if (followCopy->isTangibleObject() && (!cast<TangibleObject*>(followCopy)->isAttackableBy(_this.get()) || cast<TangibleObject*>(followCopy)->isDestroyed()))
		return false;
...
int AiAgentImplementation::setDestination() {
	...
	case AiAgent::FLEEING:
		...
		if (!isRetreating() && (followCopy == NULL || !isInRange(followCopy, fleeRange))) {
			...
			activateAwarenessEvent(cast<CreatureObject*>(followCopy));
