...
void AiAgentImplementation::selectDefaultAttack() {
	if (npcTemplate == NULL)
		nextActionCRC = String::hashCode("defaultattack");
	else
	...
	while (!found && getPatrolPointSize() != 0) {
		...
		for (int i = 1; i < path->size(); ++i) { // i = 0 is our position
			...
			if (showNextMovementPosition) {
				...
				Reference<SceneObject*> movementMarker = getZoneServer()->createObject(String::hashCode("object/path_waypoint/path_waypoint.iff"), 0);
				...
				for (int i = 0; i < path->size(); ++i) {
					...
					movementMarker = getZoneServer()->createObject(String::hashCode("object/path_waypoint/path_waypoint.iff"), 0);
...
bool AiAgentImplementation::isScentMasked(CreatureObject* target) {
	...
	if (!effectiveTarget->hasBuff(String::hashCode("skill_buff_mask_scent_self"))) {
...
bool AiAgentImplementation::isConcealed(CreatureObject* target) {
	...
	if (!effectiveTarget->hasBuff(String::hashCode("skill_buff_mask_scent"))) {
		if(camouflagedObjects.contains(effectiveTarget)) camouflagedObjects.removeElement(effectiveTarget);
		return false;
	}
	if (isDroidObject())
...
void AiAgentImplementation::setupBehaviorTree(AiTemplate* aiTemplate) {
	...
	for (int i = 0; i < parents.size(); i++) {
		...
		if (element.getKey() == String::hashCode("none")) // this is the parent of the root node, just skip it.
			continue;
	...
	Vector<uint32>& roots = parents.get(String::hashCode("none"));
	if (roots.size() > 1) {
...
void AiAgentImplementation::setupBehaviorTree(AiTemplate* getTarget, AiTemplate* selectAttack, AiTemplate* combatMove, AiTemplate* idle) {
	...
	rootSelector->setID(String::hashCode("root"));
	attackSequence->setID(String::hashCode("attackSequence"));
	...
	behaviors.put(String::hashCode("root"), rootSelector);
	behaviors.put(String::hashCode("attackSequence"), attackSequence);
...
void AiAgentImplementation::resetBehaviorList() {
	...
	currentBehaviorID = String::hashCode("root");
