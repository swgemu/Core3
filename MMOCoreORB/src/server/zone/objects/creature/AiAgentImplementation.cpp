...
bool AiAgentImplementation::validateTarget(SceneObject* target) {
	...
	if (targetCreatureObject != NULL && (!targetCreatureObject->isAttackableBy(_this.getReferenceUnsafe()) || targetCreatureObject->isDead() || targetCreatureObject->isIncapacitated()))
		return false;

	TangibleObject* targetTangibleObject = target->asTangibleObject();

	if (targetTangibleObject != NULL && (!targetTangibleObject->isAttackableBy(_this.getReferenceUnsafe()) || targetTangibleObject->isDestroyed()))
		return false;
...
void AiAgentImplementation::notifyInsert(QuadTreeEntry* entry) {
	...
	if (scno == _this.getReferenceUnsafe())
		return;
...
void AiAgentImplementation::notifyDespawn(Zone* zone) {
	...
	for (int i = 0; i < movementMarkers.size(); ++i) {
		...
		Locker clocker(marker, _this.getReferenceUnsafe());
	...
	if (creatureInventory != NULL) {
		Locker clocker(creatureInventory, _this.getReferenceUnsafe());
	...
	Reference<Task*> task = new RespawnCreatureTask(_this.getReferenceUnsafe(), zone, level);
