...
bool CreatureObject::isWearingArmor() {
	CreatureObjectImplementation* _implementation = static_cast<CreatureObjectImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_ISWEARINGARMOR__);

		return method.executeWithBooleanReturn();
	} else
		return _implementation->isWearingArmor();
}
...
bool CreatureObjectImplementation::readObjectMember(ObjectInputStream* stream, const String& _name) {
	...
	if (_name == "CreatureObject.wearablesVector") {
...
int CreatureObjectImplementation::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_name = "CreatureObject.wearablesVector";
	_name.toBinaryStream(stream);
	_offset = stream->getOffset();
	stream->writeInt(0);
...
void CreatureObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_CHECKLASTKNOCKDOWN__:
		...
		break;
	case RPC_ISWEARINGARMOR__:
		resp->insertBoolean(isWearingArmor());
		break;
...
bool CreatureObjectAdapter::isWearingArmor() {
	return (static_cast<CreatureObject*>(stub))->isWearingArmor();
}
