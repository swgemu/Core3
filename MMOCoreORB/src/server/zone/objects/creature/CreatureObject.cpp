...
bool CreatureObjectImplementation::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x6ba640e4: //CreatureObject.lastCombatActionTime
		TypeInfo<Time >::parseFromBinaryStream(&lastCombatActionTime, stream);
		return true;

	case 0xdb3dc71d: //CreatureObject.lastCombatPosition
		TypeInfo<Vector3 >::parseFromBinaryStream(&lastCombatPosition, stream);
		return true;
...
int CreatureObjectImplementation::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_nameHashCode = 0x6ba640e4; //CreatureObject.lastCombatActionTime
	TypeInfo<uint32>::toBinaryStream(&_nameHashCode, stream);
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<Time >::toBinaryStream(&lastCombatActionTime, stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);

	_nameHashCode = 0xdb3dc71d; //CreatureObject.lastCombatPosition
	TypeInfo<uint32>::toBinaryStream(&_nameHashCode, stream);
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<Vector3 >::toBinaryStream(&lastCombatPosition, stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
