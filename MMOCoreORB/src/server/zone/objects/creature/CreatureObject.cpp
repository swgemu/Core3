...
void CreatureObject::addWearableSkillMod(const String& skillMod, long long value, bool notifyClient) {
	CreatureObjectImplementation* _implementation = static_cast<CreatureObjectImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_ADDWEARABLESKILLMOD__STRING_LONG_BOOL_);
		method.addAsciiParameter(skillMod);
		method.addSignedLongParameter(value);
		method.addBooleanParameter(notifyClient);
		...
		_implementation->addWearableSkillMod(skillMod, value, notifyClient);
}
...
int CreatureObjectImplementation::addDotState(unsigned long long dotType, unsigned long long objectID, unsigned int strength, byte type, unsigned int duration, float potency, unsigned int defense) {
	...
	return (&damageOverTimeList)->addDot(_this, objectID, duration, dotType, type, strength, potency, defense);
}
...
void CreatureObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ADDSKILLMOD__STRING_LONG_BOOL_:
		...
		break;
	case RPC_ADDWEARABLESKILLMOD__STRING_LONG_BOOL_:
		addWearableSkillMod(inv->getAsciiParameter(_param0_addWearableSkillMod__String_long_bool_), inv->getSignedLongParameter(), inv->getBooleanParameter());
		break;
...
void CreatureObjectAdapter::addWearableSkillMod(const String& skillMod, long long value, bool notifyClient) {
	(static_cast<CreatureObject*>(stub))->addWearableSkillMod(skillMod, value, notifyClient);
}
