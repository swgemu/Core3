...
public:
	...
	void outputTimes(CreatureObject* caller) {
		...
		while (iter.hasNext()) {
			...
			list->addMenuItem("Average time spent in " + key + ": " + String::valueOf(averageTime) + "ms with " + String::valueOf(call.get()) + " calls totaling " + String::valueOf((float)nextTime.get()/1000.f) + "ms.");
		}
