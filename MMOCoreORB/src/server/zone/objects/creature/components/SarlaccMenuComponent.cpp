...
int SarlaccMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectedID) {
	if (selectedID == 20) {
		...
		for (int i=0; i< inventory->getContainerObjectsSize(); i++) {
			...
			if (sco == NULL)
				continue;
			if (sco->getServerObjectCRC() == String::hashCode("object/tangible/loot/quest/quest_item_sarlacc_bile_jar.iff")) {
			...
			if (sco->getContainerObjectsSize() > 0) {
				for (int j=0; j < sco->getContainerObjectsSize(); j++) {
					...
					if (child->getServerObjectCRC() == String::hashCode("object/tangible/loot/quest/quest_item_sarlacc_bile_jar.iff")) {
						player->sendSystemMessage("@mob/sarlacc:bile_already"); // You already have a good sample of bile. It probably isn't wise to risk getting more.
						return 0;
					}
				}
		...
		Reference<SceneObject*> bileSceno = player->getZoneServer()->createObject(String::hashCode("object/tangible/loot/quest/quest_item_sarlacc_bile_jar.iff"), 1);
		if (bileSceno == NULL)
			return 1;
