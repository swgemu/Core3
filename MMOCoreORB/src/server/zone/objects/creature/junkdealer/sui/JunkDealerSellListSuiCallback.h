...
void run(CreatureObject* player, SuiBox* suiBox, bool cancel, Vector<UnicodeString>* args) {
	...
	/*for (int i = 0; i < inventory->getContainerObjectsSize(); i++) {
		if (inventory->getContainerObject(i)->getObjectID()==itemId){
			iCredits = cast<TangibleObject*>(inventory->getContainerObject(i))->getJunkValue();
			tst = inventory->getContainerObject(i);
			break;}
	}*/
	ManagedReference<SceneObject*> tst =inventory->getContainerObject(itemId);
	if (tst == NULL || otherPressed==true ){
				return;
	}
	int iCredits = cast<TangibleObject*>(inventory->getContainerObject(itemId))->getJunkValue();
	UnicodeString itemName = StringIdManager::instance()->getStringId(tst->getDisplayedName().hashCode());
	tst->destroyObjectFromWorld(true);
