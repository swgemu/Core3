...
public:
	...
	Vector<ManagedReference<ArmorObject*> > getArmorAtHitLocation(uint8 hl) {
		...
		switch(hl) {
		case 1: // HIT_BODY
			return protectionArmorMap.get(CHEST); // CHEST
		case 2: // HIT_LARM
		case 3: // HIT_RARM
		{
			Vector<ManagedReference<ArmorObject*> > armArmor = protectionArmorMap.get(ARMS); // ARMS
		...
		case 4: // HIT_LLEG
		case 5: // HIT_RLEG
			return protectionArmorMap.get(LEGS); // LEGS
		case 6: // HIT_HEAD
			return protectionArmorMap.get(HEAD); // HEAD
		default:
			return protectionArmorMap.get(NOLOCATION); // NOLOCATION - This is never set and should remain empty
		}
	}
