...
int ForceHealQueueCommand::runCommand(CreatureObject* creature, CreatureObject* targetCreature) const {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (woundAttributesToHeal & (int)pow(2, i)) {
			for (int j = 0; j < 3; j++) {
	...
	for (int i = 0; i < 3; i++) {
		// Attrib Values: Health = 1, Action = 2, Mind = 4
		if (attributesToHeal & (int)pow(2, i)) {
			if (totalCost < currentForce) {
				uint8 attrib = i * 3;
				int curHam = targetCreature->getHAM(attrib);
				int maxHam = targetCreature->getMaxHAM(attrib);
	...
	if (totalCost < currentForce) {
		...
		int healedStates = 0;
		for (int i = 12; i <= 15; i++) {
			if ((statesToHeal & (int)pow(i, 2)) && targetCreature->hasState((int)pow(i, 2))) {
				totalStates++;
				int newTotal = totalCost + healStateCost;

				if (newTotal < currentForce) {
					targetCreature->removeStateBuff((int)pow(i, 2));
					totalCost = newTotal;
					healPerformed = true;
					healedStates++;
				}
