...
public:
	...
	int doCombatAction(CreatureObject* creature, const uint64& target, const UnicodeString& arguments = "", ManagedReference<WeaponObject*> weapon = NULL) const {
		...
		try {
			Reference<CreatureAttackData*> attackData = new CreatureAttackData(arguments, this, target);
			int res = combatManager->doCombatAction(creature, weapon, cast<TangibleObject*>(targetObject.get()), attackData);
	...
	virtual void sendAttackCombatSpam(TangibleObject* attacker, TangibleObject* defender, int attackResult, int damage, CreatureAttackData *data) const {
