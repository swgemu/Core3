					...
					if (targetTano->isCreatureObject()) {
						...
						if (targetCreature != NULL) {
							if (targetCreature->isPlayerCreature()) {
								...
								if (targetOwner != NULL && !targetOwner->hasBountyMissionFor(creature) && !CombatManager::instance()->areInDuel(creature, targetOwner) && targetOwner->getFactionStatus() == FactionStatus::OVERT) {
										ghost->doFieldFactionChange(FactionStatus::OVERT);
								}
							} else {
