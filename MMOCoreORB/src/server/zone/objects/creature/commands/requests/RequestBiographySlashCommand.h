...
class RequestBiographySlashCommand : public QueueCommand {
public:

	RequestBiographySlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		if (object->isPlayer()) {
			...
			try {
				...
				System::out
						<< "Unreported exception in ObjectControllerMessage::parseBiographyRequest(PlayerCreature* player, Message *pack)";
				if (play != player)
					play->unlock();
				return false;
			}
