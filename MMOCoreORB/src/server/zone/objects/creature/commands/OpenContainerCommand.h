...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target,
		...
		if (objectToOpen->getParent() != NULL
				&& objectToOpen->getParent().get()->isCraftingStation())
			return GENERALERROR;
		...
		if (objectToOpen->checkContainerPermission(creature,
				ContainerPermissions::OPEN)) {

			if (objectToOpen->getGameObjectType()
					== SceneObjectType::STATICLOOTCONTAINER) {

				ManagedReference<Container*> container = objectToOpen.castTo<
						Container*> ();
				if (container == NULL)
					return GENERALERROR;
			...
			objectToOpen->notifyObservers(ObserverEventType::OPENCONTAINER,
					creature);
