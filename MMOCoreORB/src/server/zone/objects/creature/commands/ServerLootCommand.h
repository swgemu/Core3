					...
					if (correctSyntax) {
						...
						if (itemTemplate != nullptr) {
							...
							for (int i = 0; count > i; ++i) {
								if (compareLoot) {
									obj = lootManager->createNewLootObject(itemTemplate, level, false, logLoot);
									obj->setCustomObjectName(obj->getDisplayedName() + " (new)", false);

									ref = lootManager->createLootObject(itemTemplate, level, false, logLoot);
									ref->setCustomObjectName(ref->getDisplayedName() + " (old)", false);
								} else {
									if (lootManager->getLootVersion() == 1) {
										obj = lootManager->createNewLootObject(itemTemplate, level, false, logLoot);
										obj->setCustomObjectName(obj->getDisplayedName() + " (new)", false);
									} else {
								...
								systemMessage << "itemTemplate: " << obj->getDisplayedName() << " successfully generated." << endl;
...
//				+-------------------------------------------------------------------------------------------------------------------------------------------------------+
						...
						if (correctSyntax) {
							...
							Reference<const LootGroupTemplate*> group = lootManager->getLootMap()->getLootGroupTemplate(lootGroup);
...
//				+-------------------------------------------------------------------------------------------------------------------------------------------------------+
					...
					for (int i = 0; templateVector.size() > i; ++i) {
						...
						ManagedReference<TangibleObject*> prototype = zoneServer->createObject(itemTemplate->getDirectObjectTemplate().hashCode(), 2)->asTangibleObject();
