...
creature->sendSystemMessage("Checked all the band members. " + String::valueOf(bandMembers.size()) + " of them playing.");
		for (int j = 0; j < bandMembers.size(); ++j) {
			...
			ManagedReference<Instrument*> bandMemberInstrument = bandMemberSession->getInstrument(bandMember);
			...
			bandMemberSession->sendEntertainingUpdate(bandMember, /*0x3C4CCCCD*/0.0125, instrumentAnimation, 0x07339FF8, instrid);
			bandMemberSession->setPerformanceName(args);
			creature->sendSystemMessage("Changed " + bandMember->getCustomObjectName() + " to the new song.");
		}

		Locker locker(group);

		group->setBandSong(args);
creature->sendSystemMessage("Returning Success.");
		return SUCCESS;
	}
