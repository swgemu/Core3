	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (res == SUCCESS) {
			...
			if (creatureTarget != NULL) {
				...
				ManagedReference<AiAgent*> target = cast<AiMoveEvent*>(target.get());

				Locker locker(target);

				CombatManager::instance()->attemptPeace(target);

				target->clearCombatState(creature);
				target->setCombatState(AiAgent::OBLIVIOUS);
				target->setNextPosition();
				target->activateMovementEvent();
