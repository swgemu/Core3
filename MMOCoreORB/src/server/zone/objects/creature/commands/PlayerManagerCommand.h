...
public:
	...
	static String dumpCOV(ZoneServer* zoneServer, uint64_t oid, bool showAll) {
		StringBuffer resp;

		if (zoneServer == nullptr) {
			resp << "oid: " << (int)oid << " zoneServer == nullptr";
		...
		if (targetObject == nullptr) {
			resp << "oid: " << (int)oid << " getObject() failed.";
			return resp.toString();
		}
		...
		if (vec == nullptr) {
			resp << "oid: " << (int)oid << " does not have a close objects vector.";
			return resp.toString();
		}
