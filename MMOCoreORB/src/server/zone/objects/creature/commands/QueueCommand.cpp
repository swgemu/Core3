	...
	if (creoParentID != tarParentID) {
		...
		if (targetCell != nullptr) {
			...
			if (parentSceneObject != nullptr) {
				...
				if (buildingObject->isAllowedEntry(creature)) {
					creature->sendSystemMessage(" building object allowed entry = true");
				}
			}
			...
			if (!perms->hasInheritPermissionsFromParent() && (creature->getRootParent() == target->getRootParent())) {
				creature->sendSystemMessage(" inherent perms are false");
