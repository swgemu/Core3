...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if(targetPlayer->hasBuff(String::hashCode("skill_buff_mask_scent")) || targetPlayer->getSkillModFromBuffs("private_conceal") > 0) {
			creature->sendSystemMessage("@skl_use:sys_target_concealed"); // Your target is already concealed.
			return false;
		}

		if(targetPlayer->hasBuff(String::hashCode("skill_buff_mask_scent_self")) || (targetPlayer->getOptionsBitmask() & CreatureState::MASKSCENT)) {
		...
		uint32 crc = String::hashCode("skill_buff_mask_scent");
