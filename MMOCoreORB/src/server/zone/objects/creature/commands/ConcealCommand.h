...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!targetPlayer->checkCooldownRecovery("camo_" + String::valueOf(crc))) {
			StringIdChatParameter delayMsg("skl_use", "sys_conceal_delay");
			delayMsg.setDI(targetPlayer->getCooldownTime("camo_" + String::valueOf(crc))->miliDifference()/1000);
			targetPlayer->sendSystemMessage(delayMsg);
			return GENERALERROR;
		}
