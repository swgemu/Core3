...
: JediQueueCommand(name, server) {
		forceCost = 150;
		uint32 buffCRC1 = BuffCRC::JEDI_FORCE_ARMOR_2;
		uint32 buffCRC2 = BuffCRC::JEDI_FORCE_ARMOR_1;
		buffCRCs.add(buffCRC1);
		buffCRCs.add(buffCRC2);
		duration = 1800;
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		return doJediBuffCommand(creature, target, arguments);
	}
