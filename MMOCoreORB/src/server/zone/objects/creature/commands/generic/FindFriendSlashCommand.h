...
class FindFriendSlashCommand : public QueueCommand {
public:

	FindFriendSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		/*PlayerCreature* playerManager = server->getPlayerManager();
		...
		if (name != "") {
			player->getPlayerCreature* playerManager);
		}*/
		return true;
	}
