...
class ResourceContainerSplitSlashCommand : public QueueCommand {
public:

	ResourceContainerSplitSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		if (player->hasFullInventory()) {
			player->sendSystemMessage("You dont have enough space in your inventory");
			return false;
		}
