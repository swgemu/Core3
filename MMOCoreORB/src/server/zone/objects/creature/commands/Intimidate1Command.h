...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (res == SUCCESS && creature->isPlayerCreature() && creature->getPlayerObject()->getCommandMessageString(String::hashCode("intimidate1")).isEmpty()==false && creature->checkCooldownRecovery("command_message")) {
			UnicodeString shout(creature->getPlayerObject()->getCommandMessageString(String::hashCode("intimidate1")));
