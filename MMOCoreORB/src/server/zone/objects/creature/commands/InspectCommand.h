...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (targetSceneO == nullptr || !targetSceneO->isShipAiAgent()) {
			return INVALIDTARGET;
		}

		auto shipAgent = targetSceneO->asShipAiAgent();
		...
		if (!shipAgent->isInRange3d(ship, INSPECT_RANGE)) {
		...
		auto inspectTask = new ShipInspectTask(creature, shipAgent);
