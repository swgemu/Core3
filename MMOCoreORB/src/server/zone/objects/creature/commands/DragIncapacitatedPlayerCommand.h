...
public:
	...
	void getCoordinates(SceneObject* object, SceneObject* targetObject, float maxMove, WorldCoordinates* newPosition) const {
		...
		float newZ = zone->getPlanetManager()->findClosestWorldFloor(newPosition->getX(), newPosition->getY(), objectPosition.getZ(), 0, &intersections, (CloseObjectsVector*) object->getCloseObjects());
	...
	void drag(CreatureObject* player, CreatureObject* targetPlayer, float maxRange, float maxMovement, bool needsConsent) const {
		...
		targetPlayer->setDirection(*player->getDirection());

		targetPlayer->updateZone(false, false);
		targetPlayer->notifySelfPositionUpdate();
		targetGhost->setClientLastMovementStamp(0);
		targetPlayer->setMovementCounter(0);
