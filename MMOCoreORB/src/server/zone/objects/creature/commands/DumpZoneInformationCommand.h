...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		int total = MAX(heightCacheMissCount + heightCacheMissCount, 1);

		msg << "height cache hit count = " << heightCacheHitCount << ", miss count = " << heightCacheHitCount << ", hit rate = " << heightCacheHitCount / total << "%" << endl;
