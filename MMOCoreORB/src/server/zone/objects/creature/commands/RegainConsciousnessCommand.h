...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (creature->isDead()){
			...
			ManagedReference<PrivateBuff *> regenDebuff = new PrivateBuff(creature, STRING_HASHCODE("private_force_regen__debuff"), 60*30, BuffType::JEDI);
			Locker regenLocker(regenDebuff);
			regenDebuff->setSkillModifier("private_force_regen_percentage", 50);
