...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (targetObject == NULL || !targetObject->isPlayerObject()) {
		...
		if(res == SUCCESS) {
			...
			SortedVector<ManagedReference<TangibleObject*> > targets = CombatManager::instance()->getAreaTargets(creature, creature->getWeapon(), tarCreo, data);
			if(targets.size() != 0) {
