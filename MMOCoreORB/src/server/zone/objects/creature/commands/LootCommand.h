...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		int pickupResult = pickupOwnedItems(creature, lootContainer);
		switch (pickupResult) {
		case 0: //No items available for anyone to pickup.
			break;
		case 1: //No items available for looter to pickup, but one is available for someone else.
			ai->notifyObservers(ObserverEventType::LOOTCREATURE, creature, 0);
			lootContainer->openContainerTo(creature);
			break;
		case 2: //An item was available for the looter, there are items remaining.
			return SUCCESS;
		case 3: //An item was available for the looter, there are NO items remaining.
			creature->getZoneServer()->getPlayerManager()->rescheduleCorpseDestruction(creature, ai);
			return SUCCESS;
		default:
			break;
	...
	int pickupOwnedItems(CreatureObject* creature, SceneObject* lootContainer) const {
		/* Return codes:
		 * 0: No items available for anyone to pickup.
		 * 1: No items available for looter to pickup, but one is available for someone else.
		 * 2: An item was available for the looter, there are items remaining.
		 * 3: An item was available for the looter, there are NO items remaining.
		 */
