...
class HealAllSelf1Command : public JediQueueCommand {
public:

	HealAllSelf1Command(const String& name, ZoneProcessServer* server)
: JediQueueCommand(name, server) {
		clientEffect = "clienteffect/pl_force_heal_self.cef";
		healAttributesDamage.put(CreatureAttribute::HEALTH, 500);
		healAttributesDamage.put(CreatureAttribute::ACTION, 500);
		healAttributesDamage.put(CreatureAttribute::MIND, 500);
		healingType = JediQueueCommand::SELFONLY;
	}
