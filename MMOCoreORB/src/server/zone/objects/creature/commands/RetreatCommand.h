...
public:
	...
	bool checkRetreat(CreatureObject* creature) const {
		...
		uint32 burstCRC = String::hashCode("burstrun");
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (player->isPlayerCreature() && player->getPlayerObject()->getCommandMessageString(String::hashCode("retreat")).isEmpty()==false && creature->checkCooldownRecovery("command_message")) {
			UnicodeString shout(player->getPlayerObject()->getCommandMessageString(String::hashCode("retreat")));
	...
	void doRetreat(CreatureObject* player) const {
		...
		uint32 actionCRC = String::hashCode("retreat");
