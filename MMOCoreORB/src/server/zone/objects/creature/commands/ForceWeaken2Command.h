		...
		if (res == SUCCESS) {
			...
			if (creatureTarget != NULL) {
				...
				if (!creatureTarget->hasBuff(STRING_HASHCODE("forceweaken2"))) {
					...
					String buffname = "forceweaken2b";

					if (currentHealth <= 25000) {
						int debuffHam = 100 + (randTick * 5);//  debuff of HAMs should TICK between 25-150 per weaken1, 100-250 weaken2
						...
						int debuffHam = (randTick * currentHealth) * .0015;//  but for higher level mobs, debuff of HAMs should 'scale' up
