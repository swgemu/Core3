	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (res == SUCCESS) {
			...
			if (targetObject != NULL) {
				...
				ManagedReference<Buff*> choke = new Buff(creature, getNameCRC(), 35, BuffType::JEDI);
				creature->addBuff(choke);

				Reference<ForceChokeTickTask*> tickTask = new ForceChokeTickTask(creature, tickDamage);
