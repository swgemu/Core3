...
public:
	Wait(const String& className, const uint32 id, const LuaObject& args) : Behavior(className, id, args), durationMin(-1.f), durationMax(-1.f) {
		parseArgs(args);
	}
	...
	void parseArgs(const LuaObject& args) {
		durationMin = (getArg<float>()(args, "durationMin") * 1000);
		durationMax = (getArg<float>()(args, "durationMax") * 1000);
	}
	...
	Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		...
		float totalWait = System::frandom(fabs(durationMax - durationMin)) + durationMin;

		// agent->info(true) << "setting wait: " << totalWait << " Num of players in range: " << agent->getNumberOfPlayersInRange();

		agent->setWait((uint64)totalWait);
