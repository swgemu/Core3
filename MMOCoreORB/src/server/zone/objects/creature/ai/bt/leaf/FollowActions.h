...
class GetFollowFromTarget : public Behavior {
public:
    GetFollowFromTarget(const String& className, const uint32 id, const LuaObject& args)
        : Behavior(className, id, args) {}

    GetFollowFromTarget(const GetFollowFromTarget& a)
        : Behavior(a) {}

    Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		SceneObject* target = agent->getTargetFromTargetsDefenders();
        agent->storeFollowObject();
        agent->setFollowObject(target);
        
        return agent->getFollowObject() != NULL ? SUCCESS : FAILURE;
    }
};

class GetFollowFromCommand : public Behavior {
public:
    GetFollowFromCommand(const String& className, const uint32 id, const LuaObject& args)
        : Behavior(className, id, args) {}

    GetFollowFromCommand(const GetFollowFromCommand& a)
        : Behavior(a) {}
    ...
    Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		...
		agent->storeFollowObject();
		agent->setFollowObject(cd->getLastCommandTarget().get());
