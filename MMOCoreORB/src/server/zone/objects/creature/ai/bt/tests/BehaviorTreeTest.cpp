...
public:
	BehaviorTreeTest()
			: agent(new NiceMock<MockAiAgent>()), aiMap(AiMap::instance()), mockZone(new NiceMock<MockZone>()), followMock(new NiceMock<MockAiAgent>()), mockWeao(
					new NiceMock<MockWeaponObject>()), mockDefaultWeao(new NiceMock<MockWeaponObject>()) {
	...
	void SetUp() {
		...
		ON_CALL(*agent, getZoneUnsafe()).WillByDefault(Return(mockZone));
		...
		ON_CALL(*followMock, isDestroyed()).WillByDefault(Return(false));
	...
	void TearDown() {
		...
		Mock::VerifyAndClear(followMock);
	}
...
TEST_F(BehaviorTreeTest, LoadTemplateTest) {
	...
	// we need to spoof the "has patrol" check
	ON_CALL(*agent, getPatrolPointSize()).WillByDefault(Return(true));
...
TEST_F(BehaviorTreeTest, CombatTest) {
	...
	ON_CALL(*followMock, isAttackableBy(TypedEq<CreatureObject*>(agent.get()))).WillByDefault(Return(true));

	ON_CALL(*mockThreat, getHighestThreatCreature()).WillByDefault(Return(followMock));
	ON_CALL(*followMock, isInRange(TypedEq<SceneObject*>(agent.get()), _)).WillByDefault(Return(true));
	ON_CALL(*followMock, hasState(CreatureState::PEACE)).WillByDefault(Return(false));
	ON_CALL(*agent, getDistanceTo(TypedEq<SceneObject*>(followMock))).WillByDefault(Return(5.f));
	...
	EXPECT_TRUE( agent->getWeapon() == mockDefaultWeao ) << "getWeapon() should be " << mockDefaultWeao.get() << ", got " << agent->getWeapon();
...
TEST_F(BehaviorTreeTest, MoveTest) {
	...
	ON_CALL(*pPoint, isInRange(TypedEq<SceneObject*>(followMock), FloatEq(256.f))).WillByDefault(Return(true));

	EXPECT_CALL(*agent, findNextPosition(FloatEq(10.f), TypedEq<bool>(true))).WillOnce(Return(true));
...
TEST_F(BehaviorTreeTest, SetWaitTest) {
	...
	EXPECT_CALL(*agent, findNextPosition(FloatEq(10.f), TypedEq<bool>(true))).WillOnce(Return(false));
...
TEST_F(BehaviorTreeTest, PetFollowTest) {
	...
	EXPECT_CALL(*agent, findNextPosition(FloatEq(10.f), TypedEq<bool>(false))).WillOnce(Return(true));
...
TEST_F(BehaviorTreeTest, PetPatrolTest) {
	...
	// TODO: Is this all we need to test here?
	EXPECT_CALL(*agent, findNextPosition(FloatEq(10.f), TypedEq<bool>(true))).WillOnce(Return(true)).WillOnce(Return(false));
...
TEST_F(BehaviorTreeTest, PetAttackTest) {
	...
	ON_CALL(*followMock, isAttackableBy(TypedEq<CreatureObject*>(agent.get()))).WillByDefault(Return(false));

	ON_CALL(*attackMock, asCreatureObject()).WillByDefault(Return(attackMock.get()));
	ON_CALL(*attackMock, asTangibleObject()).WillByDefault(Return(attackMock.get()));
	ON_CALL(*attackMock, isDestroyed()).WillByDefault(Return(false));
	ON_CALL(*attackMock, getPosture()).WillByDefault(Return(CreaturePosture::UPRIGHT));
	ON_CALL(*attackMock, isAttackableBy(TypedEq<CreatureObject*>(agent.get()))).WillByDefault(Return(true));
	ON_CALL(*attackMock, isInRange(TypedEq<SceneObject*>(agent.get()), FloatEq(128.f))).WillByDefault(Return(true));
	...
	ON_CALL(*agent, getDistanceTo(TypedEq<SceneObject*>(attackMock))).WillByDefault(Return(5.f));
	ON_CALL(*agent, getReadyWeapon()).WillByDefault(Return(mockWeao));
...
TEST_F(BehaviorTreeTest, LookTest) {
	...
	ON_CALL(*agent, isAggressiveTo(TypedEq<CreatureObject*>(followMock.get()))).WillByDefault(Return(true));
	ON_CALL(*agent, checkLineOfSight(TypedEq<SceneObject*>(followMock.get()))).WillByDefault(Return(true));
	ON_CALL(*agent, isAttackableBy(TypedEq<CreatureObject*>(followMock.get()))).WillByDefault(Return(true));
	ON_CALL(*followMock, isAttackableBy(TypedEq<CreatureObject*>(agent.get()))).WillByDefault(Return(true));
	...
	EXPECT_TRUE( agent->getFollowState() == AiAgent::WATCHING ) << "FollowState should be 1, got " << agent->getFollowState();
	EXPECT_TRUE( agent->getFollowObject() == followMock ) << "FollowObject should be " << followMock.get() << ", got " << agent->getFollowObject().get();
}
...
TEST_F(BehaviorTreeTest, ScareTest) {
	...
	ON_CALL(*agent, isInRange(TypedEq<SceneObject*>(followMock.get()), _)).WillByDefault(Return(true));
	ON_CALL(*agent, isAggressiveTo(TypedEq<CreatureObject*>(followMock.get()))).WillByDefault(Return(false));
	ON_CALL(*agent, checkLineOfSight(TypedEq<SceneObject*>(followMock.get()))).WillByDefault(Return(true));

	Reference<NiceMock<MockPatrolPoint>*> pPoint = new NiceMock<MockPatrolPoint>();
	ON_CALL(*agent, getHomeLocation()).WillByDefault(Return(pPoint));
	ON_CALL(*pPoint, isInRange(TypedEq<SceneObject*>(agent), FloatEq(128.f))).WillByDefault(Return(false));

	ON_CALL(*followMock, getLevel()).WillByDefault(Return(2));
	ON_CALL(*followMock, isAggressiveTo(TypedEq<CreatureObject*>(agent.get()))).WillByDefault(Return(true));
	ON_CALL(*followMock, getPosture()).WillByDefault(Return(CreaturePosture::UPRIGHT));
	ON_CALL(*followMock, getLocomotion()).WillByDefault(Return(CreatureLocomotion::RUNNING));
	ON_CALL(*followMock, isFacingObject(TypedEq<SceneObject*>(agent.get()))).WillByDefault(Return(true));

	agent->doMovement();

	EXPECT_TRUE( agent->getFollowState() == AiAgent::FLEEING ) << "FollowState should be 5, got " << agent->getFollowState();
	EXPECT_TRUE( agent->getFollowObject() == followMock ) << "FollowObject should be " << followMock.get() << ", got " << agent->getFollowObject().get();
}
