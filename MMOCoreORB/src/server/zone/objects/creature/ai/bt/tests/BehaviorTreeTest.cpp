...
TEST_F(BehaviorTreeTest, GetTargetTest) {
	...
	EXPECT_CALL(*followMock, isInRange(TypedEq<SceneObject*>(agent.get()), FloatEq(128.f)))
...
TEST_F(BehaviorTreeTest, PetFollowTest) {
	...
	Behavior* idleRoot = aiMap->getTemplate("idlePet");
	ASSERT_TRUE( idleRoot != NULL );
	agent->setTree(idleRoot, BehaviorTreeSlot::NONE);

	Behavior* moveRoot = aiMap->getTemplate("movePet");
	ASSERT_TRUE( moveRoot != NULL );
	agent->setTree(moveRoot, BehaviorTreeSlot::MOVE);

	ON_CALL(*agent, getFollowState()).WillByDefault(Return(AiAgent::FOLLOWING));
	ON_CALL(*agent, getPosture()).WillByDefault(Return(CreaturePosture::UPRIGHT));
	ON_CALL(*agent, setDestination()).WillByDefault(Return(1));
	ON_CALL(*agent, getCurrentSpeed()).WillByDefault(Return(1.f));
	ON_CALL(*agent, completeMove()).WillByDefault(Return(true));
...
TEST_F(BehaviorTreeTest, PetPatrolTest) {
	...
	ON_CALL(*agent, getFollowState()).WillByDefault(Return(AiAgent::PATROLLING));
	ON_CALL(*agent, getPosture()).WillByDefault(Return(CreaturePosture::UPRIGHT));
	ON_CALL(*agent, setDestination()).WillByDefault(Return(1));
	ON_CALL(*agent, getCurrentSpeed()).WillByDefault(Return(1.f));
	ON_CALL(*agent, completeMove()).WillByDefault(Return(true));
...
TEST_F(BehaviorTreeTest, PetAttackTest) {
	...
	Behavior* gtRoot = aiMap->getTemplate("targetPet");
	ASSERT_TRUE( gtRoot != NULL );
	agent->setTree(gtRoot, BehaviorTreeSlot::NONE);
	...
	// We've already tested the same threatmap/defender logic so make we won't test it here.
	// Make sure it never gets called
	EXPECT_CALL(*agent, getThreatMap()).Times(0);
	EXPECT_CALL(*agent, getMainDefender()).Times(0);

	// the end result should be that we DON'T exit combat, this test is for a commanded attack
	EXPECT_CALL(*agent, getFollowObject())
		.WillOnce(Return(followMock.get()))
		.WillRepeatedly(Return(attackMock.get()));
	EXPECT_CALL(*attackMock, isInRange(TypedEq<SceneObject*>(agent.get()), FloatEq(128.f)))
		.WillOnce(Return(true));
	EXPECT_CALL(*agent, hasDefender(TypedEq<SceneObject*>(attackMock.get()))).WillOnce(Return(true));
	EXPECT_CALL(*agent, clearCombatState(_)).Times(0);
...
TEST_F(BehaviorTreeTest, PetGuardTest) {
	...
	Reference<NiceMock<MockAiAgent>* > attackMock = new NiceMock<MockAiAgent>();
	followMock->addDefender(attackMock);
...
TEST_F(BehaviorTreeTest, NoBitmaskAggroTest) {
	Locker locker(agent);
	// The basic idea of this test is to ensure that by default, aggroing with a fill tree works
	...
	ON_CALL(*agent, getFollowState()).WillByDefault(Return(AiAgent::WATCHING));
	ON_CALL(*agent, isAggressiveTo(TypedEq<CreatureObject*>(followMock.get())))
		.WillByDefault(Return(true));
	ON_CALL(*agent, checkLineOfSight(TypedEq<SceneObject*>(followMock.get())))
		.WillByDefault(Return(true));
	ON_CALL(*agent, hasDefender(TypedEq<SceneObject*>(followMock.get()))).WillByDefault(Return(false));

	EXPECT_CALL(*agent, setDefender(TypedEq<SceneObject*>(followMock.get())))
		.Times(1);
	EXPECT_CALL(*agent, hasDefender(TypedEq<SceneObject*>(followMock.get())))
		.WillOnce(Return(false))
		.WillOnce(Return(true));
