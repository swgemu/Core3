...
TEST_F(BehaviorTreeTest, WaitTest) {
	...
	ASSERT_TRUE( root->getChild(STRING_HASHCODE("wander")) != NULL );
	ASSERT_TRUE( root->getChild(STRING_HASHCODE("wander"))->getChild(STRING_HASHCODE("wander0")) != NULL );
	Behavior* wtRoot = root->getChild(STRING_HASHCODE("wander"))->getChild(STRING_HASHCODE("wander0"))->getChild(STRING_HASHCODE("wait0"));
	...
	// running chain logic since Wait should return RUNNING. To do this though, we will need a real
	// AiAgent, so let's make one.
	// the running chain methods aren't mocked, so they will be called directly (this is desirable)
	//ConfigManager::instance()->loadConfigData();
	//ConfigManager::instance()->setProgressMonitors(false);
	//Reference<ZoneServer*> zoneServer = new ZoneServer(ConfigManager::instance());
	//Reference<ZoneProcessServer*> processServer = new ZoneProcessServer(zoneServer);
	//Reference<Zone*> zone = new Zone(processServer, "aiagent_test");
	//zone->createContainerComponent();
	//zone->_setObjectID(1);
	//Reference<AiAgent*> realAgent = new AiAgent();
	//realAgent->setContainerComponent("ContainerComponent");
	//realAgent->setZoneComponent("ZoneComponent");
	//realAgent->_setObjectID(23);

	//Locker locker(realAgent);
	//realAgent->initializePosition(0, 0, 0);
	//zone->transferObject(realAgent, -1);
	...
	//realAgent->setRootBehavior(wanderRoot);
	Locker locker(agent);
	wanderRoot->doAction(agent.get());
	wanderRoot->doAction(agent.get());
	...
	//realAgent->destroyObjectFromWorld(false);
	delete pPoint;
}
