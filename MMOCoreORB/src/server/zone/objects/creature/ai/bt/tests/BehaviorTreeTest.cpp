...
TEST_F(BehaviorTreeTest, ParentChildDuplicateTest) {
	...
	while (b_it.hasNext()) {
		unsigned int k = -1;
		Reference<Behavior*> b = NULL;
		b_it.getNextKeyAndValue(k, b);
		ASSERT_TRUE( b != NULL ) << k << " has a NULL behavior!";
...
TEST_F(BehaviorTreeTest, LoadTemplateTest) {
	...
	Behavior* root = aiMap->getTemplate("rootTest");
	ASSERT_TRUE( root != NULL ) << "Failed to load rootTest";
	agent->setRootBehavior(root);

	// first test the direct lookup functionality
	Behavior* wdRoot = aiMap->getTemplate("idlewander");
	...
	// now set a tree in the same slot and make sure it replaces the old one
	wdRoot = aiMap->getTemplate("idlestatic");
	agent->setTree(wdRoot, BehaviorTreeSlot::IDLE);
	agent->doMovement();
...
TEST_F(BehaviorTreeTest, GetTargetTest) {
	...
	Behavior* gtRoot = aiMap->getTemplate("targetTest");
...
TEST_F(BehaviorTreeTest, SelectWeaponTest) {
	...
	Behavior* swRoot = aiMap->getTemplate("weaponTest");
...
TEST_F(BehaviorTreeTest, SelectAttackTest) {
	...
	Behavior* saRoot = aiMap->getTemplate("attackTest");
...
TEST_F(BehaviorTreeTest, MoveTest) {
	...
	Behavior* cmRoot = aiMap->getTemplate("moveTest");
...
TEST_F(BehaviorTreeTest, WaitTest) {
	...
	Behavior* wanderRoot = aiMap->getTemplate("wanderTest");
	ASSERT_TRUE( wanderRoot != NULL );
	agent->setRootBehavior(wanderRoot);

	Behavior* moveRoot = aiMap->getTemplate("moveTest");
