...
enum class DataVal : uint32 {
	WALK	= STRING_HASHCODE("WALK"),
	RUN		= STRING_HASHCODE("RUN"),
	DEFAULT	= STRING_HASHCODE("DEFAULT"),
	WEAPON	= STRING_HASHCODE("WEAPON"),
	RANDOM	= STRING_HASHCODE("RANDOM"),
	BADVAL	= 0
};
...
public:
	...
	static DataVal valueOf(const String& val) {
		try {
			return static_cast<DataVal>(val.hashCode());
		} catch (std::bad_cast&) {
			return DataVal::BADVAL;
		}
	}
