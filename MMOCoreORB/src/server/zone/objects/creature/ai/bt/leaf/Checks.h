...
enum CheckType {
	...
	CHECK_OUTDOORS,
	CHECK_FOLLOWLEVEL,
	CHECK_FOLLOWBACKAGGRESSION,
	CHECK_FOLLOWFACING,
	CHECK_PETCOMMAND,
	CHECK_FOLLOWISCOMMAND
};
...
typedef _Check<uint32, CHECK_FOLLOWSPEED> CheckFollowSpeed;
template <> bool CheckFollowSpeed::check(AiAgent* agent) const;

typedef _Check<uint32, CHECK_FOLLOWLOS> CheckFollowLOS;
...
typedef _Check<bool, CHECK_FOLLOWLEVEL> CheckFollowLevel;
template <> bool CheckFollowLevel::check(AiAgent* agent) const;

typedef _Check<bool, CHECK_FOLLOWBACKAGGRESSION> CheckFollowBackAggression;
template <> bool CheckFollowBackAggression::check(AiAgent* agent) const;

typedef _Check<bool, CHECK_FOLLOWFACING> CheckFollowFacing;
...
typedef _Check<bool, CHECK_FOLLOWISCOMMAND> CheckFollowIsCommand;
template <> bool CheckFollowIsCommand::check(AiAgent* agent) const;
