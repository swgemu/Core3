...
public:
	...
	bool hasChild(Behavior* c) const {
		for (int idx = 0; idx < children.size(); ++idx)
			if (children.get(idx) == c) return true;
	...
	Behavior* getChild(uint32 cID) const {
		for (int idx = 0; idx < children.size(); ++idx)
			if (children.get(idx)->getID() == cID) return children.get(idx);
	...
	const Vector<Reference<Behavior*> > getChildren(Reference<AiAgent*> agent) const;
};
