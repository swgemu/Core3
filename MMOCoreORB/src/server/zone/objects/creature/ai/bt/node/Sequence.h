...
public:
	ParallelSequence(const String& className, const uint32 id, const String& argString = "")
		: Composite(className, id, argString) {}
	
	virtual Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const;
};

class NonDeterministicSequence : public Composite {
public:
	NonDeterministicSequence(const String& className, const uint32 id, const String& argString = "")
		: Composite(className, id, argString) {}
