...
public:
    ...
    Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		...
		SceneObject* currObj = agent->getFollowObject().get();
		if (currObj != NULL)
		...
		for (int i = 0; i < closeObjects.size(); ++i) {
			...
			// result was a failure, which means that we don't want to pay attention to this target
			agent->setTargetObject(NULL);
		}
