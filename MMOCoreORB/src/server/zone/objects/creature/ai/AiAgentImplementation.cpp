...
/* This function determins whether or not an AI should try to continue
...
bool AiAgentImplementation::findNextPosition(float maxDistance, bool walk) {
	...
	if (endDistanceSq > 256 * 256) {
		//error("AiAgent - findNextPosition endPosition > 250m from currentPosition");
		setMovementState(AiAgent::PATHING_HOME);
		return false;
	}
	...
	*/
	...
	Reference<Vector<WorldCoordinates>* > path = currentFoundPath.get();
	...
	if (path == nullptr) {
		...
		if ((movementState == AiAgent::FOLLOWING || movementState == AiAgent::PATHING_HOME) && endMovementPosition.getCell() == nullptr && currentParent == nullptr) {
			WorldCoordinates pathEnd = path->get(currentFoundPath->size() - 1);
			float endDiffSq = path->get(currentFoundPath->size() - 1).getWorldPosition().squaredDistanceTo(endMovementCoords.getWorldPosition());
	...
	if (currentParent != nullptr && endMovementPosition.getCell() != nullptr)
	...
	float nextMovementDistance = nextMovementPosition.getWorldPosition().distanceTo(currentWorldPos);

	if (nextMovementDistance <= 0 && path->size() > 2) {
		path->remove(1);
		return false;
	}

#ifdef DEBUG_PATHING
	printf("findNexPosition - Path Size = %i ---  ", path->size());
	...
	if (!(nextMovementDistance > 0 && newSpeed > 0.1)) {
		...
		path->set(0, nextMovementPosition);
	}
...
void AiAgentImplementation::activateMovementEvent() {
	...
	bool activate = ConfigManager::instance()->getBool("Core3.Tweaks.ActivateAllAIMovement", false);

	if (!activate && numberOfPlayersInRange.get() <= 0 && getFollowObject().get() == nullptr && !isRetreating()) {
		if (moveEvent != nullptr) {
			moveEvent->clearCreatureObject();
			moveEvent = nullptr;
		}
