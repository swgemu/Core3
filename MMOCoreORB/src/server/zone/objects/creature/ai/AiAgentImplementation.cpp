...
void AiAgentImplementation::addPatrolPoint(PatrolPoint& point) {
	...
	if (patrolPoints.size() > 5) {
		info(true) << "patrol points size greater than 5 !!!";

		E3_ASSERT(patrolPoints.size() < 6);
	}
...
int AiAgentImplementation::setDestination() {
	...
	if (patrolPoints.size() > 20) {
		info(true) << getObjectID() << " Patrol points have overflowed - Total points: " << patrolPoints.size() << " Movement State: " << stateCopy << " Saved Patrol point size: " << savedPatrolPoints.size();
