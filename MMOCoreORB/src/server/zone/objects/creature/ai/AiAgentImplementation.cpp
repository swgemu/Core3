...
bool AiAgentImplementation::findNextPosition(float maxDistance, bool walk) {
	...
	if (endDistanceSq <= maxSquared) {
		//info(true) << "endDistSquared = " << endDistanceSq << "  maxSquared = " << maxSquared << "   For:  " << getObjectID();
	...
	*/
	...
	if (currentFoundPath == nullptr) {
		...
		if ((movementState == AiAgent::FOLLOWING || movementState == AiAgent::PATHING_HOME) && endMovementCell == nullptr && currentParent == nullptr &&
			...
			currentFoundPath->set(0, WorldCoordinates(asAiAgent()));
	...
	/*if (nextMovementDistance <= 0) {
		info(true) << " ----- >>>>>>>> nextMovementDistance = " << nextMovementDistance << "   For: " << getObjectID() << " Path size: " << path->size() << "  Patrol points total = " << getPatrolPointSize() << "  Location: " << currentPosition.toString() << "    Next Position = " << nextMovementPosition.getWorldPosition().toString();
	}*/
...
bool AiAgentImplementation::generatePatrol(int num, float dist) {
	...
	// TODO: Fix - We are generating patrol point vectors with the home location repeated * 5
