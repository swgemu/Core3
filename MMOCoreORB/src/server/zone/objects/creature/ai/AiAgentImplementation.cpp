...
bool AiAgentImplementation::findNextPosition(float maxDistance, bool walk) {
	...
	if (endDistanceSq > 185 * 185) {
		error("AiAgent - findNextPosition endPosition > 185m from currentPosition");
	...
	*/
	...
	/*if (!(endDistanceSq > 0 && newSpeed > 0.1)) {
	...
	uint64 endParentID = endMovementCell != nullptr ? endMovementCell->getObjectID() : 0;

	if (currentParentID == endParentID && endDistanceSq <= maxSquared) {
		printf("path set null and removed patrol point \n");
