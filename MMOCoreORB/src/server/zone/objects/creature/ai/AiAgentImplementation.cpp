...
void AiAgentImplementation::activateHAMRegeneration(int latency) {
    ...
    uint32 healthTick = Math::max(1, ceil(getMaxHAM(CreatureAttribute::HEALTH) / 300000.f * latency));
    uint32 actionTick = Math::max(1, ceil(getMaxHAM(CreatureAttribute::ACTION) / 300000.f * latency));
    uint32 mindTick   = Math::max(1, ceil(getMaxHAM(CreatureAttribute::MIND)   / 300000.f * latency));
...
bool AiAgentImplementation::findNextPosition(float maxDistance, bool walk) {
	...
	while (!found && getPatrolPointSize() != 0) {
		...
		if (targetDistance > maxDistance)
			...
			// maxDistance radius
			maxDist = Math::min(maxSpeed, targetDistance - maxDistance + 0.1);
		else
	...
	if (found) {
		...
		if (dist > 0 && newSpeed > 0.1) {
			nextMovementInterval = Math::min((int)((Math::min(dist, maxDist)/newSpeed)*1000 + 0.5), UPDATEMOVEMENTINTERVAL);
