...
void AiAgentImplementation::activatePostureRecovery() {
	if ((isProne() || isKnockedDown() || isKneeling()) && checkPostureChangeDelay()) {
		if(!isInCombat()) {
			...
			if (isDizzied() && System::random(100) < 85) {
				queueDizzyFallEvent();
			} else {
				setPosture(CreaturePosture::UPRIGHT, false, true);
				CombatAction *action = new CombatAction(_this.getReferenceUnsafeStaticCast(), STRING_HASHCODE("change_posture"));
