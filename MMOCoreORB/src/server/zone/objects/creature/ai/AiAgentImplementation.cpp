...
bool AiAgentImplementation::isCamouflaged(CreatureObject* creature) {
	...
	camoDebug << "Roll after calc = " << roll << ". ";

	bool success = (System::random(100) + 1) < roll;
	...
	if (success) {
		...
		// break mask scent add cooldown
		int timer = (70 - (effectiveTarget->getSkillMod("mask_scent") / 2)) * 1000;
	...
	Reference<Task*> camoTask = new CamoTask(effectiveTarget, asAiAgent(), scentMasked, success);
	camoTask->execute();
