...
int DroidObjectImplementation::rechargeFromBattery(CreatureObject* player) {
	...
	ManagedReference<SceneObject*> droidObj = NULL;
	ManagedReference<DroidObject*> droidPet = cast<DroidObject*>(droidObj.get());
...
void DroidObjectImplementation::rechargeFromDroid() {
	ManagedReference<SceneObject*> droidTar = NULL;
	ManagedReference<DroidObject*> targetDroid = cast<DroidObject*>(droidTar.get());
	// Reset power to max and heal that droid's BF too
	power = MAX_POWER;
	int targetdroidsBf = targetDroid->getShockWounds();
	targetDroid->addShockWounds(-targetdroidsBf, true, false);
