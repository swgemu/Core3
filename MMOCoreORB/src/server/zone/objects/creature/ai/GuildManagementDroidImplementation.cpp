...
void GuildManagementDroidImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	if (!ConfigManager::instance()->getBool("Core3.PlayerManager.GuildEnhancements", false))
		return;

	if (player == nullptr)
		return;

	player->sendSystemMessage("GuildManagementDroidImplementation - Menu selected");
...
int GuildManagementDroidImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	info(true) << "handleObjectMenuSelect -- selectedID = " << selectedID;
