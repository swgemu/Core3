...
void PetDeedImplementation::adjustPetLevel(CreatureObject* player, CreatureObject* pet) {
	...
	if (newLevel < 1) {
		...
		info("Pet Level adjustment went below 1.");
		return;
	}
	if (newLevel > 75) {
...
void PetDeedImplementation::adjustPetStats(CreatureObject* player, CreatureObject *pet) {
	...
	// we calculated a level higher than what the pet is set to, downgrade the stats to the expectd level.
