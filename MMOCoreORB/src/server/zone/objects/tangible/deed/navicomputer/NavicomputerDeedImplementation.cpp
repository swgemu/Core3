...
int NavicomputerDeedImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if (selectedID == 20) {
		...
		if (flightComputer == nullptr) {
			error() << "Error in flight computer: " << controlDeviceTemplate;
			return 1;
		}
		...
		if (computerStorage == nullptr) {
			error() << "Storage error in flight computer: " << controlDeviceTemplate;
			return 1;
		}
		...
		if (!flightComputer->transferObject(computerStorage, 4)) {
			...
			error() << "Storage transfer error in flight computer: " << controlDeviceTemplate;
			return 1;
		}
		...
		if (!datapad->transferObject(flightComputer, -1)) {
			flightComputer->destroyObjectFromDatabase(true);
			error() << "Control device transfer error in flight computer: " << controlDeviceTemplate;
			return 1;
		}
