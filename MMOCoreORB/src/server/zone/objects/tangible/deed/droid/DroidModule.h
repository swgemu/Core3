...
public:
	...
	virtual bool toBinaryStream(ObjectOutputStream* stream) {
		std::cout << "DroidModule::toBinaryStream()\n";
		stream->writeInt(moduleType);
		return true;
	}

	virtual bool parseFromBinaryStream(ObjectInputStream* stream) {
		std::cout << "DroidModule::toBinaryStream()\n";
		moduleType = stream->readInt();
		return true;
	}
