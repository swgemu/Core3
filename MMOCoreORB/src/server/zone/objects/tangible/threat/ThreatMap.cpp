...
void ThreatMap::removeAll(bool forceRemoveAll) {
	...
	for (int i = size() - 1; i >= 0; i--) {
		...
		if (key == nullptr || selfStrong == nullptr || keyPlanetCRC != selfPlanetCRC || forceRemoveAll) {
			remove(i);

			if (threatMapObserver != nullptr) {
				key->dropObserver(ObserverEventType::HEALINGRECEIVED, threatMapObserver);
			}
		} else if (key->isCreatureObject() && (key->asCreatureObject()->isDead() || !key->asCreatureObject()->isOnline())) {
			remove(i);
...
void ThreatMap::dropDamage(TangibleObject* target) {
	...
	if (target == nullptr || selfStrong == nullptr || target->getPlanetCRC() != selfStrong->getPlanetCRC()) {
		drop(target);

		if (threatMapObserver != nullptr) {
			target->dropObserver(ObserverEventType::HEALINGRECEIVED, threatMapObserver);
		}
	} else if (target->isCreatureObject() && (target->asCreatureObject()->isDead() || !target->asCreatureObject()->isOnline())) {
		drop(target);
...
TangibleObject* ThreatMap::getHighestThreatAttacker() {
	...
	if (currentThreat != nullptr && currentThreat->isCreatureObject()) {
		CreatureObject* currentCreo = currentThreat->asCreatureObject();

		if (currentCreo != nullptr && !currentCreo->isDead() && !currentCreo->isIncapacitated() && !currentCreo->isDestroyed() &&
	...
	for (int i = 0; i < size(); ++i) {
		...
		if (creature != nullptr && !creature->isDead() && !creature->isIncapacitated() && creature->isInRange(selfStrong, 128.f) && creature->isAttackableBy(selfStrong)) {
	...
	// ManagedReference<TangibleObject*> highestTano = ;
	// ManagedReference<CreatureObject*> highestCreo = highestTano->asCreatureObject();
