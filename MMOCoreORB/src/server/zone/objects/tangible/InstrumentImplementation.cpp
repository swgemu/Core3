...
int InstrumentImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if (selectedID == 69) {
		...
		if (player->getDistanceTo(_this.get()) >= 5) {
			...
			player->executeObjectControllerAction(String::hashCode("startmusic"), getObjectID(), "");
	}
...
void InstrumentImplementation::spawnInAdminCell(CreatureObject* player) {
	...
	player->executeObjectControllerAction(String::hashCode("transferitemmisc"), _this.get()->getObjectID(), uni);
