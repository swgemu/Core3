...
void ScavengerDroidImplementation::updatePlayerScore(CreatureObject* player) {
	int playerScore = getTurnedInItemListSize(player);
	...
	if (currentWinner != NULL)
		leaderScore = getTurnedInItemListSize(currentWinner.get());

	if (currentWinner == NULL || currentWinner == player || playerScore >= leaderScore) {
		player->sendSystemMessage("@event_perk:scavenger_you_lead");
		currentWinner = player;
	} else {
		StringIdChatParameter msg("event_perk", "scavenger_not_leader");
		msg.setTO(currentWinner.get()->getDisplayedName());
...
void ScavengerDroidImplementation::addToPlayerItemList(CreatureObject* player, const String& item) {
	if (!turnedInItemList.contains(player))
		turnedInItemList.put(player, new Vector<String>());

	turnedInItemList.get(player)->add(item);
}
