...
int JukeboxImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	// Non owners / non privileged cannot pick up
	if (selectedID == 10 && !isOwner && (player->getPlayerObject() == NULL || !player->getPlayerObject()->isPrivileged()))
		return 0;
...
String JukeboxImplementation::getDurationString() {
