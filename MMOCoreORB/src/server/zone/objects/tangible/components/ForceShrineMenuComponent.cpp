...
int ForceShrineMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) {
	...
	if (!creature->hasSkill("force_title_jedi_rank_02")) {
		...
		// Find a trainer.
		findTrainerObject(creature, ForceShrineMenuComponent::TRAINER_FIND_RETRIES, ghost.get());
...
void ForceShrineMenuComponent::findTrainerObject(CreatureObject* player, int retriesCounter, PlayerObject* ghost) {
