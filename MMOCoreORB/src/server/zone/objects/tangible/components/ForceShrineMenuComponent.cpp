...
int ForceShrineMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) {
	if (selectedID == 213) {
		...
		if (!creature->hasSkill("force_title_jedi_rank_02") && (creature->getScreenPlayState("padawan:trials:started") == 1)) {
		...
		} else if (!creature->hasSkill("force_title_jedi_rank_02") && (creature->getScreenPlayState("VillageJediProgression") == 32)) {
