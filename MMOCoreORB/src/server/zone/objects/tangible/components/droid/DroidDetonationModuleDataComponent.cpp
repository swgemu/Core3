...
int DroidDetonationModuleDataComponent::handleObjectMenuSelect(CreatureObject* player, byte selectedID, PetControlDevice* controller) {
	...
	if( selectedID == DETONATE_DROID ){
		...
		if(droid->getPendingTask("droid_detonation") != NULL) {
			System::out << "Pending task still running\n";
...
void DroidDetonationModuleDataComponent::addToStack(BaseDroidModuleComponent* other){
	...
	if (droidComponent != NULL) {
		...
		System::out << "Added rating its now " << droidComponent->getAttributeValue("bomb_level") << "\n";
		System::out << "Added count its now " << droidComponent->getAttributeValue("module_count") << "\n";
	}
...
void DroidDetonationModuleDataComponent::copy(BaseDroidModuleComponent* other) {
	...
	System::out << "Other module rating " << rating << "\n";
	moduleCount = 1;
	DroidComponent* droidComponent = cast<DroidComponent*>(getParent());
	if (droidComponent != NULL) {
		...
		System::out << "Copied rating its now " << droidComponent->getAttributeValue("bomb_level") << "\n";
		System::out << "Copied count its now " << droidComponent->getAttributeValue("module_count") << "\n";
	}
}
