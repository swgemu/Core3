...
DroidMaintenanceModuleDataComponent::DroidMaintenanceModuleDataComponent() {
	...
	moduleRating = 0;
	totalCost = 0; // we track total cost for assignments to the list.
	maxStructures = 3;
...
void DroidMaintenanceModuleDataComponent::payStructures() {
	...
	for(int i=0;i< runAssignments.size();i++) {
		uint64 objectID = runAssignments.elementAt(i).getKey();
		Vector<uint64> cost = runAssignments.elementAt(i).getValue();
		ManagedReference<SceneObject*> obj = droid->getZoneServer()->getObject(objectID);
		StructureObject* structureObject = cast<StructureObject*>(obj.get());
		uint64 maintToPay = cost.elementAt(0);
		uint64 fee = 0;
		if (cost.size() > 1) {
			fee = cost.elementAt(1);
		}
		structureObject->payMaintenance(maintToPay,linkedCreature.get(),true);
		// take money from player,
		CreatureObject *p;
		if (linkedCreature->getCashCredits() > (fee)) {
			linkedCreature->subtractCashCredits(fee);
		} else {
			int paid = linkedCreature->getCashCredits();
			int remainder = fee - paid;
			if (linkedCreature->getBankCredits() > remainder) {
				linkedCreature->subtractBankCredits(remainder);
			} else {
...
long DroidMaintenanceModuleDataComponent::calculateRunTime() {
	long duration = 0;
	ManagedReference<DroidObject*> droid = getDroidObject();
	long localPlanet = droid->getPlanetCRC();
	ManagedReference<SceneObject*> primeStructure;
	for(int i=0;i< runAssignments.size();i++) {
		uint64 objectID = runAssignments.elementAt(i).getKey();
		if(isValidStructure(objectID)) {
			ManagedReference<SceneObject*> obj = droid->getZoneServer()->getObject(objectID);
			if (obj->getPlanetCRC() != localPlanet) {
