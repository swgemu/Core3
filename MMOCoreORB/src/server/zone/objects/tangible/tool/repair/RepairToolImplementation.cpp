...
void RepairToolImplementation::sendRepairListTo(CreatureObject* player) {
	...
	Vector<ManagedReference<SceneObject*> > repairableItems =
                        playerManager->getInventoryItemsOfType(player, repairToolData -> getRepairType() );
        ...
        for (int i = 0; i < repairableItems.size(); i++) {
                ...
                if (tano != nullptr  ) {
                        ...
                        if (inNeedOfRepair) {
                                listbox->addMenuItem( tano->getDisplayedName(),
                                                      tano->getObjectID());
                        }
                }
        }
