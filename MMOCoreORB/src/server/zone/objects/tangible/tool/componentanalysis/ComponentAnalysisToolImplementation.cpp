...
void ComponentAnalysisToolImplementation::loadTemplateData(SharedObjectTemplate* templateData) {
...
void ComponentAnalysisToolImplementation::reverseEngineer(CreatureObject* player) {
	...
	for (int i = 0; i < componentCount; ++i) {
		...
		combineComponent(candidate);
	}

	SceneObject* finalComponent = savedBaseComponent.get();

	//destroy all objects that aren't the final component
	for (int i = componentCount; i > 0; --i) {
		SceneObject* candidate = getContainerObject(i - 1);
		if (candidate != finalComponent) {
			Locker locker(candidate);
...
int ComponentAnalysisToolImplementation::canAddObject(SceneObject* object, int containmentType, String& errorDescription) {
	...
	if (component->getReverseEngineeringLevel() != engineeringLevel) {
		errorDescription = "@reverse_engineering_tool:invalid";
		return TransferErrorCode::INVALIDTYPE;
	}
...
int ComponentAnalysisToolImplementation::notifyObjectRemoved(SceneObject* object) {
	//if the first object placed in the tool has been removed, select a new base
...
void ComponentAnalysisToolImplementation::combineComponent(ShipComponent* component) {
	SceneObject* base = savedBaseComponent.get();

	//basic stats for all ship components
	ShipComponent* baseComponent = static_cast<ShipComponent*>(base);
	...
	switch (component->getGameObjectType()) {
		...
		case SceneObjectType::SHIPBOOSTER: {
			ShipBoosterComponent* baseBooster = cast<ShipBoosterComponent*>(base);
		...
		case SceneObjectType::SHIPWEAPONCAPACITOR: {
			ShipCapacitorComponent* baseCapacitor = cast<ShipCapacitorComponent*>(base);
		...
		case SceneObjectType::SHIPDROIDINTERFACE: {
			ShipDroidInterfaceComponent* baseInterface = cast<ShipDroidInterfaceComponent*>(base);
		...
		case SceneObjectType::SHIPENGINE: {
			ShipEngineComponent* baseEngine = cast<ShipEngineComponent*>(base);
		...
		case SceneObjectType::SHIPREACTOR: {
			ShipReactorComponent* baseReactor = cast<ShipReactorComponent*>(base);
		...
		case SceneObjectType::SHIPSHIELDGENERATOR: {
			ShipShieldComponent* baseShield = cast<ShipShieldComponent*>(base);
		...
		case SceneObjectType::SHIPWEAPON: {
			ShipWeaponComponent* baseWeapon = cast<ShipWeaponComponent*>(base);
