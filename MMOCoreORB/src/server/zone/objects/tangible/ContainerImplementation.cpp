...
void ContainerImplementation::loadTemplateData(
	...
	ContainerTemplate* containerTemplate =
			dynamic_cast<ContainerTemplate*> (templateData);
 ...
 */
void ContainerImplementation::fillObjectMenuResponse(
		ObjectMenuResponse* menuResponse, CreatureObject* player) {
	TangibleObjectImplementation::fillObjectMenuResponse(menuResponse, player);

	if (checkContainerPermission(
			player,
			ContainerPermissions::MOVECONTAINER && getParent().get() != NULL
					&& getParent().get()->checkContainerPermission(player,
							ContainerPermissions::MOVEOUT)
					&& !(_this.get()->isRecycleToolObject())))

		menuResponse->addRadialMenuItem(50, 3, "@base_player:set_name"); //Set Name

	if (!isSliced() && isSliceable() && isContainerLocked()
			&& player->hasSkill("combat_smuggler_novice"))
		menuResponse->addRadialMenuItem(69, 3, "@slicing/slicing:slice"); // Slice
}

int ContainerImplementation::handleObjectMenuSelect(CreatureObject* player,
		byte selectedID) {
	if (selectedID == 50) {
		if (checkContainerPermission(
				player,
				ContainerPermissions::MOVECONTAINER && getParent().get()
						!= NULL && getParent().get()->checkContainerPermission(
						player, ContainerPermissions::MOVEOUT))) {
		...
		ManagedReference<Facade*> facade = player->getActiveSession(
				SessionFacadeType::SLICING);
		ManagedReference<SlicingSession*> session =
				dynamic_cast<SlicingSession*> (facade.get());
	...
	return TangibleObjectImplementation::handleObjectMenuSelect(player,
			selectedID);
}

int ContainerImplementation::canAddObject(SceneObject* object,
		int containmentType, String& errorDescription) {
	//	if (locked)
	...
	if (containmentType == -1) {
		if ((gameObjectType == SceneObjectType::WEARABLECONTAINER
				&& object->getGameObjectType()
						== SceneObjectType::WEARABLECONTAINER)) {
		...
		if (getContainerVolumeLimit() < getCountableObjectsRecursive()
				+ objectSize) {
		...
		ManagedReference<SceneObject*> wearableParent = getParentRecursively(
				SceneObjectType::WEARABLECONTAINER);
		ManagedReference<SceneObject*> playerParent = getParentRecursively(
				SceneObjectType::PLAYERCREATURE);

		// If there's a wearable container parent, return if it doesn't have enough room
		if (wearableParent != NULL) {
			if (wearableParent->getContainerVolumeLimit()
					< wearableParent->getCountableObjectsRecursive()
							+ objectSize) {
				errorDescription = "@container_error_message:container03"; // This container is full.
			...
			ManagedReference<WearableContainerObject*> wearable =
					cast<WearableContainerObject*> (wearableParent.get());
			if (!wearable->isEquipped() && playerParent != NULL) {
				SceneObject* inventory = playerParent->getSlottedObject(
						"inventory");
				SceneObject* bank = playerParent->getSlottedObject("bank");
				SceneObject* parentOfWearableParent =
						wearable->getParent().get();

				// Return if it's in a player inventory which doesn't have room
				if (parentOfWearableParent == inventory) {
					if (inventory->getContainerVolumeLimit()
					...
					if (bank->getContainerVolumeLimit()
							< bank->getCountableObjectsRecursive() + objectSize) {
						errorDescription
								= "@container_error_message:container03"; // This container is full.
			...
			if (gameObjectType == SceneObjectType::WEARABLECONTAINER) {
				WearableContainerObject* pack =
						cast<WearableContainerObject*> (_this.get().get());

				if (pack != NULL && !pack->isEquipped()) {
					// This is a wearable container, and it's not equipped.
					if (playerParent != NULL) {
						SceneObject* inventory =
								playerParent->getSlottedObject("inventory");
						SceneObject* bank = playerParent->getSlottedObject(
								"bank");
						...
						if (thisParent == inventory) {
							...
							if (bank->getContainerVolumeLimit()
									< bank->getCountableObjectsRecursive()
											+ objectSize) {
								errorDescription
										= "@container_error_message:container03"; // This container is full.
		...
		if (playerParent == NULL) {
			...
			if (rootParent != NULL) {
				if (rootParent->isBuildingObject()) {
					BuildingObject* building = rootParent.castTo<
							BuildingObject*> ();
					...
					if (!building->isStaticBuilding()
						...
						errorDescription
								= "@container_error_message:container13"; // This house has too many items in it

						return TransferErrorCode::TOOMANYITEMSINHOUSE;
					}
				} else if (rootParent->isFactory()) {
					FactoryObject* factory =
							rootParent.castTo<FactoryObject*> ();
					SceneObject* hopper = factory->getSlottedObject(
							"ingredient_hopper");

					if (hopper->getContainerVolumeLimit()
							< hopper->getCountableObjectsRecursive()
									+ objectSize) {
						errorDescription
								= "@container_error_message:container03"; // This container is full.
						return TransferErrorCode::CONTAINERFULL;
		...
		if (myParent != NULL && otherParent != NULL) {
			if (otherParent->isCreatureObject()) {
				...
				if (ai != NULL) {
					SceneObject* creatureInventory = ai->getSlottedObject(
							"inventory");
	...
	return TangibleObjectImplementation::canAddObject(object, containmentType,
			errorDescription);
}
