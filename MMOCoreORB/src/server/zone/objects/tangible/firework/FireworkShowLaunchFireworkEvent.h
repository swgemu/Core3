	...
	void run() {
		...
		try {
			...
			if (fireworkShowData->getTotalFireworkCount() > 0) {
				...
				Reference<FireworkShowLaunchFireworkEvent*> fireworkShowLaunchFireworkEvent = new FireworkShowLaunchFireworkEvent(player, fireworkShowData, worldFireworkShow);
		...
		this->worldFireworkShow = NULL;
		this->fireworkShowData = NULL;
		this->player = NULL;
	}
};
