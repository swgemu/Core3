...
int VitalityPackImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if ( ownerID != player->getObjectID()){
		...
		if (targetOwner != NULL)
		{
			Locker crossLock(targetOwner, _this.get() );
		...
		if (!hasConsent){
			player->sendSystemMessage("You require consent to revitalize another player's pet");
			return 0;
		}
	}
	...
	int vitalityLoss = (controlDevice->getMaxVitality()-(controlDevice->getVitality() + effectiveness))/4.5;
	if( vitalityLoss <= 0 )
		vitalityLoss = 1;
	controlDevice->setMaxVitality( controlDevice->getMaxVitality() - vitalityLoss );

	// Calculate restored vitality
	int newVitality = MIN( controlDevice->getVitality() + effectiveness, controlDevice->getMaxVitality() );
	controlDevice->setVitality( newVitality );

	// Use a charge
	decreaseUseCount();

	player->sendSystemMessage("You restore your pet to " + String::valueOf( newVitality ) +
