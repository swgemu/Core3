...
void StructureMaintenanceTask::run() {
	...
	// Handle buildings that were condemned prior to bank withdrawal being added
	if (strongRef->isDecayed()) {
		int uncondemnCost = -strongRef->getSurplusMaintenance();

		if (creditObj->getBankCredits() >= uncondemnCost) {
			strongRef->payMaintenance(uncondemnCost, creditObj , false);
