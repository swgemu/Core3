...
public:
	//Factory Types
	static const uint8 EQUIPMENT = 0;
	static const uint8 FOOD = 1;
	static const uint8 STRUCTURE = 2;
	static const uint8 WEARABLES = 3;

public:
	FactoryObjectImplementation(uint64 objid);
	FactoryObjectImplementation(uint64 objid, InstallationDeed* deed);
	...
	inline void setFactoryType(uint8 type) {
		factoryType = type;
		String attr("factoryType");
		itemAttributes->setIntAttribute(attr, (int) factoryType);
	}
