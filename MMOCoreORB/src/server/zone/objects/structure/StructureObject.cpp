...
void StructureObject::payMaintenance(int maintenance, CreatureObject* payer) {
	StructureObjectImplementation* _implementation = static_cast<StructureObjectImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_PAYMAINTENANCE__INT_CREATUREOBJECT_);
		method.addSignedIntParameter(maintenance);
		method.addObjectParameter(payer);

		method.executeWithVoidReturn();
	} else
		_implementation->payMaintenance(maintenance, payer);
}
...
Packet* StructureObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ADDMAINTENANCE__INT_:
		...
		break;
	case RPC_PAYMAINTENANCE__INT_CREATUREOBJECT_:
		payMaintenance(inv->getSignedIntParameter(), static_cast<CreatureObject*>(inv->getObjectParameter()));
		break;
...
void StructureObjectAdapter::payMaintenance(int maintenance, CreatureObject* payer) {
	(static_cast<StructureObject*>(stub))->payMaintenance(maintenance, payer);
}
