...
class SarlaccEruptTask: public Task {
	...
	ManagedReference<CreatureObject*> triggerPlayer;


public:
	SarlaccEruptTask(SarlaccArea* area, CreatureObject* player) {
		sarlaccArea = area;
		triggerPlayer = player;
	}

	void run() {
		if (sarlaccArea == NULL || triggerPlayer == NULL)
			return;

		PlayClientEffectLoc* effectLoc = new PlayClientEffectLoc("clienteffect/cr_sarlacc_erupt.cef", "tatooine", sarlaccArea->getPositionX(), sarlaccArea->getPositionZ(), sarlaccArea->getPositionY(), 0, 3);
		triggerPlayer->broadcastMessage(effectLoc, true);

		Zone* zone = triggerPlayer->getZone();
