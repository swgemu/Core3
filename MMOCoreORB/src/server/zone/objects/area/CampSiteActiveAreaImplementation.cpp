...
void CampSiteActiveAreaImplementation::setAbandoned(bool isAbandoned) {
	...
	if (getZoneServer() == NULL || getZone() == NULL)
		return;
	...
	ManagedReference<StaticObject*> fire = (getZoneServer()->createObject(fireTemplate.hashCode())).castTo< StaticObject*>();
	...
	float posZ = getZone()->getHeight(posX, posY);

	fire->initializePosition(posX, posZ, posY);

	getZone()->transferObject(fire, -1, true);
