...
void PetControlDeviceImplementation::setTrainingCommand( unsigned int commandID ){
	...
	if( petType == PetManager::DROIDPET) {
		...
		// Check for required droid type
		if(((commandID == PetManager::ATTACK || commandID == PetManager::GUARD || commandID == PetManager::SPECIAL_ATTACK1 || commandID == PetManager::SPECIAL_ATTACK2 || commandID == PetManager::RANGED_ATTACK) && !droid->isCombatDroid()) ||
		...
		//check for required player skills
