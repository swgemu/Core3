...
int LuaSceneObject::getDirection(lua_State* L) {
	...
	if (!lua_checkstack(L, 5)) {
		Logger::console.fatal("lua_checkstack failed.");
	}

	/*
	lua_pushfloat(L, direction->getW());
	lua_rawseti(L, -2, 4);

	lua_pushfloat(L, direction->getX());
	lua_rawseti(L, -2, 3);

	lua_pushfloat(L, direction->getY);
	lua_rawseti(L, -2, 2);

	lua_pushfloat(L, direction->getZ());
	lua_rawseti(L, -2, 1);
	*/
