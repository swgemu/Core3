...
void SceneObjectImplementation::initializeTransientMembers() {
	...
	if(dataObjectComponent != NULL) {
		dataObjectComponent->setParent(_this.getReferenceUnsafe());
...
void SceneObjectImplementation::close(SceneObject* client) {
	BaseMessage* msg = new SceneObjectCloseMessage(_this.getReferenceUnsafe());

	client->sendMessage(msg);
}

void SceneObjectImplementation::link(SceneObject* client, uint32 containmentType) {
	BaseMessage* msg = new UpdateContainmentMessage(_this.getReferenceUnsafe(), getParent().get(), containmentType);
	client->sendMessage(msg);
}
...
void SceneObjectImplementation::updateToDatabaseWithoutChildren() {
	...
	server->updateObjectToDatabase(_this.getReferenceUnsafe());*/
}
...
void SceneObjectImplementation::destroyObjectFromDatabase(bool destroyContainedObjects) {
	...
	_this.getReferenceUnsafe()->setPersistent(0);
...
uint64 SceneObjectImplementation::getObjectID() const {
	return _this.getReferenceUnsafe()->_getObjectID();
}

void SceneObjectImplementation::sendWithoutParentTo(SceneObject* player) {
	BaseMessage* msg = new SceneObjectCreateMessage(_this.getReferenceUnsafe());
...
void SceneObjectImplementation::notifyLoadFromDatabase() {
	if (!containerObjects.hasDelayedLoadOperationMode()) {
		for (int i = 0; i < slottedObjects.size(); ++i) {
			...
			if (obj->getParent().get() != _this.getReferenceUnsafe()) {
				obj->setParent(_this.getReferenceUnsafe());
		...
		for (int i = 0; i < containerObjects.size(); ++i) {
			...
			if (obj->getParent() != _this.getReferenceUnsafe()) {
				obj->setParent(_this.getReferenceUnsafe());
	...
	if (zone != NULL) {
		zone->transferObject(_this.getReferenceUnsafe(), -1, true);
	} /*else if (parent != NULL && getParent()->isCellObject()) {
		getRootParent()->notifyObjectInsertedToChild(_this.getReferenceUnsafe(), getParent(), NULL);
...
void SceneObjectImplementation::sendDestroyTo(SceneObject* player) {
	...
	BaseMessage* msg = new SceneObjectDestroyMessage(_this.getReferenceUnsafe());
	player->sendMessage(msg);
}

void SceneObjectImplementation::sendAttributeListTo(CreatureObject* object) {

	AttributeListMessage* alm = new AttributeListMessage(_this.getReferenceUnsafe());

	try {

		attributeListComponent->fillAttributeList(alm, object, _this.getReferenceUnsafe());
...
void SceneObjectImplementation::broadcastObjectPrivate(SceneObject* object, SceneObject* selfObject) {
	...
	if (parent != NULL) {
		ManagedReference<SceneObject*> grandParent = getRootParent().get().castTo<SceneObject*>();
...
void SceneObjectImplementation::broadcastObject(SceneObject* object, bool sendSelf) {
	SceneObject* selfObject = sendSelf ? NULL : _this.getReferenceUnsafe();
...
int SceneObjectImplementation::inRangeObjects(unsigned int gameObjectType, float range) {
	...
	for (int i = 0; i < maxInRangeObjectCount; ++i) {
		...
		if (scno->isInRange(_this.getReferenceUnsafe(), range) && scno->getGameObjectType() == gameObjectType)
			++numberOfObjects;
	}
...
void SceneObjectImplementation::updateZone(bool lightUpdate, bool sendPackets) {
	zoneComponent->updateZone(_this.getReferenceUnsafe(), lightUpdate, sendPackets);
}
...
void SceneObjectImplementation::notifyPositionUpdate(QuadTreeEntry* entry) {
	...
	zoneComponent->notifyPositionUpdate(_this.getReferenceUnsafe(), entry);
}

void SceneObjectImplementation::updateZoneWithParent(SceneObject* newParent, bool lightUpdate, bool sendPackets) {
	zoneComponent->updateZoneWithParent(_this.getReferenceUnsafe(), newParent, lightUpdate, sendPackets);
}

void SceneObjectImplementation::notifyInsertToZone(Zone* newZone) {
	zoneComponent->notifyInsertToZone(_this.getReferenceUnsafe(), newZone);
}

void SceneObjectImplementation::teleport(float newPositionX, float newPositionZ, float newPositionY, uint64 parentID) {
	zoneComponent->teleport(_this.getReferenceUnsafe(), newPositionX, newPositionZ, newPositionY, parentID);
}
...
void SceneObjectImplementation::switchZone(const String& newTerrainName, float newPostionX, float newPositionZ, float newPositionY, uint64 parentID, bool toggleInvisibility) {
	zoneComponent->switchZone(_this.getReferenceUnsafe(), newTerrainName, newPostionX, newPositionZ, newPositionY, parentID, toggleInvisibility);
}
...
void SceneObjectImplementation::updateDirection(float fw, float fx, float fy, float fz) {
	...
	if (parent.get() != NULL) {
		DataTransformWithParent* pack = new DataTransformWithParent(_this.getReferenceUnsafe());
		broadcastMessage(pack, true, true);
	} else {
		DataTransform* pack = new DataTransform(_this.getReferenceUnsafe());
		broadcastMessage(pack, true, true);
	}
}
...
void SceneObjectImplementation::notifyRemoveFromZone() {
	zoneComponent->notifyRemoveFromZone(_this.getReferenceUnsafe());
}

int SceneObjectImplementation::canAddObject(SceneObject* object, int containmentType, String& errorDescription) {
	return containerComponent->canAddObject(_this.getReferenceUnsafe(), object, containmentType, errorDescription);
}
...
bool SceneObjectImplementation::transferObject(SceneObject* object, int containmentType, bool notifyClient, bool allowOverflow) {
	return containerComponent->transferObject(_this.getReferenceUnsafe(), object, containmentType, notifyClient, allowOverflow);
}

bool SceneObjectImplementation::removeObject(SceneObject* object, SceneObject* destination, bool notifyClient) {
	return containerComponent->removeObject(_this.getReferenceUnsafe(), object, destination, notifyClient);
}

void SceneObjectImplementation::openContainerTo(CreatureObject* player) {
	ClientOpenContainerMessage* cont = new ClientOpenContainerMessage(_this.getReferenceUnsafe());
...
void SceneObjectImplementation::closeContainerTo(CreatureObject* player, bool notify) {
	...
	if(notify) {
		ClientOpenContainerMessage* cont = new ClientOpenContainerMessage(_this.getReferenceUnsafe(), true);
		player->sendMessage(cont);
	}
}
...
ManagedWeakReference<SceneObject*> SceneObjectImplementation::getRootParent() {
	...
	while ((tempParent = grandParent->getParent()) != NULL && grandParent != _this.getReferenceUnsafe()) {
	...
	if (grandParent == _this.getReferenceUnsafe())
		return NULL;
...
ManagedWeakReference<SceneObject*> SceneObjectImplementation::getParentRecursively(uint32 gameObjectType) {
	...
	while ((temp = temp->getParent()) != NULL && temp != _this.getReferenceUnsafe()) {
		if (temp->getGameObjectType() == gameObjectType) {
	...
	if (temp == _this.getReferenceUnsafe())
		return NULL;
...
void SceneObjectImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	return objectMenuComponent->fillObjectMenuResponse(_this.getReferenceUnsafe(), menuResponse, player);
}

int SceneObjectImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	return objectMenuComponent->handleObjectMenuSelect(_this.getReferenceUnsafe(), player, selectedID);
}
...
void SceneObjectImplementation::createChildObjects() {
	...
	for (int i = 0; i < templateObject->getChildObjectsSize(); ++i) {
		...
		obj->initializeChildObject(_this.getReferenceUnsafe());
	}
}
...
void SceneObjectImplementation::showFlyText(const String& file, const String& aux, uint8 red, uint8 green, uint8 blue) {
	ShowFlyText* fly = new ShowFlyText(_this.getReferenceUnsafe(), file, aux, red, green, blue);
...
ManagedWeakReference<SceneObject*> SceneObjectImplementation::getParent() {
	...
	assert(parent != _this.getReferenceUnsafe());
...
void SceneObjectImplementation::playEffect(const String& file,
	...
	PlayClientEffectObjectMessage* effect = new PlayClientEffectObjectMessage(
			_this.getReferenceUnsafe(), file, aux);
