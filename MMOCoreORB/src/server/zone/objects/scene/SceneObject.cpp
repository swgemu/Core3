...
bool SceneObject::isVehicleCustomKit() {
	SceneObjectImplementation* _implementation = static_cast<SceneObjectImplementation*>(_getImplementationForRead());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_ISVEHICLECUSTOMKIT__);

		return method.executeWithBooleanReturn();
	} else
		return _implementation->isVehicleCustomKit();
}
...
bool SceneObjectImplementation::isJunkDealer() {
	// server/zone/objects/scene/SceneObject.idl():  		return false;
	return false;
}
...
bool SceneObjectImplementation::isVehicleCustomKit() {
	// server/zone/objects/scene/SceneObject.idl():  		return gameObjectType == SceneObjectType.VEHICLECUSTOMKIT;
	return gameObjectType == SceneObjectType::VEHICLECUSTOMKIT;
}
...
void SceneObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_GETCONTAINEROBJECTRECURSIVE__LONG_:
		{
			resp->insertLong(getContainerObjectRecursive(inv->getUnsignedLongParameter())->_getObjectID());
		}
		break;
	...
	case RPC_ISDECORATION__:
		{
			resp->insertBoolean(isDecoration());
		}
		break;
	case RPC_SETZONECOMPONENT__STRING_:
		{
			String name; 
			setZoneComponent(inv->getAsciiParameter(name));
		}
		break;
	...
	case RPC_ISREPAIRTOOL__:
		...
		break;
	case RPC_ISVEHICLECUSTOMKIT__:
		{
			resp->insertBoolean(isVehicleCustomKit());
		}
		break;
...
bool SceneObjectAdapter::isJunkDealer() {
	return (static_cast<SceneObject*>(stub))->isJunkDealer();
}
...
bool SceneObjectAdapter::isVehicleCustomKit() {
	return (static_cast<SceneObject*>(stub))->isVehicleCustomKit();
}
