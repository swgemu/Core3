...
PendingTasksMap* SceneObject::getPendingTasks() {
	...
	if (_implementation == NULL) {
		...
		DistributedMethod method(this, RPC_GETPENDINGTASKS__);

		return static_cast<PendingTasksMap*>(method.executeWithObjectReturn());
	} else
...
void SceneObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_GETPARENTID__:
		...
		break;
	case RPC_GETPENDINGTASKS__:
		{
			resp->insertLong(getPendingTasks()->_getObjectID());
		}
		break;
...
PendingTasksMap* SceneObjectAdapter::getPendingTasks() {
	return (static_cast<SceneObject*>(stub))->getPendingTasks();
}
