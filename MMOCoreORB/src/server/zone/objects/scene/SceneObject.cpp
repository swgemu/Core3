...
bool SceneObject::isSkillBuff() {
	SceneObjectImplementation* _implementation = static_cast<SceneObjectImplementation*>(_getImplementationForRead());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_ISSKILLBUFF__);

		return method.executeWithBooleanReturn();
	} else
		return _implementation->isSkillBuff();
}
...
bool SceneObjectImplementation::isSkillBuff() {
	// server/zone/objects/scene/SceneObject.idl():  		return gameObjectType == SceneObjectType.SKILLBUFF;
	return gameObjectType == SceneObjectType::SKILLBUFF;
}
...
void SceneObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ISREPAIRTOOL__:
		...
		break;
	case RPC_ISSKILLBUFF__:
		{
			resp->insertBoolean(isSkillBuff());
		}
		break;
...
bool SceneObjectAdapter::isSkillBuff() {
	return (static_cast<SceneObject*>(stub))->isSkillBuff();
}
