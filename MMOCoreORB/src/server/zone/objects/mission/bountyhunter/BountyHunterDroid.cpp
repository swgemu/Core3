...
Reference<FindTargetTask*> BountyHunterDroid::findTarget(SceneObject* droidObject, CreatureObject* player, MissionObject* mission, bool track) {
	...
	ManagedReference<AiAgent*> droid = cast<AiAgent*>(player->getZone()->getCreatureManager()->spawnCreature(String::hashCode("seeker"), 0, player->getPositionX(), player->getPositionZ(), player->getPositionY(), 0));
