...
void ShipShieldComponentImplementation::install(CreatureObject* owner, ShipObject* ship, int slot, bool notifyClient) {
    ...
    ship->setFrontShieldMax(frontHitpoints, notifyClient, message2);
    ship->setRearShieldMax(rearHitpoints, notifyClient, message2);

    
    DeltaMessage *message3 = notifyClient ? new DeltaMessage(ship->getObjectID(), 'SHIP', 6) : NULL;
    ship->setFrontShield(frontHitpoints, true, message3);
    ship->setRearShield(rearHitpoints, true, message3);
