...
void ShipObjectImplementation::receiveConvoMessage(SceneObject* sender, SceneObject* player, const String& messageString, bool customString) {
	...
	SpaceCommTimerTask* task = new SpaceCommTimerTask(creo, oid, 10000);

	task->schedule();

}

bool ShipObjectImplementation::checkConvoInRange(SceneObject* targetObject) {
