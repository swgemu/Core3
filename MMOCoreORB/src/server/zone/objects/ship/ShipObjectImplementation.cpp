...
void ShipObjectImplementation::resetComponentFlag(uint32 slot, bool notifyClient, ShipDeltaVector* deltaVector) {
	...
	const int transientFlags[] = {
			ShipComponentFlag::DISABLED,
			ShipComponentFlag::LOW_POWER,
			ShipComponentFlag::ACTIVE,
			ShipComponentFlag::DISABLED_NEEDS_POWER,
	};

	for (int i = 0; i < 4; ++i) {
		uint32 transientFlag = transientFlags[i];

		if (componentFlag & transientFlag) {
			componentFlag &= ~transientFlag;
		}
	}

	if (componentOptions.get(slot) != componentFlag) {
		if (deltaVector == nullptr && notifyClient) {
			deltaVector = getDeltaVector();
		}
