...
void ShipObjectImplementation::notifyInsert(TreeEntry* object) {
	...
	try {
		...
		if (!isShipAiAgent() && !sceneO->isSpaceStation() && (!isInRange3d(sceneO, 2048))) {
			info(true) << getDisplayedName() << " ShipObjectImplementation --- notifyInsert -- OBJECT IS OUT OF RANGE -- Object " << sceneO->getDisplayedName() << " Type: " << sceneO->getGameObjectType() << " String Name: " << sceneO->getObjectNameStringIdName();
			info(true) << "Distance: " << getPosition().squaredDistanceTo(sceneO->getWorldPosition()) << " My Ship World: " << getWorldPosition().toString() << " Object Position: " << sceneO->getWorldPosition().toString();
