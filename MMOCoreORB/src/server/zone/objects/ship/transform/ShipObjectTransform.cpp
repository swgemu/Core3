...
void ShipObjectTransform::setRotation(ShipObject* ship) {
	...
	if (currentRotation != nextRotation) {
		float rollRadius = ship->getBoundingRadius() / 96.f;
		float rollSpeed = ship->getCurrentSpeed() / ship->getEngineMaxSpeed();
		float rollDamp =  1.f - Math::clamp(0.f, (rollRadius * rollSpeed), 1.f);
