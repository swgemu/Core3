...
private:
	...
	bool validateTransform(const SpaceTransform& transform) const {
		...
		if (isnan(position.getX()) || isnan(position.getY()) || isnan(position.getZ())) {
			return false;
		}

		const auto& rotation = transform.getRotation();

		if (isnan(rotation.getX()) || isnan(rotation.getY()) || isnan(rotation.getZ())) {
			return false;
		}
