...
public:
	Evade(const String& className, const uint32 id, const LuaObject& args) : BehaviorSpace(className, id, args), evadeDelay(2000) {
		parseArgs(args);
	}
...
public:
	...
	BehaviorSpace::Status execute(ShipAiAgent* agent, unsigned int startIdx = 0) const {
		...
		ThreatMap* agentMap = agent->getThreatMap();
		...
		if (targetShip != nullptr && topThreat->getObjectID() == targetShip->getObjectID())
			return SUCCESS;
