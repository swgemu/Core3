...
template<> bool CheckStopEvading::check(ShipAiAgent* agent) const {
	...
	if (!evadePoint.isEvadePoint())
		return true;

	float endEvadeDist = agent->getBoundingRadius() + agent->getEngineMaxSpeed();

	return evadePoint.isReached() || agent->isInRangePosition(evadePoint.getWorldPosition(), endEvadeDist);
}
...
template<> bool CheckEvadeChance::check(ShipAiAgent* agent) const {
	...
	float maxDistance = agent->getMaxDistance() + (ShipAiAgent::DEFAULTAGGRORADIUS / 2.f);
	float maxSquared = maxDistance * maxDistance;

	float minDistance = targetShip->getBoundingRadius() + agent->getBoundingRadius() + agent->getCurrentSpeed();
