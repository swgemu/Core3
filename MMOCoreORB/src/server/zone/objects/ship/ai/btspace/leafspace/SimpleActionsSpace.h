...
public:
	...
	BehaviorSpace::Status execute(ShipAiAgent* agent, unsigned int startIdx = 0) const {
		...
		//	return agent->findNextPosition(agent->getMaxDistance(), mode == DataValSpace::WALK) ? SUCCESS : FAILURE;
		//}

		return SUCCESS; //agent->findNextPosition(agent->getMaxDistance(), mode == DataValSpace::WALK) ? RUNNING : SUCCESS;
	}
};
