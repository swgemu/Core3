...
Vector3 ShipAiAgentImplementation::getNextAttackPosition(ShipObject* targetShip) {
	if (targetShip == nullptr) {
		return getPosition();
	}

	setNextPathPosition(targetShip);
	...
	float sqrDistance = currentPosition.squaredDistanceTo(nextPosition);
	float currentSpeed = getActualSpeed();
	float sqrSpeed = currentSpeed * currentSpeed;

	float intercept = sqrDistance > sqrSpeed ? sqrSpeed / sqrDistance : sqrDistance / sqrSpeed;
	return ((nextPosition - currentPosition) * (5.f * intercept)) + nextPosition;
}

Vector3 ShipAiAgentImplementation::getNextEvadePosition() {
	Vector3 currentPosition = getPosition();
	...
	float distance = getEngineMaxSpeed() * 2.f;
	float maxMove = 200.f;

	Vector3 evadePosition;

	evadePosition.setX(currentPosition.getX() + (maxMove * currentDirection.getX()));
	evadePosition.setY(currentPosition.getY() + (maxMove * currentDirection.getY()));
	evadePosition.setZ(currentPosition.getZ() + (maxMove * currentDirection.getZ()));

	//Vector3 evadePosition = (currentSpeed * 2.0f) * currentPosition;

#ifdef DEBUG_FINDNEXTPOSITION
	info(true) << getDisplayedName() << " -- Next evade position = " << evadePosition.toString();sa
#endif
...
int ShipAiAgentImplementation::setDestination() {
	...
	case ShipAiAgent::ATTACKING: {
		...
		SpacePatrolPoint nextPoint = getNextAttackPosition(targetShip);
	...
	case ShipAiAgent::EVADING: {
		...
		Vector3 evadePosition = getNextEvadePosition();
...
void ShipAiAgentImplementation::setSpeed() {
	// Check if the engines are disabled
	auto componentMap = getShipComponentMap();

	if (componentMap != nullptr && (componentMap->get(Components::ENGINE) & ShipComponentFlag::DISABLED)) {
		clearPatrolPoints();
	...
	ManagedReference<ShipObject*> followCopy = nullptr;

	switch (movementState) {
	case ShipAiAgent::FOLLOWING:
		followCopy = getFollowShipObject().get();
		break;
	case ShipAiAgent::ATTACKING: {
		followCopy = getTargetShipObject().get();
		break;
	}
	default:
		break;
	...
	if (followCopy != nullptr) {
		...
		currentSpeed = followCopy->getActualSpeed();
...
bool ShipAiAgentImplementation::findNextPosition(int maxDistance) {
	...
	int nextDist = currentPosition.squaredDistanceTo(nextPosition);
