...
void ShipAiAgentImplementation::loadTemplateData(SharedShipObjectTemplate* shipTemp) {
	...
	info(true) << getDisplayedName() << " has custom template # " << customShipAiMap;
...
void ShipAiAgentImplementation::initializeTransientMembers() {
	...
	if (aiLogLevel >= 0) {
		...
		logFilename << "log/ai_space/" << now.getFormattedTime("%Y-%m-%d/%H-%M")
			<< "/" << getObjectID() << "/ShipAiAgent-" << now.getTime() << "-" << getObjectID() << ".log";
...
const Quaternion* ShipAiAgentImplementation::unitVectorToQuaternion() {
	...
	float turnYaw = getTurnRadian(yaw, lastYaw) * M_1_PI * BEHAVIORINTERVAL;
	float roll = Math::clamp(-1.f, turnYaw, 1.f) * M_PI_2;
...
bool ShipAiAgentImplementation::findNextPosition(float maxDistance) {
	...
	float distSqr = currentPosition.squaredDistanceTo(endMovementPosition.getWorldPosition());
	...
	Vector3 thisMove = ((currentSpeed * BEHAVIORINTERVAL) * currentDirection) + currentPosition;

	if (thisMove == currentPosition) {
		endMovementPosition.setReached(true);
	...
	broadcastTransform(currentPosition);
...
void ShipAiAgentImplementation::setNextPosition(float x, float z, float y) {
	//info("setNextPosition(" + String::valueOf(x) + ", " + String::valueOf(z) + ", " + String::valueOf(y) + ")", true);

	Locker locker(&targetMutex);

	SpacePatrolPoint point(Vector3(x, z, y));

	patrolPoints.add(0, point);
}

void ShipAiAgentImplementation::setNextStepPosition(float x, float z, float y) {
	Locker locker(&targetMutex);
...
bool ShipAiAgentImplementation::generatePatrol(int totalPoints, float distance) {
	...
	for (int i = 0; i < totalPoints; i++) {
		...
		float x = homePosition.getX();
		float y = homePosition.getY();
		float z = homePosition.getZ();
