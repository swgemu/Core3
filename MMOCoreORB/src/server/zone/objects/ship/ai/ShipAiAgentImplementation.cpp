...
SpacePatrolPoint ShipAiAgentImplementation::getNextAttackPosition(ShipObject* targetShip) {
	...
	SpacePatrolPoint nextPoint = (((nextPosition - currentPosition) * (5.f * intercept)) + nextPosition);
...
void ShipAiAgentImplementation::setSpeed() {
	...
	Vector3 nextPosition = getFinalPosition().getWorldPosition();
	...
	lastSpeed = currentSpeed;
	currentSpeed = Math::max(throMin, (distMax * throttle));
