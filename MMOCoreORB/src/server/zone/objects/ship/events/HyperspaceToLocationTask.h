...
public:
	...
	void run() {
		...
		case 10: {
			...
			// Phase 1: Clear stale close objects and send CmdStartScene to all passengers.
			...
			for (int i = 0; i < totalPlayers; i++) {
				...
				try {
					...
					shipMember->setMovementCounter(0);

					client->balancePacketCheckupTime();
					client->sendMessage(new unkByteFlag());
					client->sendMessage(new CmdStartScene(shipMember));
					client->sendMessage(new ParametersMessage());

					auto zoneServer = shipObject->getZoneServer();

					if (zoneServer != nullptr) {
						auto guildManager = zoneServer->getGuildManager();

						if (guildManager != nullptr) {
							guildManager->sendBaselinesTo(shipMember);
						}
					}
				} catch (...) {
					shipMember->error() << "Failed to send CmdStartScene during hyperspace - ShipID: " << shipObject->getObjectID() << " Ship Member ID: " << shipMember->getObjectID();
				}
			}
