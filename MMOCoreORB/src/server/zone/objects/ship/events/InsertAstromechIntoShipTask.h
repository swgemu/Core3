...
public:
	...
	void run() {
		...
		auto controlDevice = droidObject->getControlDevice().get();

		if (controlDevice == nullptr || !controlDevice->isPetControlDevice()) {
			return;
		}

		auto droidControl = dynamic_cast<PetControlDevice*>(controlDevice.get());
		...
		if (droidObject->getLocalZone() != nullptr) {
			Locker cLock(controlDevice, player);

			auto storeTask = new PetControlDeviceStoreTask(controlDevice.castTo<PetControlDevice*>(), player, true);
		...
		Locker cLock(droidObject, player);
