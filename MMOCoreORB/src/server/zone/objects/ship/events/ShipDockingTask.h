...
private:
	...
	void setSpeed(ShipObject* ship) {
		...
		if (positionTime > 0.f && positionTime != rotationTime) {
			throttle = Math::min(positionTime, rotationTime) / Math::max(positionTime, rotationTime);
		}
	...
	Vector3 getBoundingPosition(ShipObject* ship, const Vector3& axis = Vector3::UNIT_Z);
