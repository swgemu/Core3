...
void ZoneServer::finishInitialization() {
	ZoneServerImplementation* _implementation = static_cast<ZoneServerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_FINISHINITIALIZATION__);

		method.executeWithVoidReturn();
	} else
		_implementation->finishInitialization();
}
...
void ZoneServerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_INITIALIZE__:
		...
		break;
	case RPC_FINISHINITIALIZATION__:
		{
...
void ZoneServerAdapter::finishInitialization() {
	(static_cast<ZoneServer*>(stub))->finishInitialization();
}
