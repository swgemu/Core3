...
public:
	...
	void updateTransform(ShipObject* ship, CreatureObject* pilot, const Vector3& position, bool reorthonormalize) {
		...
		const Vector3 updatedVelocity = velocity.get();
		if (ship->getCurrentVelocity() != updatedVelocity) {
			ship->setCurrentVelocity(updatedVelocity);

			// velocity has updated, which is necessary for speed to update, but not sufficient
			// so check speed before updating speed
			...
			if (ship->getCurrentSpeed() != updatedSpeed) {
				...
				const float ypFactor = ship->getComponentEfficiencyMap()->get(Components::ENGINE) * ship->getRotationFactor(updatedSpeed);
				...
				if (ship->getActualYawRate() != actualYawRate) {
					ship->setActualYawRate(ship->getEngineYawRate() * ypFactor, false, nullptr, deltaVector);
				}

				if (ship->getActualPitchRate() != actualPitchRate) {
					ship->setActualPitchRate(ship->getEnginePitchRate() * ypFactor, false, nullptr, deltaVector);
				}
