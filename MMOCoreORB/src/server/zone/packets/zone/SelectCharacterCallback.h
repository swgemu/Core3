...
public:
	...
	static void connectPlayer(SceneObject* obj, uint64_t characterID, CreatureObject* player, ZoneClientSession* client) {
		...
		if (obj != NULL && obj->isPlayerCreature()) {
			...
			if (oldClient != NULL && client != oldClient) {
				...
				creature->executeOrderedTask([=] () {
					...
					connectPlayer(obj, characterID, player, client);
				}, lambdaName);
