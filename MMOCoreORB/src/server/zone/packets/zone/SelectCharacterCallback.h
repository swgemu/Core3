...
public:
	...
	static void connectApprovedPlayer(SceneObject* obj, uint64_t characterID, CreatureObject* player, ZoneClientSession* client, ZoneServer* zoneServer) {
		...
		Player is in the zone:
			- Both playerParent and currentParent are nullptr's
		...
		*/
		...
		debugMsg << "---------- SelectCharacterCallback ---------- Player: " << player->getDisplayedName() << endl;
		debugMsg << "Player Arrangement: " << playerArrangement << " savedParentID: " << savedParentID << " Last Logout World Position: " << lastWorldPosition.toString() << endl;

		if (playerParent != nullptr)
			debugMsg << "playerParent: " << playerParent->getObjectName()->getFullPath() << " ID: " << playerParent->getObjectID() << endl;
		else
			debugMsg << "playerParentplayerParent: nullptr" << endl;
