...
public:
	...
	void updatePosition(CreatureObject* creO, SceneObject* parent) {
		...
		if (!playerManager->checkSpeedHackTests(creO, ghost, lastValidatedWorldPosition, transform.getPosition(), transform.getTimeStamp(), transform.getPositionZ(), parent)) {
			return updateError(creO, "!checkSpeedHackTests_POSITION");
		}
	...
	void updateStatic(CreatureObject* creO, SceneObject* parent) {
		...
		if (!playerManager->checkSpeedHackTests(creO, ghost, lastValidatedWorldPosition, transform.getPosition(), transform.getTimeStamp(), lastValidatedWorldPosition.getZ(), parent)) {
			return updateError(creO, "!checkSpeedHackTests_STATIC");
		}
