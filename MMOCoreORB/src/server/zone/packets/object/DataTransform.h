...
public:
	...
	void updatePosition(CreatureObject* creO, SceneObject* parent) {
		...
		if (!privilegedPlayer && (parent == nullptr || (parent != nullptr && (parent->isVehicleObject() || parent->isMount()))) && !CollisionManager::checkMovementCollision(creO, closeObjects, zone, Vector3(lastValidatedWorldPosition.getX(), lastValidatedWorldPosition.getY(), validWorldZ), Vector3(position.getX(), position.getY(), positionZ))) {
			return updateError(creO, "!checkCollision", true, (transformSpeed < 3.f ? 3.f : transformSpeed));
		}
