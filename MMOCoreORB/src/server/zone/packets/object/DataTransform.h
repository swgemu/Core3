...
public:
	...
	void updatePosition(CreatureObject* creO, SceneObject* parent) {
		...
		if (!ghost->isForcedTransform()) {
			...
			if (deltaTime < Transform::MIDDELTA && transform.getSpeed() == creO->getCurrentSpeed() && !transform.isYawUpdate(creO->getDirection())) {
	...
	void updateStatic(CreatureObject* creO, SceneObject* parent) {
		bool synchronize = creO->getCurrentSpeed() == 0.f && transform.getSpeed() == 0.f && !transform.isYawUpdate(creO->getDirection());
