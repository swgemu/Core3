...
public:
	JtlShipListResponse(CreatureObject* creo, SceneObject* terminal) : ObjectControllerMessage(creo->getObjectID(), 0x1B, 0x41D) {
		...
		VectorMap<String, uint64> shipMap;
		...
		for (int i = 0; i < datapad->getContainerObjectsSize(); ++i) {
			...
			shipMap.put(control->getStoredCityName(), object->getObjectID());
		}
		...
		for (int i = 0; i < shipMap.size(); ++i) {
			auto cityName = shipMap.elementAt(i).getKey();
			auto shipID = shipMap.elementAt(i).getValue();
