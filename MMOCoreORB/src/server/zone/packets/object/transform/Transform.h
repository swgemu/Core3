...
#undef isinf
	...
	float getMoveScale(const Vector3& creoPosition, float range) const {
		...
		float sqrDifference = sqrDistance > sqrRange ? sqrRange / sqrDistance : sqrDistance / sqrRange;

		return sqrDifference * sqrDifference;
	}

	Vector3 predictPosition(const Vector3& creoPosition, int deltaTime) const {
