...
void LootValues::setLevel(const LootItemTemplate* lootTemplate, int lootLevel) {
	...
	int levelMin = lootTemplate->getLevelMax();
...
void LootValues::setModifier(const LootItemTemplate* lootTemplate, float lootModifier) {
	...
	int modMax = BonusType::STATIC;
	int modMin = BonusType::STATIC;
	...
	modifier = modMax == modMin ? modMin : getDistributedValue(modMin, modMax, level) + EXPERIMENTAL;
}
