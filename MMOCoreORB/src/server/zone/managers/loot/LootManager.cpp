...
enum {RPC_INITIALIZE__ = 6,RPC_CALCULATELOOTCREDITS__INT_,RPC_CREATELOOT__SCENEOBJECT_AIAGENT_,RPC_CREATELOOT__SCENEOBJECT_STRING_INT_BOOL_};
...
TangibleObject* LootManager::createLootObject(LootItemTemplate* templateObject, int level, bool maxCondition) {
	LootManagerImplementation* _implementation = static_cast<LootManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
...
LootManagerImplementation::LootManagerImplementation(CraftingManager* craftman, ObjectManager* objMan, ZoneServer* server) {
	...
	lootGroupMap = NULL;
	// server/zone/managers/loot/LootManager.idl():  		lootableAttachmentMods.setNoDuplicateInsertPlan();
	(&lootableAttachmentMods)->setNoDuplicateInsertPlan();
...
void LootManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_CREATELOOT__SCENEOBJECT_STRING_INT_BOOL_:
		{
			String lootGroup; 
			resp->insertBoolean(createLoot(static_cast<SceneObject*>(inv->getObjectParameter()), inv->getAsciiParameter(lootGroup), inv->getSignedIntParameter(), inv->getBooleanParameter()));
		}
		break;
	default:
