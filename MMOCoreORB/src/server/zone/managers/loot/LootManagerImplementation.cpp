	...
	if (object->isWeaponObject()) {
		...
		if (shouldGenerateDots) {
			...
			case 1: // Poison, only does the main 3 pools.
				...
				strength = round(MAX(dotGatesPoison.elementAt(0).getKey(), MIN(dotGatesPoison.elementAt(0).getValue(), round(System::random(rand - level) + level)))); // Poisons apparently have higher strength.
				duration = round(MAX(dotGatesPoison.elementAt(2).getKey(), MIN(dotGatesPoison.elementAt(2).getValue(), round(System::random(rand - level) + level)))); // Lower duration.
				potency = round(MAX(dotGatesPoison.elementAt(1).getKey(), MIN(dotGatesPoison.elementAt(1).getValue(), round(System::random(rand - level) + level))));
				break;
			case 2: // Disease, drops on all 9 pools.
				attribute = System::random(8);
				strength = round(MAX(dotGatesDisease.elementAt(0).getKey(), MIN(dotGatesDisease.elementAt(0).getValue(), round(System::random(rand - level) + level)))); // Diseases apparently have lower strength.
				duration = round(MAX(dotGatesDisease.elementAt(2).getKey(), MIN(dotGatesDisease.elementAt(2).getValue(), round(System::random(rand - level) + level)))); // Higher duration.
				potency = round(MAX(dotGatesDisease.elementAt(1).getKey(), MIN(dotGatesDisease.elementAt(1).getValue(), round(System::random(rand - level) + level)))); // Potency is anywhere from 1 to 100%.
				break;
			case 3: // Fire, same as Poison.
				attribute = numbers[chooseAttribute];
				strength = round(MAX(dotGatesFire.elementAt(0).getKey(), MIN(dotGatesFire.elementAt(0).getValue(), round(System::random(rand - level) + level)))); // Fires apparently have higher strength.
				duration = round(MAX(dotGatesFire.elementAt(2).getKey(), MIN(dotGatesFire.elementAt(2).getValue(), round(System::random(rand - level) + level)))); // Lower duration.
				potency = round(MAX(dotGatesFire.elementAt(1).getKey(), MIN(dotGatesFire.elementAt(1).getValue(), round(System::random(rand - level) + level))));
				break;
			default:
				break;
			...
			int uses = System::random(7000); // Uses are global, and not dependent on type like other stats.
