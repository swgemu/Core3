...
void LootManagerImplementation::initialize() {
	...
	info("Loaded " + String::valueOf(lootableAttachmentMods.size()) + " lootable attachment stat mods.", true);
...
bool LootManagerImplementation::loadConfigData() {
	...
	LuaObject modsTable = lua->getGlobalObject("lootableAttachmentStatMods");
	loadLootableMods( &modsTable, &lootableAttachmentMods );
...
String LootManagerImplementation::getRandomLootableMod( unsigned int sceneObjectType ) {
	...
	if( sceneObjectType == SceneObjectType::CLOTHINGATTACHMENT || sceneObjectType == SceneObjectType::ARMORATTACHMENT ){
		return lootableAttachmentMods.get(System::random(lootableAttachmentMods.size() - 1));
	}
	...
	else if( sceneObjectType == SceneObjectType::HEAVYWEAPON || sceneObjectType == SceneObjectType::SPECIALHEAVYWEAPON ){
