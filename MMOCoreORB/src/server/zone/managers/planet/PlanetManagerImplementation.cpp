...
void PlanetManagerImplementation::initialize() {
	...
	if (zone->getZoneName() == "dathomir") {
		Reference<ActiveArea*> area = zone->getZoneServer()->createObject(String::hashCode("object/fs_village_area.iff"), 0).castTo<ActiveArea*>();
		...
		Reference<ActiveArea*> sarlaccArea = zone->getZoneServer()->createObject(String::hashCode("object/sarlacc_area.iff"), 0).castTo<ActiveArea*>();
		...
		Reference<ActiveArea*> sarlaccPreArea = zone->getZoneServer()->createObject(String::hashCode("object/sarlacc_area.iff"), 0).castTo<ActiveArea*>();
	...
	if (zone->getZoneName() == "tatooine") {
		Reference<ActiveArea*> area = zone->getZoneServer()->createObject(String::hashCode("object/sarlacc_area.iff"), 0).castTo<ActiveArea*>();
		...
		Reference<ActiveArea*> preArea = zone->getZoneServer()->createObject(String::hashCode("object/sarlacc_area.iff"), 0).castTo<ActiveArea*>();
...
void PlanetManagerImplementation::loadLuaConfig() {
	...
	if (badges.isValidTable()) {
		uint32 hashCode = String::hashCode("object/badge_area.iff");
	...
	for (int i = 0; i < dtiff.getTotalRows(); ++i) {
		...
		if (region != NULL) {
			...
			if (strongholdFaction == GCWManager::IMPERIALHASH || regionName.contains("imperial")) {
				scenery = zone->getZoneServer()->createObject(String::hashCode("object/static/particle/particle_distant_ships_imperial.iff"), 0);
			} else if (strongholdFaction == GCWManager::REBELHASH || regionName.contains("rebel")) {
				scenery = zone->getZoneServer()->createObject(String::hashCode("object/static/particle/particle_distant_ships_rebel.iff"), 0);
			} else {
				scenery = zone->getZoneServer()->createObject(String::hashCode("object/static/particle/particle_distant_ships.iff"), 0);
			}
		...
		ManagedReference<ActiveArea*> noBuild = zone->getZoneServer()->createObject(String::hashCode("object/active_area.iff"), 0).castTo<ActiveArea*>();
...
Reference<SceneObject*> PlanetManagerImplementation::createTicket(const String& departurePoint, const String& arrivalPlanet, const String& arrivalPoint) {
	ManagedReference<SceneObject*> obj = server->getZoneServer()->createObject(String::hashCode("object/tangible/travel/travel_ticket/base/base_travel_ticket.iff"), 1);
