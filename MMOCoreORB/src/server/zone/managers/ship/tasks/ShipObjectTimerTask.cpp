...
ShipObjectTimerTask::ShipObjectTimerTask(ShipManager* shipManager) : Task() {
	...
	shipVector.setAllowOverwriteInsertPlan();
	agentVector.setAllowOverwriteInsertPlan();
	queueVector.setAllowOverwriteInsertPlan();
...
void ShipObjectTimerTask::updateAgents() {
	...
	for (int i = agentVector.size(); -1 < --i;) {
		...
		if (agent == nullptr || agent->getLocalZone() == nullptr) {
