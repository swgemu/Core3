	...
	for (uint32 slot = 0; slot < Components::FIGHTERSLOTMAX; ++slot) {
		...
		switch (slot) {
			...
			case Components::ENGINE: {
				...
				ship->setEnginePitchAccelerationRate(pitch * 0.01f, false);
				ship->setEnginePitchRate(pitchRate * 0.01f, false);
				ship->setEngineYawAccelerationRate(yaw * 0.01f, false);
				ship->setEngineYawRate(yawRate * 0.01f, false);
				ship->setEngineRollAccelerationRate(roll * 0.01f, false);
				ship->setEngineRollRate(rollRate * 0.01f, false);
