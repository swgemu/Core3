	...
	for (int i = 0; i < 20; i++) {
		...
		if (componentValues.contains(k)) {
			...
			case Components::ENGINE: {
				...
				engine->setPitchAccelerationRate(pitch);
				engine->setYawAccelerationRate(yaw);
				engine->setRollAccelerationRate(roll);
				engine->setPitchRateMaximum(pitchRate);
				engine->setYawRateMaximum(yawRate);
				engine->setRollRateMaximum(rollRate);
