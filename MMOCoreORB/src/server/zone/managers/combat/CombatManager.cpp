...
int CombatManager::doCombatAction(CreatureObject* attacker, WeaponObject* weapon, TangibleObject* defenderObject, const CreatureAttackData& data) const {
	...
	/*
		All Defenders will be added to targetDefenders with a DefenderHitList
	...
	*/
	...
	return 0; // function returns > 0 when successful
...
/*=================================================================================================*/
	...
	// Damage and TEF checks will happen after we have our targets and do calculations
	...
	///doTargetCombatAction will no longer called here.
	...
	/*if (damage > 0) {
		...
		if (attacker->isPlayerCreature() && data.getCommandCRC() != STRING_HASHCODE("attack")) <---- weapon decay
		...
		if (!data.isForceAttack())
			weapon->decreasePowerupUses(attacker); 																 <-- powerups
	}
...
void CombatManager::doTargetCombatAction(CreatureObject* attacker, WeaponObject* weapon, const CreatureAttackData& data, bool* shouldGcwCrackdownTef, bool* shouldGcwTef, bool* shouldBhTef) const {
	...
	for (int i = 0; i < targetDefenders.size(); i++) {
		...
		if (defender->isCreatureObject()) {
			...
			damage = doTargetCombatAction(attacker, weapon, defenderCreo, data, shouldGcwCrackdownTef, shouldGcwTef, shouldBhTef);
		} else {
...
/*
	int doTargetCombatAction #2 - CreO attacker && CreO defender
*/
...
// This needs to be sent all together after all calculations are complete
