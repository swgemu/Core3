...
ArmorObject* CombatManager::getArmorObject(CreatureObject* defender, uint8 hitLocation) {
	...
	case HIT_BODY:
	{
		Vector<ManagedReference<ArmorObject*> > chestArmor = defender->getWearablesDeltaVector()->getArmorAtHitLocation(CombatManager::CHEST);
		if(chestArmor.isEmpty())
			return NULL;
		return chestArmor.get(System::random(chestArmor.size()-1));
	}
	case HIT_HEAD:
	{
		Vector<ManagedReference<ArmorObject*> > headArmor = defender->getWearablesDeltaVector()->getArmorAtHitLocation(CombatManager::HEAD);
		if(headArmor.isEmpty())
			return NULL;
		return headArmor.get(System::random(headArmor.size()-1));
	}
	case HIT_LLEG:
	case HIT_RLEG:
	{
		Vector<ManagedReference<ArmorObject*> > legArmor = defender->getWearablesDeltaVector()->getArmorAtHitLocation(CombatManager::LEGS);
		if(legArmor.isEmpty())
			return NULL;
		return legArmor.get(System::random(legArmor.size()-1));
	}
	case HIT_LARM:
	case HIT_RARM:
	{

		Vector<ManagedReference<ArmorObject*> > armArmor = defender->getWearablesDeltaVector()->getArmorAtHitLocation(CombatManager::ARMS);
		Vector<ArmorObject*> armorAtLocation;

		if(armArmor.isEmpty())
			return NULL;

		if(hitLocation == HIT_LARM) {
		...
		if(armorAtLocation.isEmpty())
			return armArmor.get(System::random(armArmor.size()-1));
		else
			return armorAtLocation.get(System::random(armorAtLocation.size()-1));
	}
	default:
		warning("Invalid hitLocation sent to CombatManager::getArmorObject");
	};
...
float CombatManager::doDroidDetonation(CreatureObject* droid, CreatureObject* defender, float damage) {
	...
	if (defender->isCreatureObject()) {
		...
		if(defender->isVehicleObject()) {
			...
			// player
			int bodyHitLocation = HIT_BODY + System::random(2);

			ArmorObject* healthArmor = getArmorObject(defender, bodyHitLocation);
...
int CombatManager::applyDamage(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, int damage, float damageMultiplier, int poolsToDamage, uint8& hitLocation, const CreatureAttackData& data) {
	...
	if (poolsToDamage & HEALTH) {
		...
		hitLocation = bodyLocations[System::random(2)];
...
void CombatManager::broadcastCombatAction(CreatureObject * attacker, TangibleObject * defenderObject, WeaponObject* weapon, const CreatureAttackData & data, int damage, uint8 hit, uint8 hitLocation) {
	...
	if (animationCRC == 0)
		animationCRC = getDefaultAttackAnimation(attacker, weapon, damage, hitLocation);
