...
int CombatManager::applyDamage(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, int damage, float damageMultiplier, int poolsToDamage, const CreatureAttackData& data) {
	...
	int spillOverDamage = (damage/10.f + 0.5f);
	damage -= spillOverDamage * (3-numberOfPoolsDamaged);
	...
	if ((poolsToDamage ^ 0x7) & HEALTH)
		defender->inflictDamage(attacker, CreatureAttribute::HEALTH, (maxDamage/10.f + 0.5f), true, xpType, true, true);
	if ((poolsToDamage ^ 0x7) & ACTION)
		defender->inflictDamage(attacker, CreatureAttribute::ACTION, (maxDamage/10.f + 0.5f), true, xpType, true, true);
	if ((poolsToDamage ^ 0x7) & MIND)
		defender->inflictDamage(attacker, CreatureAttribute::MIND, (maxDamage/10.f + 0.5f), true, xpType, true, true);
