...
/*
	void doTargetCombatAction # 1 - TanO attacker
...
/*
	void doTargetCombatAction # 2 - CreO attacker
...
void CombatManager::doTargetCombatAction(CreatureObject* attacker, WeaponObject* weapon, const CreatureAttackData& data, bool* shouldGcwCrackdownTef, bool* shouldGcwTef, bool* shouldBhTef) const {
	...
	for (int i = 0; i < targetDefenders.size(); i++) {
		...
		if (defender->isCreatureObject()) {
			...
			//doTargetCombatAction(attacker, weapon, defenderCreo, data, shouldGcwCrackdownTef, shouldGcwTef, shouldBhTef);
		} else {
...
/*
	int doTargetCombatAction #3 - CreO attacker && CreO defender
