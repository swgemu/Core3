...
/* doCombatAction - CreO attacker & TanO defender
	Function will do checks and then add defender(s) to targetDefendersList
...
int CombatManager::doCombatAction(CreatureObject* attacker, WeaponObject* weapon, TangibleObject* defenderObject, const CreatureAttackData& data) const {
	...
	/*
		All Defenders will be added to targetDefenders and as a DefenderHitList
		The targetted or primaryDefender is added first
	*/
	...
	DefenderHitList* targetList = new DefenderHitList(defenderObject);

	if (targetList != nullptr) {
		attacker->addTargetDefenderList(targetList);	// AddPrimary Defender before checking for area targets if needed
	}
	...
	if (data.getCommand()->isAreaAction() || data.getCommand()->isConeAction()) {
		...
		while (areaDefenders->size() > 0) {
			...
			for (int i = 0; i < areaDefenders->size(); i++) {
				...
				DefenderHitList* targetList = new DefenderHitList(tano);

				if (targetList != nullptr) {
					attacker->addTargetDefenderList(targetList);
					areaDefenders->remove(i);
				}
			}
		}
...
// =================================================================================================
	...
	// Damage and TEF checks will happen after we have our targets
