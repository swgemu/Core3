...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int hitLocation, const CreatureAttackData& data) {
	...
	FrsManager* frsMan = defender->getZoneServer()->getFrsManager();
	...
	if (!data.isForceAttack()) {
		...
		if (forceArmor > 0) {
			if (defender->isPlayerCreature()) {
				if (defender->hasBuff(BuffCRC::JEDI_FORCE_ARMOR_1))
					forceArmor = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_ARMOR_1, forceArmor);
				else if (defender->hasBuff(BuffCRC::JEDI_FORCE_ARMOR_2))
					forceArmor = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_ARMOR_2, forceArmor);
			}
		...
		if (forceShield > 0) {
			if (defender->isPlayerCreature()) {
				if (defender->hasBuff(BuffCRC::JEDI_FORCE_SHIELD_1))
					forceShield = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_SHIELD_1, forceShield);
				else if (defender->hasBuff(BuffCRC::JEDI_FORCE_SHIELD_2))
					forceShield = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_SHIELD_2, forceShield);
			}
		...
		if (forceFeedback > 0 && (defender->hasBuff(BuffCRC::JEDI_FORCE_FEEDBACK_1) || defender->hasBuff(BuffCRC::JEDI_FORCE_FEEDBACK_2))) {
			if (defender->isPlayerCreature()) {
				if (defender->hasBuff(BuffCRC::JEDI_FORCE_FEEDBACK_1))
					forceFeedback = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_FEEDBACK_1, forceFeedback);
				else if (defender->hasBuff(BuffCRC::JEDI_FORCE_FEEDBACK_2))
					forceFeedback = frsMan->getFrsModifiedBuffValue(defender, BuffCRC::JEDI_FORCE_FEEDBACK_2, forceFeedback);
			}
