				...
				if (terminalSpawn.isValidTable()) {
					...
					for (int j = 1; j <= terminalSpawnLocs.getTableSize(); ++j) {
						...
						if (terminalLoc.isValidTable()) {
							...
							float spawnRot = terminalLoc.getFloatAt(4);
							int cellID = terminalLoc.getIntAt(5);

							Vector3 spLoc(spawnX, spawnY, spawnZ);
							Reference<TerminalSpawn*> termSpawn = new TerminalSpawn(baseTemplate, spLoc, cellID, spawnRot);
...
void GCWManagerImplementation::refreshExpiredVulnerability(BuildingObject* building) {
	...
	if (!testTime.isPast()) {
		...
		String baseName = building->getObjectTemplate()->getTemplateFileName();
		info("Loaded while vulnerable in refresh - " + baseName, true);
...
void GCWManagerImplementation::spawnBaseTerminals(BuildingObject* bldg) {
	...
	for (int i = 0; i < terminalTemplates.size(); i++) {
		...
		float degrees = termLoc->getRotation();
