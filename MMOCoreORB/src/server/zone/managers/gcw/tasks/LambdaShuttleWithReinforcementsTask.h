...
class LambdaShuttleWithReinforcementsTask : public Task {
	...
	void spawnSingleTroop(SceneObject* lambdaShuttle, CreatureObject* player, const String& creatureTemplate, float xOffset, float zOffset) {
		float rotationRadians = spawnDirection.getRadians();
		float xOffsetRotated = xOffset * Math::cos(rotationRadians) + zOffset * Math::sin(rotationRadians);
		float zOffsetRotated = -xOffset * Math::sin(rotationRadians) + zOffset * Math::cos(rotationRadians);

		Zone* zone = lambdaShuttle->getZone();
		float x = lambdaShuttle->getPositionX() + xOffsetRotated;
		float y = lambdaShuttle->getPositionY() + zOffsetRotated;
