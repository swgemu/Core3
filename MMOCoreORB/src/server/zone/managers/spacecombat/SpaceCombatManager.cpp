...
void SpaceCombatManager::applyDamage(ShipObject* ship, const ShipProjectile* projectile, const SpaceCollisionResult& result) const {
	...
	if (damage > 0.f) {
		// Add Threat Map Damage
		ThreatMap* targetThreatMap = target->getThreatMap();
		...
		if (collisionSlot != Components::CHASSIS) {
		...
		if (target->getShipComponentMap()->get(Components::BRIDGE) == 0) {
