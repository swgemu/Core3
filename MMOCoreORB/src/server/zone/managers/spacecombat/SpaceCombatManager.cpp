...
void SpaceCombatManager::applyDamage(ShipObject* ship, const ShipProjectile* projectile, const SpaceCollisionResult& result) const {
	...
	applyDamage(target, position, damage, armorEffect, shieldEffect, collisionSlot, targetSlot, hitFront);
}

void SpaceCombatManager::applyDamage(ShipObject* target, const Vector3& collisionPoint, float damage, float armorEffect, float shieldEffect, int collisionSlot, int targetSlot, bool hitFront) const {
	Locker tLock(target);
