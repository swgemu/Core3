...
void ObjectVersionUpdateManager::updateWeaponsDots() {
	...
	int count = 0;
	uint32 classNameHashCode = String::hashCode("_className");
	try {
		...
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			try {
				if (!Serializable::getVariable<String>(classNameHashCode, &className, &objectData) ||
						!Serializable::getVariable<int>(String::hashCode("WeaponObject.dotType"), &oldType, &objectData)) {
			...
			if (className == "WeaponObject") {
				...
				ObjectOutputStream* newData = changeVariableData(String::hashCode("WeaponObject.dotType"), &objectData, &newDotsValue);
				newData->reset();

				ObjectInputStream* inputStream = new ObjectInputStream(newData->getBuffer(), newData->size());
				delete newData;

				ObjectOutputStream* newNextData = changeVariableData(String::hashCode("WeaponObject.dotAttribute"), inputStream, &newDotsValue);
				newNextData->reset();
				...
				newData = changeVariableData(String::hashCode("WeaponObject.dotStrength"), inputStream, &newDotsValue);
				newData->reset();
				...
				newNextData = changeVariableData(String::hashCode("WeaponObject.dotDuration"), inputStream, &newDotsValue);
				newNextData->reset();
				...
				newData = changeVariableData(String::hashCode("WeaponObject.dotPotency"), inputStream, &newDotsValue);
				newData->reset();
				...
				newNextData = changeVariableData(String::hashCode("WeaponObject.dotUses"), inputStream, &newDotsValue);
				newNextData->reset();
	...
	try {
		...
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			if (className == "BuildingObject" || className == "InstallationObject" || className == "GarageInstallation" || className == "ShuttleInstallation") {
				...
				if( Serializable::getVariable<uint64>(String::hashCode("StructureObject.ownerObjectID"), &ownerID, &objectData)) {
					StructurePermissionList permissionList;

					if ( Serializable::getVariable<StructurePermissionList>(String::hashCode("StructureObject.structurePermissionList"), &permissionList, &objectData)){
						ObjectOutputStream newOutputStream;
						permissionList.setOwner(ownerID);
						permissionList.toBinaryStream(&newOutputStream);

						ObjectOutputStream* test = changeVariableData(String::hashCode("StructureObject.structurePermissionList"), &objectData, &newOutputStream);
						test->reset();
...
void ObjectVersionUpdateManager::updateResidences(){
	...
	try {
		...
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			try {
				if (!Serializable::getVariable<String>(String::hashCode("_className"), &className, &objectData) ||
						!Serializable::getVariable<uint64>(String::hashCode("PlayerObject.declaredResidence"), &residence, &objectData)) {
			...
			//	info("we found a Player " + String::valueOf(objectID) + " with residence " + String::valueOf(residence),true);
				...
				if( Serializable::getVariable< SortedVector<unsigned long long> >(String::hashCode("PlayerObject.ownedStructures"), &structureList, &objectData) &&
						Serializable::getVariable<uint64>(String::hashCode("PlayerObject.declaredResidence"), &residence, &objectData)){
	...
	if(!database->getData(buildingID,&objectData)){
		...
		if ( Serializable::getVariable<String>(String::hashCode("_className"), &className, &objectData)){
			...
			if(className == "BuildingObject"){
				if ( !Serializable::getVariable<bool>(String::hashCode("BuildingObject.isOwnerResidence"), &isResidence, &objectData)){
					...
					ObjectOutputStream* test = changeVariableData(String::hashCode("BuildingObject.isOwnerResidence"), &objectData, &newResidenceValue);
					test->reset();
	...
	try {
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			if (className == "CityRegion") {
				...
				if ( Serializable::getVariable<int>(String::hashCode("CityRegion.cityTreasury"), &funds, &objectData)){

					floatFunds = funds;
					ObjectOutputStream newFunds;
					TypeInfo<float>::toBinaryStream(&floatFunds, &newFunds );
					ObjectOutputStream* test = changeVariableData(String::hashCode("CityRegion.cityTreasury"), &objectData, &newFunds);
					test->reset();
					database->putData(objectID, test, NULL);
	...
	try {
		while (iterator.getNextKeyAndValue(objectID, &objectData)) {
			...
			if (className == "CityRegion") {
				count++;
				printf("\r\tUpdating city treasury [%d] / [?]\t", count);
				if ( Serializable::getVariable<float>(String::hashCode("CityRegion.cityTreasury"), &funds, &objectData)){
