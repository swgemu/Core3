...
void ObjectVersionUpdateManager::updateTangibleObjectsVersion6() {
	...
	ObjectDatabase *schemdb = ObjectDatabaseManager::instance()->loadObjectDatabase("draftschematics", true);
	ObjectDatabaseIterator schemIter(schemdb);
	if(!schemIter.getNextKey(objectID) || objectID == 0) {
				...
				if (className == "PlayerObject") {
					if (Serializable::getVariable<AbilityList>(STRING_HASHCODE("PlayerObject.abilityList"), &abilityList, &objectData)) {
						...
						if(abilityList.size() != size) {
							...
							//info("Updating abilityList", true);
						}
					} else {
