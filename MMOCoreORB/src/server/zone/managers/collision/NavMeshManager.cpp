...
void NavMeshManager::enqueueJob(Zone* zone, NavMeshRegion* region, AABB areaToBuild, const RecastSettings& recastConfig, String queue      ) {
...
void NavMeshManager::startJob(Reference<NavMeshJob*> job) {
    ...
    AABB bBox = region->getBoundingBox();

    float range = bBox.extents()[bBox.longestAxis()];
    Vector3 center = bBox.center();//Vector3(bSphere.getCenter()[0], 0, -bSphere.getCenter()[2]);
    ...
    if(initialBuild) {
        ...
        zone->getPlanetManager()->addNavMesh(region);
    }
