...
bool CollisionManager::checkLineOfSight(SceneObject* object1, SceneObject* object2) {
	...
	if (rootParent1 != nullptr || rootParent2 != nullptr) {
		if (rootParent1 == rootParent2) {
			if (object1->isCreatureObject() && object2->isCreatureObject()) {
				...
				if (creO != nullptr && tarCreO != nullptr) {
					if (creO->isPlayerCreature() && tarCreO->getParentID() != 0) {
						if (!playerEntryCheck(creO, tarCreO)) {
							return false;
						}
					} else if (tarCreO->isPlayerCreature() && creO->getParentID() != 0) {
						if (!playerEntryCheck(tarCreO, creO)) {
							return false;
						}
					} else {
