...
bool CollisionManager::checkMovementCollision(CreatureObject* player, CloseObjectsVector* closeObjectsVector, Zone* zone, const Vector3& lastValidWorld, const Vector3& transformPosition) {
	...
	Vector3 rayStart = player->getWorldPosition();
	...
	rayEnd.setX(rayStart.getX() - (dx * 0.5));
	rayEnd.setY(rayStart.getY() - (dy * 0.5));
	...
	if (maxDistance < 0.5) {
#ifdef DEBUG_MOVEMENT_COLLISION
		player->info(true) << "CollisionManager::checkMovementCollision -- MaxtDistance is less than 0.5f";
#endif // DEBUG_MOVEMENT_COLLISION
...
#ifdef DEBUG_MOVEMENT_COLLISION
	player->info(true) << "Movement Collision -- Ray Start: " << rayStart.toString() << " Ray End: " << rayEnd.toString() << " Max Dist: " << maxDistance;
#endif // DEBUG_MOVEMENT_COLLISION
		...
		if (areas.size() > 0) {
			...
			if (collisions.size() == 0) {
				...
				for (int i = 0; i < areas.size(); i++) {
					if (!pathFinderMan->getRecastPath(rayStart, rayEnd, areas.get(i), path, maxDistance, false)) {
						delete path;
						path = nullptr;
