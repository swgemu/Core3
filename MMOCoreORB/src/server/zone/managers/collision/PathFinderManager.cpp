...
#ifdef DEBUG_PATHING
		info(true) << "getFloorPath -- our Floor node or the target floor node is null";
#endif
		return 1;
	} else if (objectFloor == targetFloor) {
#ifdef DEBUG_PATHING
		info(true) << "getFloorPath -- we are on the same triangle, returning nearest node";
#endif

		// This needs to return the nearest node to the position or we risk going out of the cell boundary
		const PathNode* nearestNode = CollisionManager::findNearestPathNode(targetFloor, floor, pointB);
