...
void PathFinderManager::getNavMeshCollisions(SortedVector<Reference<NavCollision*>> *collisions,
	...
	for (const ManagedReference<NavMeshRegion*>& region : *regions) {
		const Vector3& sPos = region->getWorldPosition();
...
bool PathFinderManager::getRecastPath(const Vector3& start, const Vector3& end, NavMeshRegion* region, Vector<WorldCoordinates>* path, float& len) {
	...
	if (pointInSphere(targetPosition, sphere) || pointInSphere(startPosition, sphere)) {
		...
		if (!((status = query->findPath(startPoly, endPoly, polyStart.toFloatArray(), polyEnd.toFloatArray(), &m_filter, polyPath, &numPolys, 2048)) & DT_SUCCESS))
			return false;
	...
	for(const WorldCoordinates& pointB : endPoints) {
		...
		try {
			if (collisions.size() == 1) { // we're entering a navmesh
				...
				if (!getRecastPath(collision->getPosition(), targetTemp, collision->getRegion(), path, len) && !allowPartial)
					continue;
