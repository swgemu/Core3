...
Vector<WorldCoordinates>* PathFinderManager::findPathFromWorldToWorld(const WorldCoordinates& pointA, Vector<WorldCoordinates>& endPoints, Zone* zone, bool allowPartial) {
	...
	Reference<Vector<WorldCoordinates>*> finalpath = new Vector<WorldCoordinates>();
	...
	for(const WorldCoordinates& pointB : endPoints) {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			if (pointInSphere(targetPosition, sphere) || pointInSphere(startPosition, sphere)) {
				...
				if (query->findPath(startPoly, endPoly, polyStart.toFloatArray(), polyEnd.toFloatArray(), &m_filter, polyPath, &numPolys, 2048) != DT_SUCCESS && !allowPartial)
					continue;
				...
				if (numPolys) {
					...
					if (numPoints > 0) {
						Reference<Vector<WorldCoordinates>*> path = new Vector<WorldCoordinates>();
						float len = 0;
