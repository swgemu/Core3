...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	lua_register(luaEngine->getLuaState(), "getServerEvent", getServerEvent);
	...
	try {
		...
		while (iterator.getNextKey(objectID)) {
			...
			if (eventName == pEventCheck->getEventName()) {
				instance()->error("Event already exists. Exiting...");
				ERROR_CODE = GENERAL_ERROR;
				continue;
			}
		}
	} catch (DatabaseException& e) {
		instance()->error("Error in checking event database in DirectorManager::createServerEvent");
		ERROR_CODE = GENERAL_ERROR;
	}
...
int DirectorManager::getServerEvent(lua_State* L) {
	if (checkArgumentCount(L, 1) == 1) {
