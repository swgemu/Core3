...
void DirectorManager::loadPersistentEvents() {
	...
	try {
		...
		while (iterator.getNextKey(objectID)) {
			...
			info("Loading persistent event object id: " + String::valueOf(objectID), true);
			++i;
		}
...
int DirectorManager::createEvent(lua_State* L) {
	...
	if (parameterCount > 4) {
		...
		if (save) {
			...
			instance()->info("Persistent event object id: " + String::valueOf(pevent->_getObjectID()), true);
...
void DirectorManager::activateEvent(ScreenPlayTask* task) {
	...
	info("Activated persistent event object id: " + String::valueOf(task->getPersistentEventObjectID()), true);
