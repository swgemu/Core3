...
int DirectorManager::removeObservers(lua_State* L) {
	...
	SceneObject* sceneObject = (SceneObject*) lua_touserdata(L, -1);
	SortedVector<ManagedReference<Observer*> > observers = sceneObject->getObservers(ObserverEventType::OBJECTDESTRUCTION);

	for (int i = 0; i < observers.size(); i++) {
		sceneObject->dropObserver(ObserverEventType::OBJECTDESTRUCTION, observers.get(i).get());
	}
