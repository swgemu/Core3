...
//	SortedVector<ManagedReference<AuctionItem* > > items = auctionMap->getVendorItems("");
//
...
//				vendorData->dropVendorItem(objectId);
//
...
int AuctionManagerImplementation::checkRetrieve(CreatureObject* player, uint64 objectIdToRetrieve, TangibleObject* vendor) {
...
//	ManagedReference<AuctionItem*> item = NULL;
//
//	ManagedReference<TangibleObject*> vendor = cast<TangibleObject*>(zoneServer->getObject(vendorID));
//
...
//	}
//
//	if (item->isOfferToVendor() && item->getOwnerID() != player->getObjectID() && vendorData->isVendorOwner(player)) {
	...
	case 7: // Vendor items screen (including Vendor location search)
		...
		if(vendorData != NULL) {
			...
			for (int i = 0; (i < items->size()) && (displaying < (offset + 100)); i++) {
				...
				if (vendorData->isVendorOwner(player)) {
					...
					if (vendor->isBazaarTerminal() && !vendorData->isVendorSearchEnabled())
						continue;
...
void AuctionManagerImplementation::getAuctionData(CreatureObject* player, TangibleObject* vendor, String& search, int screen, uint32 category, int clientcounter, int offset) {
	...
	if (vendor->isBazaarTerminal() && screen != 7) { // This is to prevent bazaar items from showing on Vendor Search
		items = auctionMap->getBazaarItems(search);
	} else {
		items = auctionMap->getVendorItems(search);
	}
