...
void AuctionManagerImplementation::addSaleItem(CreatureObject* player, uint64 objectid, SceneObject* vendor, const UnicodeString& description, int price, uint32 duration, bool auction, bool premium) {
	...
	if (vendor == nullptr || (!vendor->isVendor() && !vendor->isBazaarTerminal())) {
		error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Not valid vendor object.";
	...
	if(oldItem == nullptr) {
		if (objectToSell == nullptr || !objectToSell->isASubChildOf(player)) {
			if (objectToSell != nullptr)
				error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Selling invalid object.";
		...
		if (oldItem->getStatus() == AuctionItem::DELETED) {
			error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Attempt to sell a DELETED item, auctionItem: " << *oldItem;
			ItemSoldMessage* soldMessage = new ItemSoldMessage(objectid, ItemSoldMessage::INVALIDITEM);
			player->sendMessage(soldMessage);
			return;
		}
		...
		if (oldItem->getStatus() == AuctionItem::RETRIEVED) {
			error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Attempt to sell a RETRIEVED item, auctionItem: " << *oldItem;
			ItemSoldMessage* soldMessage = new ItemSoldMessage(objectid, ItemSoldMessage::INVALIDITEM);
			player->sendMessage(soldMessage);
			return;
		}
		...
		if (oldItem->getOwnerID() != player->getObjectID()) {
			error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Selling object owned by another player: " << oldItem->getOwnerID();
			ItemSoldMessage* soldMessage = new ItemSoldMessage(objectid, ItemSoldMessage::INVALIDITEM);
			player->sendMessage(soldMessage);
			return;
		}
	...
	if (zone == nullptr) {
		error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): Vendor has null zone?";
		ItemSoldMessage* soldMessage = new ItemSoldMessage(objectid, ItemSoldMessage::UNKNOWNERROR);
		player->sendMessage(soldMessage);
		return;
	}
	...
	if(item == nullptr) {
		error() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): createVendorItem failed.";
		ItemSoldMessage* soldMessage = new ItemSoldMessage(objectid, ItemSoldMessage::UNKNOWNERROR);
		player->sendMessage(soldMessage);
		return;
	}
	...
	if(result != ItemSoldMessage::SUCCESS) {
		info() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): failed to add to auctionMap, result=" << ItemSoldMessage::statusToString(result) << ", auctionItem: " << *item;
	...
	info() << "addSaleItem(plyer=" << player->getObjectID() << ", objetid=" << objectid << "): SUCCESS, auctionItem: " << *item;
}
