...
* Flytext over bobber - DONE
* Bobber to move closer with tugs - DONE
* Junk loot catch
* Fix Bait chance to change state - DONE
* fishing_manager.lua - load lists - DONE
* Trx log loot
...
void FishingManagerImplementation::fishingStep(CreatureObject* player) {
	...
	if (bait != nullptr) {
		...
		if ((state < CATCH) && (baitState > SOGGY)) {
			if (System::random(15) < baitState) {
...
void FishingManagerImplementation::success(CreatureObject* player, int fish, SceneObject* marker, uint32 boxID) {
	...
	if (fish > 70) { // Non-fish Item Caught
		...
		if (badLuck <= 1) {
			...
			if (lootFishObject != nullptr) {
				...
				trx.addState("Fish Type: ", fishName);
				trx.addState("Fish Length: ", length);
				trx.addState("XP Awarded: ", xp);
				trx.addState("Resource Awarded: ", resourceString);
				trx.addState("Total Resources Given: ", amount);
