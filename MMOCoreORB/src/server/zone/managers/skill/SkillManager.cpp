...
void SkillManager::surrenderAllSkills(CreatureObject* creature, bool notifyClient, bool removeJediProgression) {
	ManagedReference<PlayerObject*> ghost = creature->getPlayerObject();
	...
	for (int i = 0; i < copyOfList.size(); i++) {
		...
		if (skill->getSkillPointsRequired() > 0) {
			if (!removeJediProgression and skill->getSkillName().contains("force_title"))
				continue;
