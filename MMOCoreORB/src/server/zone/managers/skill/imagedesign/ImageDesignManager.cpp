...
void ImageDesignManager::updateColorCustomization(CreatureObject* imageDesigner, CustomizationData* customData, uint32 value, bool isHairUpdate, TangibleObject* hairObject, CreatureObject* creo) {
...
void ImageDesignManager::updateColorCustomization(CreatureObject* imageDesigner, const String& customizationName, uint32 value, bool isHairUpdate, TangibleObject* hairObject, CreatureObject* creo) {
	if (value > 255 || creo == NULL)
		return;
	...
	for (int i = 0; i < data->size(); ++i) {
		CustomizationData* customData = &data->get(i);
		updateColorCustomization(imageDesigner, customData, value, isHairUpdate, hairObject, creo);
	}
}
