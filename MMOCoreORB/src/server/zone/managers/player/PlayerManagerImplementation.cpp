...
void PlayerManagerImplementation::completeUnity( CreatureObject* respondingPlayer, unsigned long long respondingPlayerRingId ){
	...
	for (int i = 0; i < respondingPlayerInventory->getContainerObjectsSize(); i++) {
		respondingRing = cast<WearableObject*>(respondingPlayerInventory->getContainerObject(i).get());
		if( respondingRing != NULL && respondingRing->getObjectID() == respondingPlayerRingId && !respondingRing->isEquipped() ){
			break;
		}
	}
	...
	for (int i = 0; i < askingPlayerInventory->getContainerObjectsSize(); i++) {
		askingRing = cast<WearableObject*>(askingPlayerInventory->getContainerObject(i).get());
		if( askingRing != NULL && askingPlayerInventory->getObjectID() == proposeUnitySession->getAskingPlayerRing() && !askingRing->isEquipped() ){
			break;
		}
	}
	...
	// Exchange rings
	ManagedReference<ObjectController*> objectController = server->getObjectController();
	if (objectController->transferObject(askingRing, respondingPlayerInventory, -1, true)){
		askingRing->sendDestroyTo(askingPlayer);
		respondingPlayerInventory->broadcastObject(askingRing, true);
	}

	if (objectController->transferObject(respondingRing, askingPlayerInventory, -1, true)){
