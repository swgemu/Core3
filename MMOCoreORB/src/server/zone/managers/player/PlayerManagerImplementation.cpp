...
void PlayerManagerImplementation::claimVeteranRewards(CreatureObject* player){
	...
	// Get account
	Account* account = getAccount( playerGhost->getAccountID() );
	if( account == NULL )
		return;
	...
	if( player->getActiveSession(SessionFacadeType::VETERANREWARD) != NULL ){
		player->sendSystemMessage( "You are already attemping to claim a veteran reward." );
		return;
	}
	...
	box->setPromptText("You may choose one of the items listed below.  This item will be placed in your inventory" );
	box->setPromptTitle("Veteran Reward");
	box->setOkButton(true, "@ok");
	box->setCancelButton(true, "@cancel");
...
void PlayerManagerImplementation::generateVeteranReward(CreatureObject* player, int itemIndex ){
