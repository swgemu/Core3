...
void PlayerManagerImplementation::generateVeteranReward(CreatureObject* player) {
	...
	// Set the stats for the Sorosuub Veteran Reward Deed
	if (rewardSceno->isShipDeedObject()) {
		ShipDeed* shipDeed = rewardSceno.castTo<ShipDeed*>();

		if (shipDeed != nullptr) {
			Locker lock(shipDeed);

			// set hitpoints, mass, location
			shipDeed->setMass(10000);
			shipDeed->setMaxHitPoints(1512);
			shipDeed->setCreateComponents(true);
