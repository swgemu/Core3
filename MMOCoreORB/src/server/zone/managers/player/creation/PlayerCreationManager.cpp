				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 3) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 3 seconds. Repeat attempts prior to 1 hour elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 3) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 3 seconds. Repeat attempts prior to 1 hour elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
