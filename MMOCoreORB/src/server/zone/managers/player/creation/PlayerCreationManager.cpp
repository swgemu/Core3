		...
		if (!freeGodMode) {
			try {
				...
				if(accountPermissionLevel > 0 && (accountPermissionLevel == 9 || accountPermissionLevel == 10 || accountPermissionLevel == 12 || accountPermissionLevel == 15)) {
					...
					/*
					Reference<ShipControlDevice*> shipControlDevice = zoneServer->createObject(String::hashCode("object/intangible/ship/sorosuub_space_yacht_pcd.iff"), 1).castTo<ShipControlDevice*>();
					//ShipObject* ship = (ShipObject*) server->createObject(String::hashCode("object/ship/player/player_sorosuub_space_yacht.iff"), 1);
					Reference<ShipObject*> ship = zoneServer->createObject(String::hashCode("object/ship/player/player_basic_tiefighter.iff"), 1).castTo<ShipObject*>();
