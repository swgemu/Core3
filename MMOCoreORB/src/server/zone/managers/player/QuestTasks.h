...
public:
	...
	void parseDataTableRow(const DataTableRow* row) {
		if (row->getCellsSize() == 28) {
			...
			throw UnknownDatatableException("Number of columns in quest task datatable is of an unknown number(" + String::valueOf(row->getCellsSize()) + "), please implement parsing of it.");
		}
	}
