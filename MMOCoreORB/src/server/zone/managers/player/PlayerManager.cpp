...
bool PlayerManager::hasCorrectZCoordinate(CreatureObject* player) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_HASCORRECTZCOORDINATE__CREATUREOBJECT_);
		method.addObjectParameter(player);

		return method.executeWithBooleanReturn();
	} else
		return _implementation->hasCorrectZCoordinate(player);
}
...
void PlayerManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_CREATESKIPPEDTUTORIALBUILDING__CREATUREOBJECT_:
		...
		break;
	case RPC_HASCORRECTZCOORDINATE__CREATUREOBJECT_:
		{
			resp->insertBoolean(hasCorrectZCoordinate(static_cast<CreatureObject*>(inv->getObjectParameter())));
		}
		break;
...
bool PlayerManagerAdapter::hasCorrectZCoordinate(CreatureObject* player) {
	return (static_cast<PlayerManager*>(stub))->hasCorrectZCoordinate(player);
}
