...
void PlayerManager::ejectPlayerFromDungeon(CreatureObject* player) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_EJECTPLAYERFROMDUNGEON__CREATUREOBJECT_);
		method.addObjectParameter(player);

		method.executeWithVoidReturn();
	} else
		_implementation->ejectPlayerFromDungeon(player);
}
...
void PlayerManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SENDPLAYERTOCLONER__CREATUREOBJECT_LONG_INT_:
		...
		break;
	case RPC_EJECTPLAYERFROMDUNGEON__CREATUREOBJECT_:
		{
			ejectPlayerFromDungeon(static_cast<CreatureObject*>(inv->getObjectParameter()));
		}
		break;
	...
	case RPC_SHOULDDELETECHARACTER__LONG_INT_:
		{
			resp->insertBoolean(shouldDeleteCharacter(inv->getUnsignedLongParameter(), inv->getSignedIntParameter()));
		}
		break;
...
void PlayerManagerAdapter::ejectPlayerFromDungeon(CreatureObject* player) {
	(static_cast<PlayerManager*>(stub))->ejectPlayerFromDungeon(player);
}
