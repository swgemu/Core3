...
void PlayerManager::updateSwimmingState(CreatureObject* player, float newZ) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_UPDATESWIMMINGSTATE__CREATUREOBJECT_FLOAT_);
		method.addObjectParameter(player);
		method.addFloatParameter(newZ);

		method.executeWithVoidReturn();
	} else
		_implementation->updateSwimmingState(player, newZ);
}
...
void PlayerManager::cleanupCharacters(bool deleteCharacter) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_CLEANUPCHARACTERS__BOOL_);
		method.addBooleanParameter(deleteCharacter);

		method.executeWithVoidReturn();
	} else
		_implementation->cleanupCharacters(deleteCharacter);
}

bool PlayerManager::shouldDeleteCharacter(unsigned long long characterID) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (_implementation == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_SHOULDDELETECHARACTER__LONG_);
		method.addUnsignedLongParameter(characterID);

		return method.executeWithBooleanReturn();
	} else
		return _implementation->shouldDeleteCharacter(characterID);
}
...
void PlayerManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SHOULDRESCHEDULECORPSEDESTRUCTION__CREATUREOBJECT_CREATUREOBJECT_:
		{
			resp->insertBoolean(shouldRescheduleCorpseDestruction(static_cast<CreatureObject*>(inv->getObjectParameter()), static_cast<CreatureObject*>(inv->getObjectParameter())));
		}
		break;
	case RPC_RESCHEDULECORPSEDESTRUCTION__CREATUREOBJECT_CREATUREOBJECT_:
		{
			rescheduleCorpseDestruction(static_cast<CreatureObject*>(inv->getObjectParameter()), static_cast<CreatureObject*>(inv->getObjectParameter()));
		}
		break;
	case RPC_CANGROUPMEMBERHARVESTCORPSE__CREATUREOBJECT_CREATURE_:
		{
			resp->insertBoolean(canGroupMemberHarvestCorpse(static_cast<CreatureObject*>(inv->getObjectParameter()), static_cast<Creature*>(inv->getObjectParameter())));
		}
		break;
	...
	case RPC_CREATESKIPPEDTUTORIALBUILDING__CREATUREOBJECT_:
		...
		break;
	case RPC_UPDATESWIMMINGSTATE__CREATUREOBJECT_FLOAT_:
		{
			updateSwimmingState(static_cast<CreatureObject*>(inv->getObjectParameter()), inv->getFloatParameter());
		}
		break;
	...
	case RPC_FIXHAM__CREATUREOBJECT_:
		{
			fixHAM(static_cast<CreatureObject*>(inv->getObjectParameter()));
		}
		break;
	case RPC_CLEANUPCHARACTERS__BOOL_:
		{
			cleanupCharacters(inv->getBooleanParameter());
		}
		break;
	case RPC_SHOULDDELETECHARACTER__LONG_:
		{
			resp->insertBoolean(shouldDeleteCharacter(inv->getUnsignedLongParameter()));
		}
		break;
	default:
...
void PlayerManagerAdapter::updateSwimmingState(CreatureObject* player, float newZ) {
	(static_cast<PlayerManager*>(stub))->updateSwimmingState(player, newZ);
}
...
void PlayerManagerAdapter::cleanupCharacters(bool deleteCharacter) {
	(static_cast<PlayerManager*>(stub))->cleanupCharacters(deleteCharacter);
}

bool PlayerManagerAdapter::shouldDeleteCharacter(unsigned long long characterID) {
	return (static_cast<PlayerManager*>(stub))->shouldDeleteCharacter(characterID);
}
