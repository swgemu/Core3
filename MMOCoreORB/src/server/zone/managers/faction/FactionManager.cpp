...
void FactionManager::awardSpaceFactionPoints(CreatureObject* player, const String& shipType, const String& factionName, uint32 shipLevel, int totalShipmates, float factionMultiplier) {
	if (player == nullptr || !factionMap.contains(factionName)) {
		return;
	}
	...
	uint32 typeHash = shipType.hashCode();
