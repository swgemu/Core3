...
public:
	...
	int compareTo(QuadTreeEntryInterfaceBase* obj) {
...
class HeightCacheFunction : public LRUFunction2<uint64, float, float, TerrainCache::lru_value_t > {
...
TerrainCache::TerrainCache(TerrainManager* terrainManager) :
		lru_cache_t(new HeightCacheFunction(terrainManager),
