...
void MissionManagerImplementation::populateMissionList(MissionTerminal* missionTerminal, CreatureObject* player, int counter) {
	...
	for (int i = 0; i < bagSize; ++i) {
		...
		if (missionTerminal->isGeneralTerminal()) {
			...
			if (i < (bagSize / maximumNumberOfMissionTypesInOneTerminal)) {
				randomizeGenericEntertainerMission(player, mission, Faction::FACTIONNEUTRAL, MissionTypes::DANCER);
			} else if (i < (bagSize * 2 / maximumNumberOfMissionTypesInOneTerminal)) {
				randomizeGenericEntertainerMission(player, mission, Faction::FACTIONNEUTRAL, MissionTypes::MUSICIAN);
			} else {
				mission->setTypeCRC(0);
			}
			...
			if (i < bagSize / maximumNumberOfMissionTypesInOneTerminal) {
				...
				if (enableFactionalCraftingMissions && numberOfCraftingMissions < 6) {
					...
					randomizeGenericEntertainerMission(player, mission, Faction::FACTIONIMPERIAL, MissionTypes::DANCER);
					numberOfDancerMissions++;
				} else if (enableFactionalEntertainerMissions && numberOfMusicianMissions < 6) {
					randomizeGenericEntertainerMission(player, mission, Faction::FACTIONIMPERIAL, MissionTypes::MUSICIAN);
			...
			if (i < bagSize / maximumNumberOfMissionTypesInOneTerminal) {
				...
				if (enableFactionalCraftingMissions && numberOfCraftingMissions < 6) {
					...
					randomizeGenericEntertainerMission(player, mission, Faction::FACTIONREBEL, MissionTypes::DANCER);
					numberOfDancerMissions++;
				} else if (enableFactionalEntertainerMissions && numberOfMusicianMissions < 6) {
					randomizeGenericEntertainerMission(player, mission, Faction::FACTIONREBEL, MissionTypes::MUSICIAN);
					numberOfMusicianMissions++;
				} else {
					mission->setTypeCRC(0, true);
				}
...
void MissionManagerImplementation::randomizeGenericHuntingMission(CreatureObject* player, MissionObject* mission, const uint32 faction) {
	LairSpawn* randomLairSpawn = getRandomLairSpawn(player, Faction::FACTIONNEUTRAL, MissionTypes::HUNTING);
