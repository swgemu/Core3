...
public:
	...
	void checkLootGroupEntryRecursive(LootGroupMap* lootGroupMap, String& entryName, Vector<String>* parentGroups) {
		...
		if (lootGroupMap->lootGroupExisits(entryName)) {
...
TEST_F(LuaMobileTest, LuaLootGroupsTest) {
	...
	while (itemIter.hasNext()) {
		...
		EXPECT_FALSE( lootGroupMap->lootGroupExisits(itemTemplateName) ) << "Loot item " << std::string(itemTemplateName.toCharArray()) << " has the same name as a loot group.";
	}
