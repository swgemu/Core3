...
private:
	...
	void registerBehaviors() {
		...
		_REGISTERLEAF(CheckFollowDead);
		_REGISTERLEAF(CheckFollowIncapped);
		_REGISTERLEAF(CheckFollowDestroyed);
	...
	static int addAiTemplate(lua_State* L) {
		...
		for (int idx = 0; idx < loadMap.size(); ++idx) {
			...
			if (parent->isComposite())
				static_cast<server::zone::objects::creature::ai::bt::node::Composite*>(parent.get())->addChild(child);
			else if (parent->isDecorator())
				static_cast<server::zone::objects::creature::ai::bt::decorator::Decorator*>(parent.get())->setChild(child);
		}
