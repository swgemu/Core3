...
SceneObject* CreatureManagerImplementation::spawnLair(unsigned int lairTemplate, int difficultyLevel, int lairBuildingLevel, float x, float z, float y, float size) {
	...
	if (lairTmpl == nullptr || lairTmpl->getBuildingType() != LairTemplate::LAIR)
 	...
 	buildingToSpawn = lairTmpl->getBuilding((uint32)lairBuildingLevel);
	...
	int conditionCalc = difficultyLevel * (900 + System::random(200));
	int maxCondition = Math::min(CreatureManager::CREATURE_LAIR_MAX, (uint32)conditionCalc);

	building->setMaxCondition(maxCondition);
