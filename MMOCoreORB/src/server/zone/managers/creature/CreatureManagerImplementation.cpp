...
SceneObject* CreatureManagerImplementation::spawnDynamicSpawn(unsigned int lairTemplate, int difficulty, float x, float z, float y, float size) {
	...
	ManagedReference<TheaterObject*> theater = zoneServer->createObject(String::hashCode("object/intangible/theater/base_theater.iff"), 0).castTo<TheaterObject*>();
...
bool CreatureManagerImplementation::createCreatureChildrenObjects(CreatureObject* creature, uint32 templateCRC, bool persistent, uint32 mobileTemplateCRC) {
	if (creature->hasSlotDescriptor("default_weapon")) {
		...
		if (creature->isNonPlayerCreatureObject()) {
			defaultWeaponCRC = String::hashCode("object/weapon/melee/unarmed/unarmed_default.iff");
		} else {
			defaultWeaponCRC = String::hashCode("object/weapon/creature/creature_default_weapon.iff");
		}
	...
	if (creature->hasSlotDescriptor("inventory")) {
		Reference<SceneObject*> creatureInventory = zoneServer->createObject(String::hashCode("object/tangible/inventory/creature_inventory.iff"), persistent);
