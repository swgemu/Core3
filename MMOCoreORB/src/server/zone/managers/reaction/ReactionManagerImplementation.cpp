...
void ReactionManagerImplementation::doKnockdown(CreatureObject* victim, AiAgent* attacker) {
	...
	victim->setPosture(CreaturePosture::KNOCKEDDOWN, false, true);
	attacker->doCombatAnimation(victim, knockdownAnim.hashCode(), HIT, 0xFF);

	CombatAction *kdAnim = new CombatAction(victim, STRING_HASHCODE("change_posture"));

	victim->broadcastMessage(kdAnim, true);
