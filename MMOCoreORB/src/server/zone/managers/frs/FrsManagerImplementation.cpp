...
void FrsManagerImplementation::setPlayerRank(CreatureObject* player, int rank) {
	...
	printf("%s set to rank %i\n", player->getFirstName().toCharArray(), rank);
...
void FrsManagerImplementation::promotePlayer(CreatureObject* player) {
	...
	printf("Promoted %s to rank %i\n", player->getFirstName().toCharArray(), newRank);
...
void FrsManagerImplementation::adjustFrsExperience(CreatureObject* player, int amount) {
	...
	if (amount > 0) {
		...
		printf("Granting %s %i frs experience\n", player->getFirstName().toCharArray(), amount);
		...
		ghost->addExperience("force_rank_xp", amount, true);
		printf("Removing %i frs experience from %s\n", amount, player->getFirstName().toCharArray());
...
void FrsManagerImplementation::deductMaintenanceXp(CreatureObject* player) {
	...
	printf("Adding %i maint xp debt to %s\n", maintXp, player->getFirstName().toCharArray());
...
void FrsManagerImplementation::deductDebtExperience(CreatureObject* player) {
	...
	printf("Deducted %i maint xp debt from %s\n", debtAmt, player->getFirstName().toCharArray());
...
void FrsManagerImplementation::handleVoteRecordSui(CreatureObject* player, SceneObject* terminal, short enclaveType, int rank, int index) {
	...
	printf("%s has voted for petitioner %s\n", player->getFirstName().toCharArray(), playerName.toCharArray());
...
void FrsManagerImplementation::handleVotePetitionSui(CreatureObject* player, SceneObject* terminal, short enclaveType, int rank) {
	...
	printf("%s has petitioned for a promotion\n", player->getFirstName().toCharArray());
...
void FrsManagerImplementation::handleAcceptPromotionSui(CreatureObject* player, SceneObject* terminal, short enclaveType, int rank) {
	...
	if (availSlots <= 0) {
		printf("%s has attempted to accept promotion but there are no full slots, closing phase\n", player->getFirstName().toCharArray());
		SortedVector<uint64>* winnerList = rankData->getWinnerList();
	...
	if (rankData->getTotalWinners() <= 0) {
		printf("no more winners in list, closing phase\n");
	...
	if ((availSlots - 1) <= 0) {
		printf("No more available slots, but winners still in list, notifying them and closing phase\n");
		SortedVector<uint64>* winnerList = rankData->getWinnerList();
...
void FrsManagerImplementation::runVotingUpdate(FrsRank* rankInfo) {
	...
	if (status == VOTING_CLOSED) {
		printf("%i, %i current voting status: closed\n", councilType, rank);
		if (availSlots > 0) {
			SortedVector<uint64>* winnerList = rankInfo->getWinnerList();

			if (winnerList->size() > 0) {
				printf("%i, %i Winners in list from previous phase, notifying them they can accept, setting phase to waiting\n", councilType, rank);
		...
		if (totalPetitioners == 0) { // No one petitioned
			...
			if (totalPetitioners <= availSlots || totalPetitioners == 1) { // More open slots than petitioners, or only one petitioner so everyone wins
				printf("%i, %i more slots than petitioners, no voting phase needed, notifying winners and setting phase to waiting\n", councilType, rank);
				rankInfo->setVoteStatus(WAITING);
				...
				printf("%i, %i More petitioners than open slots, moving to voting phase\n", councilType, rank);
		...
		printf("%i, %i current voting status: open\n", councilType, rank);
		VectorMap<uint64, int>* petitionerList = rankInfo->getPetitionerList();

		if (petitionerList->size() == 0) { // Empty petitioner list. This shouldn't happen, but just in case.
			printf("%i, %i Petitioner list empty somehow in voting phase, closing phase\n", councilType, rank);
			rankInfo->resetVotingData();
			rankInfo->setVoteStatus(VOTING_CLOSED);
		} else {
			if (availSlots > 0) { // Add top X (where X = available slots) winners to winner list so they can accept next phase
				...
				printf("%i, %i Grabbed %i winners, notifying them and moving to waiting phase\n", councilType, rank, winnerList->size());
				for (int i = 0; i < winnerList->size(); i++) {
				...
				printf("%i, %i No available slots, notifying winners and adding them to list for next open slot, setting phase closed\n", councilType, rank);
		...
		printf("%i, %i current voting status: waiting\n", councilType, rank);
		SortedVector<uint64>* winnerList = rankInfo->getWinnerList();

		if (winnerList->size() > 0) {
			printf("%i, %i - %i people didnt accept promotion, notifying\n", councilType, rank, winnerList->size());
		...
		printf("%i, %i Setting phase closed\n", councilType, rank);
		rankInfo->resetVotingData();
		rankInfo->setVoteStatus(VOTING_CLOSED);
	}
		...
		for (int j = 0; j < playerList->size(); j++) {
			...
			if (!rankInfo->isOnVotedList(playerID)) {
				...
				if (player != NULL) {
					...
					printf("%s didnt vote, adding %i vote penalty\n", player->getFirstName().toCharArray(), votePenalty);
