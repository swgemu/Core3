...
bool FrsManagerImplementation::isValidFrsBattle(CreatureObject* attacker, CreatureObject* victim) {
	...
	if (attackerGhost == NULL || victimGhost == NULL || victimGhost->hasOnKillerList(attacker->getObjectID()))
		return false;

	// No credit if either player is neutral or if they are same faction
	int attackerFaction = attacker->getFaction();
	int victimFaction = victim->getFaction();

	if (attackerFaction == 0 or victimFaction == 0 or attackerFaction == victimFaction)
		return false;
	...
	// No credit if they are in the same council. BH's killing a FRS target still pass this check
