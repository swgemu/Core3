...
void CreditManager::addBankCredits(uint64 creatureID, int amount, bool notifyClient) {
	ManagedReference<CreditObject*> obj = CreditManager::instance()->getCreditObject(creatureID);
...
ManagedReference<CreditObject*> CreditManager::getCreditObject(uint64 creoID) {
	...
	ManagedReference<CreditObject*> creditObj = creditObjects.get(oid);
	if (creditObj != NULL)
		return creditObj;

	creditObj = Core::getObjectBroker()->lookUp(oid).castTo<CreditObject*>();

	if (creditObj == NULL) {
		Locker locker(&dbMutex);

		// gotta duplicate logic after lock. Because multithreading
		ManagedReference<CreditObject*> creditObj = creditObjects.get(oid);
		if (creditObj != NULL)
			return creditObj;
	...
	creditObjects.put(oid, creditObj);
	return creditObj;
}