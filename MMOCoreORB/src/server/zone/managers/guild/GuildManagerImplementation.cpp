...
bool GuildManagerImplementation::validateGuildRename(CreatureObject* player, const String& guildName, GuildObject* guild) {
	if (guildName.length() < 1 || guildName.length() > 25) {
		player->sendSystemMessage("@guild:namechange_fail_name_bad_length"); //Guild names must be 1-25 characters in length.
		return false;
	}

	if (guildName.contains("\\") || guildName.contains("\n") || guildName.contains("\r") || guildName.contains("#")) {
		player->sendSystemMessage("@guild:namechange_fail_name_not_allowed"); //That guild name is not allowed
		return false;
	}
	...
	if (guildNameExists(guildName) && (guildName != guild->getGuildName())) {
		player->sendSystemMessage("@guild:create_fail_name_in_use"); //That guild name is already in use.
		return false;
	}
...
bool GuildManagerImplementation::validateGuildRenameAbbrev(CreatureObject* player, const String& guildAbbrev, GuildObject* guild) {
	if (guildAbbrev.length() < 1 || guildAbbrev.length() > 5) {
		player->sendSystemMessage("@guild:namechange_fail_abbrev_bad_length"); // Guild abbreviations must be 1-5 characters in length.
		return false;
	}
	if (guildAbbrev.contains("\\") || guildAbbrev.contains("\n") || guildAbbrev.contains("\r") || guildAbbrev.contains("#")) {
		player->sendSystemMessage("@guild:namechange_fail_abbrev_not_allowed"); // That guild name is not allowed
		return false;
	}
	NameManager* nameManager = processor->getNameManager();

	if (!nameManager->validateName(guildAbbrev, 0)) {
		player->sendSystemMessage("@guild:namechange_fail_abbrev_not_allowed"); // That guild abbreviation is not allowed.
		return false;
	}

	if (guildAbbrevExists(guildAbbrev) && (guildAbbrev != guild->getGuildAbbrev())) {
		player->sendSystemMessage("@guild:create_fail_abbrev_in_use"); // That guild abbreviation is already in use.
		return false;
	}
...
void GuildManagerImplementation::scheduleGuildRename(CreatureObject* player, GuildObject* guild) {
	...
	player->sendSystemMessage(params); // You have set your guild's name and abbreviation to be changed to '%TU' and '%TT' respectively. The change will take place in approximately 7 days, if there are no conflicts at that time.
