	...
	if (node->hasChildNodes()) {
		...
		if (templatePath.indexOf(".iff") < 0) { // Non-item selections
			...
			if (templatePath == "unlearn_all_skills") {
				...
				ManagedReference<SceneObject*> scob = cbSui->getUsingObject();
				player->addDotState(player, CreatureState::POISONED, scob->getObjectID(), 100, CreatureAttribute::HEALTH, 60, -1, 0);
				player->addDotState(player, CreatureState::BLEEDING, scob->getObjectID(), 100, CreatureAttribute::ACTION, 60, -1, 0);
				player->addDotState(player, CreatureState::DISEASED, scob->getObjectID(), 100, CreatureAttribute::ACTION, 60, -1, 0);
				player->addDotState(player, CreatureState::ONFIRE, scob->getObjectID(), 100, CreatureAttribute::HEALTH, 60, -1, 0, 20);
