...
void PortalLayout::readPortalGeometry_0003(IffStream *iff, int numPortals) {
...
void PortalLayout::readPortalGeometry_0004(IffStream *iff, int numPortals) {
	...
	for(int i=0; i<numPortals; i++) {
		...
		uint32 numIdx = vertexDataChunk->getChunkSize() / 12;
...
void PortalLayout::parse(IffStream* iffStream) {
	try {
		...
		if (type == '0003')
			readPortalGeometry_0003(iffStream, numPortals);
		else
			readPortalGeometry_0004(iffStream, numPortals);
