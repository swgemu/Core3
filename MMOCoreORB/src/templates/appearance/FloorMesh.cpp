...
Vector <Reference<MeshData*>> FloorMesh::getTransformedMeshData(const Matrix4& parentTransform) {
	...
	for (const auto& edge : uncrossableEdges) {
		...
		start = Vector3(start.getZ(), start.getY(), -start.getX());
		end = Vector3(end.getZ(), end.getY(), -end.getX());
	...
	data->transformMeshData(parentTransform);
	meshData.add(data);
	return meshData;
}
