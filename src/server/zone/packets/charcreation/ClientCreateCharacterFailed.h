...
public:
	ClientCreateCharacterFailed(const String& errorstr) : BaseMessage() {
		insertShort(0x03);
		insertInt(0xDF333C6E); //ClientCreateCharacterFailed
		insertInt(0x00); //Unicode String, used instead of stf file. We'll never use it.
		insertAscii("ui"); //StfFile - Should always be "ui".
		insertInt(0x00); //static spacer
		insertAscii(errorstr); //StfName - Should be from the ui StfFile.
