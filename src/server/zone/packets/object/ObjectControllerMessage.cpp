...
#include "../../objects/scene/variables/TransformVariables.h"
...
bool ObjectControllerMessage::parseDataTransform(Player* player, Message* pack) {
	...
	/*TransformVariables* variable = new TransformVariables();
	variable->setMovementStamp(pack->parseInt());
	variable->setMovementCounter(pack->parseInt());
	...
	variable->setDirection(dx, dz, dy, dw);

	float x = pack->parseFloat();
	float z = pack->parseFloat();
	float y = pack->parseFloat();

	variable->initializePosition(x, z, y);

	//bool success = playerManager->updatePlayerPosition(player, variable);
	 return success;*/
