...
void CreatureObjectImplementation::onConverse(Player* player) {
	...
	if(tOptStr == "init") {
	...
	} else if(screenId.indexOf("mission:") >= 0) { // Do an objective check for the mission. tOptStr = mission:missionkey
		System::out << "onConverse: handling mission check for conversation" << endl;
		String key;
		...
		System::out << "onConverse: checking mission key: " << key << endl;

		if(key.isEmpty() || key == "mission")
			return;

		System::out << "onConverse: evaluating mission..." << endl;

		// Grab the status string from the mission
		MissionManager* mm = server->getMissionManager();
		String retSay;
		mm->evalMission(player, key, retSay, false);
		...
		System::out << "handing off to sendConvoScreen (default)" << endl;
...
void CreatureObjectImplementation::onKilled(SceneObject* killer) {
	...
	System::out << "CreatureObject,onKilled: checking for NPC and killer is player" << endl;
	if(_this->isNonPlayerCreature() && killer->isPlayer()) {
		Player* killerPl = (Player*) killer;
		if(killerPl->missionCount() > 0) {
			killerPl->updateMissions(MissionObjectiveImplementation::HAS_KILLS, _this->getObjectCRC(), _this->getCustomName().toString(), 1);
			System::out << "CreatureObject,onKilled: updatedMissions finished" << endl;
		}
	}
