...
class TellpetSlashCommand : public QueueCommand {
public:

	TellpetSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		/*
		uint64 petID = packet->parseLong();

		UnicodeString msg = UnicodeString("");
		packet->parseUnicode(msg);
