...
class SocialInternalSlashCommand : public QueueCommand {
public:

	SocialInternalSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		chatManager->handleEmote(player, packet);*/
		return true;
	}
