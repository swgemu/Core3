...
class OpenContainerSlashCommand : public QueueCommand {
public:

	OpenContainerSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		if (parent == (SceneObject*)player->getInventory() || parent->isCell() || ((player->getBankContainer() != NULL) && (parent =  player->getBankContainer()))) {
			conti->sendTo(player);
			conti->openTo(player);
		}*/
		return true;
