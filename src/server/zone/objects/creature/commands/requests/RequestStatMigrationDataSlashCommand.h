...
class RequestStatMigrationDataSlashCommand : public QueueCommand {
public:

	RequestStatMigrationDataSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		/*uint64 objectid = packet->parseLong(); //TargetID?
