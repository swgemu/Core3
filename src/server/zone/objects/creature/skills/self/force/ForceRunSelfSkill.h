...
public:
	...
	uint32 buffState;
	 ...
	 ForceRunSelfSkill(const string& name, const string& effect, float sp, float acc, int sl, int du, ZoneProcessServerImplementation* serv) : SelfSkill(name, effect.c_str(), SELF, serv) {
		 speed = sp;
		 acceleration = acc;
		 slope = sl;
		 duration = du;
	...
	bool calculateCost(CreatureObject* creature) {
		...
		if(
			...
			player->isDizzied()) {
			//unicode msg = unicode("You are incapable of such speed right now.");
