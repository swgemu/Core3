...
class DizzyFallDownEvent : public Event {
	CreatureObjectImplementation* creature;

public:
	DizzyFallDownEvent(CreatureObjectImplementation* cr) : Event() {
		creature = cr;
	...
	bool activate() {
		try {
			...
			if (!creature->isIncapacitated() && !creature->isDead()) {
				int combatEquil = creature->getSkillMod("combat_equillibrium");
			...
			creature->error("Unreported exception caught in DizzyFallDownEvent::activate");
			creature->unlock();
		}
