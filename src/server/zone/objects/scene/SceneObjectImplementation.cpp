...
void SceneObjectImplementation::addSerializableVariables() {
	...
	addSerializableVariable("moving", &moving);
	addSerializableVariable("persistent", &persistent);
	addSerializableVariable("updated", &updated);
	addSerializableVariable("deleted", &deleted);
	addSerializableVariable("precisionDirectionAngle", &precisionDirectionAngle);
	addSerializableVariable("linkType", &linkType);
	addSerializableVariable("keepObject", &keepObject);
	addSerializableVariable("canPickup", &canPickup);
	addSerializableVariable("associatedArea", &associatedArea);
	addSerializableVariable("attackable", &attackable);
	addSerializableVariable("groupDamageList", &groupDamageList);
...
void SceneObjectImplementation::disseminateXp(int levels) {
	...
	for (int i = 0; i < playerDamageList.size(); i++) {
		SerializableVectorMapEntry<SceneObjectReference<CreatureObject>, ReferenceSlot<DamageDone> > entry = playerDamageList.SerializableSortedVector<SerializableVectorMapEntry<SceneObjectReference<CreatureObject>, ReferenceSlot<DamageDone> > >::get(i);
...
void SceneObjectImplementation::cleanupDamageDone() {
	while ( !playerDamageList.isEmpty() ) {
		SerializableVectorMapEntry<SceneObjectReference<CreatureObject>, ReferenceSlot<DamageDone> > entry = playerDamageList.SerializableSortedVector<SerializableVectorMapEntry<SceneObjectReference<CreatureObject>, ReferenceSlot<DamageDone> > >::get(0);
		CreatureObject *creature = entry.getKey();
		DamageDone *dmg = entry.getValue();
