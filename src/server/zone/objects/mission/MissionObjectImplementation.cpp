...
void MissionObjectImplementation::assetSetup() {
	...
	waypoint->setName(missionName + " Waypoint");
 ...
 */
bool MissionObjectImplementation::updateStatus(int type, uint32 objCrc, const String& str, int increment) {
	bool doUpdate = false;
	...
	for(int i = 0; i < objectiveList.size(); i++) {
		...
		//System::out << "MissionObject,updateStatus: doUpdate = " << doUpdate << endl;

		if(doUpdate) {
			switch(type) {
				case MissionObjectiveImplementation::HAS_KILLS:
					mo->incrementVar(increment);
					//statusStr += "(" + String::valueOf(mo->getVar()) + "/" + String::valueOf(mo->getLimit()) + ") confirmed kills";
					if(mo->getStrVar().compareTo("null") != 0)
						//statusStr += " on " + String::valueOf(mo->getStrVar());
				break;
