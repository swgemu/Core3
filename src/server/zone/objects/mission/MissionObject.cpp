...
bool MissionObject::updateStatus(int type, unsigned int objCrc, const String& str, int increment) {
	if (_impl == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
		...
		return ((MissionObjectImplementation*) _impl)->updateStatus(type, objCrc, str, increment);
}
...
void MissionObject::setMissionName(const String& tstr) {
	if (_impl == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
		...
		((MissionObjectImplementation*) _impl)->setMissionName(tstr);
}

String& MissionObject::getMissionName() {
	if (_impl == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
		...
		method.executeWithAsciiReturn(_return_getMissionName);
		return _return_getMissionName;
	} else
		return ((MissionObjectImplementation*) _impl)->getMissionName();
}
...
Packet* MissionObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case 32:
		...
		break;
	case 33:
		resp->insertBoolean(updateStatus(inv->getSignedIntParameter(), inv->getUnsignedIntParameter(), inv->getAsciiParameter(_param2_updateStatus__int_int_String_int_), inv->getSignedIntParameter()));
		break;
	case 34:
		checkComplete();
		break;
	case 35:
		setMissionName(inv->getAsciiParameter(_param0_setMissionName__String_));
		break;
	case 36:
		resp->insertAscii(getMissionName());
		break;
	case 37:
...
bool MissionObjectAdapter::updateStatus(int type, unsigned int objCrc, const String& str, int increment) {
	return ((MissionObjectImplementation*) impl)->updateStatus(type, objCrc, str, increment);
}
...
void MissionObjectAdapter::setMissionName(const String& tstr) {
	return ((MissionObjectImplementation*) impl)->setMissionName(tstr);
}

String& MissionObjectAdapter::getMissionName() {
	return ((MissionObjectImplementation*) impl)->getMissionName();
}
