...
#include "events/RemoveFireworkEvent.h"

FireworkImplementation::FireworkImplementation(Player* player, uint32 tempCRC, const UnicodeString& n, const String& tempn, int anim)
...
void FireworkImplementation::init() {
	switch (objectCRC) {
	case 0xF9E2CBB8: // 1
	case 0x15ADE9E5: // s01
		animationType = 1;
...
void FireworkImplementation::generateAttributes(SceneObject* obj) {
...
void FireworkImplementation::addAttributes(AttributeListMessage* alm) {
...
void FireworkImplementation::updateCraftingValues(DraftSchematic* draftSchematic){
...
int FireworkImplementation::useObject(Player* player) {
...
void FireworkImplementation::launchFirework(Player* player) {
	...
	server->addEvent(new RemoveFireworkEvent(firework));
}
