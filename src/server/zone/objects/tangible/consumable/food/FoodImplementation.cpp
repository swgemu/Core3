...
FoodImplementation::FoodImplementation(uint64 oid, uint32 tempCRC, const UnicodeString& n, const String& tempn)
		: FoodServant(oid, tempCRC, n, tempn, TangibleObjectImplementation::FOOD) {
...
FoodImplementation::FoodImplementation(CreatureObject* creature, uint64 oid, uint32 tempCRC, const UnicodeString& n, const String& tempn)
		: FoodServant(creature, oid, tempCRC, n, tempn, TangibleObjectImplementation::FOOD) {
...
int FoodImplementation::useObject(Player* player) {
	//TODO: add an effect to the player, increase filling, etc.
	return 1;
}

void FoodImplementation::generateAttributes(SceneObject* obj) {
	if (!obj->isPlayer())
		return;
...
void FoodImplementation::updateCraftingValues(DraftSchematic* draftSchematic){
...
void FoodImplementation::parseItemAttributes() {
...
void FoodImplementation::addHeaderAttributes(AttributeListMessage* alm) {
...
void FoodImplementation::addFooterAttributes(AttributeListMessage* alm) {

}

void FoodImplementation::addAttributes(AttributeListMessage* alm) {
