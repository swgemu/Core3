...
class TransferItemMiscSlashCommand : public SlashCommand {
public:
	...
	TangibleObject* validateDropAction(Player* player, uint64 target) {
		SceneObject* obj;
	...
	void transferItemToContainer(Player* player, TangibleObject* item, uint64 destinationID) {
		Zone* zone = player->getZone();
		if (zone == NULL)
			return;
