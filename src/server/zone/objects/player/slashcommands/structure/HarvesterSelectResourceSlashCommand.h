...
#ifndef HARVESTERSELECTRESOURCESLASHCOMMAND_H_
#define HARVESTERSELECTRESOURCESLASHCOMMAND_H_

#include "../../../scene/SceneObject.h"
#include "../../../structure/installation/InstallationObject.h"
#include "../../../../packets.h"

class HarvesterSelectResourceSlashCommand : public SlashCommand {
public:

	HarvesterSelectResourceSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(Player* player, Message* packet) {
		...
		uint64 objectid = packet->parseLong(); // Pop the Harvester ID - there might be some other int afterwards?
		SceneObject* object = player->getZone()->lookupObject(objectid);

		// Unicode - common!
		UnicodeString uresourceid("");
		packet->parseUnicode(uresourceid);
...
#endif //HARVESTERSELECTRESOURCESLASHCOMMAND_H_
