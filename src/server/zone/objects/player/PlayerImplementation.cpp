...
void PlayerImplementation::updateMissions(int type, uint32 objCrc, const String& str, int increment) {
	for(int i = 0; i < missionMap.size(); i++) {
		...
		if(mo != NULL) {
			if(!mo->isComplete()) {
				System::out << "Player,updateMissions: Calling update on mission status " << i << endl;
				if(mo->updateStatus(type, objCrc, str, increment))
					break;
			}
		}
	}
...
void PlayerImplementation::saveMissions() {
	...
	for(int i = 0; i < missionMap.size(); i++) {
		mm->savePlayerMission(_this, missionMap.get(i), false);
	}

	mm->savePlayerKeys(_this, curMisoKeys, finMisoKeys, false);
}
...
void PlayerImplementation::dropMission(const String& key, bool finished) {
	...
	//Add to finished keys if the mission was completed successfully:
	if(finished && !hasCompletedMisoKey(key))
		finMisoKeys += (key + ",");
