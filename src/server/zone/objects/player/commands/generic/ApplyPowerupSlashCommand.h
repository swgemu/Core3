...
class ApplyPowerupSlashCommand : public QueueCommand {
public:

	ApplyPowerupSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		uint64 powerupID = packet->parseLong();
