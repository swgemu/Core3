...
class SpatialChatInternalSlashCommand : public QueueCommand {
public:

	SpatialChatInternalSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		chatManager->handleSpatialChatInternalMessage(player, arguments);
		return true;
	}
