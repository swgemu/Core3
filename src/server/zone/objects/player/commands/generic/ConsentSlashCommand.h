...
#ifndef CONSENTSLASHCOMMAND_H_

#define CONSENTSLASHCOMMAND_H_
...
class ConsentSlashCommand : public QueueCommand {
public:

	ConsentSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		packet->parseUnicode(UnicodeStringName);
		...
		PlayerCreature* player->getZone()->getZoneServer()->getPlayerManager();
		consentName = consentName.toLowerCase();
		PlayerCreature* playerManager->getPlayer(consentName);
