...
class ChangeDanceSlashCommand : public QueueCommand {
public:

	ChangeDanceSlashCommand(const String& name, ZoneProcessServerImplementation* server)
	...
	bool doSlashCommand(PlayerCreature* player, const uint64& target, const UnicodeString& arguments) {
		...
		/*packet->parseLong(); // skip passed target

		UnicodeString option = UnicodeString("");
		packet->parseUnicode(option);
