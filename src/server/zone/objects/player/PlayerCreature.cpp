...
void PlayerCreature::sendExecuteConsoleCommand(const String& command) {
	if (_impl == NULL) {
		...
		((PlayerCreatureImplementation*) _impl)->sendExecuteConsoleCommand(command);
}
...
void PlayerCreature::setSpawnedBlueFrog() {
	if (_impl == NULL) {
		...
		((PlayerCreatureImplementation*) _impl)->setSpawnedBlueFrog();
}
...
bool PlayerCreature::hasSpawnedBlueFrog() {
	if (_impl == NULL) {
		...
		return ((PlayerCreatureImplementation*) _impl)->hasSpawnedBlueFrog();
}
...
void PlayerCreature::addToConsentList(const String& name) {
	if (_impl == NULL) {
		...
		((PlayerCreatureImplementation*) _impl)->addToConsentList(name);
}
...
bool PlayerCreature::hasInConsentList(const String& name) {
	if (_impl == NULL) {
		...
		return ((PlayerCreatureImplementation*) _impl)->hasInConsentList(name);
}
...
void PlayerCreature::removeFromConsentList(const String& name) {
	if (_impl == NULL) {
		...
		((PlayerCreatureImplementation*) _impl)->removeFromConsentList(name);
}
...
String PlayerCreature::getConsentName(int i) {
	if (_impl == NULL) {
		...
		return ((PlayerCreatureImplementation*) _impl)->getConsentName(i);
}
...
int PlayerCreature::getConsentListSize() {
	if (_impl == NULL) {
		...
		return ((PlayerCreatureImplementation*) _impl)->getConsentListSize();
}
...
void PlayerCreature::closeSuiWindowType(unsigned int windowType) {
	if (_impl == NULL) {
		...
		((PlayerCreatureImplementation*) _impl)->closeSuiWindowType(windowType);
}
...
PlayerCreatureImplementation::PlayerCreatureImplementation() {
	_initializeImplementation();
	// server/zone/objects/player/PlayerCreature.idl(172):  		Logger.setLoggingName("PlayerCreature");
	Logger::setLoggingName("PlayerCreature");
	// server/zone/objects/player/PlayerCreature.idl(174):  		owner = null;
	owner = NULL;
	// server/zone/objects/player/PlayerCreature.idl(176):  		disconnectEvent = null;
	disconnectEvent = NULL;
	// server/zone/objects/player/PlayerCreature.idl(177):  		recoveryEvent = null;
	recoveryEvent = NULL;
	// server/zone/objects/player/PlayerCreature.idl(178):  		accountID = 0;
	accountID = 0;
	// server/zone/objects/player/PlayerCreature.idl(180):  		savedZoneID = -1;
	savedZoneID = -1;
	// server/zone/objects/player/PlayerCreature.idl(181):  		savedParentID = 0;
	savedParentID = 0;
	// server/zone/objects/player/PlayerCreature.idl(183):  		skillPoints = 0;
	skillPoints = 0;
	// server/zone/objects/player/PlayerCreature.idl(185):  		teachingOrLearning = false;
	teachingOrLearning = false;
	// server/zone/objects/player/PlayerCreature.idl(187):  		suiBoxNextID = 0;
	suiBoxNextID = 0;
	// server/zone/objects/player/PlayerCreature.idl(189):  		factionStatus = 0;
	factionStatus = 0;
	// server/zone/objects/player/PlayerCreature.idl(190):  		lotsRemaining = 10;
	lotsRemaining = 10;
	// server/zone/objects/player/PlayerCreature.idl(192):  		centeredBonus = 0;
	centeredBonus = 0;
	// server/zone/objects/player/PlayerCreature.idl(194):  		clientLastMovementStamp = 0;
	clientLastMovementStamp = 0;
	// server/zone/objects/player/PlayerCreature.idl(196):  		super.pvpStatusBitmask = CreatureFlag.PLAYER;
	CreatureObjectImplementation::pvpStatusBitmask = CreatureFlag::PLAYER;
	// server/zone/objects/player/PlayerCreature.idl(198):  		conversatingCreature = null;
	conversatingCreature = NULL;
	// server/zone/objects/player/PlayerCreature.idl(200):  		spawnedBlueFrog = false;
	spawnedBlueFrog = false;
	// server/zone/objects/player/PlayerCreature.idl(202):  		persistentMessages.setNoDuplicateInsertPlan();
	(&persistentMessages)->setNoDuplicateInsertPlan();
	// server/zone/objects/player/PlayerCreature.idl(203):  		duelList.setNoDuplicateInsertPlan();
	(&duelList)->setNoDuplicateInsertPlan();
	// server/zone/objects/player/PlayerCreature.idl(204):  		consentList.setNoDuplicateInsertPlan();
	(&consentList)->setNoDuplicateInsertPlan();
	// server/zone/objects/player/PlayerCreature.idl(206):  		setOffline();
	setOffline();
}

void PlayerCreatureImplementation::loadTemplateData(SharedObjectTemplate* templateData) {
	// server/zone/objects/player/PlayerCreature.idl(213):  		super.loadTemplateData(templateData);
	CreatureObjectImplementation::loadTemplateData(templateData);
	// server/zone/objects/player/PlayerCreature.idl(215):  		super.pvpStatusBitmask = CreatureFlag.PLAYER;
	CreatureObjectImplementation::pvpStatusBitmask = CreatureFlag::PLAYER;
}

bool PlayerCreatureImplementation::isAggressiveTo(PlayerCreature* object) {
	// server/zone/objects/player/PlayerCreature.idl(320):  		return 
	if ((&duelList)->contains(object) && object->requestedDuelTo(_this))	// server/zone/objects/player/PlayerCreature.idl(321):  			return true;
	return true;
	// server/zone/objects/player/PlayerCreature.idl(323):  false;
	return false;
}

void PlayerCreatureImplementation::setBadge(unsigned int badge) {
	// server/zone/objects/player/PlayerCreature.idl(364):  		badges.setBadge(badge);
	(&badges)->setBadge(badge);
}

int PlayerCreatureImplementation::getNumBadges() {
	// server/zone/objects/player/PlayerCreature.idl(370):  		return badges.getNumBadges();
	return (&badges)->getNumBadges();
}

int PlayerCreatureImplementation::getBadgeTypeCount(byte type) {
	// server/zone/objects/player/PlayerCreature.idl(374):  		return badges.getTypeCount(type);
	return (&badges)->getTypeCount(type);
}

void PlayerCreatureImplementation::resetIncapacitationCounter() {
	// server/zone/objects/player/PlayerCreature.idl(380):  		incapacitationCounter = 0;
	incapacitationCounter = 0;
}

void PlayerCreatureImplementation::updateIncapacitationCounter() {
	// server/zone/objects/player/PlayerCreature.idl(386):  		incapacitationCounter++;
	incapacitationCounter ++;
	// server/zone/objects/player/PlayerCreature.idl(388):  	}
	if (isFirstIncapacitation() || isFirstIncapacitationExpired())	// server/zone/objects/player/PlayerCreature.idl(389):  			resetFirstIncapacitationTime();
	resetFirstIncapacitationTime();
}

bool PlayerCreatureImplementation::isFirstIncapacitation() {
	// server/zone/objects/player/PlayerCreature.idl(393):  		return incapacitationCounter == 1;
	return incapacitationCounter == 1;
}

byte PlayerCreatureImplementation::getIncapacitationCounter() {
	// server/zone/objects/player/PlayerCreature.idl(397):  		return incapacitationCounter;
	return incapacitationCounter;
}

void PlayerCreatureImplementation::setLastNpcConvStr(const String& conv) {
	// server/zone/objects/player/PlayerCreature.idl(402):  		lastNpcConvo = conv;
	lastNpcConvo = conv;
}

void PlayerCreatureImplementation::setLastNpcConvMessStr(const String& mess) {
	// server/zone/objects/player/PlayerCreature.idl(406):  		lastNpcConvoMessage = mess;
	lastNpcConvoMessage = mess;
}

String PlayerCreatureImplementation::getLastNpcConvStr() {
	// server/zone/objects/player/PlayerCreature.idl(410):  		return lastNpcConvo;
	return lastNpcConvo;
}

String PlayerCreatureImplementation::getLastNpcConvMessStr() {
	// server/zone/objects/player/PlayerCreature.idl(414):  		return lastNpcConvoMessage;
	return lastNpcConvoMessage;
}

String PlayerCreatureImplementation::getLastNpcConvOption(int idx) {
	// server/zone/objects/player/PlayerCreature.idl(418):  		return lastNpcConvoOptions.get(idx);
	return (&lastNpcConvoOptions)->get(idx);
}

void PlayerCreatureImplementation::addLastNpcConvOptions(const String& option) {
	// server/zone/objects/player/PlayerCreature.idl(422):  		lastNpcConvoOptions.add(option);
	(&lastNpcConvoOptions)->add(option);
}

int PlayerCreatureImplementation::countLastNpcConvOptions() {
	// server/zone/objects/player/PlayerCreature.idl(426):  		return lastNpcConvoOptions.size();
	return (&lastNpcConvoOptions)->size();
}

void PlayerCreatureImplementation::clearLastNpcConvOptions() {
	// server/zone/objects/player/PlayerCreature.idl(430):  		lastNpcConvoOptions.removeAll();
	(&lastNpcConvoOptions)->removeAll();
}

void PlayerCreatureImplementation::setSpawnedBlueFrog() {
	// server/zone/objects/player/PlayerCreature.idl(434):  		spawnedBlueFrog = true;
	spawnedBlueFrog = true;
}

bool PlayerCreatureImplementation::hasSpawnedBlueFrog() {
	// server/zone/objects/player/PlayerCreature.idl(438):  		return spawnedBlueFrog;
	return spawnedBlueFrog;
}

void PlayerCreatureImplementation::addToConsentList(const String& name) {
	// server/zone/objects/player/PlayerCreature.idl(442):  		consentList.put(name);
	(&consentList)->put(name);
}

bool PlayerCreatureImplementation::hasInConsentList(const String& name) {
	// server/zone/objects/player/PlayerCreature.idl(446):  		return consentList.contains(name);
	return (&consentList)->contains(name);
}

void PlayerCreatureImplementation::removeFromConsentList(const String& name) {
	// server/zone/objects/player/PlayerCreature.idl(450):  		consentList.drop(name);
	(&consentList)->drop(name);
}

String PlayerCreatureImplementation::getConsentName(int i) {
	// server/zone/objects/player/PlayerCreature.idl(454):  		return consentList.get(i);
	return (&consentList)->get(i);
}

int PlayerCreatureImplementation::getConsentListSize() {
	// server/zone/objects/player/PlayerCreature.idl(458):  		return consentList.size();
	return (&consentList)->size();
}

void PlayerCreatureImplementation::addToDuelList(PlayerCreature* targetPlayer) {
	// server/zone/objects/player/PlayerCreature.idl(463):  		duelList.put(targetPlayer);
	(&duelList)->put(targetPlayer);
}

void PlayerCreatureImplementation::removeFromDuelList(PlayerCreature* targetPlayer) {
	// server/zone/objects/player/PlayerCreature.idl(467):  		duelList.drop(targetPlayer);
	(&duelList)->drop(targetPlayer);
}

PlayerCreature* PlayerCreatureImplementation::getDuelListObject(int index) {
	// server/zone/objects/player/PlayerCreature.idl(471):  		return duelList.get(index);
	return (&duelList)->get(index);
}

bool PlayerCreatureImplementation::requestedDuelTo(PlayerCreature* targetPlayer) {
	// server/zone/objects/player/PlayerCreature.idl(475):  		return duelList.contains(targetPlayer);
	return (&duelList)->contains(targetPlayer);
}

bool PlayerCreatureImplementation::isDuelListEmpty() {
	// server/zone/objects/player/PlayerCreature.idl(479):  		return duelList.isEmpty();
	return (&duelList)->isEmpty();
}

int PlayerCreatureImplementation::getDuelListSize() {
	// server/zone/objects/player/PlayerCreature.idl(483):  		return duelList.size();
	return (&duelList)->size();
}

SortedVector<unsigned long long>* PlayerCreatureImplementation::getPersistentMessages() {
	// server/zone/objects/player/PlayerCreature.idl(490):  		return persistentMessages;
	return (&persistentMessages);
}

void PlayerCreatureImplementation::addPersistentMessage(unsigned long long id) {
	// server/zone/objects/player/PlayerCreature.idl(494):  		persistentMessages.put(id);
	(&persistentMessages)->put(id);
}

void PlayerCreatureImplementation::dropPersistentMessage(unsigned long long id) {
	// server/zone/objects/player/PlayerCreature.idl(498):  		persistentMessages.drop(id);
	(&persistentMessages)->drop(id);
}

PlayerObject* PlayerCreatureImplementation::getPlayerObject() {
	// server/zone/objects/player/PlayerCreature.idl(502):  		return (PlayerObject) super.getSlottedObject("ghost");
	return (PlayerObject*) CreatureObjectImplementation::getSlottedObject("ghost");
}

bool PlayerCreatureImplementation::isOnline() {
	// server/zone/objects/player/PlayerCreature.idl(506):  		return onlineStatus != OFFLINE && onlineStatus != LINKDEAD;
	return onlineStatus != OFFLINE && onlineStatus != LINKDEAD;
}

bool PlayerCreatureImplementation::isOffline() {
	// server/zone/objects/player/PlayerCreature.idl(510):  		return onlineStatus == OFFLINE;
	return onlineStatus == OFFLINE;
}

bool PlayerCreatureImplementation::isLoading() {
	// server/zone/objects/player/PlayerCreature.idl(514):  		return onlineStatus == LOADING || onlineStatus == LOGGINGOUT;
	return onlineStatus == LOADING || onlineStatus == LOGGINGOUT;
}

bool PlayerCreatureImplementation::isLinkDead() {
	// server/zone/objects/player/PlayerCreature.idl(518):  		return onlineStatus == LINKDEAD;
	return onlineStatus == LINKDEAD;
}

bool PlayerCreatureImplementation::isLoggingIn() {
	// server/zone/objects/player/PlayerCreature.idl(522):  		return onlineStatus == LOGGINGIN;
	return onlineStatus == LOGGINGIN;
}

bool PlayerCreatureImplementation::isLoggingOut() {
	// server/zone/objects/player/PlayerCreature.idl(526):  		return onlineStatus == LOGGINGOUT;
	return onlineStatus == LOGGINGOUT;
}

unsigned int PlayerCreatureImplementation::getAccountID() {
	// server/zone/objects/player/PlayerCreature.idl(530):  		return accountID;
	return accountID;
}

ZoneClientSession* PlayerCreatureImplementation::getClient() {
	// server/zone/objects/player/PlayerCreature.idl(534):  		return owner;
	return owner;
}

byte PlayerCreatureImplementation::getRaceID() {
	// server/zone/objects/player/PlayerCreature.idl(538):  		return raceID;
	return raceID;
}

unsigned long long PlayerCreatureImplementation::getSavedParentID() {
	// server/zone/objects/player/PlayerCreature.idl(542):  		return savedParentID;
	return savedParentID;
}

int PlayerCreatureImplementation::getSavedZoneID() {
	// server/zone/objects/player/PlayerCreature.idl(546):  		return savedZoneID;
	return savedZoneID;
}

void PlayerCreatureImplementation::setSavedParentID(unsigned long long id) {
	// server/zone/objects/player/PlayerCreature.idl(553):  		savedParentID = id;
	savedParentID = id;
}

void PlayerCreatureImplementation::setSavedZoneID(int id) {
	// server/zone/objects/player/PlayerCreature.idl(557):  		savedZoneID = id;
	savedZoneID = id;
}

void PlayerCreatureImplementation::setSkillPoints(int points) {
	// server/zone/objects/player/PlayerCreature.idl(561):  		skillPoints = points;
	skillPoints = points;
}
...
void PlayerCreatureImplementation::setClient(ZoneClientSession* cli) {
	// server/zone/objects/player/PlayerCreature.idl(569):  		owner = cli;
	owner = cli;
}

void PlayerCreatureImplementation::setBiography(const UnicodeString& bio) {
	// server/zone/objects/player/PlayerCreature.idl(573):  		biography = bio;
	biography = bio;
}

void PlayerCreatureImplementation::setRaceID(byte id) {
	// server/zone/objects/player/PlayerCreature.idl(577):  		raceID = id;
	raceID = id;
}

void PlayerCreatureImplementation::setConversatingCreature(CreatureObject* creature) {
	// server/zone/objects/player/PlayerCreature.idl(581):  		conversatingCreature = creature;
	conversatingCreature = creature;
}

void PlayerCreatureImplementation::setClientLastMovementStamp(unsigned int stamp) {
	// server/zone/objects/player/PlayerCreature.idl(585):  		clientLastMovementStamp = stamp;
	clientLastMovementStamp = stamp;
}

void PlayerCreatureImplementation::setOffline() {
	// server/zone/objects/player/PlayerCreature.idl(589):  		onlineStatus 
	if (isLinkDead()){
}
	// server/zone/objects/player/PlayerCreature.idl(593):  = OFFLINE;
	onlineStatus = OFFLINE;
}

void PlayerCreatureImplementation::setLoggingOut() {
	// server/zone/objects/player/PlayerCreature.idl(601):  		onlineStatus = LOGGINGOUT;
	onlineStatus = LOGGINGOUT;
}

void PlayerCreatureImplementation::setAccountID(unsigned int id) {
	// server/zone/objects/player/PlayerCreature.idl(605):  		accountID = id;
	accountID = id;
}

void PlayerCreatureImplementation::clearDisconnectEvent() {
	// server/zone/objects/player/PlayerCreature.idl(609):  		disconnectEvent = null;
	disconnectEvent = NULL;
}

void PlayerCreatureImplementation::clearRecoveryEvent() {
	// server/zone/objects/player/PlayerCreature.idl(613):  		recoveryEvent = null;
	recoveryEvent = NULL;
}

void PlayerCreatureImplementation::addChatRoom(ChatRoom* room) {
	// server/zone/objects/player/PlayerCreature.idl(617):  		chatRooms.put(room);
	(&chatRooms)->put(room);
}

void PlayerCreatureImplementation::removeChatRoom(ChatRoom* room) {
	// server/zone/objects/player/PlayerCreature.idl(621):  		chatRooms.drop(room);
	(&chatRooms)->drop(room);
}

int PlayerCreatureImplementation::getSkillPoints() {
	// server/zone/objects/player/PlayerCreature.idl(625):  		return skillPoints;
	return skillPoints;
}

bool PlayerCreatureImplementation::hasSuiBox(unsigned int boxID) {
	// server/zone/objects/player/PlayerCreature.idl(631):  		return suiBoxes.contains(boxID);
	return (&suiBoxes)->contains(boxID);
}

SuiBox* PlayerCreatureImplementation::getSuiBox(unsigned int boxID) {
	// server/zone/objects/player/PlayerCreature.idl(635):  		return suiBoxes.get(boxID);
	return (&suiBoxes)->get(boxID);
}

bool PlayerCreatureImplementation::hasSuiBoxWindowType(unsigned int windowType) {
	// server/zone/objects/player/PlayerCreature.idl(646):  		unsigned int type = 0;
	unsigned int type = 0;
	// server/zone/objects/player/PlayerCreature.idl(648):  
	for (	// server/zone/objects/player/PlayerCreature.idl(648):  		for (int i = 0;
	int i = 0;
	i < (&suiBoxes)->size();
 ++i) {
	// server/zone/objects/player/PlayerCreature.idl(649):  			SuiBox sui = suiBoxes.get(i);
	SuiBox* sui = (&suiBoxes)->get(i);
	// server/zone/objects/player/PlayerCreature.idl(651):  			type = sui.getWindowType();
	type = sui->getWindowType();
	// server/zone/objects/player/PlayerCreature.idl(653):  		}
	if (windowType == type)	// server/zone/objects/player/PlayerCreature.idl(654):  				return true;
	return true;
}
	// server/zone/objects/player/PlayerCreature.idl(657):  		return false;
	return false;
}

void PlayerCreatureImplementation::closeSuiWindowType(unsigned int windowType) {
	// server/zone/objects/player/PlayerCreature.idl(661):  	}
	if (hasSuiBoxWindowType(windowType)){
	// server/zone/objects/player/PlayerCreature.idl(662):  			SuiBox sui = getSuiBoxFromWindowType(windowType);
	SuiBox* sui = getSuiBoxFromWindowType(windowType);
	// server/zone/objects/player/PlayerCreature.idl(664):  		}
	if (sui != NULL){
	// server/zone/objects/player/PlayerCreature.idl(665):  				sendMessage(sui.generateCloseMessage());
	sendMessage(sui->generateCloseMessage());
	// server/zone/objects/player/PlayerCreature.idl(666):  				removeSuiBox(sui.getBoxID());
...
SuiBox* PlayerCreatureImplementation::getSuiBoxFromWindowType(unsigned int windowType) {
	// server/zone/objects/player/PlayerCreature.idl(672):  		unsigned int type = 0;
	unsigned int type = 0;
	// server/zone/objects/player/PlayerCreature.idl(674):  
	for (	// server/zone/objects/player/PlayerCreature.idl(674):  		for (int i = 0;
	int i = 0;
	i < (&suiBoxes)->size();
 ++i) {
	// server/zone/objects/player/PlayerCreature.idl(675):  			SuiBox sui = suiBoxes.get(i);
	SuiBox* sui = (&suiBoxes)->get(i);
	// server/zone/objects/player/PlayerCreature.idl(677):  		}
	if (windowType == sui->getWindowType())	// server/zone/objects/player/PlayerCreature.idl(678):  				return sui;
	return sui;
}
	// server/zone/objects/player/PlayerCreature.idl(681):  		return null;
	return NULL;
}

void PlayerCreatureImplementation::addSuiBox(SuiBox* sui) {
	// server/zone/objects/player/PlayerCreature.idl(685):  		unsigned int key = sui.getBoxID();
	unsigned int key = sui->getBoxID();
	// server/zone/objects/player/PlayerCreature.idl(686):  		suiBoxes.put(key, sui);
	(&suiBoxes)->put(key, sui);
}

int PlayerCreatureImplementation::getLotsRemaining() {
	// server/zone/objects/player/PlayerCreature.idl(690):  		return lotsRemaining;
	return lotsRemaining;
}

void PlayerCreatureImplementation::setLotsRemaining(int lots) {
	// server/zone/objects/player/PlayerCreature.idl(694):  		lotsRemaining = lots;
	lotsRemaining = lots;
}

int PlayerCreatureImplementation::getFactionStatus() {
	// server/zone/objects/player/PlayerCreature.idl(698):  		return factionStatus;
	return factionStatus;
}

UnicodeString PlayerCreatureImplementation::getBiography() {
	// server/zone/objects/player/PlayerCreature.idl(702):  		return biography;
	return biography;
}

unsigned int PlayerCreatureImplementation::getClientLastMovementStamp() {
	// server/zone/objects/player/PlayerCreature.idl(706):  		return clientLastMovementStamp;
	return clientLastMovementStamp;
}

void PlayerCreatureImplementation::setSurveyTool(SurveyTool* tool) {
	// server/zone/objects/player/PlayerCreature.idl(714):  		surveyTool = tool;
	surveyTool = tool;
}

SurveyTool* PlayerCreatureImplementation::getSurveyTool() {
	// server/zone/objects/player/PlayerCreature.idl(721):  		return surveyTool;
	return surveyTool;
}

void PlayerCreatureImplementation::clearTradeContainer() {
	// server/zone/objects/player/PlayerCreature.idl(726):  		tradeContainer.clear();
	(&tradeContainer)->clear();
}

void PlayerCreatureImplementation::setTeachingOrLearning(bool value) {
	// server/zone/objects/player/PlayerCreature.idl(730):  	 teachingOrLearning = value;
	teachingOrLearning = value;
}

bool PlayerCreatureImplementation::isTeachingOrLearning() {
	// server/zone/objects/player/PlayerCreature.idl(735):  	 return teachingOrLearning;
	return teachingOrLearning;
}

CreatureObject* PlayerCreatureImplementation::getConversatingCreature() {
	// server/zone/objects/player/PlayerCreature.idl(744):  		return conversatingCreature;
	return conversatingCreature;
}

int PlayerCreatureImplementation::getCenteredBonus() {
	// server/zone/objects/player/PlayerCreature.idl(748):  		return centeredBonus;
	return centeredBonus;
}

void PlayerCreatureImplementation::setCenteredBonus(int bonus) {
	// server/zone/objects/player/PlayerCreature.idl(752):  		centeredBonus = bonus;
	centeredBonus = bonus;
}

TradeContainer* PlayerCreatureImplementation::getTradeContainer() {
	// server/zone/objects/player/PlayerCreature.idl(757):  		return tradeContainer;
	return (&tradeContainer);
}

bool PlayerCreatureImplementation::hasBadge(unsigned int badge) {
	// server/zone/objects/player/PlayerCreature.idl(761):  		return badges.hasBadge(badge);
	return (&badges)->hasBadge(badge);
}

CraftingTool* PlayerCreatureImplementation::getLastCraftingToolUsed() {
	// server/zone/objects/player/PlayerCreature.idl(765):  		return lastCraftingToolUsed;
	return lastCraftingToolUsed;
}

void PlayerCreatureImplementation::setLastCraftingToolUsed(CraftingTool* tool) {
	// server/zone/objects/player/PlayerCreature.idl(769):  		lastCraftingToolUsed = tool;
