...
void PlayerManager::updatePlayerPosition(Player* player, TransformVariables* newPosition) {
	if (_impl == NULL) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
		...
		method.addObjectParameter(player);
		method.addObjectParameter(newPosition);

		method.executeWithVoidReturn();
	} else
		((PlayerManagerImplementation*) _impl)->updatePlayerPosition(player, newPosition);
}
...
Packet* PlayerManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case 32:
		...
		break;
	case 33:
		updatePlayerPosition((Player*) inv->getObjectParameter(), (TransformVariables*) inv->getObjectParameter());
		break;
	case 34:
...
void PlayerManagerAdapter::updatePlayerPosition(Player* player, TransformVariables* newPosition) {
	return ((PlayerManagerImplementation*) impl)->updatePlayerPosition(player, newPosition);
}
