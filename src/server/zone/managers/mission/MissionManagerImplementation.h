...
public:
	...
	MissionObject* poolMission(const String& dbKey, int termMask, const String& typeStr, uint32 descKey, uint32 titleKey, uint32 diffLv, float destX, float destY, uint32 destPlanetCrc,
			...
			const String& descriptionStf, const String& titleStf, uint32 typeCrc, const String& objectiveDefaults, bool doLock = true);
	...
	void evalMission(Player* player, const String& tKey, String& retSay);
	...
	void savePlayerMission(Player* player, MissionObject* miso, bool doLock = true);

	//Saves mission keys to the DB, called automatically on player save event
	void savePlayerKeys(Player* player, const String& curMisoKeys, const String& finMisoKeys, bool doLock = true);
