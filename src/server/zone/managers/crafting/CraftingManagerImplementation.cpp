...
void CraftingManagerImplementation::loadDraftSchematicsFromDatabase() {
	lock();
...
DraftSchematic* CraftingManagerImplementation::loadDraftSchematic(
		ResultSet* result) {
	...
	Vector<string> parsedIngredientTemplateNames =
			parseStringsFromString(unparIngredientTemplateNames);
	...
	Vector<string> parsedResourceTypes =
			parseStringsFromString(unparResourceTypes);

	// example: 3,8,1
	string unparResourceQuantities = result->getString(10);
	Vector<uint32> parsedResourceQuantities =
			parseUnsignedInt32sFromString(unparResourceQuantities);
	...
	Vector<uint32> parsedNumberExperimentalProperties =
			parseUnsignedInt32sFromString(unparNumberExperimentalProperties);
...
void CraftingManagerImplementation::prepareCraftingSession(Player* player,
		CraftingTool * ct, DraftSchematic* ds) {
...
void CraftingManagerImplementation::createDraftSchematic(Player* player, CraftingTool * ct) {
...
void CraftingManagerImplementation::createTangibleObject(Player* player, CraftingTool * ct) {			
...
void CraftingManagerImplementation::setupIngredients(Player* player, CraftingTool * ct) {
	...
	ManufactureSchematicObjectMessage7* msco7 =
			new ManufactureSchematicObjectMessage7(ds->getObjectID(), ds);
...
void CraftingManagerImplementation::addResourceToCraft(Player * player,
	...
	ResourceContainerObjectDeltaMessage3* dRcno3 =
			new ResourceContainerObjectDeltaMessage3(rcno);
	...
	ManufactureSchematicObjectDeltaMessage6* dMsco6 =
			new ManufactureSchematicObjectDeltaMessage6(ds->getObjectID());
	...
	ManufactureSchematicObjectDeltaMessage7* dMsco7 =
			new ManufactureSchematicObjectDeltaMessage7(ds->getObjectID());
	if (ct->getInsertCount() == 1) {
		dMsco7->fullUpdate(ds, ds->getIngredientListSize(), slot,
				rcno->getObjectID(), dsi->getResourceQuantity());
	} else {
		dMsco7->partialUpdate(slot, ds->getIngredientListSize()
				+ ct->getInsertCount(), rcno->getObjectID(),
				dsi->getResourceQuantity());
	}
	dMsco7->close();
...
void CraftingManagerImplementation::removeResourceFromCraft(Player * player,
		uint64 resID, int slot, int counter) {

	CraftingTool * ct = player->getCurrentCraftingTool();
	DraftSchematic * ds = ct->getDs();
	
	cout << "Removing resource\n";
...
void CraftingManagerImplementation::createPrototype(Player * player, string test) {
...
void CraftingManagerImplementation::assembleWithoutExperimenting(
		Player * player, CraftingTool * ct, DraftSchematic * ds, int counter) {

	ManufactureSchematicObjectDeltaMessage3* dMsco3 =
			new ManufactureSchematicObjectDeltaMessage3(ds->getObjectID());
