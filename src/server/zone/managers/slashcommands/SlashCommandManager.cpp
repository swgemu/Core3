...
SlashCommandManager::SlashCommandManager(ZoneProcessServerImplementation* serv)
	: Mutex("SlashCommand Manager") {

	server = serv;
	configManager = new SlashCommandConfigManager(server);
...
void SlashCommandManager::addSlashCommand(SlashCommand* command) {
	slashCommands.put(command);
}

SlashCommand* SlashCommandManager::getSlashCommand(const String& name) {
	return slashCommands.getSlashCommand(name);
}

SlashCommand* SlashCommandManager::getSlashCommand(uint32 crc) {
