...
bool ResourceTree::buildTreeFromClient() {
	...
	for (int i = 0; i < dtiff.getTotalRows(); ++i) {
		...
		row->getCell(10)->getValue(intvalue);
		entry->setMaxtype(intvalue);

		row->getCell(11)->getValue(intvalue);
		entry->setMintype(intvalue);

		row->getCell(12)->getValue(intvalue);
		entry->setMinpool(intvalue);

		row->getCell(13)->getValue(intvalue);
		entry->setMaxpool(intvalue);

		row->getCell(14)->getValue(intvalue);
		entry->setRecycled(intvalue);
