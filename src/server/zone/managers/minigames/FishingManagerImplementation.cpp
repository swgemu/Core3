	...
	if (fish > 93){ // NON-FISH ITEM WAS CAUGHT!
		...
		if (badLuck == 0) {
			...
			if (lootFishObject != NULL) {
				...
				lootFishObject->setAttributes(name, zone->getTerrainName(), time, length / 100);
				...
				lootFishObject->setCustomizationVariable(CustomizationVariableTypes::PRIVATE_INDEX_COLOR_1, color.get(zone->getTerrainName()));
				...
				String resourceString = zone->getTerrainName();
